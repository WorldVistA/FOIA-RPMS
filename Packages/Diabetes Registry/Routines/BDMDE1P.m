BDMDE1P ; IHS/CMI/LAB - 2017 DIABETES AUDIT PRINT 15 Dec 2016 3:05 PM ; 
 ...
 S BDMPD=0,BDMGUIC=0 F  S BDMPD=$O(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD!(BDMQUIT)  D
 ...
 .I $$AGE^AUPNPAT(BDMPD,BDMADAT)<40!($E($G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))=3) W !?5,"This is only calculated for patients 40 years of age and older without,",!,"a statin allegry or intolerance." I 1
...
DONE ;
 ...
 Q
I(I) ;
 Q $G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,I))
 ;
PAGE ;
 Q:$G(BDMDSP)
