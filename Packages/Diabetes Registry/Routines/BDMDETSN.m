BDMDETSN ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
PAUSE ;EP
 ...
 Q
HDR ; -- header code
 S VALMHDR(1)="SNOMED LISTS TO SUPPORT 2017 DIABETES AUDIT REPORTING"
 S VALMHDR(2)="* View SNOMED Lists"
 Q
 ;
INIT ;EP -- init variables and list array
 K BDMTAX S BDMHIGH="",C=0
 S BDMYR=$O(^BDMSNME("B",2017,0))
...
DISP ;EP - add an item to the selected list - called from a protocol
 ...
 S XBRP="PRINT^BDMDETSN",XBRC="",XBRX="XIT^BDMDETSN",XBNS="BDM"
 D ^XBDBQUE
 D DISPX
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BDMDETSN"")"
 S XBRC="",XBRX="XIT^BDMDETSN",XBIOP=0 D ^XBDBQUE
 Q
PHDR ;
 I 'BDMPG G HEAD1
