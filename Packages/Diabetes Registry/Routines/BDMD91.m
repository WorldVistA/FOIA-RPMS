BDMD91 ; IHS/CMI/LAB - IHS Diabetes Audit 2009 ; 
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**2,3,10**;JUN 14, 2007;Build 12
 ;
BEGIN ;EP - called from option
 D TAXCHK^BDMD919
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2009")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM91",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMPREP'=2 S XBRP="^BDMD91P",XBRC="^BDMD910",XBRX="XIT^BDMD91",XBNS="BDM"
 I BDMPREP=2 S XBRP="",XBRC="^BDMD910",XBRX="XIT^BDMD91",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
IAEP ;EP
 ...
 G IF2
P ;
 S BDMSTP=0 K ^XTMP("BDMDM91",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM91",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM91",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMDM91",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM91",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM91",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2009 EPI REC 1",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2009 EPI INFO RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPCP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2) ;PEP - gui call
 ...
 I BDMFILE]"",$$UP^XLFSTR(BDMFILE)'[".REC" S BDMFILE=BDMFILE_".rec"
 ...
 N BDMOPT  ;maw
 S BDMOPT="2009 Diabetes Program Audit"
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 S X=BDMJOB_"."_BDMBTH
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMD91",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMD91P","^TMP($J,""BDMDM91"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM91",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM91",X)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMDM91",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMD91(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
