BDMPE15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPE=0 F  S BDMPE=$O(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE)) Q:BDMPE'=+BDMPE  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,20))
 S P=$S($E(V)="F":3,$E(V)="M":4,1:5)
 S $P(BDMCUML(10),U,P)=$P(BDMCUML(10),U,P)+1
AGE ;
 S V=$$AGE^AUPNPAT(BDMPE,BDMADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMDE14(BDMPE,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1
 .I $$OB^BDMDE14(BDMPE,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
BPC ;blood pressure control
 ;take last 3 bp's and get mean systolic and mean diastolic
 S $P(BDMCUML(60),U,2)=$P(BDMCUML(60),U,2)+1
 S S=$$SYSMEAN(BDMPE,BDMRBD,BDMRED)
 S D=$$DIAMEAN(BDMPE,BDMRBD,BDMRED)
 D
...
TOBACCO ;
 ...
 S V=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,27))
 S V1=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMPE16(BDMPE,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 ;S P=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,94))
 S H=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,34))
 S A=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMPE1",BDMJOB,BDMBTH,"AUDIT",BDMPE,61))
 S L=$$LDL^BDMDE18(BDMPE,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDE18(BDMPE,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
...
SYSMEAN(P,BDATE,EDATE) ;EP
 NEW X S X=$$BPS^BDMDE13(P,BDATE,EDATE,"I")
 I X="" Q ""
