BDMPF15 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT ;
...
CUML ;EP
 ...
 ;
PROCESS ;
 S BDMPF=0 F  S BDMPF=$O(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF)) Q:BDMPF'=+BDMPF  D CUML1
 Q
 ;
CUML1 ;
GENDER ;
 ...
 S V=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,20))
 S P=$S($E(V)="F":3,$E(V)="M":4,1:5)
 S $P(BDMCUML(10),U,P)=$P(BDMCUML(10),U,P)+1
AGE ;
 S V=$$AGE^AUPNPAT(BDMPF,BDMADAT)
...
BMI ;
 ...
 S V=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,112))
 S $P(BDMCUML(40),U,2)=$P(BDMCUML(40),U,2)+1
 D
 .I V="" S $P(BDMCUML(40),U,5)=$P(BDMCUML(40),U,5)+1 Q
 .I $$OW^BDMDF14(BDMPF,V,BDMADAT) S $P(BDMCUML(40),U,3)=$P(BDMCUML(40),U,3)+1
 .I $$OB^BDMDF14(BDMPF,V,BDMADAT) S $P(BDMCUML(40),U,4)=$P(BDMCUML(40),U,4)+1
BPC ;blood pressure control
 ;take last 3 bp's and get mean systolic and mean diastolic
 S $P(BDMCUML(60),U,2)=$P(BDMCUML(60),U,2)+1
 S S=$$SYSMEAN(BDMPF,BDMRBD,BDMRED)
 S D=$$DIAMEAN(BDMPF,BDMRBD,BDMRED)
 D
...
TOBACCO ;
 ...
 S V=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,27))
 S V1=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,28))
...
DMTX ;diabetes treatment
 ...
 S V=$$THERAPY^BDMPF16(BDMPF,BDM6MBD,BDMRED)
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,62))
...
ACE ;110 title^total pts^total pts with protein^# of those on ace^# with htn^# of those on ace"
 ...
 ;S P=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,94))
 S H=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,34))
 S A=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,60))
...
LIPID ;115
 S V=$G(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,61))
 S L=$$LDL^BDMDF18(BDMPF,BDMBDAT,BDMADAT,"I"),L=$P(L,U)
 S T=$$CHOL^BDMDF18(BDMPF,BDMBDAT,BDMADAT,"I"),T=$P(T,U)
