...
PAUSE ;EP
 ...
 Q
HDR ; -- header code
 S VALMHDR(1)="SNOMED LISTS TO SUPPORT 2018 DIABETES AUDIT REPORTING"
 S VALMHDR(2)="* View SNOMED Lists"
 Q
 ;
INIT ;EP -- init variables and list array
 K BDMTAX S BDMHIGH="",C=0
 S BDMYR=$O(^BDMSNME("B",2018,0))
...
DISP ;EP - add an item to the selected list - called from a protocol
 ...
 S XBRP="PRINT^BDMDFTSN",XBRC="",XBRX="XIT^BDMDFTSN",XBNS="BDM"
 D ^XBDBQUE
 D DISPX
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^BDMDFTSN"")"
 S XBRC="",XBRX="XIT^BDMDFTSN",XBIOP=0 D ^XBDBQUE
 Q
PHDR ;
 I 'BDMPG G HEAD1
