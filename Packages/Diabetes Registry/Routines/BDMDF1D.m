BDMDF1D ; IHS/CMI/LAB - BDM Continuation of BDMDF1C 1/11/2014 1:54:57 PM ;
...
MICRO ;EP
 ...
 ...D SETV
 ...Q
 I '$D(BDM(1)) S %1="" D ACRATIOM^BDMDF1C Q %1
 D SETN
...
ASPIRIN ;EP
 ...
 S BDM=$$PRESD^BDMDF18(P,$O(^ATXAX("B","DM AUDIT ASPIRIN DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 S BDM=$$PRESD^BDMDF18(P,$O(^ATXAX("B","DM AUDIT ANTIPLT/ANTICOAG RX",0)),ED,186)
 I BDM]"" Q BDM
 S BDM=$$PRESD^BDMDF18(P,$O(^ATXAX("B","DM AUDIT ANTI-PLATELET DRUGS",0)),ED,186)
 I BDM]"" Q BDM
 Q "2  None"
 ;
 ;
HEPCDX(P,EDATE) ;EP - dx of HEP C ever?
 ...
 .I $P($G(^AUPNPROB(X,800)),U,1)]"",$$SNOMED^BDMUTL(2018,"PXRM HEPATITIS C",$P(^AUPNPROB(X,800),U,1)) S G=1
 .Q
 I G Q G
 Q "2  No"
HEPSCR(P,EDATE) ;
 ...
 ...Q
 I BGPC Q BGPC
 S %="",E=+$$CODEN^ICPTCOD(86803),%=$$CPTI^BDMDFDU(P,$$DOB^AUPNPAT(P),EDATE,E)
...
DMRETDX(P,EDATE) ;EP - is DM RETINOPATHY on problem list
 ...
 I 'T S T=$O(^ATXAX("B","DM AUDIT DIABETIC RETINOPATHY",0))
 S (X,G)=0 F  S X=$O(^AUPNPROB("AC",P,X)) Q:X'=+X!(G)  D
 ...
 ..I '$$SNO("PXRM BGP DM RETINOPATHY"),$$SNOMED^BDMUTL(2018,"DIABETIC RETINOPATHY",$P(^AUPNPROB(X,800),U,1)) S G=X
 ..I $$SNOMED^BDMUTL(2018,"PXRM BGP DM RETINOPATHY",$P(^AUPNPROB(X,800),U,1)) S G=X
 .Q
