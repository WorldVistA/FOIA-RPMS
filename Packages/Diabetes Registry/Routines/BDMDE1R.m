BDMDE1R ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
PROCESS ;
 S BDMNOGO=0
 S BDMPD=0 F  S BDMPD=$O(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD)) Q:BDMPD'=+BDMPD  D CUML1
 Q
 ;
CUML1 ;
ASPIRIN ;
 I $$DODX^BDMDE16(BDMPD,BDMDMRG,"I")>BDMADAT S BDMNOGO=BDMNOGO+1 Q
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))  ;CVD
 I $E(V)=1 S $P(BDMCUML(10,1),U,3)=$P(BDMCUML(10,1),U,3)+1  ;CVD DENOM
 ;CVD AND ASPIRIN
 S A=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
CKD ;
 ...
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,79)),V=$$STV^BDMDE18($P(V,U,2),5)
 S T=$P($G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,92)),U,5)  ;type of test
...
IMM ;
HEPB ;
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,115))
...
FLU ;
 ...
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,64))
 I $E(V)="1" S $P(BDMCUML(110,1),U,2)=$P(BDMCUML(110,1),U,2)+1
PNEUM ;
 S $P(BDMCUML(120,1),U,3)=$P(BDMCUML(120,1),U,3)+1
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,66))
 I $E(V)="1" S $P(BDMCUML(120,1),U,2)=$P(BDMCUML(120,1),U,2)+1
TD ;
 S $P(BDMCUML(130,1),U,3)=$P(BDMCUML(130,1),U,3)+1
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,68))
 I $E(V)="1" S $P(BDMCUML(130,1),U,2)=$P(BDMCUML(130,1),U,2)+1
LIPID ;
 S V=$E($G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))  ;STATIN
 S C=0
 S A=$$AGE^AUPNPAT(BDMPD,BDMADAT)
 I $E($G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 S C=1  ;CVD
 I C=1!(A>39&(A<76)) D
...
NUTR ;
 ...
 S G=0,V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))
...
TOB ;
 ...
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,215))
 I +V=1!(+V=2) S $P(BDMCUML(170,1),U,2)=$P(BDMCUML(170,1),U,2)+1
TB ;
 S $P(BDMCUML(180,1),U,3)=$P(BDMCUML(180,1),U,3)+1
 S V=$G(^XTMP("BDMDMDATA",BDMJOB,BDMBTH,"AUDIT",BDMPD,70))
