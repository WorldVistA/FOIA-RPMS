BDMD61E ; IHS/CMI/LAB - IHS Diabetes Audit 2006 ;
...
ZIS ;
 I BDMPREP'=2 S XBRP="^BDMD61P",XBRC="EAUDIT^BDMD610",XBRX="XIT^BDMD61",XBNS="BDM"
 I BDMPREP=2 S XBRP="",XBRC="EAUDIT^BDMD610",XBRX="XIT^BDMD61",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMDM61",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM61",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM61",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMDM61",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
C ;get register, status, random or not
 K ^XTMP("BDMDM61",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMCMS="",BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM61",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM61",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMDM61",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2006 EPI REC 1",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2006 EPI INFO RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPCP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="Run the 2006 Audit w/predefined set of Pts"  ;maw
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 S X=BDMJOB_"."_BDMBTH
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMD61E",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMD61P","^TMP($J,""BDMDM61"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM61",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM61",X)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMDM61",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMD61(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
