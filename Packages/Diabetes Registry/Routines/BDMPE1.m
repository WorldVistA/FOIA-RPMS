BDMPE1 ; IHS/CMI/LAB - IHS PRE Diabetes Audit 2015 ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**10**;JUN 14, 2007;Build 12
 ;
BEGIN ;EP - called from option
 D TAXCHK^BDMPE19  ;
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF PREDIABETES/METABOLIC SYNDROME CARE, 2017")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMPE1",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 S XBRP="^BDMPE1P",XBRC="^BDMPE10",XBRX="XIT^BDMPE1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMPE1P"")"
 S XBRC="^BDMPE10",XBRX="XIT^BDMPE1",XBIOP=0 D ^XBDBQUE
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMPE1",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMPE1",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMPE1",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMPE1",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
 ...
 S BDMCNT=0 F BDMPE=0:0 S BDMPE=$O(^DIBT(+Y,1,BDMPE)) Q:'BDMPE  S BDMCNT=BDMCNT+1,^TMP($J,"PATS",BDMCNT,BDMPE)=""
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDE1B(P,BDMBDAT,BDMADAT,1,1) K ^TMP($J,"PATS",X)
...
PCP ;
 ...
 I Y=-1 G PCP
 S BDMPEP=+Y
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) I $P(^AUPNPAT(P,0),U,14)'=BDMPEP K ^TMP($J,"PATS",X,P)
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMPE1",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMPE1",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMPE1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 K ^XTMP("BDMPE1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPEP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN) ;PEP - gui call
 F X="BDMJOB","BDMBTH","BDMDMRG","BDMADAT","BDMTYPE","BDMSTMP","BDMPEP","BDMCOM","BDMRAND","BDMRCNT","BDMCMS","BDMSTAT","BDMPREP","BDMFILE","BDMDSP","BDMPPN","BDMDUZ2" S @X=$G(@X)
 ...
 S BDMNOW=$G(%)
 N BDMOPT  ;maw
 S BDMOPT="DM 2017 Pre-Diabetes Audit"  ;maw
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) D
 .I BDMBEN=1,$$BEN^AUPNPAT(P,"C")'="01" K ^TMP($J,"PATS",X)
 .I BDMBEN=2,$$BEN^AUPNPAT(P,"C")="01" K ^TMP($J,"PATS",X)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMPE1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMPE1P","^TMP($J,""BDMPE1"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMPE1",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMPE1",X)
 .;I BDMDATA="ZZZZZZZ" S BDMDETA=$C(12)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMPE1",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMPE1(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
