BDMDF14 ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 14 Oct 2014 9:54 AM ; 
...
COMOR ;
 ...
 W !?3,"Diagnosed hypertension ever",?49,$$C($P(V,U,6)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,6))
 W !?6,"Diagnosed hypertension & mean BP <140/<90",?49,$$C($P(V,U,7)),?61,$$C($P(V,U,6)),?73,$$P($P(V,U,6),$P(V,U,7))
COCVD ;
 I $Y>(BDMIOSL-8) D HEADER Q:BDMQUIT
 W !!?3,"Diagnosed CVD ever",?49,$$C($P(V,U,8)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,8))
 ...
 W !?6,"Diagnosed CVD & statin prescribed*",?49,$$C($P(V,U,11)),?61,$$C($P(V,U,34)),?73,$$P($P(V,U,34),$P(V,U,11))
...
CKD ;
 I $Y>(BDMIOSL-6) D HEADER Q:BDMQUIT
 W !!?3,"In age 18+ chronic kidney disease (CKD) [1]",?49,$$C($P(V,U,14)),?61,$$C($P(V,U,13)),?73,$$P($P(V,U,13),$P(V,U,14))
 W !?6,"CKD [1] & mean BP <140/<90",?49,$$C($P(V,U,15)),?61,$$C($P(V,U,14)),?73,$$P($P(V,U,14),$P(V,U,15))
 W !?6,"CKD [1] & ACE Inhibitor or ARB prescribed",?49,$$C($P(V,U,16)),?61,$$C($P(V,U,14)),?73,$$P($P(V,U,14),$P(V,U,16))
GU ;
 ...
 W !?6,"Stages 1 & 2: eGFR =>60 ml/min",?49,$$C($P(V,U,19)),?61,$$C($P(V,U,13)),?73,$$P($P(V,U,13),$P(V,U,19)),!?9,"& UACR =>30 mg/g"
 ...
 W !?6,"Chronic Kidney",?49,$$C($P(V,U,23)),?61,$$C($P(V,U,13)),?73,$$P($P(V,U,13),$P(V,U,23)),!?9,"Disease Stage undetermined"
HEPC ;P11
 W !!,?2,"Diagnosed hepatitis C (HCV) ever",?49,$$C($P(V,U,35)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,35))
 W !,?6,"If born 1945-1965 and no HCV dx,",?49,$$C($P(V,U,37)),?61,$$C($P(V,U,36)),?73,$$P($P(V,U,36),$P(V,U,37)),!?6,"screened ever"
 W !!,?2,"Diagnosed retinopathy ever",?49,$$C($P(V,U,38)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,38))
COM ;
 I $Y>(BDMIOSL-10) D HEADER Q:BDMQUIT
 W !!?3,"Number of comorbid conditions [2]"
 ...
 W !?6,"Five or more",?49,$$C($P(V,U,30)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,30))
 ;W !?6,"Six",?49,$$C($P(V,U,31)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,31))
TOBSCR ;
 I $Y>(BDMIOSL-9) D HEADER Q:BDMQUIT
 S V=$G(BDMCUML(65)) W !!,"Tobacco & Nicotine Use",!?3,$P(V,U)
...
ENDS ;
 ...
 W !,"use screening during the Audit period:"
 W !?6,"Screened",?49,$$C($P(V,U,3)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,3))
 W !?6,"Not screened",?49,$$C($P(V,U,4)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,4))
 W !,"ENDS use status:"
...
TX ;
 ...
 W !?3,"None of the following meds",?49,$$C($P(V,U,3)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,3))
 ...
 W !!?3,"Repaglinide (Prandin) or Nateglinide ",?49,$$C($P(V,U,6)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,6)),!?10,"(Starlix)"
...
HEADER1 ;
 ...
 N BDMDHDR
 S BDMDHDR="AUDIT REPORT FOR 2018 (Audit Period "_$$DATE^BDMS9B1(BDMBDAT)_" to "_$$DATE^BDMS9B1(BDMADAT)_")"
 W $$CTR(BDMDHDR,80),!
