BDMD71 ; IHS/CMI/LAB - IHS Diabetes Audit 2007 ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**2,10**;JUN 14, 2007;Build 12
 ;
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM71",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMDEMO=-1 G IF
 I BDMPREP'=2 S XBRP="^BDMD71P",XBRC="^BDMD710",XBRX="XIT^BDMD71",XBNS="BDM"
 I BDMPREP=2 S XBRP="",XBRC="^BDMD710",XBRX="XIT^BDMD71",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMDM71",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM71",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM71",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMDM71",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM71",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM71",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMDM71",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 S I=$O(^BDMRECD("B","DM AUDIT 2007 EPI REC 1",0))
 ...
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2007 EPI INFO RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
 ...
 K ^XTMP("BDMDM71",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPCP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2007 Diabetes Program Audit"
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 S X=BDMJOB_"."_BDMBTH
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMD71",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMD71P","^TMP($J,""BDMDM71"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM71",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMDM71",X)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMDM71",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMD71(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
