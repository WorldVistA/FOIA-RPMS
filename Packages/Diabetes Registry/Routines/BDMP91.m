BDMP91 ; IHS/CMI/LAB - IHS PRE Diabetes Audit 2009 ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**2,3,10**;JUN 14, 2007;Build 12
 ;
BEGIN ;EP - called from option
 D TAXCHK^BDMP919  ;
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF PREDIABETES/METABOLIC SYNDROME CARE, 2009")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMP91",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 ...
 I BDMDEMO=-1 G IF
 S XBRP="^BDMP91P",XBRC="^BDMP910",XBRX="XIT^BDMP91",XBNS="BDM"
 ;I BDMPREP=2 S XBRP="",XBRC="^BDMP910",XBRX="XIT^BDMP91",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMP91",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMP91",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMP91",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMP91",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMP91",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMP91",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMP91",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 K ^BDMDATA("BDMEPI",$J)
 S XBGL="BDMDATA("
 K XBUF I $P($G(^APCCCTRL(DUZ(2),0)),U,11)]"" S XBUF=$P(^APCCCTRL($S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0),U,11)
...
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPCP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2) ;PEP - gui call
 ...
 S BDMNOW=$G(%)
 N BDMOPT  ;maw
 S BDMOPT="DM 2009 Pre-Diabetes Audit"  ;maw
 K DD,D0,DIC
 S X=BDMJOB_"."_BDMBTH
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMP91",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMP91P","^TMP($J,""BDMP91"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMP91",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMP91",X)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMP91",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMP91(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
