BDMPF1P ; IHS/CMI/LAB - 2003 DIABETES AUDIT PRINT ;
 ...
 S BDMPF=0,BDMGUIC=0 F  S BDMPF=$O(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF)) Q:BDMPF'=+BDMPF!(BDMQUIT)  D
 .I $G(BDMGUI),BDMGUIC W !,"ZZZZZZZ"
 .S BDMGUIC=1
 .W:$D(IOF) @IOF
 .W !,$$CTR("ASSESSMENT PREDIABETES/METABOLIC SYNDROME CARE, FY 2018",80)
 ...
 .W !,$S($G(BDMPPN):"NAME: "_$P($G(^DPT(BDMPF,0)),U),1:"") S J=$S($G(BDMPPN):$L($P(^DPT(BDMPF,0),U))+10,1:0) W ?J,"PRIMARY CARE PROVIDER: ",$$I(15)
 .W !!,"CLASSIFICATION (all that apply):"
 .W !?1,"1 IFG - ",$$I(200)
 .S X=200 F  S X=$O(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)) Q:X>200.99!(X="")  D
 ..W !?3,^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)
 .W !?1,"2 IGT - ",$$I(210)
 .S X=210 F  S X=$O(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)) Q:X>210.99!(X="")  D
 ..W !?3,^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)
 .W !?1,"3 METABOLIC SYNDROME - ",$$I(220)
 .S X=220 F  S X=$O(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)) Q:X>220.99!(X="")  D
 ..W !?3,^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)
 .W !?1,"OTHER ABNORMAL GLUCOSE - ",$$I(230)
 .S X=230 F  S X=$O(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)) Q:X>230.99!(X="")  D
 ..W !?3,^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,X)
 .W !?1,"CMS Register DX: ",$$I(22)_" "_$$I(24)
 .I $D(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,25)) W !?1,^(25)
 .I $D(^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,26)) W !?1,^(26)
...
CUML ;
 I BDMPREP=2!(BDMPREP=3) D CUML^BDMPF14
DONE ;
 K ^TMP($J)
 K ^XTMP("BDMPF1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
I(I) ;
 Q ^XTMP("BDMPF1",BDMJOB,BDMBTH,"AUDIT",BDMPF,I)
 ;
PAGE ;
