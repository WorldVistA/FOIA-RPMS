BDMPF1 ; IHS/CMI/LAB - IHS PRE Diabetes Audit 2015 ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**11**;JUN 14, 2007;Build 30
 ;
BEGIN ;EP - called from option
 D TAXCHK^BDMPF19  ;
 W:$D(IOF) @IOF
 W !!!,$$CTR("ASSESSMENT OF PREDIABETES/METABOLIC SYNDROME CARE, 2018")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMPF1",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
DEMO ;
 S BDMDFMO="I"
 I BDMTYPE'="P" D DEMOCHK^BDMUTL(.BDMDFMO) I BDMDFMO=-1 G IF
 I BDMTYPE="P" S BDMDFMO="I"
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMPF1P",XBRC="^BDMPF10",XBRX="XIT^BDMPF1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMPF1P"")"
 S XBRC="^BDMPF10",XBRX="XIT^BDMPF1",XBIOP=0 D ^XBDBQUE
 Q
P ;
 S BDMSTP=0 K ^XTMP("BDMPF1",BDMJOB,BDMBTH),^TMP($J,"PATS")
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMPF1",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMPF1",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 Q
S ; Get patient name or cohort
 K ^XTMP("BDMPF1",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
 ...
 S BDMCNT=0 F BDMPF=0:0 S BDMPF=$O(^DIBT(+Y,1,BDMPF)) Q:'BDMPF  S BDMCNT=BDMCNT+1,^TMP($J,"PATS",BDMCNT,BDMPF)=""
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDF1B(P,BDMBDAT,BDMADAT,1,1) K ^TMP($J,"PATS",X)
...
PCP ;
 ...
 I Y=-1 G PCP
 S BDMPFP=+Y
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) I $P(^AUPNPAT(P,0),U,14)'=BDMPFP K ^TMP($J,"PATS",X,P)
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMPF1",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMPF1",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
XIT1 ;
 ...
 K ^XTMP("BDMPF1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 ...
 K ^XTMP("BDMPF1",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPFP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDFMO,BDMBEN,BDMDQ) ;PEP - gui call
 F X="BDMJOB","BDMBTH","BDMDMRG","BDMADAT","BDMTYPE","BDMSTMP","BDMPFP","BDMCOM","BDMRAND","BDMRCNT","BDMCMS","BDMSTAT","BDMPREP","BDMFILE","BDMDSP","BDMPPN","BDMDUZ2" S @X=$G(@X)
 ...
 S BDMNOW=$G(%)
 N BDMOPT  ;maw
 S BDMOPT="DM 2018 Pre-Diabetes Audit"  ;maw
 S X=0 F  S X=$O(^TMP($J,"PATS",X)) Q:X'=+X  S P=$O(^TMP($J,"PATS",X,0)) D
 .I BDMBEN=1,$$BEN^AUPNPAT(P,"C")'="01" K ^TMP($J,"PATS",X)
 .I BDMBEN=2,$$BEN^AUPNPAT(P,"C")="01" K ^TMP($J,"PATS",X)
 K DD,D0,DIC
 ...
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMPF1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMPF1P","^TMP($J,""BDMPF1"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMPF1",X)) Q:X'=+X  D
 .S BDMDATA=^TMP($J,"BDMPF1",X)
 .;I BDMDATA="ZZZZZZZ" S BDMDFTA=$C(12)
...
TEST ;
 ...
 F X=1:1:10 S ^XTMP("BDMPF1",BDMJOB,BDMBTH,"PATS",X)=""
 D BDMG^BDMPF1(BDMJOB,BDMBTH,1,DT,"P","","","","","","","",4,"TESTEPI",.BDMIEN)
 Q
