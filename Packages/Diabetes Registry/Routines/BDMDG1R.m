BDMDG1R ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 03 Feb 2014 5:38 PM ;
...
ASPIRIN ;
 ...
 S V=$G(^XTMP("BDMDM19",BDMJOB,BDMBTH,"AUDIT",BDMPD,116))  ;CVD
 I $E(V)=1 S $P(BDMCUML(10,1),U,3)=$P(BDMCUML(10,1),U,3)+1  ;CVD DENOM
 ;CVD AND ASPIRIN
 S A=$G(^XTMP("BDMDM19",BDMJOB,BDMBTH,"AUDIT",BDMPD,62))
...
BPC ;blood pressure control
 ...
 S S=$$SYSMEAN^BDMDG15(BDMPD,BDMRBD,BDMRED)
 S D=$$DIAMEAN^BDMDG15(BDMPD,BDMRBD,BDMRED)
 D
 .I S=""!(D="") Q
...
EDUC ;
 ...
 S V=$G(^XTMP("BDMDM19",BDMJOB,BDMBTH,"AUDIT",BDMPD,44))  ;NUTR
...
HEPC ;
 S V=$G(^XTMP("BDMDM19",BDMJOB,BDMBTH,"AUDIT",BDMPD,222))  ;HEPC DX
 I $E(V)'=1 D
...
LIPID ;
 S V=$E($G(^XTMP("BDMDM19",BDMJOB,BDMBTH,"AUDIT",BDMPD,300)))  ;STATIN
 S C=0
 S A=$$AGE^AUPNPAT(BDMPD,BDMADAT)
 I $E($G(^XTMP("BDMDM19",BDMJOB,BDMBTH,"AUDIT",BDMPD,116)))=1 S C=1  ;CVD
 I C=1!(A>39&(A<76)) D
