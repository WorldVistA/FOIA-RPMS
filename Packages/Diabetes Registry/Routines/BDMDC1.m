BDMDC1 ; IHS/CMI/LAB - IHS Diabetes Audit 2015 ;
 ;;2.0;DIABETES MANAGEMENT SYSTEM;**8,10**;JUN 14, 2007;Build 12
 ;
BEGIN ;EP - called from option
 D TAXCHK^BDMDC19
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2015")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM15",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 K DIR S DIR(0)="S^1:Print Individual Reports;2:Create AUDIT EXPORT file;3:Cumulative Audit Only;4:Both Individual and Cumulative Audits;5:SDPI Key Measures Report",DIR("A")="Enter Print option",DIR("B")="1" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I $D(DIRUT) G TYPE
 S BDMPREP=Y
...
DEMO ;
 ...
 I BDMPREP=2 S XBRP="",XBRC="^BDMDC10",XBRX="XIT^BDMDC1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDC1P",XBRC="^BDMDC10",XBRX="XIT^BDMDC1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDC1P"")"
 S XBRC="^BDMDC10",XBRX="XIT^BDMDC1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF2
P ;
 S BDMSTP=0 K ^XTMP("BDMDM15",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM15",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM15",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 ;
S ; Get patient name or cohort
 K ^XTMP("BDMDM15",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM15",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM15",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
TIME ;PEP - called from BDM Get fiscal year or time frame
 ...
 ;
XIT1 ;
 K ^BDMDCTA($J),^BDMDCTA("BDMEPI",$J)
 K ^XTMP("BDMTAX",BDMJOB,BDMBTH)  ;cmi/maw kill tmp storage of taxonomies
 K ^XTMP("BDMDM15",BDMJOB,BDMBTH),BDMJOB,BDMBTH
XIT ;
...
WRITEF ;EP write flat file
 K ^BDMDCTA($J)
 Q:'$D(^BDMDCTA("BDMEPI",$J))
 ;load in epi definition to ^BDMDCTA($J,"BDM EPI"
 S I=$O(^BDMRECD("B","DM AUDIT 2015 EXPORT RECORD",0))
 ...
 S ^BDMDCTA($J,1)=N
 S N=1
 ;MOVE RECORDS TO ^BDMDCTA($J,"BDM EPI"
 S X=0 F  S X=$O(^BDMDCTA("BDMEPI",$J,X)) Q:X'=+X  S N=N+1,^BDMDCTA($J,N)=^BDMDCTA("BDMEPI",$J,X)
 K ^BDMDCTA("BDMEPI",$J)
 S XBGL="BDMDCTA("
 K XBUF I $P($G(^APCCCTRL($S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0)),U,11)]"" S XBUF=$P(^APCCCTRL($S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0),U,11)
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2015 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
 ...
 K ^XTMP("BDMDM15",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPCP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2015 Diabetes Program Audit"
 I BDMPREP=5 S BDMOPT="SDPI Key Measures Report"
 D NOW^%DTC
 S BDMNOW=$G(%)
 K DD,D0,DIC
 S X=DUZ_"."_BDMBTH
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDC1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDC1P","^TMP($J,""BDMDM15"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM15",X)) Q:X'=+X  D
 .S BDMDCTA=^TMP($J,"BDMDM15",X)
 .;I BDMDCTA="ZZZZZZZ" S BDMDCTA=$C(12)
 .S ^BDMGUI(BDMIEN,11,X,0)=BDMDCTA,C=C+1
