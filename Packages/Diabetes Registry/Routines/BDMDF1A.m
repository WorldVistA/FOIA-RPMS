BDMDF1A ; IHS/CMI/LAB -IHS -CUMULATIVE REPORT 14 Dec 2016 11:59 AM ; 
...
ACE ;
 ...
 W !?3,"In patients with known hypertension [3]",?49,$$C($P(V,U,7)),?61,$$C($P(V,U,5)),?73,$$P($P(V,U,5),$P(V,U,7))
 W !?3,"In patients age 18+ with CKD [1]",?49,$$C($P(V,U,8)),?61,$$C($P(V,U,4)),?73,$$P($P(V,U,4),$P(V,U,8))  ;,!?10,"urine albumin excretion**"
ASPIRIN ;
...
LDL ;
 ...
 W !?7,"LDL 100-189 mg/dl",?49,$$C($P(V,U,4)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,4))
...
TRIG ;
 ...
 W !!?3,"Triglycerides [4]",?49,$$C(T),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),T)
 W !?7,"TG <150 mg/dl",?49,$$C($P(V,U,3)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,3))
 W !?7,"TG 150-499 mg/dl",?49,$$C($P(V,U,8)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,8))
 W !?7,"TG 500-999 mg/dl",?49,$$C($P(V,U,9)),?61,$$C($P(V,U,2)),?73,$$P($P(V,U,2),$P(V,U,9))
...
SDM ;
 I $Y>(BDMIOSL-8) D HEADER Q:BDMQUIT
 W !!,"[1] CKD: eGFR <60 or UACR =>30"
 W !,"[2] Comorbid conditions counted are: active depression, current tobacco use,",!,"severely obese (BMI 40 or higher), diagnosed hypertension, diagnosed CVD,",!,"CKD (eGFR<60 or uACR>=30), hepatitis C, and retinopathy."
 W !,"[3] Known hypertension: Has hypertension listed as an active problem, or ",!,"three visits with a diagnosis of hypertension ever (prior to the end ",!,"of the Audit period)."
 W !,"[4] For triglycerides: >150 is a marker of CVD risk, not a treatment",!,"target; >1000 is a risk marker for pancreatitis."
 W !
 Q
EXIT ;
