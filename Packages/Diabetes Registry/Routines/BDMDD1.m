BDMDD1 ; IHS/CMI/LAB - IHS Diabetes Audit 2016 ;
...
BEGIN ;EP - called from option
 ...
 W:$D(IOF) @IOF
REGASK ;
 W !!!,$$CTR("ASSESSMENT OF DIABETES CARE, 2016")
...
TYPE ;
 S BDMSTP=0
 K ^XTMP("BDMDM16",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMTYPE=""
...
IF ;PEP - called from BDM indivdual or epi
 S BDMSTP=0
 K DIR
 S DIR(0)="S^1:Print Individual Reports;2:Create AUDIT EXPORT file;3:Cumulative Audit Only;4:Both Individual and Cumulative Audits;5:SDPI Key Measures Report (2015 Version);6:SDPI Key Measures Report (2016 Version)"
...
DEMO ;
 ...
 I BDMPREP=2 S XBRP="",XBRC="^BDMDD10",XBRX="XIT^BDMDD1",XBNS="BDM" D ^XBDBQUE,XIT Q
 W ! S DIR(0)="S^P:PRINT Output;B:BROWSE Output on Screen",DIR("A")="Do you wish to",DIR("B")="P" K DA D ^DIR K DIR
 I $D(DIRUT) D XIT Q
 S BDMOPT=Y
 I Y="B" D BROWSE,XIT Q
 S XBRP="^BDMDD1P",XBRC="^BDMDD10",XBRX="XIT^BDMDD1",XBNS="BDM"
 D ^XBDBQUE
 D XIT
 Q
BROWSE ;
 S XBRP="VIEWR^XBLM(""^BDMDD1P"")"
 S XBRC="^BDMDD10",XBRX="XIT^BDMDD1",XBIOP=0 D ^XBDBQUE
 Q
IAEP ;EP
 S BDMTYPE="P"
 S BDMPREP=1
 ...
 G IF2
P ;
 S BDMSTP=0 K ^XTMP("BDMDM16",BDMJOB,BDMBTH),^TMP($J,"PATS")
 S BDMBEN=3
P1 ;
 K DIC S DIC="^AUPNPAT(",DIC(0)="AEMQ" D ^DIC K DIC
 I Y=-1,'$D(^XTMP("BDMDM16",BDMJOB,BDMBTH,"PATS")) W !,"No patients selected" S BDMSTP=1 Q
 I Y=-1 Q
 S ^XTMP("BDMDM16",BDMJOB,BDMBTH,"PATS",+Y)=""
 G P1
 ;
S ; Get patient name or cohort
 K ^XTMP("BDMDM16",BDMJOB,BDMBTH),^TMP($J,"PATS") S BDMSTP=0
...
PREG ;
 ...
 .Q:$P(^DPT(P,0),U,2)'="F"
 .;W ".",P
 .I $$PREG^BDMDD1B(P,BDMBDAT,BDMADAT,1,1) K ^TMP($J,"PATS",X)
...
RAND ;random sample or not
 ...
 G:$D(DIRUT) C
 I Y="A" S C=0 F  S C=$O(^TMP($J,"PATS",C)) Q:C'=+C  S X=$O(^TMP($J,"PATS",C,0)),^XTMP("BDMDM16",BDMJOB,BDMBTH,"PATS",X)=""
 ...
 I $D(DIRUT) S BDMSTP=1 Q
 S C=0 F N=1:1:BDMCNT Q:C=Y  S I=$R(BDMCNT) I I,$D(^TMP($J,"PATS",I)) S X=$O(^TMP($J,"PATS",I,0)),^XTMP("BDMDM16",BDMJOB,BDMBTH,"PATS",X)="",C=C+1 K ^TMP($J,"PATS",I,X)
 K ^TMP($J,"PATS")
 Q
...
WRITEF ;EP write flat file
 K ^BDMDDTA($J)
 Q:'$D(^BDMDDTA("BDMEPI",$J))
 ;load in epi definition to ^BDMDDTA($J,"BDM EPI"
 S I=$O(^BDMRECD("B","DM AUDIT 2016 EXPORT RECORD",0))
 ...
 S ^BDMDDTA($J,1)=N
 S N=1
 ;MOVE RECORDS TO ^BDMDDTA($J,"BDM EPI"
 S X=0 F  S X=$O(^BDMDDTA("BDMEPI",$J,X)) Q:X'=+X  S N=N+1,^BDMDDTA($J,N)=^BDMDDTA("BDMEPI",$J,X)
 K ^BDMDDTA("BDMEPI",$J)
 S XBGL="BDMDDTA("
 K XBUF I $P($G(^APCCCTRL($S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0)),U,11)]"" S XBUF=$P(^APCCCTRL($S($G(BDMDUZ2):BDMDUZ2,1:DUZ(2)),0),U,11)
 S XBMED="F",XBFN=BDMFILE,XBTLE="SAVE OF DM AUDIT 2016 EXPORT RECORDS GENERATED BY -"_$P(^VA(200,DUZ,0),U)
 ...
 K ^XTMP("BDMDM16",BDMJOB,BDMBTH),BDMJOB,BDMBTH
 Q
BDMG(BDMJOB,BDMBTH,BDMDMRG,BDMADAT,BDMTYPE,BDMSTMP,BDMPDP,BDMCOM,BDMRAND,BDMRCNT,BDMCMS,BDMSTAT,BDMPREP,BDMFILE,BDMDSP,BDMGIEN,BDMSDPI,BDMSDPG,BDMPPN,BDMDUZ2,BDMDEMO,BDMBEN) ;PEP - gui call
 ...
 N BDMOPT  ;maw
 S BDMOPT="2016 Diabetes Program Audit"
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BDMDD1",ZTDESC="GUI DM AUDIT" D ^%ZTLOAD
 D XIT
 Q
GUIEP ;EP - called from taskman
 ...
 D GUIR^XBLM("^BDMDD1P","^TMP($J,""BDMDM16"",")
 Q:$G(BDMDSP)  ;quit if to screen
 S X=0,C=0 F  S X=$O(^TMP($J,"BDMDM16",X)) Q:X'=+X  D
 .S BDMDDTA=^TMP($J,"BDMDM16",X)
 .;I BDMDDTA="ZZZZZZZ" S BDMDDTA=$C(12)
 .S ^BDMGUI(BDMIEN,11,X,0)=BDMDDTA,C=C+1
