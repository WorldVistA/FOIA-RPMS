BUDDRP6I ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE 6B 14 Jan 2016 2:27 PM ; 
...
S(V) ;
 ...
 ;
LISTS ;EP
 I $G(BUDPRGAL) S BUDGPG=0 D PRGAL^BUDDRP6W
 I $G(BUDIMM1L) S BUDGPG=0 D IMM1L
 I $G(BUDIMM2L) S BUDGPG=0 D IMM2L
 I $G(BUDPAP1L) S BUDGPG=0 D PAP1L^BUDDRP6P
 I $G(BUDPAP2L) S BUDGPG=0 D PAP2L^BUDDRP6P
 I $G(BUDWAC1L) S BUDGPG=0 D WAC1L^BUDDRP6F
 I $G(BUDWAC2L) S BUDGPG=0 D WAC2L^BUDDRP6F
 I $G(BUDAWS1L) S BUDGPG=0 D AWS1L^BUDDRP6G
 I $G(BUDAWS2L) S BUDGPG=0 D AWS2L^BUDDRP6G
 I $G(BUDTUA1L) S BUDGPG=0 D TUA1L^BUDDRP6T
 I $G(BUDTUA2L) S BUDGPG=0 D TUA2L^BUDDRP6T
 I $G(BUDAPT1L) S BUDGPG=0 D APT1L^BUDDRP6S
 I $G(BUDAPT2L) S BUDGPG=0 D APT2L^BUDDRP6S
 I $G(BUDCAD1L) S BUDGPG=0 D CAD1L^BUDDRP6J
 I $G(BUDCAD2L) S BUDGPG=0 D CAD2L^BUDDRP6J
 I $G(BUDIVD1L) S BUDGPG=0 D IVD1L^BUDDRP6K
 I $G(BUDIVD2L) S BUDGPG=0 D IVD2L^BUDDRP6K
 I $G(BUDDRC1L) S BUDGPG=0 D CRC1L^BUDDRP6L
 I $G(BUDDRC2L) S BUDGPG=0 D CRC2L^BUDDRP6L
 I $G(BUDHIV1L) S BUDGPG=0 D HIV1L^BUDDRP6A
 I $G(BUDHIV2L) S BUDGPG=0 D HIV2L^BUDDRP6A
 I $G(BUDDEP1L) S BUDGPG=0 D DEP1L^BUDDRP6N
 I $G(BUDDEP2L) S BUDGPG=0 D DEP2L^BUDDRP6N
 I $G(BUDDS1L) S BUDGPG=0 D DS1L^BUDDRP61
 I $G(BUDDS2L) S BUDGPG=0 D DS2L^BUDDRP61
 Q
PAUSE ;
...
IMM1L ;EP
 ...
 D IMM1H Q:BUDQUIT
 I '$D(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM1")) W:BUDROT="P" !!,"No patients to report." D:BUDROT="D" S() D:BUDROT="D" S("No patients to report.") Q
 D IMM1L1
...
IMM1L1 ;
 ...
 S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM1",BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 .S BUDCCOM="" F  S BUDCCOM=$O(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM1",BUDNAME,BUDCCOM)) Q:BUDCCOM=""!(BUDQUIT)  D
 ..S DFN=0 F  S DFN=$O(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM1",BUDNAME,BUDCCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...I BUDROT="P",$Y>(IOSL-3) D IMM1H Q:BUDQUIT
 ...
 ...S BUDTOT=BUDTOT+1
 ...S BUDALL=$P(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM1",BUDNAME,BUDCCOM,DFN),"|||",1)
...
IMM2L ;EP
 S BUDP=0,BUDQUIT=0,BUDTOT=0
 D IMM2H Q:BUDQUIT
 I '$D(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM2")) W:BUDROT="P" !!,"No patients to report." D:BUDROT="D" S() D:BUDROT="D" S("No patients to report.") Q
 D IMM2L1
...
IMM2L1 ;
 ...
 S BUDNAME="" F  S BUDNAME=$O(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM2",BUDNAME)) Q:BUDNAME=""!(BUDQUIT)  D
 .S BUDCCOM="" F  S BUDCCOM=$O(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM2",BUDNAME,BUDCCOM)) Q:BUDCCOM=""!(BUDQUIT)  D
 ..S DFN=0 F  S DFN=$O(^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM2",BUDNAME,BUDCCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ...I BUDROT="P",$Y>(IOSL-3) D IMM2H Q:BUDQUIT
 ...I BUDROT="P" W $E($P(^DPT(DFN,0),U,1),1,25),?29,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?41,$E(BUDCCOM,1,25),?70,$P(^DPT(DFN,0),U,2),?75,$$AGE^AUPNPAT(DFN,BUDED),!
 ...S BUDTOT=BUDTOT+1
 ...S BUDALL=^XTMP("BUDDRP6B",BUDJ,BUDH,"IMM2",BUDNAME,BUDCCOM,DFN)
 ...S X="" F BUDX=1:1 S BUDV=$P(BUDALL,U,BUDX) Q:BUDV=""!(BUDQUIT)  D
 ....I BUDROT="P",$Y>(IOSL-3) D IMM2H Q:BUDQUIT
 ....I BUDROT="P" W ?4,BUDV,!
 ....I BUDROT="D" S X=X_$S(X]"":"; ",1:"")_BUDV
