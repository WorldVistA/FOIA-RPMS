BUDERPC2 ;IHS/CMI/LAB - UDS TABLE 6A;
...
L26C(BUDV) ;EP
 ...
 I C,$D(^BUDETSC(BUDY,11,"B",C)) Q "CLIN "_C
...
L26(BUDV) ;EP
 ...
 .I $D(^BUDETSC(BUDY,8,"B",Z)) S G=Z Q
 Q G
L26A(BUDV) ;EP
 ;age 9-72 months
 S G=""
 S A=$$AGE^BUDEUTL2(DFN,2,$$VD^APCLV(BUDV))
 I A<9 Q G
 I A>72 Q G
 S G="" S X=0 F  S X=$O(^AUPNVCPT("AD",BUDV,X)) Q:X'=+X!(G]"")  D
 .S Z=$$VAL^XBDIQ1(9000010.18,X,.01)
...
SHI(V,LV) ;
 ...
 I $G(BUDT4IPP) D
 .S ^XTMP("BUDERPT1",BUDJ,BUDH,"T4IPPL",$G(V),$G(LV),BUDCCOM,BUDSEX,BUDAGE,DFN)=$G(LV)
 Q
HI ;
...
TINS(A,P) ;
 ...
 .S ^XTMP("BUDERPT1",BUDJ,BUDH,"T4PMIS",$G(P),$G(T),BUDAGE,BUDCCOM,BUDSEX,DFN)=""
 Q
 ;
MCD(P,D,T) ;EP - Is patient P medicaid eligible on date D.
 ...
 .I Z,$D(^BUDESITE(BUDSITE,12,"B",Z)),$P(^BUDESITE(BUDSITE,12,Z,0),U,2)="NI" Q
 .I Z,$D(^BUDESITE(BUDSITE,12,"B",Z)),$P(^BUDESITE(BUDSITE,12,Z,0),U,2)'="MCD" Q  ;not to be used as MCD
 ...
 . Q:$P(^BUDESITE(BUDSITE,12,Z,0),U,2)'="MCD"
...
MCR(P,D) ;EP - Is patient P medicare eligible on date D.  1 = yes, 0 = no.
 ...
 . S Z=$P(^AUPNMCR(P,11,I,0),U,4)
 . I Z,$D(^BUDESITE(BUDSITE,12,"B",Z)),$P(^BUDESITE(BUDSITE,12,Z,0),U,2)="NI" Q
 . I Z,$D(^BUDESITE(BUDSITE,12,"B",Z)),$P(^BUDESITE(BUDSITE,12,Z,0),U,2)'="MCR" Q
 . S Y=1
 .Q
MCRX ;
 I Y Q Y
 I '$$OV^BUDERPC3(BUDSITE,"MCR") Q Y
 ...
 . Q:$P(^AUPNPRVT(P,11,I,0),U)=""
 . S X=$P(^AUPNPRVT(P,11,I,0),U) Q:X=""
 . I $D(^BUDESITE(BUDSITE(12,"B",X))) S T=$P(^BUDESITE(BUDSITE,12,X,0),U,2) I T'="MCR" Q  ;not supposed to be medicare
 ...
 .Q:$P(^BUDESITE(BUDSITE,12,N,0),U,2)'="MCR"  ;not to be counted as PI
 .;I N S N=$$VALI^XBDIQ1(9999999.18,N,.21)
 .;I T="K" Q:N'="K"
 .;I T=""  Q:N="K"
 .S J=0 F  S J=$O(^AUPNMCD(I,11,J)) Q:J'=+J  D
...
MIGRANT ;
 ...
 .Q:'$G(BUDT4CHA)
 .S T="Migratory" S ^XTMP("BUDERPT1",BUDJ,BUDH,"T4CHAR",14,T,BUDCCOM,BUDSEX,BUDAGE,DFN)=""
 I $P(M,U,1)="S" S BUDT4V(15)=BUDT4V(15)+1,BUDT4V(16)=BUDT4V(16)+1 D
 .Q:'$G(BUDT4CHA)
 .S T="Seasonal"
 .S ^XTMP("BUDERPT1",BUDJ,BUDH,"T4CHAR",15,T,BUDCCOM,BUDSEX,BUDAGE,DFN)=""
 Q
HOMELESS ;
 ...
 S ^XTMP("BUDERPT1",BUDJ,BUDH,"T4CHAR",L,T,BUDCCOM,BUDSEX,BUDAGE,DFN)=""
 Q
VET ;
 S V=$$VALI^XBDIQ1(2,DFN,1901)
 I V="Y" S T="Veteran" S BUDT4V(25)=BUDT4V(25)+1,^XTMP("BUDERPT1",BUDJ,BUDH,"T4CHAR",25,T,BUDCCOM,BUDSEX,BUDAGE,DFN)=""
 Q
SCHOOL ;IF ALL VISITS ARE CLINIC SCHOOL??
 ...
 S ^XTMP("BUDERPT1",BUDJ,BUDH,"T4CHAR",24,"School Based Patient",BUDCCOM,BUDSEX,BUDAGE,DFN)=""
 Q
MIG(P,D) ;EP
