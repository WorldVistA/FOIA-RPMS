BUDERP71 ; IHS/CMI/LAB - UDS REPORT DRIVER TABLE ;
...
PL D LISTS
 ...
 K ^XTMP("BUDERP7",BUDJ,BUDH)
 Q
P71 ;
 D HEADER^BUDERPTP Q:BUDQUIT  D T7H
...
R2C ;EP
 ...
 Q
LISTS ;
 I $G(BUDPRGHL) S BUDGPG=0 D PRGHL^BUDERP7I
 I $G(BUDPRGRL) S BUDGPG=0 D PRGRL^BUDERP7I
 I $G(BUDHTRL) S BUDGPG=0 D HTRL^BUDERP7J
 I $G(BUDHTCRL) S BUDGPG=0 D HTCRL^BUDERP7J
 I $G(BUDHTURL) S BUDGPG=0 D HTURL^BUDERP7U
 I $G(BUDDMRL) S BUDGPG=0 D DMRL^BUDERP7L
 I $G(BUDDMR1L) S BUDGPG=0 D DMR1L^BUDERP7L
 I $G(BUDDMR2L) S BUDGPG=0 D DMR2L^BUDERP7T
 K ^XTMP("BUDERP7",BUDJ,BUDH)
 Q
 ;
 ;
