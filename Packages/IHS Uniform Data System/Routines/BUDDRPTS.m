BUDDRPTS ; IHS/CMI/LAB - UDS REPORT DRIVER 12 Dec 2016 8:11 AM 01 Dec 2016 6:01 PM ; 
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUDDRPTS",ZTDTH="",ZTDESC="UDS 16 REPORT SEARCH TEMPLATE" D ^%ZTLOAD D EOJ Q
 Q
PRINTCP ;
 W !
 S X=$$CTR($P(^DIC(4,BUDSITE,0),U),60),$E(X,3)=$P(^VA(200,DUZ,0),U,2),$E(X,10)="UDS 2016",$E(X,70)="Cover Page" W !,X
...
T3AI ;intro for table 3A
 W !!,"TABLE 3A:  PATIENTS BY AGE & SEX ASSIGNED AT BIRTH ",!,"WITH SEARCH TEMPLATE CREATION"
 W !,"This report will produce UDS Table 3A, an itemization of users (patients) by age"
 W !,"and sex assigned at birth.  Users must have at least one visit during the "
 W !,"selected time period as defined above.  ",!,"Age is calculated as of June 30th of the year you select.",!
...
GENI ;general introductions
 ...
 W !
 D PAUSE
 W !,"TABLE 3A: PATIENTS BY AGE & SEX ASSIGNED AT BIRTH W/ SEARCH TEMPLATE CREATION"
 ...
 W !,"(patients) by age and sex assigned at birth."
 W !
 Q
 ;
PAUSE ;
...
NRY ;
 ...
 Q
STEMP ;EP
 S X=0 F  S X=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"3ATEMP",X)) Q:X'=+X  S ^DIBT(BUDSTMP,1,X)=""
 Q
