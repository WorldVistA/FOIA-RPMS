BUDHRP7A ; IHS/CMI/LAB - UDS REPORT PROCESSOR ;
...
PRGHLST ;EP - list of pregnant females
 ...
 Q:BUDSEX'="F"
 S BUDP=$$PREG^BUDHRP6B(DFN,$$FMADD^XLFDT(BUDED,-609),BUDED,BUDBD)
 ...
 I BUDRACEP=8,(+BUDHTHN=3) S BUDR=17 G SETSECTH  ;BOTH BLANK OR REFUSED
 I +BUDHTHN=1 S BUDR=BUDRACEP G SETSECTH
 I +BUDHTHN=2!(+BUDHTHN=3) S BUDR=BUDRACEP+8
SETSECTH ;
 S $P(BUDSECTH(1),U,BUDR)=$P($G(BUDSECTH(1)),U,BUDR)+1
 S $P(BUDSECTH(1),U,18)=$P($G(BUDSECTH(1)),U,18)+1
 S ^XTMP("BUDHRP7",BUDJ,BUDH,"PRGH",BUDAGEP,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDP,"*",2)_"#"_$P(BUDHIV,"*",2)
 Q
 ;
...
PROB S T=$O(^ATXAX("B","BGP PREGNANCY DIAGNOSES 2",0))
 ...
 S BUDD=G,BUDALL=BUDALL_"Problem List: "_$$VAL^XBDIQ1(9000011,Z,.01)_" on "_$$DATE^BUDHUTL1(G)
...
PRGRLST ;EP - list of pregnant females
 ...
 I '$P(BUDP,U) Q  ;not pregnant
 S BUDHISPN=$$HISP^BUDHRPTC(DFN)
 S BUDHISP=$P($$HISP^BUDHRPTC(DFN),U,1)  ;1=hispanic 2=non hispanic
 ;S BUDHISP1=BUDHISP+2  ;set piece
 ;
 S BUDR1=$$RACE^BUDHRPTC(DFN)
 S BUDR=$P(BUDR1,U,1)  ;LINE
 S ^XTMP("BUDHRP7",BUDJ,BUDH,"PRGR",$P(BUDR1,U,5),BUDHISP,BUDCCOM,BUDAGEP,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"*",2)
 Q
 ;
PRGELST ;EP - list of pregnant females
 ...
 I '$P(BUDP,U) Q  ;not pregnant
 S BUDRACE=$$HISP^BUDHRPTC(DFN)
 I +BUDRACE=1 S BUDRACE="Hispanic or Latino"
 I +BUDRACE=2 S BUDRACE="All Others"
 S ^XTMP("BUDHRP7",BUDJ,BUDH,"PRGE",BUDRACE,BUDCCOM,BUDAGEP,$P(^DPT(DFN,0),U),DFN)=$P(BUDP,"*",2)
 Q
 ;
