...
PROC ;EP - called from xbdbque
 ...
 K ^BUDETSC("AC")
 S BUD1=0 F  S BUD1=$O(^BUDETSC(BUD1)) Q:BUD1'=+BUD1  D
 .S T=$P(^BUDETSC(BUD1,0),U,4)
 .Q:T=""
 .K ^BUDETSC(BUD1,7)
 .K BUDTAX
 ...
 ..S C=C+1
 ..S ^BUDETSC(BUD1,7,C,0)=$P(BUDTAX(X),U,1)_U_X_U_$P(BUDTAX(X),U,3)
 .S ^BUDETSC(BUD1,7,0)="^90669.77^"_C_"^"_C
 .S DIK="^BUDETSC(",DA=BUD1 D IX1^DIK
 S ^XTMP("BUDERPT1",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"UDS REPORT"
 S ^XTMP("BUDERP6B",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^UDS TABLE 6B LISTS"
 S ^XTMP("BUDERP7",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BUD TABLE 7 LISTS"
