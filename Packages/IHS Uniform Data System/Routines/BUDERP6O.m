BUDERP6O ; IHS/CMI/LAB - UDS REPORT PROCESSOR ;
...
I ;EP ;CAD
 ...
 I BUD18TH'=BUDED,'$$VBBD^BUDERP6V(DFN,BUD18TH,BUDED) Q  ;quit if no visiT AFTER 18TH BIRTHDAY
 I BUD18TH=BUDED,'$$VBBD^BUDERP6V(DFN,BUD18TH,BUDED) Q  ;quit if no visiT AFTER 18TH BIRTHDAY
 ...
 I X Q
 ;I X S ^XTMP("BUDERP6B",BUDJ,BUDH,"ALG","CAD2",BUDAGE,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDCADV,U)_U_$P(X,U,2)_U_BUDLDL Q  ;eliminate those with an allergy to a LIPID LOWERING DRUG
 ;I BUDCCADVU="" S X="",X=$$LIPITHER(DFN,BUDBD,BUDED) I X]"" S ^XTMP("BUDERP6B",BUDJ,BUDH,"ALG","CAD2",BUDAGE,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDCCADVU,U)_U_$P(X,U,2) Q
 ...
 .I $G(BUDCAD2L) D
 ..I BUDCADT="" S ^XTMP("BUDERP6B",BUDJ,BUDH,"CAD2",BUDAGE,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDCADV,U)_U_$P(BUDCADV,U,2)_U_$P(BUDLDL,U,1)_" "_$$DATE^BUDEUTL1($P(BUDLDL,U,2))_U_$P(BUDCADT,U,2)
 .I $G(BUDCAD1L) D
 ..I BUDCADT]"" S ^XTMP("BUDERP6B",BUDJ,BUDH,"CAD1",BUDAGE,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDCADV,U)_U_$P(BUDCADV,U,2)_U_$P(BUDLDL,U,1)_" "_$$DATE^BUDEUTL1($P(BUDLDL,U,2))_U_$P(BUDCADT,U,2)
 Q
LIPIDALG(P,ED) ;
 ;allergy tracking
...
PROBCAD(P,BDATE,EDATE) ;EP
 NEW G
 S G=$$PLCL^BUDEDU(P,"CAD DIAGNOSES")
 I 'G Q ""
...
CAD(P,BDATE,EDATE) ;EP
 ...
 S TIEN=$O(^BUDETSSC("B","T6B CAD DIAGNOSES",0))
 S TIEN1=$O(^BUDETSSC("B","T6B CAD SURGICAL DIAGNOSES",0))
 ...
 ..Q:Y=""
 ..I $D(^BUDETSSC("AC",Y,TIEN)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" CPT: "_Y Q
 ..I $D(^BUDETSSC("AC",Y,TIEN1)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" CPT: "_Y Q
 .;V TRANS
 .S X=0 F  S X=$O(^AUPNVTC("AD",VIEN,X)) Q:X'=+X  D
 ..Q:'$D(^AUPNVTC(X,0))
 ..S Y=$$VAL^XBDIQ1(9000010.33,X,.07)
 ..Q:Y=""
 ..I $D(^BUDETSSC("AC",Y,TIEN)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" CPT/TRAN: "_Y Q
 ..I $D(^BUDETSSC("AC",Y,TIEN1)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" CPT/TRAN: "_Y Q
 .;POV/SNOMED
 .S X=0 F  S X=$O(^AUPNVPOV("AD",VIEN,X)) Q:X'=+X  D
 ..Q:'$D(^AUPNVPOV(X,0))
 ..S Y=$$VALI^XBDIQ1(9000010.07,X,.01)
 ..I $D(^BUDETSSC("AD",Y,TIEN)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" DX: "_$$VAL^XBDIQ1(9000010.07,X,.01) Q
 ..I $D(^BUDETSSC("AD",Y,TIEN1)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" DX: "_$$VAL^XBDIQ1(9000010.07,X,.01) Q
 ..S Y=$$VAL^XBDIQ1(9000010.07,X,1101)
 ..Q:Y=""
 ..I $D(^BUDETSSC("AS",Y,TIEN)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" SNOMED: "_Y Q
 ..I $D(^BUDETSSC("AS",Y,TIEN1)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" SNOMED: "_Y Q
 .;PROC
 .S X=0 F  S X=$O(^AUPNVPRC("AD",VIEN,X)) Q:X'=+X  D
 ..Q:'$D(^AUPNVPRC(X,0))
 ..S Y=$$VALI^XBDIQ1(9000010.08,X,.01)
 ..I $D(^BUDETSSC("AP",Y,TIEN)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" PROC: "_$$VAL^XBDIQ1(9000010.08,X,.01) Q
 ..I $D(^BUDETSSC("AP",Y,TIEN1)) S BUDAST=$$DATE^BUDEUTL1(VDATE)_" PROC: "_$$VAL^XBDIQ1(9000010.08,X,.01) Q
 I BUDAST]"" Q BUDAST  ; S X=$O(BUDAST(0)),X=BUDAST(X) Q X
 S Y=$$PLCL^BUDEDU(P,"T6B CAD DIAGNOSES",EDATE,1)
 I Y Q "PL "_$P(Y,U,2)_" on "_$$DATE^BUDEUTL1($P(Y,U,3))_U_$P(X,U,3)
 S Y=$$PLCL^BUDEDU(P,"T6B CAD SURGICAL DIAGNOSES",EDATE,1)
 I Y Q "PL "_$P(Y,U,2)_" on "_$$DATE^BUDEUTL1($P(Y,U,3))_U_$P(X,U,3)
 ...
 ..Q:'$D(^BUDETSSC("AP",X,TIEN1))
 ..S G=$$DATE^BUDEUTL1((9999999-D))_" PROC: "_$$VAL^XBDIQ1(9000010.08,Y,.01) Q
 I G]"" Q G
 S X="" F  S X=$O(^BUDETSSC(TIEN1,14,"B",X)) Q:X=""!(G]"")  D
 .S Y=+$$CODEN^ICPTCOD(X)
 .S Z=$$CPTI^BUDEDU(P,$$DOB^AUPNPAT(P),BUDED,Y) I Z S G=$$DATE^BUDEUTL1($P(Z,U,2))_" CPT: "_X
 Q ""
 S Y=$$LASTPRC^BUDEUTL1(P,"BUD CABG PTCA PROCS",$$DOB^AUPNPAT(P),EDATE)
...
LIPITHER(P,BD,ED) ;
 ...
 S G=""
 D GETMEDS^BUDEUTL2(P,BD,ED,"BUD LIPID LOWERING MEDS","BGPMU LIPID LOWERING NDCS",,,.BUDMEDS1)
...
EHRO ;EPRES
 ;EHR OUTSIDE
 S C=$$PRES^BUDERP6W(P,$O(^ATXAX("B","BUD LIPID LOWERING MEDS",0)),BD,ED,$O(^ATXAX("B","BGPMU LIPID LOWERING NDCS",0)))
...
GETV(P,BD,ED,SITE) ;EP - get all visits for this patient and COUNT MEDICAL VISITS
 ...
 .Q:VLOC=""
 .Q:'$D(^BUDESITE(SITE,11,VLOC))  ;not valid location
 .Q:"AHSORMI"'[$P(^AUPNVSIT(VSIT,0),U,7)
 .S CLINC=$$CLINIC^APCLV(VSIT,"C")
 .S TIEN=$O(^BUDECNTL("B","UDS PT/TABLE 5 CLINIC EXCL",0))
 .I CLINC]"",$D(^BUDECNTL(TIEN,11,"B",CLINC)) Q  ;not a clinic code we want in any table
