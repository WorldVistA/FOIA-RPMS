BUDERP6U ; IHS/CMI/LAB - UDS REPORT PROCESSOR ;
...
H ;EP ; ASTHMA
 ...
 .I $G(BUDAPT2L) D
 ..I BUDASTT="" S ^XTMP("BUDERP6B",BUDJ,BUDH,"APT2",BUDAGE,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDAST,U)_U_$P(BUDASTT,U,2)
 .I $G(BUDAPT1L) D
 ..I BUDASTT]"" S ^XTMP("BUDERP6B",BUDJ,BUDH,"APT1",BUDAGE,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDAST,U)_U_$P(BUDASTT,U,2)
 Q
SABA(P,BD,ED) ;
 NEW BUDMEDS1,G,A,C,M,V,V1D,BUDHSABA
 S G="",BUDHSABA=""
 D GETMEDS^BUDEUTL2(P,BD,ED,"BGP PQA SABA MEDS","BGP PQA SABA NDC",,,.BUDMEDS1)
 ...
 I 'BUDHSABA Q ""
 S G=""
 D GETMEDS^BUDEUTL2(P,BD,ED,"BGP PQA CONTROLLER MEDS","BGP PQA CONTROLLER NDC",,,.BUDMEDS1)
...
ASTHMA(P,BDATE,EDATE) ;EP
 ...
 S TIEN=$O(^BUDETSSC("B","T6B ASTHMA PERSISTENT CODES",0))
 ...
 ..Q:Y=""
 ..I $D(^BUDETSSC("AC",Y,TIEN)) S BUDAST(9999999-VDATE)=Y_U_$$DATE^BUDEUTL1(VDATE)_U_VDATE Q
 .;V TRANS
 ...
 ..Q:'$D(^AUPNVPOV(X,0))
 ..S Y=$$VALI^XBDIQ1(9000010.07,X,.01) I $D(^BUDETSSC("AD",Y,TIEN)) S BUDAST(9999999-VDATE)=$$VAL^XBDIQ1(9000010.07,X,.01)_U_$$DATE^BUDEUTL1(VDATE)_U_VDATE Q
 ..S Y=$$VAL^XBDIQ1(9000010.07,X,1101)
 ..Q:Y=""
 ..I $D(^BUDETSSC("AS",Y,TIEN)) S BUDAST(9999999-VDATE)=Y_U_$$DATE^BUDEUTL1(VDATE)_U_VDATE Q
 I $O(BUDAST(0)) S X=$O(BUDAST(0)),X=BUDAST(X) Q $P(X,U,1)_" on "_$P(X,U,2)_U_$P(X,U,3)
 S Y=$$PLCL^BUDEDU(P,"T6B ASTHMA PERSISTENT CODES",EDATE,1)
...
EXCL(P,BDATE,EDATE) ;EP - EMPHYSEMA, COPD, CYSTIC FIBROSIS, ARF
 ...
 S BUDEX=0
 D ALLV^APCLAPIU(P,BDATE,EDATE,"BUDVS")
 S TIEN=$O(^BUDETSSC("B","T6B ASTHMA EXCLUSION CODES",0))
 ...
 ..Q:'$D(^AUPNVPOV(X,0))
 ..S Y=$$VALI^XBDIQ1(9000010.07,X,.01) I $D(^BUDETSSC("AD",Y,TIEN)) S BUDEX=1 Q
 ..S Y=$$VAL^XBDIQ1(9000010.07,X,1101)
 ..Q:Y=""
 ..I $D(^BUDETSSC("AS",Y,TIEN)) S BUDEX=1 Q
 I BUDEX Q 1
 S Y=$$PLCL^BUDEDU(P,"T6B ASTHMA EXCLUSION CODES",EDATE,1)
 I Y Q 1
 Q ""
 ;
AST1039(P,BDATE,EDATE) ;EP
 ...
 I '$D(^TMP($J,"A",1)) Q ""
 S Y=""
 S Y=$$CPTI^BUDEDU(P,BDATE,EDATE,$P($$CPT^ICPTCOD("1039F"),U,1))
...
EHRO ;EPRES
 ;EHR OUTSIDE
 S C=$$PRES^BUDERP6W(P,$O(^ATXAX("B","BGP PQA CONTROLLER MEDS",0)),BD,ED,$O(^ATXAX("B","BGP PQA CONTROLLER NDC",0)))
