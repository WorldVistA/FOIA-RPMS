...
T6 ;EP
 ...
 D HEADER^BUDHRPTP Q:BUDQUIT  D T6SH
 K BUDFNP
 S BUDORD=0 F  S BUDORD=$O(^BUDHTSC("B",BUDORD)) Q:BUDORD'=+BUDORD!(BUDORD>30)!(BUDQUIT)  D PRN
 D HEADER^BUDHRPTP Q:BUDQUIT  D T6SH1
 S BUDORD=30,BUDTYPE="S" F  S BUDORD=$O(^BUDHTSC("B",BUDORD)) Q:BUDORD'=+BUDORD!(BUDQUIT)  D PRN
 Q
PRN ;
 D
 .S BUDY=0 F  S BUDY=$O(^BUDHTSC("B",BUDORD,BUDY)) Q:BUDY'=+BUDY!(BUDQUIT)  D
 ..;gather all lines into an array
 ..K BUDARR
 ..S BUDHD=$P(^BUDHTSC(BUDY,0),U,2)
 ..S (X,C,M)=0 F  S X=$O(^BUDHTSC(BUDY,2,X)) Q:X'=+X  S C=C+1,$P(BUDARR(C),U,2)=$P(^BUDHTSC(BUDY,2,X,0),U,1),M=C
 ..S (X,C)=0 F  S X=$O(^BUDHTSC(BUDY,3,X)) Q:X'=+X  S C=C+1,$P(BUDARR(C),U,3)=$P(^BUDHTSC(BUDY,3,X,0),U,1) S:C>M M=C
 ..S (X,C)=0 F  S X=$O(^BUDHTSC(BUDY,4,"B",X)) Q:X'=+X  S Y=$O(^BUDHTSC(BUDY,4,"B",X,0)) S C=C+1,$P(BUDARR(C),U,4)=$P(^BUDHTSC(BUDY,4,Y,0),U,2) S:C>M M=C
 ..S M=M\2 S:M=0 M=1
 ..S $P(BUDARR(1),U,1)=$P(^BUDHTSC(BUDY,0),U,3)
 ..S $P(BUDARR(1),U,5)=$$C($P(BUDT6("V"),U,BUDORD))
 ..S $P(BUDARR(1),U,6)=$$C($P(BUDT6("P"),U,BUDORD))
 ..I $Y>(IOSL-6) D HEADER^BUDHRPTP Q:BUDQUIT  D:BUDTYPE="D" T6SH D:BUDTYPE="S" T6SH1
