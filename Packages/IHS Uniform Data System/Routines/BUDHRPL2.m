BUDHRPL2 ; IHS/CMI/LAB - UDS print lists ;
 ;;13.0;IHS/RPMS UNIFORM DATA SYSTEM;;OCT 12, 2018;Build 90
 ;
T52 ;EP
 S BUDP=0,BUDX2L=35
 I BUDROT="P",'$D(^XTMP("BUDHRPT1",BUDJ,BUDH,"T5",BUDX2L)) D T52H W !!,"No Uncategorized Provider visits to report." Q
 I BUDROT="D",'$D(^XTMP("BUDHRPT1",BUDJ,BUDH,"T5",BUDX2L)) D T52H D S(""),S("No Uncategorized Provider visits to report.") Q
 D T52H
 S BUDX2L=35,BUDX2L2=0,BUDY=$O(^BUDHTFIV("B",BUDX2L,0)),BUDY=$P(^BUDHTFIV(BUDY,0),U,2)
 ...
 Q
T5W ;
 I BUDROT="P" W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCCOM,1,12)
 I BUDROT="P" W ?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCCAD),?60,$E($P($$RACE^BUDHRPTC(DFN),U,4),1,15)," (",$P($$RACE^BUDHRPTC(DFN),U,3),")"
 ...
 ...S X=X_U_$E($P($$RACE^BUDHRPTC(DFN),U,4),1,15)_" ("_$P($$RACE^BUDHRPTC(DFN),U,3)_")"
...
LOC() ;EP - Return location name from file 4 based on DUZ(2).
 ...
 ;----------
T51 ;EP
 S BUDNEWR=1 ;,BUDSCTC=$G(BUDSCTC)+1
 I BUDROT="D" D T51DH
 S BUDX2L="" F  S BUDX2L=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"T51",BUDX2L)) Q:BUDX2L=""!(BUDQUIT)  D
 .S BUDX2L2="" F  S BUDX2L2=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2)) Q:BUDX2L2=""!(BUDQUIT)  D
 ...
 ..S BUDPROV="" F  S BUDPROV=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2,BUDPROV)) Q:BUDPROV=""!(BUDQUIT)  D
 ...I BUDROT="P",$Y>(IOSL-3) D T51H Q:BUDQUIT  W !!,"Line ",BUDX2LL,"   ",BUDY
 ...I BUDROT="P" W !,BUDPROV,?35,^XTMP("BUDHRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2,BUDPROV)
 ...I BUDROT="D" S X=BUDPROV_U_^XTMP("BUDHRPT1",BUDJ,BUDH,"T51",BUDX2L,BUDX2L2,BUDPROV) D S(X)
 .Q
 I BUDROT="P" W !
 Q
T51H ;
...
T3A ;EP
 S BUDP=0
 D T3H Q:BUDQUIT
 S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"3A",BUDAGE)) Q:BUDAGE'=+BUDAGE!(BUDQUIT)  D
 .S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ..S BUDCCOM="" F  S BUDCCOM=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCCOM)) Q:BUDCCOM=""!(BUDQUIT)  D
 ...S DFN="" F  S DFN=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCCOM,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D
 ....I BUDROT="P",$Y>(IOSL-3) D T3H Q:BUDQUIT
 ....I BUDROT="P" W !,$E($P(^DPT(DFN,0),U,1),1,22),?24,$S($$HRN^AUPNPAT(DFN,BUDSITE)]"":$$HRN^AUPNPAT(DFN,BUDSITE,2),1:$$HRN^AUPNPAT(DFN,DUZ(2),2)),?36,$E(BUDCCOM,1,12),?51,$P(^DPT(DFN,0),U,2),?55,$$AGE^AUPNPAT(DFN,BUDCCAD) ;
 ....S BUDV=0 F  S BUDV=$O(^XTMP("BUDHRPT1",BUDJ,BUDH,"3A",BUDAGE,BUDSEX,BUDCCOM,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .....I BUDROT="P",$Y>(IOSL-3) D T3H Q:BUDQUIT
...
T3H1 ;
 ...
 W !?3,$P(^VA(200,DUZ,0),U,2),?35,$$FMTE^XLFDT(DT),?55,"Section "_BUDSCTC_" of "_BUDTSCTC_", Page "_BUDGPG,!
 W !,$$CTR("***  RPMS Uniform Data System (UDS) "_$$VER^BUDHBAN()_"  ***",80)
 W !,$$CTR("Patient List for Table 3A, Patients by Age and Sex Assigned at Birth",80),!
 W $$CTR($P(^DIC(4,BUDSITE,0),U),80),!
 S X="Reporting Period: "_$$FMTE^XLFDT(BUDBD)_" to "_$$FMTE^XLFDT(BUDED) W $$CTR(X,80),!
 S X="Population:  "_$S($G(BUDBEN)=1:"Indian/Alaskan Native (Classification 01)",$G(BUDBEN)=2:"Not Indian Alaskan/Native (Not Classification 01)",$G(BUDBEN)=3:"All (both Indian/Alaskan Natives and Non 01)",BUDBEN=4:"Homeless",1:"")
 W $$CTR(X,80),!
 ...
 .W !,"with sex assigned at birth, age, and visit information. "
 .W !,"Age on the patient list is calculated as of June 30.",!
 W !,"PATIENT NAME",?24,"HRN",?36,"COMMUNITY",?50,"SEX",?55,"AGE",!?46,"ASSIGNED",!?46,"AT BIRTH"
...
T3HD ;
 ...
 D S($P(^VA(200,DUZ,0),U,2)_"    "_$$FMTE^XLFDT(DT))
 D S("***  RPMS Uniform Data System (UDS) "_$$VER^BUDHBAN()_"  ***")
 D S("Patient List for Table 3A, Patients by Age and Sex Assigned at Birth")
 D S($P(^DIC(4,BUDSITE,0),U))
 S X="Reporting Period: "_$$FMTE^XLFDT(BUDBD)_" to "_$$FMTE^XLFDT(BUDED) D S(X)
 S X="Population:  "_$S($G(BUDBEN)=1:"Indian/Alaskan Native (Classification 01)",$G(BUDBEN)=2:"Not Indian Alaskan/Native (Not Classification 01)",$G(BUDBEN)=3:"All (both Indian/Alaskan Natives and Non 01)",BUDBEN=4:"Homeless",1:"") D S(X)
 D S(" ")
 D S("List of all patients with one or more visits during the calendar year,")
 D S("with sex assigned at birth, age, and visit information.")
 D S("Age on the patient list is calculated as of June 30.")
 D S(" "),S("PATIENT NAME^HRN^COMMUNITY^SEX ASSIGNED AT BIRTH^AGE^VISIT DATE^PROV TYPE^SRV^CLINIC^LOCATION")
 Q
S(V) ;
