...
GETV ;EP - get all visits for this patient and tally in BUDTV
 ...
 .Q:BUDVLOC=""
 .Q:'$D(^BUDESITE(BUDSITE,11,BUDVLOC))  ;not valid location
 .Q:"AHSORMI"'[$P(^AUPNVSIT(BUDVSIT,0),U,7)  ;ignore cr/telephone/ events
 .;V12.0 ADD PROVIDER EXCLUSIONS
 .S J=$$PRIMPROV^APCLV(BUDVSIT,"F")
 .I J,$D(^BUDESITE(BUDSITE,13,"B",J)) Q  ;don't count visits with thisclass
 .S BUDCLINC=$$CLINIC^APCLV(BUDVSIT,"C")
 .S BUDTIEN=$O(^BUDECNTL("B","UDS PT/TABLE 5 CLINIC EXCL",0))
 .I BUDCLINC]"",$D(^BUDECNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;don't count UDS PATIENT CLINIC EXCLUSIONS
...
T5 ;now get visits for table 5 1 per provider type per day
 ...
 .S BUDCLINC=$$CLINIC^APCLV(BUDVSIT,"C")
 .S BUDTIEN=$O(^BUDECNTL("B","UDS PT/TABLE 5 CLINIC EXCL",0))
 .I BUDCLINC]"",$D(^BUDECNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;don't count excluded clinics
 .S BUDPP=$$PRIMPROV^APCLV(BUDVSIT,"I")  ;IEN of primary provider
 .I 'BUDPP Q  ;no primary provider
...
MEDC .;NOW CHECK FOR MEDICAL CARE, CAN ONLY HAVE 1 PER LOCATION OF ENCOUNTER
 .S S=0
 .S BUDTIEN=$O(^BUDECNTL("B","MEDICAL CARE LINE NUMBERS",0))
 .I $E($$VAL^XBDIQ1(9000010,BUDVSIT,.06),1,3)="CHS",BUDPPD=15 S BUDLINE=2 G MEDC1
 .S BUDY=$O(^BUDETFIV("C",BUDPPD,0)) I BUDY="" S BUDLINE=35 G MEDC1
 .S BUDLINE=$O(^BUDETFIV("AA",BUDPPD,""))
MEDC1 .S S=0
 .I $D(^BUDECNTL(BUDTIEN,11,"B",BUDLINE)) D
...
DENT .;NOW CHECK FOR DENTAL CARE
 .S S=0
 .S BUDTIEN=$O(^BUDECNTL("B","DENTAL LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDECNTL(BUDTIEN,11,"B",BUDLINE)) D
...
MH .;NOW CHECK FOR MH CARE
 .S S=0
 .S BUDTIEN=$O(^BUDECNTL("B","MENTAL HEALTH LINE NUMBERS",0))
...
SUB .;
 .S S=0
 .S BUDTIEN=$O(^BUDECNTL("B","SUBSTANCE LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDECNTL(BUDTIEN,11,"B",BUDLINE)) D
...
VISION .;
 .S S=0
 .S BUDTIEN=$O(^BUDECNTL("B","VISION LINE NUMBERS",0))
 .S S=0
 .I $D(^BUDECNTL(BUDTIEN,11,"B",BUDLINE)) D
...
OTH .;can have only 1 in each category
 .S BUDTIEN=$O(^BUDETFIV("B",22,0))
 .S S=0
 .I $D(^BUDETFIV(BUDTIEN,11,"B",BUDPPD)) D
...
ENAB .;NOW CHECK FOR ENABLING
 .S S=0
 .S BUDTIEN=$O(^BUDECNTL("B","ENABLING LINE NUMBERS",0))
 .I $D(^BUDECNTL(BUDTIEN,11,"B",BUDLINE)) D
...
A6 S BUDX=0 F  S BUDX=$O(^TMP($J,"VISITS",BUDX)) Q:BUDX'=+BUDX  S BUDVSIT=$P(^TMP($J,"VISITS",BUDX),U,5) D
 ...
 .Q:"AHSORMI"'[$P(^AUPNVSIT(BUDVSIT,0),U,7)  ;ignore cr/telephone/events
 .S BUDCLINC=$$CLINIC^APCLV(BUDVSIT,"C")
 .S BUDTIEN=$O(^BUDECNTL("B","TABLE 6A CLINIC EXCLUSIONS",0))
 .I BUDCLINC]"",$D(^BUDECNTL(BUDTIEN,11,"B",BUDCLINC)) Q  ;don't count excluded clinics
