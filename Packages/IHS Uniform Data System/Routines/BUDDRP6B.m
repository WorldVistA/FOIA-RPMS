...
EN2 ;
 ...
 S BUDTAXT="B6" D TAXCHK^BUDDXTCH
 D YEAR
 I BUDYEAR="" D EOJ Q
...
DRIVER ;
 ...
 Q
NODEV ;
 S XBRP="",XBRC="NODEV1^BUDDRP6B",XBRX="EOJ^BUDDRP6B",XBNS="BUD"
 D ^XBDBQUE
 Q
NODEV1 ;
 D PROC^BUDDRP6B
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BUDDRP6B",ZTDTH="",ZTDESC="UDS 16 REPORT TABLE 6B" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D EN^XBVK("BUD")
 Q
...
M ;EP - called from option
 ...
 S X=0 F  S X=$O(BUDIND(X)) Q:X'=+X  X ^BUDDLST2(X,1)
 G EN1
PROC ;EP - called from taskman
 S BUDJ=$J,BUDH=$H
 S ^XTMP("BUDDRP6B",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^BUD TABLE 6B LISTS"
 ;NOW LOOP PATS
...
N .;
 ...
 .D GETV^BUDDRPTD  ;get visits that meet criteria
...
PRGALST ;EP - list of pregnant females
 ...
 S X=$$AGB(BUDAGEP)
 S ^XTMP("BUDDRP6B",BUDJ,BUDH,"PRGA",X,BUDAGEP,$P(^DPT(DFN,0),U),BUDCCOM,DFN)=$P(BUDP,"*",2)
 Q
AGB(N) ;
...
PROB S T=$O(^ATXAX("B","BGP PREGNANCY DIAGNOSES 2",0))
 ...
 S BUDD=G,BUDALL=BUDALL_"Problem List: "_$$VAL^XBDIQ1(9000011,Z,.01)_" on "_$$DATE^BUDDUTL1(G)_U
MA ;now check for abortion or miscarriage
 ...
 S BUDG=$$LASTPRC^BUDDUTL1(P,"BGP ABORTION PROCEDURES",BDATE,EDATE)
 I BUDG Q 0
 ...
 S %=$$CPT^BUDDDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$CPT^BUDDDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT ABORTION",0))
 S %=$$TRAN^BUDDDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
 S T=$O(^ATXAX("B","BGP CPT MISCARRIAGE",0))
 S %=$$TRAN^BUDDDU(P,BUDD,EDATE,T,3)
 I %]"" Q 0
...
PRINT ;EP - called from taskman
 ...
 .S ^TMP($J,"BUDDEL",0)=0
 .S BUDDECNT=0
 D HEADER^BUDDRPTP Q:BUDQUIT  D T6BH
 ...
 D LINE1
 I $Y>(IOSL-3) D HEADER^BUDDRPTP Q:BUDQUIT  D T6BH
 ...
 I $Y>(IOSL-12) D HEADER^BUDDRPTP Q:BUDQUIT  D T6BH
 ...
 W "STATUS",?23,"|",?26,"PATIENTS WITH 2ND",?45,"|",?47,"SAMPLED OR EHR",?65,"|",?67,"PATIENTS",!
 ...
 D LINE
 W "CERVICAL CANCER",?23,"|",?26,"TOTAL NUMBER",?45,"|",?47,"NUMBER CHARTS",?65,"|",?67,"NUMBER OF",!
 W "SCREENING",?23,"|",?26,"OF FEMALE PATIENTS",?45,"|",?47,"SAMPLED OR EHR",?65,"|",?67,"PATIENTS",!
 W ?23,"|",?26,"23-64",?45,"|",?47,"TOTAL",?65,"|",?67,"TESTED",!
 W ?23,"|",?30,"(a)",?45,"|",?50,"(b)",?65,"|",?70,"(c)",!
 D LINE3
...
GENI ;general introductions
 ...
 W !,"December 31, 2016.  Based on the UDS definition, to be considered a"
