BUDEDU ;IHS/CMI/LAB - uds utility calls;
...
CTR(X,Y) ;EP - Center X in a field Y wide.
 ...
 ;
IN6B(T) ;EP
 S BUDLST2I=$O(^BUDELST2("C",T,0))
 W:$D(IOF) @IOF
 W !,$$CTR($$LOC,80)
 W !,$$CTR("UDS 2017",80)
 W !!,$P(^BUDELST2(BUDLST2I,0),U,1)_" (Table 6B)",!
 D GENI^BUDERP6B
 D PAUSE
 S X=0 F  S X=$O(^BUDELST2(BUDLST2I,11,X)) Q:X'=+X  W !,^BUDELST2(BUDLST2I,11,X,0)
 W !
 Q
HT6B(T) ;EP
 NEW L
 S L=$O(^BUDELST2("C",T,0))
 I BUDROT="P" D  Q
 .S X=0 F  S X=$O(^BUDELST2(L,12,X)) Q:X'=+X  W !,^BUDELST2(L,12,X,0)
 I BUDROT="D" D  Q
 .D S()
 .S X=0 F  S X=$O(^BUDELST2(L,12,X)) Q:X'=+X  D S(^BUDELST2(L,12,X,0))
 .D S()
 Q
DATE(D) ;EP
 I D="" Q ""
