BUDDRPL1 ; IHS/CMI/LAB - UDS print lists ;
 ;;11.0;IHS/RPMS UNIFORM DATA SYSTEM;;JAN 18, 2017;Build 66
 ;
 ;
TOL ;EP
 I '$D(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS")) D T6H W !!,"No Incomplete visits to report.",!! Q
 S BUDP=0
 S BUDX2L="" F   S BUDX2L=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L)) Q:BUDX2L=""!(BUDQUIT)  D
 .D T6H Q:BUDQUIT
 .W !!,"Line ",BUDX2L,"   ",$P($T(@BUDX2L),";;",2)
 .S BUDCCOM="" F  S BUDCCOM=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L,BUDCCOM)) Q:BUDCCOM=""!(BUDQUIT)  D
 ..S BUDAGE="" F  S BUDAGE=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L,BUDCCOM,BUDAGE)) Q:BUDAGE=""!(BUDQUIT)  D
 ...S BUDSEX="" F  S BUDSEX=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L,BUDCCOM,BUDAGE,BUDSEX)) Q:BUDSEX=""!(BUDQUIT)  D
 ....S DFN=0 F  S DFN=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L,BUDCCOM,BUDAGE,BUDSEX,DFN)) Q:DFN'=+DFN!(BUDQUIT)  D T6W
 ....Q
 ...Q
 ..Q
 .Q
...
T6W ;
 ...
 W ?55,$$AGE^AUPNPAT(DFN,BUDCCAD),?60,$E($P($$RACE^BUDDRPTC(DFN),U,3)_"-"_$P($$RACE^BUDDRPTC(DFN),U,4),1,19)
 S BUDV=0 F  S BUDV=$O(^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L,BUDCCOM,BUDAGE,BUDSEX,DFN,BUDV)) Q:BUDV'=+BUDV!(BUDQUIT)  D
 .I $Y>(IOSL-3) D T6H Q:BUDQUIT  W !!,"Line ",BUDX2L,"   ",$P($T(@BUDX2L),";;",2)
 .W !?5,$$FMTE^XLFDT($P($P(^AUPNVSIT(BUDV,0),U),".")),?25,^XTMP("BUDDRPT1",BUDJ,BUDH,"ORPHANS",BUDX2L,BUDCCOM,BUDAGE,BUDSEX,DFN,BUDV),?40,$P(^AUPNVSIT(BUDV,0),U,7),?45,$E($$CLINIC^APCLV(BUDV,"E"),1,14),?60,$E($$LOCENC^APCLV(BUDV,"E"),1,14)
 .Q
 Q
T6H ;
 G:'BUDGPG T6H1
