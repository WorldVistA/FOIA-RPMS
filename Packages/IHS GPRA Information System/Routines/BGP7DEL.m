BGP7DEL ; IHS/CMI/LAB - IHS GPRA 16 REPORT DRIVER 26 Jun 2010 3:41 PM ;
 ;;17.1;IHS CLINICAL REPORTING;;MAY 10, 2017;Build 29
 ;
 ;
 W:$D(IOF) @IOF
 W !!,$$CTR("2017 Elder Care Clinical Performance Measure Report",80)
INTRO ;
 D XIT
 ...
 I $D(DIRUT) D XIT Q
 S BGPZZ=Y
 I BGPZZ="S" D EN^BGP7DESI I '$D(BGPIND) W !!,"No measures selected" G INTRO
...
SUM ;display summary of this report
 ...
 W:$D(IOF) @IOF
 W !,$$CTR("SUMMARY OF IHS 2017 ELDER REPORT TO BE GENERATED")
 W !!,"The date ranges for this report are:"
 W !?5,"Report Period: ",?31,$$FMTE^XLFDT(BGPBD)," to ",?31,$$FMTE^XLFDT(BGPED)
 W !?5,"Previous Year Period: ",?31,$$FMTE^XLFDT(BGPPBD)," to ",?31,$$FMTE^XLFDT(BGPPED)
 W !?5,"Baseline Period: ",?31,$$FMTE^XLFDT(BGPBBD)," to ",?31,$$FMTE^XLFDT(BGPBED)
...
ZIS ;call to XBDBQUE
 ...
 I BGPRPT="" D XIT Q
 I BGPEXPT D
 .W !!,"A file will be created called BG171",$P(^AUTTLOC(DUZ(2),0),U,10)_".EL"_BGPRPT," and will reside",!,"in the ",BGPUF," directory.",!
 ...
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPEDLCG(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPEDLPG(" D ^DIK K DIK D XIT Q
 I POP W !,"Report Aborted" S DA=BGPRPT,DIK="^BGPEDLBG(" D ^DIK K DIK D XIT Q
 I $D(IO("Q")) G TSKMN
DRIVER ;
 D ^BGP7D1
 U IO
...
CNT() ;
 NEW X,C
 S X=0,C=0 F  S X=$O(^BGPELIG(X)) Q:X'=+X  S C=C+1
 Q C
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP7DEL",XBRX="XIT^BGP7DEL",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP7DEL",ZTDTH="",ZTDESC="ELDER 13 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
LISTS ;any lists with measures?
 ...
 I $D(BGPLIST) D RT^BGP7DESL I '$D(BGPLIST)!($D(BGPQUIT)) G LISTS ;get report type for each list
 Q
CHKY ;
