BGP8ELLJ ; IHS/CMI/LAB - print ind ;
...
I1AGE7 ;
 S I=I+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+24) S BGPPC=$O(^BGPELIIR("C",BGPF,0))
 S BGPNF=$P(^BGPELIIR(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90561.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGEP ;
 ...
 F X=1:1:4 S V=$P(BGPDAC(X),U,15) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ...
 S X="D. % w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,15) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 S X="D. # w/ LDL >160 w/ % of Total LDL Screen" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,15),O=$P(BGPDAP(X),U,15) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 D BL^BGP8ELLV
 Q
V(T,R,N,P) ;
