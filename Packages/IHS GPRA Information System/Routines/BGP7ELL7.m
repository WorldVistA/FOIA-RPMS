BGP7ELL7 ; IHS/CMI/LAB - print ind 03 Feb 2017 4:55 PM ; 
 ;;17.1;IHS CLINICAL REPORTING;;MAY 10, 2017;Build 29
 ;
 ;
I1AGE ;EP
 S BGPHD1="ACTIVE DIABETIC PATIENTS 55+ W/O BILATERAL BLINDNESS",BGPHD2="Active Diabetic Patients"
...
I1AGEP ;
 ...
 S X="# w/ Retinal Evalution-No Refusals (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% w/ Retinal Evalution-No Refusals (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,3) S Y=$$SB^BGP7ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="A. # w/ DM Retinal exam w/ % of Total Eval" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="A. % w/ DM Retinal exam w/ % of Total Eval" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,5) S Y=$$SB^BGP7ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="B. # w/ Other Eye Exams w/ % of Total Eval" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,6) S Y=V D S(Y,,X+1)
 S X="B. % w/ Other Eye Exams w/ % of Total Eval" D S(X,1,1)
...
BL ;
 ...
 S X="A. % w/ DM Retinal exam w/ % of Total Eval w/ % of Total Eval" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,5) S Y=$$SB^BGP7ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="B. # w/ Other Eye Exams" D S(X,1,1)
