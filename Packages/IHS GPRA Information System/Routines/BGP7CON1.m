BGP7CON1 ; IHS/CMI/LAB - measure AHR.A 30 May 2010 9:32 AM ;
...
BETA ;EP - BETA BLOCKER CONTRAINDICATION/NMI REFUSAL
 ...
 K BGPG
 S BGPG=$$LASTDX^BGP7UTL1(P,"BGP HYPOTENSION DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"Hypotension dx-Beta Blocker contraindication"  ;has hypotension dx
 S BGPG=$$LASTDX^BGP7UTL1(P,"BGP CMS 2/3 HEART BLOCK DXS",BDATE,EDATE)
 I $P(BGPG,U)=1 Q 1_U_"heart blk dx-Beta Blocker contraindication"  ;has heart block dx
 S BGPG=$$LASTDX^BGP7UTL1(P,"BGP SINUS BRADYCARDIA DXS",BDATE,EDATE)
 ...
 ...S G=1_U_"Beta Blocker contra NMI med "_$$DATE^BGP7UTL(Y)
 ..Q
 .Q
 I G Q G
 ;now cpt 8011 OR G9190 BETWEEN NMIB,NMIE
 S X=$$CPTI^BGP7DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8011"))
 I X Q "1^Beta Blocker Contra CPT code G8011: "_$$DATE^BGP7UTL($P(X,U,2))
 S X=$$TRANI^BGP7DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8011"))
 I X Q "1^Beta Blocker Contra TRAN code G8011: "_$$DATE^BGP7UTL($P(X,U,2))
 S X=$$CPTI^BGP7DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G9190"))
 I X Q "1^Beta Blocker Contra CPT code G9190: "_$$DATE^BGP7UTL($P(X,U,2))
 S X=$$TRANI^BGP7DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G9190"))
 I X Q "1^Beta Blocker Contra TRAN code G9190: "_$$DATE^BGP7UTL($P(X,U,2))
 Q ""
 ;
...
HEM ;now check for dx 459
 K BGPG S BGPG=$$LASTDX^BGP7UTL1(P,"BGP HEMORRHAGE DXS",$$DOB^AUPNPAT(P),EDATE)
 I BGPG Q 1_U_"asa Contra "_$P(BGPG,U,2)_" "_$$DATE^BGP7UTL($P(BGPG,U,3))
 S X=$$PLTAXND^BGP7DU(P,"BGP HEMORRHAGE DXS",EDATE) I X Q 1_U_"ASA Contra "_$P(X,U,2)_" "_$$DATE^BGP7UTL($P(X,U,3))   ;V17
 S X=$$IPLSNOND^BGP7DU(P,"PXRM BGP HEMORRHAGE",EDATE) I X Q 1_U_"ASA Contra "_$P(X,U,2)_" "_$$DATE^BGP7UTL($P(X,U,3))_U_$P(X,U,2)   ;V17
 ;
 ...
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_"asa Contra NMI Aspirin:  "_$$VAL^XBDIQ1(9000022,N,.04)_" "_$$DATE^BGP7UTL($P(^AUPNPREF(N,0),U,3))_" "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;now check for CPT code G8008
 S X=$$CPTI^BGP7DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_"asa Contra CPT code G8008: "_$$DATE^BGP7UTL($P(X,U,2))
 S X=$$TRANI^BGP7DU(P,NMIB,NMIE,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_"asa Contra Tran Code G8008: "_$$DATE^BGP7UTL($P(X,U,2))
 Q ""
 ;
ACEI ;EP does patient have an ACEI Contraidication
 ...
 S X=$$PLTAXND^BGP7DU(P,"BGP CMS AORTIC STENOSIS DXS",EDATE) I X Q 1_U_"ACEI Contra PL: "_$$DATE^BGP7UTL($P(X,U,3))_" ["_$P(X,U,2)_"]"  ;V17
 S X=$$IPLSNOND^BGP7DU(P,"PXRM BGP MOD SEV AORTIC STEN",EDATE) I X Q 1_U_"ACEI Contra PL: "_$$DATE^BGP7UTL($P(X,U,3))_" ["_$P(X,U,2)_"]"  ;V17   ;V17
 ;
...
STATIN ;EP does patient have an STATIN Contraidication
 ...
 ;
 ;pregnant
 S X=$$PREG^BGP7D7(P,BDATE,EDATE,0,1,,$G(BGPBDATE),$G(BGPEDATE)) I X Q 1_U_"Contra pregnant"  ;V17.1 CMI/LAB ADDED DATES FOR CURRENTLY PREGNANT
 ...
 .I T="BF-BC" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-BP" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-CS" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-EQ" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-FU" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-HC" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-ON" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-M" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-MK" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I T="BF-N" S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 .I $P(T,"-")]"",$D(^BGPSNOMG(SN,11,"B",$P(T,"-"))) S %=T_" "_$$DATE^BGP7UTL($P(BGPG(X),U)) Q
 ...
 I $D(BGPG(1)) Q 1_U_"STATIN Contra POV:  "_$$DATE^BGP7UTL($P(BGPG(1),U))_" ["_$P(BGPG(1),U,2)_"]  "_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 S X=$$PLTAXND^BGP7DU(P,"BGP ALCOHOL HEPATITIS DXS",EDATE) I X Q 1_U_"STATIN Contra PL: "_$$DATE^BGP7UTL($P(X,U,3))_"["_$P(X,U,2)_"]"  ;V17
 S X=$$IPLSNOND^BGP7DU(P,"PXRM BGP ACUTE ETOH HEPATITIS",EDATE) I X Q 1_U_"STATIN Contra PL: "_$$DATE^BGP7UTL($P(X,U,3))_"["_$P(X,U,2)_"]"  ;V17
 Q ""
