...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHHED^BGP8GHED",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI HEDIS GPRA REPORT 06" D ^%ZTLOAD Q
 Q
OTHHED ;
 D ^BGP8D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^XBLM("^BGP8DHEP","^TMP($J,""BGPGUI"",")
