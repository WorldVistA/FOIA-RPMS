BGP8PESL ; IHS/CMI/LAB - FY 06 DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for GPRA LIST DISPLAY
 D EN^VALM("BGP 18 PATED LIST SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ1
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS 2018 Patient Education Performance Measure Lists of Patients"
...
INIT ; -- init variables and list array
 ...
 .I $P(^BGPPEIR(X,0),U,4)]"" S C=C+1 D  Q
 ..S BGPGLIST(C,0)=C_")",$E(BGPGLIST(C,0),5)=$P(^BGPPEIR(X,0),U,4),BGPGLIST("IDX",C,C)=X I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 .I $P(^BGPPEIR(X,0),U,4)="" S C=C+1 D
 ..S BGPGLIST(C,0)="NO patient list available for measure:  "_$P(^BGPPEIR(X,0),U,2),BGPGLIST("IDX",C,C)=X,BGPNOLI(X)="" I $D(BGPLIST(X)) S BGPGLIST(C,0)="*"_BGPGLIST(C,0)
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
HELP ; -- help code
...
PT ;EP
 ...
 I $D(DIRUT) G PT
 S BGPDELF=Y
 W !!,"When the report is finished your delimited output will be found in the",!,$$GETDEDIR^BGP8UTL2()," directory.  The filename will be ",BGPDELF,".txt",!
 Q
