BGP8D3 ;IHS/CMI/LAB - MEASURE LOGIC;
...
I10 ;EP
 ...
 D SEAL^BGP8D3A(DFN,BGPBDATE,BGPEDATE) ;this is for the number of sealants only
 D SEALR^BGP8D3A(DFN,BGPBDATE,BGPEDATE)
 ...
 S BGPD9=0 I BGPAGEB>1,BGPAGEB<16 S X=$$DENTSRVD^BGP8D211(DFN,BGPBDATE,BGPEDATE) I X S BGPD9=1  ;V17.1 DEV DENOM
 ;S BGPN1=0 I $P(BGPVALUE,U)=1 S BGPN1=1
I101 ;
 ;get new sealant definition 2015 gpra dev
 S BGPSEALD=""
 I 'BGPN1 S BGPSEALD=$$SEALDEV^BGP8D3A(DFN,$$FMADD^XLFDT(BGPEDATE,-1096),BGPEDATE)
 ...
 .S X=0 F  S X=$O(BGPSEALS(X)) Q:X'=+X  S D=0 F  S D=$O(BGPSEALS(X,D)) Q:D'=+D  S BGPVALUE=BGPVALUE_$S(X=1:"",1:"; ")_$$DATE^BGP8UTL(D)_" "_BGPSEALS(X,D)
 .S X=0 F  S X=$O(BGPSEALR(X)) Q:X'=+X  S D=0 F  S D=$O(BGPSEALR(X,D)) Q:D'=+D  S BGPVALUE=BGPVALUE_$S(X=1:"",1:"; ")_$$DATE^BGP8UTL(D)_" "_BGPSEALR(X,D)
...
I11 ;EP
 ...
 S BGPFLUOR=$$TF^BGP8D3A(DFN,BGPBDATE,BGPEDATE)
 ...
 S BGPD9=0 I BGPAGEB>0,BGPAGEB<16 S X=$$DENTSRVD^BGP8D211(DFN,BGPBDATE,BGPEDATE) I X S BGPD9=1  ;V17.1 DEV DENOM
I111 ;
 S BGPVALUE="UP|||"_+BGPN1_" topical fluoride"
 I +BGPN1 S BGPVALUE=BGPVALUE_": " F X=2:1:5 I $P(BGPFLUOR,U,X)]"" S BGPVALUE=BGPVALUE_$S(X>2:", ",1:"")_$$DATE^BGP8UTL($P($P(BGPFLUOR,U,X),"|",1))_" "_$P($P(BGPFLUOR,U,X),"|",2)
 ;I BGPN2 S BGPVALUE=BGPVALUE_": "_$$DATE^BGP8UTL($P(BGPFLUOR,U,2))_" "_$P(BGPFLUOR,U,3)
 S BGPVALUD="UP|||"
 I +BGPN1 S BGPVALUD=BGPVALUD_+BGPN1_" topical fluoride" I +BGPN1 S BGPVALUD=BGPVALUD_": " F X=2:1:5 I $P(BGPFLUOR,U,X)]"" S BGPVALUD=BGPVALUD_$S(X>2:", ",1:"")_$$DATE^BGP8UTL($P($P(BGPFLUOR,U,X),"|",1))_" "_$P($P(BGPFLUOR,U,X),"|",2)
 ;I BGPN2 S BGPVALUD=BGPVALUD_": "_$$DATE^BGP8UTL($P(BGPFLUOR,U,2))_" "_$P(BGPFLUOR,U,3)
 K ^TMP($J,"A"),BGPFLUOR
 Q
I12 ;EP
 NEW BGPHR,V,BGPAIM
 ...
 S BGPHR=$$HIGHR^BGP8D3A(DFN,BGPEDATE)
 ...
 .I BGPD4 S BGPDV=BGPDV_$S(BGPDV]"":",AD",1:"AD")
 .I BGPHR S BGPDV=BGPDV_$S(BGPDV]"":",HR",1:"HR")
 I BGPRTYPE'=1 S BGPVALUE=BGPDV_"|||"_$$DATE^BGP8UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2)
 I BGPRTYPE=1 D
 .S BGPDV=BGPDV_"|||"
 .S BGPVALUD=BGPVALUD_"|||"
 .I BGPN7 S BGPVALUD=BGPVALUD_$$DATE^BGP8UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2),BGPVALUE=BGPDV_$$DATE^BGP8UTL($P(BGPVALUE,U,1))_" "_$P(BGPVALUE,U,2) Q
 .S BGPVALUE=BGPDV
...
I15 ;EP
 ...
 S BGPI7DA=0,BGPI7DB=0,BGPI7DC=0,BGPI7DD=0,BGPPAP4="",BGPN4=0,BGPPAP6="",BGPHPV="",BGPPAPDE=""
 S (BGPD1,BGPD2,BGPD3,BGPD4,BGPD5,BGPD6,BGPD7,BGPD8,BGPN1,BGPN2,BGPN3,BGPN4,BGPN5,BGPN6,BGPN7,BGPN8)=0
 ...
 .S BGPPAPDE=$$PAPHPV^BGP8D31(DFN,BGPEDATE,5)  ;PAP AND HPV IN PAST 5 YEARS ON SAME DAY
 .I BGPPAPDE S BGPN7=1
 I BGPN4!(BGPN5) S BGPN6=1
 I BGPN4!(BGPN7) S BGPN8=1
 ;I BGPTIME=1 W ?40,"BGPN4=",BGPN4,?50,"BGPN5=",?60,"BGPN6=",BGPN6
 S BGPVALUE=$S(BGPI7DD:"UP",1:"")_$S(BGPI7DC:",AC",1:"")_"|||"
 ;I BGPRTYPE=1 S BGPVALUE=$S(BGPI7DA:"UP",1:"")_$S(BGPI7DB:",AC",1:"")_"|||" I BGPN3 S BGPVALUE=BGPVALUE_$$DATE^BGP8UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 ;I BGPRTYPE=3 S BGPVALUE="AC|||"_$$DATE^BGP8UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 I BGPN4 S BGPVALUE=BGPVALUE_"PAP 3 YRS: "_$$DATE^BGP8UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 I BGPN5 S BGPVALUE=BGPVALUE_" PAP&HPV 5 YRS: "_$$DATE^BGP8UTL($P(BGPPAP6,U,2))_" "_$P(BGPPAP6,U,3)_"; HPV: "_$$DATE^BGP8UTL($P(BGPHPV,U,2))_" "_$P(BGPHPV,U,3)
 S BGPVALUD=$S(BGPI7DD:"UP",1:"")_$S(BGPI7DC:",AC",1:"")_"|||"
 I BGPN4 S BGPVALUD=BGPVALUD_"PAP 3 YRS: "_$$DATE^BGP8UTL($P(BGPPAP,U,2))_" "_$P(BGPPAP,U,3)
 I BGPN7 S BGPVALUD=BGPVALUD_" PAP&HPV 5 YRS SAME DAY: "_$P(BGPPAPDE,U,3)
 K BGPLPAP,BGPPAP,BGPHPV,BGPPAP4,BGPPAP6,BGPPAPDE
 Q
 ;
HPV(P,EDATE,YEARS) ;EP
 ...
 K BGP
 S T="BGP HPV DXS"
 S X=$$LASTDX^BGP8UTL1(P,T,BDATE,EDATE) I X,$P(BGPLPAP,U,2)<$P(X,U,3) S BGPLPAP="1^"_$P(X,U,3)_"^POV "_$P(X,U,2)
...
PAP(P,EDATE,YEARS,REFUSAL) ;EP
 ...
 S X=$$LASTDX^BGP8UTL1(P,T,BDATE,EDATE) I X,$P(BGPLPAP,U,2)<$P(X,U,3) S BGPLPAP="1^"_$P(X,U,3)_"^POV "_$P(X,U,2)
 ;K BGP S BGP(1)=$$LASTPRC^BGP8UTL1(P,"BGP PAP PROCEDURES",BDATE,EDATE)
 ...
 S T=$$CPTREFT^BGP8UTL1(P,$$FMADD^XLFDT(EDATE,-365),EDATE,$O(^ATXAX("B","BGP CPT PAP",0)))
...
HYSTER(P,EDATE) ;EP 
 ...
 I X Q 1
 ;S X=$$PLTAX^BGP8DU(P,"BGP HYSTERECTOMY DXS")
 ;I X Q 1
