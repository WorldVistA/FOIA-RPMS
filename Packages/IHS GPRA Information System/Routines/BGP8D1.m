BGP8D1 ;IHS/CMI/LAB - CALC MEASURES;  ; 23 Jan 2018  9:56 AM
...
PROC ;EP
 ...
 D UNFOLDTX^BGP8UTL2
 D XTMP^BGP8UTL("BGP28CPL","National GPRA Comp List")
 D XTMP^BGP8UTL("BGP8DNP","National GPRA Patient List")
 D XTMP^BGP8UTL("BGP8D","CRS Local Patient List")
...
PROCCY ;EP - current time period
 K ^TMP($J)
 S (BGPACTUP,BGPACTCL,BGPDM1,BGPDM2,BGPDMD1,BGPDMD2,BGPDMD3,BGPDMD4,BGPDMD5,BGPDMD6,BGPIHD,BGPHTN)=""
 K BGPG
 Q:'$D(^DPT(DFN,0))
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPED,BGPTIME=1,BGPBDATE=BGPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDCB(",BGPRTYPE=5:"^BGPEDLCR(",BGPRTYPE=8:"^BGPEOCB(",1:"^BGPGPDCR(")
 S BGP365=BGPBDATE
 ...
 S BGPDM2=$$DM(DFN,BGPBDATE,BGPEDATE) ;dm diagnosis in REPORT PERIOD
 ...
 .S BGPDMD6=0 I BGPFDMD,BGP2V,BGP2DMV,BGPACTUP S BGPDMD6=1  ;NEW GPRA DM USER POP DENOM V18.0
 I $G(BGPXPRP) Q  ;stop if in national gpra export
 I $G(BGPIISO) D ISS^BGP8DPA1 K ^TMP($J,"A") Q  ;SCHEDULING OPTION
 D CALCIND
 K ^TMP($J,"A")
 Q
PROCPY ;
 K ^TMP($J),BGPG
 S (BGPACTUP,BGPACTCL,BGPDM1,BGPDM2,BGPDMD1,BGPDMD2,BGPDMD3,BGPDMD4,BGPDMD5,BGPDMD6,BGPIHD)=""
 Q:'$D(^DPT(DFN,0))
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPPED,BGPTIME=2,BGPBDATE=BGPPBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDPB(",BGPRTYPE=5:"^BGPEDLPR(",BGPRTYPE=8:"^BGPEOPB(",1:"^BGPGPDPR(")
 S BGP365=BGPBDATE
 ...
 S BGPAGEE=$$AGE^AUPNPAT(DFN,BGPEDATE)
 S BGPSEX=$P(^DPT(DFN,0),U,2)
 ;I $$V2IHD^BGP8D9(DFN,BGP365,BGPEDATE),$$FIRSTIHD^BGP8D9(DFN,BGPEDATE) S BGPIHD=1
 S BGPDM1=$$DM(DFN,,BGPEDATE) ;dm diagnosis ever?
...
PROCBY ;
 ...
 Q:$P(^DPT(DFN,0),U,2)=""
 Q:"FM"'[$P(^DPT(DFN,0),U,2)
 S BGPEDATE=BGPBED,BGPTIME=3,BGPBDATE=BGPBBD,BGPGBL=$S(BGPRTYPE=3:"^BGPHEDBB(",BGPRTYPE=5:"^BGPEDLBR(",BGPRTYPE=8:"^BGPEOBB(",1:"^BGPGPDBR(")
 S BGP365=BGPBDATE
 ...
 I BGPDM1 D  ;set up 4 dm demoninators
 .S BGPFDMD=$$FIRSTDM(DFN,BGPEDATE) ;1 OR 0 FIRST DX BEFORE BEG
 .S BGP2V=$$V2(DFN,BGPBDATE,BGPEDATE)  ;2 VISITS DURING REPORT PERIOD
 .S BGP2DMV=$$V2DM(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE)  ;2 DM VISITS EVER
 .S BGPYDMV=$$V1DM(DFN,BGPBDATE,BGPEDATE) ; 1 DM VISIT IN REPORT PERIOD
 .S BGPDMD1=0 I BGPFDMD S BGPDMD1=1
 .S BGPDMD2=0 I BGPFDMD,BGP2V,BGP2DMV,BGPACTCL S BGPDMD2=1  ;ACTIVE DIABETIC
 .S BGPDMD3=0 I BGPFDMD,BGPAGEB>19,BGP2DMV,BGPYDMV,$$CREAT(DFN,BGPEDATE) S BGPDMD3=1
 .S BGPDMD5=0 I BGPFDMD,BGP2DMV,BGPYDMV,$$CREAT(DFN,BGPEDATE) S BGPDMD5=1
 .S BGPDMD6=0 I BGPFDMD,BGP2V,BGP2DMV,BGPACTUP S BGPDMD6=1  ;NEW GPRA DM USER POP DENOM V18.0
 D CALCIND
 ...
 Q
CALCIND ;
 I BGPRTYPE=5 D CALCIND^BGP8DCEI Q
 D CALCIND^BGP8DCI
 Q
...
DM(P,BDATE,EDATE) ;EP is patient diabetic 1 or 0
 I $G(BDATE)="" S BDATE=$$DOB^AUPNPAT(P)
 NEW BGPG
 S BGPG(1)=$$LASTDX^BGP8UTL1(P,"SURVEILLANCE DIABETES",BDATE,EDATE)
...
ACTCL(P,BDATE,EDATE) ;EP - clinical user
 ...
 .Q:B=""
 .I 'G,$D(^BGPCTRL($O(^BGPCTRL("B",2018,0)),11,"B",B)) S G=V  ;must be a primary clinic S G=V
 .I V'=G,$D(^BGPCTRL($O(^BGPCTRL("B",2018,0)),12,"B",B)) S S=1
 .I G,S S F=1
 .Q
 Q $S(F:1,1:0)
ACTCLBH(P,BDATE,EDATE) ;EP - clinical user
 ...
 .Q:B=""
 .I 'G,$D(^BGPCTRL($O(^BGPCTRL("B",2018,0)),11,"B",B)) S G=V  ;must be a primary clinic S G=V
 .I V'=G,$D(^BGPCTRL($O(^BGPCTRL("B",2018,0)),95,"B",B)) S S=1
 .I G,S S F=1
 .Q
 Q $S(F:1,1:0)
FIRSTDM(P,EDATE) ;EP
