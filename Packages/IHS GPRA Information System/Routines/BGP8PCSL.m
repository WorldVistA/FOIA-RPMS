BGP8PCSL ; IHS/CMI/LAB - IHS 2018 DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for GPRA LIST DISPLAY
 D EN^VALM("BGP 18 IPC LIST SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ1
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS 2018 IPC Performance Measure Lists of Patients"
 S VALMHDR(2)="* indicates the list has been selected"
 Q
 ;
INIT ; -- init variables and list array
 K BGPGLIST,BGPNOLI S BGPHIGH=""
 S (X,C,I,O)=0 F  S O=$O(^BGPINDR("AIPC",O)) Q:O'=+O  S X=$O(^BGPINDR("AIPC",O,0)) I $D(BGPIND(X)) D
