...
I0302A ;EP
 ...
 ;S BGPIHD=$$IHDCCVD^BGP7D729(DFN,BGPBDATE,BGPEDATE)
 S BGPCHD=0
...
I0302ASC ;EP
 ...
 S BGPBPC=""
 S BGPBP=$$MEANBP^BGP7D26(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE)
 I BGPBP["unknown" S BGPBPC=$$BPCPT^BGP7D22(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE) I BGPBPC]"" S BGPBP=$P(BGPBPC,U,2)_"^1"
 S BGPBPDEV=$$MEANBP^BGP7D26(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE,1)
 I BGPBPDEV["unknown" S BGPBPCD=$$BPCPT^BGP7D22(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE,1) I BGPBPCD]"" S BGPBPDEV=$P(BGPBPCD,U,2)_"^1"
 S BGPN1=$S($P(BGPBP,U,2):1,1:0)
 S BGPN16=$S($P(BGPBPDEV,U,2):1,1:0)
 ;S BGPLDL=$$LDL^BGP7D2(DFN,$$FMADD^XLFDT(BGPEDATE,-(5*365)),BGPEDATE,1) ;date^value
 ;I $P(BGPLDL,U) S BGPN11=1  ;had ldl done
 ;S BGPLDL1=$$LDL^BGP7D2(DFN,BGPBDATE,BGPEDATE,1) ;date^value
 ...
 ;S BGPBMIR="" I 'BGPN4 S BGPBMIR=$$REF^BGP7D6(DFN,BGPBDATE,BGPEDATE,BGPAGEE) I $P(BGPBMIR,U) S BGPN4=1,BGPN8=1,BGPBMIR=1_U_"BMI: Ref ht "_$$DATE^BGP7UTL($P(BGPBMIR,U,3))_" wt "_$$DATE^BGP7UTL($P(BGPBMIR,U,6))
 ...
 ;I 'BGPN6 S BGPDEP=$$DEPREF^BGP7D25(DFN,BGPBDATE,BGPEDATE) I BGPDEP S BGPN6=1
 ...
 I BGPN1,BGPN3,BGPN4,BGPN5 S BGPN7=1
 I BGPN1,BGPN3,BGPN10,BGPN5 S BGPN9=1  ;IHD GPRA
 I BGPN16,BGPN3,BGPN10,BGPN15 S BGPN14=1  ;CHD GPRA
 S BGPALL=0 I BGPN1,BGPN3,BGPN10,BGPN5,BGPN6 S BGPALL=1  ;IHD ALL
 S BGPALL1=0 I BGPN16,BGPN3,BGPN10,BGPN15,BGPN12 S BGPALL1=1 ;CHD ALL
 ...
 ;S %=$S($P(BGPLDL1,U,2):"LDL: "_$$DATE^BGP7UTL($P(BGPLDL1,U,2)),1:"")
 ...
 .I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 S %=$S($P(BGPLIFED,U):"LIFE: "_$$DATE^BGP7UTL($P(BGPLIFED,U,2))_" "_$P(BGPLIFED,U,3),1:"")
 I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
...
BPCV ;EP - called from elder care routine
 ...
 I BGPVALUE["unknown" S BGPVALUE=$$BPCPT^BGP7D22(DFN,BGPBDATE,BGPEDATE) I BGPVALUE]"" S BGPN1=1,BGPVALUE=$P(BGPVALUE,U,2) G V  ;CPTS COUNT IN NUMERATOR 1 ONLY
...
DEP(P,BDATE,EDATE) ;EP
 S X=$$DEP^BGP7D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSCR^BGP7D25(P,BDATE,EDATE) I $P(X,U) Q X
 Q ""
DEPCHD(P,BDATE,EDATE) ;EP
 S X=$$DEP^BGP7D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSCR^BGP7D25(P,BDATE,EDATE) I $P(X,U) Q X
 S X=$$DEPSUIC^BGP7D25(P,BDATE,EDATE) I $P(X,U) Q X
 Q ""
LIFE(P,BDATE,EDATE) ;EP
 S X=$$MEDNUTR^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECNUTR^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECEX^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$OTHREL^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
LIFED(P,BDATE,EDATE) ;EP
 S X=$$MEDNUTRD^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECNUTR^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$SPECEX^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 S X=$$OTHREL^BGP7D711(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
TOBACCO(P,BDATE,EDATE) ;EP
 ...
 S X=$$CPTSM^BGP7D7(P,BDATE,EDATE) I X]"" Q 1_U_X
 Q ""
...
TOBDX(P,BDATE,EDATE) ;EP
 ...
 .I $E($P($$ICDDX^BGP7UTL2(Y),U,2),1,5)'="305.1",$P($$ICDDX^BGP7UTL2(Y),U,2)'="V15.82" Q
 .S G=$P($$ICDDX^BGP7UTL2(Y),U,2)_" PL"
 .Q
 Q G
