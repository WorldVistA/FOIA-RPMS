BGP7D75 ; IHS/CMI/LAB - measure 31 ;
...
IAMT ;EP
 ...
 ;I 'BGPN2 S BGPBETA("REF")=$$BETAREF^BGP7D721(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPBETA("REF") S BGPN3=1
 I 'BGPN2 S BGPBETA("CONTRA")=$$BETACONT^BGP7D721(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPBETA("CONTRA") S BGPN4=1 ;beta Contraindication
 I 'BGPN2,'BGPN4 S BGPBETA("ADR")=$$BETAALG1^BGP7D72(DFN,BGPEDATE) I BGPBETA("ADR") S BGPN4=1
 ...
 ;I 'BGPN6 S BGPASA("REF")=$$ASAREF^BGP7D721(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPASA("REF") S BGPN7=1
 ...
 I 'BGPN6 S BGPASA("ADR")=$$ASAALLEG^BGP7D721(DFN,BGPEDATE) I BGPASA("ADR") S BGPN8=1
 ...
 ;I 'BGPN10 S BGPACE("REF")=$$ACEREF^BGP7D722(DFN,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPACE("REF") S BGPN11=1
 ...
 I 'BGPN10 S BGPACE("ADR")=$$ACEALG^BGP7D722(DFN,BGPBDATE,BGPEDATE) I BGPACE("ADR") S BGPN12=1
 ...
 I 'BGPN14 S BGPSTAT("CONTRA")=$$STATCON^BGP7D722(DFN,BGPBDATE,BGPEDATE,$P(BGPAMI,U,2),$$FMADD^XLFDT($P(BGPAMI,U,4),7)) I BGPSTAT("CONTRA") S BGPN16=1 ;beta Contraindication
 I 'BGPN14 S BGPSTAT("ADR")=$$STATALG^BGP7D722(DFN,$$DOB^AUPNPAT(DFN),BGPEDATE,BGPBDATE,BGPEDATE) I BGPSTAT("ADR") S BGPN16=1
...
AMIH(P,BDATE,EDATE) ;
 ...
 ..S %=$P(^AUPNVPOV(Y,0),U) Q:'$$ICD^BGP7UTL2(%,T,9)
...
ASACONTR(P,BDATE,EDATE,BGPADM,BGPDD) ;does patient have an aspirin allergy
 K BGPMEDS1
 S K=0,R="",BGPG=""
 D GETMEDS^BGP7UTL2(P,$$FMADD^XLFDT(BGPADM,-180),BGPDD,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) G HEM
 ...
 .I $$VAPI^BGP7D81(D,$O(^ATXAX("B","BGP CMS WARFARIN VAPI",0))) S G=1 G WAR71
 .Q:'G
WAR71 .;
 .S J=$P(^AUPNVMED(Y,0),U,8)
 .S V=$P(^AUPNVMED(Y,0),U,3)
 ...
 .I E=BGPDD S BGPG=1_U_$$DATE^BGP7UTL($P($P(^AUPNVSIT(V,0),U),"."))_U_$P(^PSDRUG(D,0),U) Q
...
HEM ;now check for dx 459
 ...
 S X=$$PLTAXND^BGP7DU(P,"BGP HEMORRHAGE DXS",EDATE) I X Q 1_U_"Contra "_$P(X,U,2)_" "_$$DATE^BGP7UTL($P(X,U,3))   ;V17
 S X=$$IPLSNOND^BGP7DU(P,"PXRM BGP HEMORRHAGE",EDATE) I X Q 1_U_"Contra "_$P(X,U,2)_" "_$$DATE^BGP7UTL($P(X,U,3))_U_$P(X,U,2)   ;V17
 ;
 ...
 ...S BGPG=1_U_$$DATE^BGP7UTL($P(^AUPNPREF(N,0),U,3))_" Contra NMI Aspirin "_$$VAL^XBDIQ1(9000022,N,.04)  ;_" "__" "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;now check for CPT code G8008
 S X=$$CPTI^BGP7DU(P,BGPADM,BGPDD,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_$$DATE^BGP7UTL($P(X,U,2))_" Contra CPT G8008"
 S X=$$TRANI^BGP7DU(P,BGPADM,BGPDD,+$$CODEN^ICPTCOD("G8008"))
 I X Q 1_U_$$DATE^BGP7UTL($P(X,U,2))_" Contra Tran G8008"
 Q ""
ACECONT(P,BDATE,EDATE,NMIB,NMIE,RPBD) ;EP does patient have an ACEI Contra
 ...
 I $D(BGPG(1)) Q 1_U_$$DATE^BGP7UTL($P(BGPG(1),U))_" Contra POV "_$P(BGPG(1),U,2)  ;_"]"  ;_$$VAL^XBDIQ1(9000010.07,+$P(BGPG(1),U,4),.04)
 S X=$$PLTAXND^BGP7DU(P,"BGP CMS AORTIC STENOSIS DXS",EDATE) I X Q 1_U_$$DATE^BGP7UTL($P(X,U,3))_U_"Contra "_$P(X,U,2)   ;V17
 S X=$$IPLSNOND^BGP7DU(P,"PXRM BGP MOD SEV AORTIC STEN",EDATE) I X Q 1_U_$$DATE^BGP7UTL($P(X,U,3))_U_"Contra "_$P(X,U,2)   ;V17
 ;
 ;nmi in Refusal
 S BGPG=""
 S T=$O(^ATXAX("B","BGP HEDIS ACEI MEDS",0))
 ...
 ..S N=0 F  S N=$O(^AUPNPREF("AA",P,50,X,D,N)) Q:N'=+N  D
 ...Q:$P($G(^AUPNPREF(N,0)),U,7)'="N"
 ...S BGPG=1_U_$$DATE^BGP7UTL($P(^AUPNPREF(N,0),U,3))_" Contra NMI "_$$VAL^XBDIQ1(9000022,N,.04)  ;_"   "__"  "_$$VAL^XBDIQ1(9000022,X,1101)
 ..Q
 .Q
 I BGPG Q BGPG
 ;nmi
 ...
 I BGPG Q BGPG
 ;PREGNANCY
 S X=$$PREG^BGP7D7(P,$S($G(RPBD):RPBD,1:NMIB),EDATE,0,1,,RPBD,EDATE) I X Q 1_U_"Contra pregnant"  ;V17.1 CMI/LAB ADDED DATES FOR CURRENTLY PREGNANT
 ;breastfeeding
