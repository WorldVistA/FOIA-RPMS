BGP8DP1S ; IHS/CMI/LAB - print ind 1 12 Nov 2010 7:38 AM ;
 ;;18.0;IHS CLINICAL REPORTING;;NOV 21, 2017;Build 51
 ;
 ;HIV SCREENING AGE DIST
I1AGE ;EP  special age tallies
 I BGPRTYPE=1!(BGPRTYPE=9) D ^BGP8DP1T Q
 Q:$G(BGPSUMON)
 I BGPINDG="W" G FEM
...
I1AGE1 ;
 ...
 Q
AGES ;
 S BGPF="WCC."_BGPZZ_"."_BGPXX_BGPZ S BGPPC=$O(^BGPINDRC("C",BGPF,0))
 D
...
I1AGEP ;
 ...
 ;I '$G(BGPSUMON),BGPPTYPE="P" D HEADER^BGP8DPH Q:BGPQUIT  D W^BGP8DP(^BGPINDR(BGPIC,53,1,0),0,1,BGPPTYPE) D:$D(^BGPINDR(BGPIC,53,2,0)) W^BGP8DP(^BGPINDR(BGPIC,53,2,0),0,1,BGPPTYPE) D AH
 I BGPPTYPE'="P" D W^BGP8DP("",0,2,BGPPTYPE) D AH
 D W^BGP8DP("PREVIOUS REPORT PERIOD",0,2,BGPPTYPE)
 D W^BGP8DP($S(BGPPTYPE="P":BGPHD2,1:BGPHD1),0,1,BGPPTYPE)
 ;I BGPPTYPE="P" D W^BGP8DP(BGPHD3,0,1,BGPPTYPE)
 S BGPARR="BGPDAP" D I1AGEP1
 I '$G(BGPSUMON),BGPPTYPE="P" D HEADER^BGP8DPH Q:BGPQUIT  D W^BGP8DP(^BGPINDR(BGPIC,53,1,0),0,1,BGPPTYPE) D:$D(^BGPINDR(BGPIC,53,2,0)) W^BGP8DP(^BGPINDR(BGPIC,53,2,0),0,1,BGPPTYPE) D AH
 I BGPPTYPE'="P" D W^BGP8DP("",0,2,BGPPTYPE) D AH
 D W^BGP8DP("BASELINE REPORT PERIOD",0,2,BGPPTYPE)
 D W^BGP8DP($S(BGPPTYPE="P":BGPHD2,1:BGPHD1),0,1,BGPPTYPE)
 ;I BGPPTYPE="P" D W^BGP8DP(BGPHD3,0,1,BGPPTYPE)
 S BGPARR="BGPDAB" D I1AGEP1
 Q
I1AGEP1 ;
 S T=30 F X=1:1:3 S V=$P(@BGPARR@(X),U) D W^BGP8DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP8DP("# w/ Comprehensive Assessment",0,2,BGPPTYPE)
 S T=31 F X=1:1:3 S V=$P(@BGPARR@(X),U,2) D W^BGP8DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP8DP("% w/ Comprehensive Assessment",0,1,BGPPTYPE)
 S T=30 F X=1:1:3 S V=$P(@BGPARR@(X),U,3) D W^BGP8DP($S(BGPPTYPE="P":$J(V,6,1),1:$$SB($J(V,6,1))),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP8DP("# w/ BMI Documented",0,2,BGPPTYPE)
 S T=31 F X=1:1:3 S V=$P(@BGPARR@(X),U,4) D W^BGP8DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP8DP("% w/ BMI Documented",0,1,BGPPTYPE)
 S T=30 F X=1:1:3 S V=$P(@BGPARR@(X),U,5) D W^BGP8DP($S(BGPPTYPE="P":$J(V,6,1),1:$$SB($J(V,6,1))),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP8DP("% w/ Nutrition Counseling",0,2,BGPPTYPE)
 S T=31 F X=1:1:3 S V=$P(@BGPARR@(X),U,6) D W^BGP8DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 D W^BGP8DP("% w/ Nutrition Counseling",0,1,BGPPTYPE)
 S T=30 F X=1:1:3 S V=$P(@BGPARR@(X),U,7) D W^BGP8DP($S(BGPPTYPE="P":$J(V,6,1),1:$$SB($J(V,6,1))),0,0,BGPPTYPE,X+1,T) S T=T+11
 I BGPPTYPE="D" D W^BGP8DP("# w/ Physical Activity Counseling",0,2,BGPPTYPE)
 I BGPPTYPE="P" D W^BGP8DP("# w/ Physical Activity",0,2,BGPPTYPE),W^BGP8DP(" Counseling",0,1,BGPPTYPE)
 S T=31 F X=1:1:3 S V=$P(@BGPARR@(X),U,8) D W^BGP8DP($S(BGPPTYPE="P":$$C(V,0,6),1:$S(V:V,1:0)),0,0,BGPPTYPE,X+1,T) S T=T+11
 I BGPPTYPE="D" D W^BGP8DP("% w/ Physical Activity Counseling",0,1,BGPPTYPE)
 I BGPPTYPE="P" D W^BGP8DP("% w/ Physical Activity",0,1,BGPPTYPE),W^BGP8DP(" Counseling",0,1,BGPPTYPE)
 S T=30 F X=1:1:3 S V=$P(@BGPARR@(X),U,9) D W^BGP8DP($S(BGPPTYPE="P":$J(V,6,1),1:$$SB($J(V,6,1))),0,0,BGPPTYPE,X+1,T) S T=T+11
 ...
 .I BGPPTYPE="P" D W^BGP8DP("# w/ Physical Activity",0,1,BGPPTYPE) S L=" Counseling",P=9 D PBY
 .I BGPPTYPE="D" S L="# w/ Physical Activity Counseling",P=9 D PBY
 .Q
 Q
PBY ;
 D W^BGP8DP(L,0,1,BGPPTYPE)
 S T=30 F X=1:1:3 S N=$P(BGPDAC(X),U,P),O=$P(@BGPARR@(X),U,3) S:N="" N=0 S:O="" O=0 S Y=$S(BGPPTYPE="P":$J($FN((N-O),"+,",1),6),1:$$SB($J((N-O),6,1))) D W^BGP8DP(Y,0,0,BGPPTYPE,X+1,T) S T=T+11
 Q
C(X,X2,X3) ;
 D COMMA^%DTC
 Q X
AH ;EP
 ...
 D W^BGP8DP(" 3 - 11",0,1,BGPPTYPE,2,34)
 D W^BGP8DP(" 12 - 17",0,0,BGPPTYPE,3,45)
 D W^BGP8DP("18+",0,0,BGPPTYPE,4,56)
