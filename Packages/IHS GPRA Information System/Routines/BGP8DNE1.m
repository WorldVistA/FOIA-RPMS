BGP8DNE1 ;IHS/CMI/LAB - NATL COMP;
...
EXPORT ;export to area or not?
 ...
 S BGPFN="CRSCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP8UTL(BGPXPBD)_$$D^BGP8UTL(BGPXPEDT)_$$D^BGP8UTL(BGPNOW)_"_001_of_001.TXT"
LOCALF ;
 S BGPLOCAL=""
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DNE1",ZTDTH="",ZTDESC="NATIONAL GPRA XP 11" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
...
CNTSF ;EP
 ...
 F BGPZ=1:1:BGPNF D
 .S BGPFN="CRSLCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP8UTL(BGPXPBD)_$$D^BGP8UTL(BGPXPEDT)_$$D^BGP8UTL(BGPNOW)_"_"_$$LZERO^BGP8UTL(BGPZ,3)_"_of_"_$$LZERO^BGP8UTL(BGPNF,3)_".TXT"
...
CNTSF1 ;EP
 ...
 S BGPZ=1,BGPFN="CRSCNT"_$P(^AUTTLOC(DUZ(2),0),U,10)_$$D^BGP8UTL(BGPXPBD)_$$D^BGP8UTL(BGPXPEDT)_$$D^BGP8UTL(BGPNOW)_"_001_of_001.TXT"
...
PROC1 ;process each patient
 ...
 ..Q:'$$ACTUP^BGP8D1(DFN,$$FMADD^XLFDT(BGPXXX,-(3*365)),BGPXXX,BGPTAXI,1)
 ..S BGPXPUP=1
...
PROCIND ;
 ...
 S $P(BGPXPPD,U,3)=$$EDT^BGP8UTL(DT)
 S $P(BGPXPPD,U,4)=$$EDT^BGP8UTL(BGPXPBD)
 S $P(BGPXPPD,U,5)=$$EDT^BGP8UTL(BGPXPEDT)
 S $P(BGPXPPD,U,6)=$$UID^BGP8DCHW(DFN)
 S $P(BGPXPPD,U,7)=$$EDT^BGP8UTL($P(^DPT(DFN,0),U,3))
 S $P(BGPXPPD,U,8)=$P(^DPT(DFN,0),U,2)
 S $P(BGPXPPD,U,9)=$$STATE^BGP8DCHW(DFN)
