BGP7D3A ; IHS/CMI/LAB - measure 11 17 Oct 2009 12:40 PM 15 Apr 2015 8:06 AM ;
...
SEALDEV(P,BDATE,EDATE) ;EP
 ...
 K BGPG
 S %=P_"^ALL ADA 0007;DURING "_$$DATE^BGP7UTL(T)_"-"_$$DATE^BGP7UTL(EDATE),E=$$START1^APCLDF(%,"BGPG(")
 S %="",X=0
 F  S X=$O(BGPG(X)) Q:X'=+X  S V=$P(BGPG(X),U,5) I '$$HAS13(V) S %=1_U_$$DATE^BGP7UTL($P(BGPG(1),U,1))_" ADA 0007"
 Q %
HAS13(V) ;
 NEW X,Y,Z
 S Z=0
 ...
 Q Z
 ;I $D(BGPG(1)) Q 1_U_$$DATE^BGP7UTL($P(BGPG(1),U,1))_" ADA 0007"
 ;S T=$$FMADD^XLFDT(EDATE,-1096)
 ;S %="",E=+$$CODEN^ICPTCOD("D0007"),%=$$CPTI^BGP7DU(P,T,EDATE,E)
 ;I % Q 1_"^"_$$DATE^BGP7UTL($P(%,U,2))_" CPT D0007"
 Q ""
TF(P,BDATE,EDATE) ;EP
 ...
 ..Q:'A
 ..I $$ICD^BGP7UTL2(A,$O(^ATXAX("B","BGP CPT TOPICAL FLUORIDE",0)),1) S T=1 S $P(BGPC,U,1)=$P(BGPC,U,1)+1,G=G+1,$P(BGPC,U,BGPC+1)=$$VD^APCLV($P(^AUPNVCPT(Y,0),U,3))_"|CPT "_$$VAL^XBDIQ1(9000010.18,Y,.01)  ;one per visit
 .Q:G  ;one per visit
 .S Y=0,G=0 F  S Y=$O(^AUPNVTC("AD",V,Y)) Q:Y'=+Y!(G)!(BGPC>3)  D
 ..Q:'$D(^AUPNVTC(Y,0))
 ..S I=$P(^AUPNVTC(Y,0),U,7)
 ..Q:I=""
 ..I $$ICD^BGP7UTL2(I,$O(^ATXAX("B","BGP CPT TOPICAL FLUORIDE",0)),1) S T=1 S $P(BGPC,U,1)=$P(BGPC,U,1)+1,G=G+1,$P(BGPC,U,BGPC+1)=$$VD^APCLV($P(^AUPNVTC(Y,0),U,3))_"|CPT/TRAN "_$$VAL^XBDIQ1(9000010.33,Y,.07)  ;one per visit
 .Q:G
 .S Y=0,G=0 F  S Y=$O(^AUPNVPOV("AD",V,Y)) Q:Y'=+Y!(G)!(BGPC>3)  D
 ..S A=$P($G(^AUPNVPOV(Y,0)),U) I A D
 ...I $$ICD^BGP7UTL2(A,$O(^ATXAX("B","BGP TOPICAL FLUORIDE DXS",0)),9) S $P(BGPC,U,1)=$P(BGPC,U,1)+1,G=1,$P(BGPC,U,BGPC+1)=$$VD^APCLV($P(^AUPNVPOV(Y,0),U,3))_"|POV "_$$VAL^XBDIQ1(9000010.07,Y,.01)  ;one per visit
 ...Q
 I BGPC Q BGPC
 Q ""
SEAL(P,BDATE,EDATE) ;EP
 ...
 ...I S="" S BGPX("NO OS")=$G(BGPX("NO OS"))+T,BGPCNT=BGPCNT+1,BGPSEALS(BGPCNT,$$VD^APCLV($P(^AUPNVDEN(Y,0),U,3)))="ADA  "_A_"("_T_")"
 ...Q
...
SEALR(P,BDATE,EDATE) ;EP
 ...
 .....S BGPX(S)=$G(BGPX(S))+1
 .....S BGPCNT=BGPCNT+1 S BGPSEALS(BGPCNT,$$VD^APCLV($P(^AUPNVDEN(Y,0),U,3)))="ADA "_A_" ("_T_")"
 ...I S="" S BGPX("NO OS")=$G(BGPX("NO OS"))+T,BGPCNT=BGPCNT+1,BGPSEALS(BGPCNT,$$VD^APCLV($P(^AUPNVDEN(Y,0),U,3)))="ADA 1353 ("_T_")"
 ...Q
...
DTAP(P,BDATE,EDATE) ;EP
 ...
 S BGPX=$$LASTITEM^BGP7DU(P,BDATE,EDATE,"IMMUNIZATION",115)
 I BGPX S BGPX=$P(BGPX,U,2)_U_$$DATE^BGP7UTL($P(BGPX,U,2))_" Imm 115"
 ...
 I $P(D,U,2)>$P(BGPX,U,1) S BGPX=$P(D,U,2)_U_$$DATE^BGP7UTL($P(D,U,2))_" CPT 90715"
 I BGPX Q 1_U_$P(BGPX,U,2)
 ;I $D(BGPVARI) Q 1_U_$$DATE^BGP7UTL($O(BGPVARI(9999999),-1))_" CPT 90715"
 F BGPZ=115 S X=$$ANCONT^BGP7D31(P,BGPZ,EDATE)
 I X]"" Q 2_U_$$DATE^BGP7UTL($P(X,U,1))_" "_$P(X,U,2)
 ;now go to Refusals
 ...
 I R Q 2_U_$$DATE^BGP7UTL(R)_" "_"NMI Dtap"
 S R="",B=+$$CODEN^ICPTCOD(90715)
 S G=$$NMIREF^BGP7UTL1(P,81,B,$$DOB^AUPNPAT(P),EDATE)
 I G Q 2_U_$$DATE^BGP7UTL($P(G,U,2))_" "_"NMI Dtap 90715"
 Q ""
DTAPTD(P,BDATE,EDATE) ;EP
 ...
 S LTD=$O(TDD(0))
 I LTD]"" S LTD=(9999999-LTD)_U_$$DATE^BGP7UTL(9999999-LTD)_" Imm "_TDD(LTD)
 ;now check cpt codes
 S T=$O(^ATXAX("B","BGP CPT TDAP/TD",0))
 S BGPVARI=$$CPT^BGP7DU(P,BDATE,EDATE,T,5)
 I $P(BGPVARI,U,1)>$P(LTD,U,1) S LTD=$P(BGPVARI,U,1)_U_$$DATE^BGP7UTL($P(BGPVARI,U,1))_" CPT "_$P(BGPVARI,U,2)
 K BGPG S %=P_"^LAST DX [BGP TD IZ DXS;DURING "_BDATE_"-"_EDATE,E=$$START1^APCLDF(%,"BGPG(")
 I $D(BGPG(1)),$P(BGPG(1),U,1)>$P(LTD,U,1) S LTD=$P(BGPG(1),U,1)_U_$$DATE^BGP7UTL($P(BGPG(1),U))_" DX V06.5"
 I LTD Q 1_U_$P(LTD,U,2)
 F BGPZ=115,9,113,138,139 S X=$$ANCONT^BGP7D31(P,BGPZ,EDATE) Q:X]""
 I X]"" Q 2_U_$$DATE^BGP7UTL($P(X,U,1))_" "_$P(X,U,2)
 ;now go to Refusals
 S B=$$DOB^AUPNPAT(P),E=EDATE,BGPNMI="",R=""
 ...
 ..Q:$P(^AUPNPREF(Y,0),U,7)'="N"
 ..S R=D    ;S:$P(^AUPNPREF(Y,0),U,7)="N" BGPNMI=1 S R=1
 I R Q 2_U_$$DATE^BGP7UTL(R)_" NMI Dtap/Td"
 S R="",B=+$$CODEN^ICPTCOD(90715)
 S G=$$NMIREF^BGP7UTL1(P,81,B,$$DOB^AUPNPAT(P),EDATE)
 I G Q 2_U_$$DATE^BGP7UTL($P(G,U,2))_" "_"NMI Dtap 90715"
 S R="",B=+$$CODEN^ICPTCOD(90714)
 S G=$$NMIREF^BGP7UTL1(P,81,B,$$DOB^AUPNPAT(P),EDATE)
 I G Q 2_U_$$DATE^BGP7UTL($P(G,U,2))_" "_"NMI Dtap 90714"
 S R="",B=+$$CODEN^ICPTCOD(90718)
 S G=$$NMIREF^BGP7UTL1(P,81,B,$$DOB^AUPNPAT(P),EDATE)
 I G Q 2_U_$$DATE^BGP7UTL($P(G,U,2))_" "_"NMI Dtap 90718"
 Q ""
 ;
LASTTDN ;
...
GA(P,BDATE,EDATE) ;EP
 ...
 ..S A=$P($G(^AUPNVCPT(Y,0)),U)
 ..Q:'A
 ..I $$ICD^BGP7UTL2(A,$O(^ATXAX("B","BGP CPT DENT GEN ANESTHESIA",0)),1) S BGPZ(V)=""
 ...
 ..S A=$P($G(^AUPNVCPT(Y,0)),U)
 ..Q:'A
 ..I $$ICD^BGP7UTL2(A,$O(^ATXAX("B","BGP CPT DENTAL SSC",0)),1) S BGPZ(V)=1
 I 'BGPGA Q ""
 S BGPGASSC=0,X=0 F  S X=$O(BGPZ(X)) Q:X'=+X  I BGPZ(X) S BGPGASSC=BGPGASSC+1
 S Z="",X=0 F  S X=$O(BGPZ(X)) Q:X'=+X  S:Z]"" Z=Z_"; " S Z=Z_$$DATE^BGP7UTL($$VD^APCLV(X))_": GEN ANESTH"_$S(BGPZ(X):", SSC",1:"")
