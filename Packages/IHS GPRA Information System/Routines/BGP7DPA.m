BGP7DPA ; IHS/CMI/LAB - COMP NATIONAL GPRA FOR PTS W/APPT 03 Jun 2017 2:54 PM 06 May 2017 1:23 PM ;
...
EP ;EP - called from option interactive
 ...
 I 'X W !!,"Exiting Report....." D PAUSE^BGP7DU,EOJ Q
RTYPE ;
 K BGPQUIT
 S BGPRT1=""
...
NT S BGPLIST="A"
 S BGPCPLC=0
 ;S XBRP="PRINT^BGP7DPAW",XBRC="PROC^BGP7DPA",XBRX="EOJ^BGP7DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
 K IOP,%ZIS
 W !! S %ZIS="PQM" D ^%ZIS
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP7DPA",ZTDTH="",ZTDESC="CRS 17 SCHED REPORT" D ^%ZTLOAD D EOJ Q
 Q
EOJ ;
 D ^XBFMK
 K DIC,DIR,DFN
...
SELPT ;
 ...
 .S DIR("?",5)="",DIR("?")="     ""[??"" will list your templates.",DIR("??")="^D LIST^BGP7DPAP"
 .D ^DIR K DIR
 .S:X[U X=U
 .I $E(X)="[" D  Q
 .. S X=$E(X,2,$L(X))
...
BDMG(BGPBD,BGPED,BGPCLN) ;EP - GUI DMS Entry Point
 ...
 S DIC="^BGPGUIG(",DIC(0)="L",DIADD=1,DLAYGO=9001004.4
 D FILE^DICN
 ...
 D ^XBFMK
 K ZTSAVE S ZTSAVE("*")=""
 S ZTIO="",ZTDTH=$$NOW^XLFDT,ZTRTN="GUIEP^BGP7DPA",ZTDESC="GUI CN GRPA APPT" D ^%ZTLOAD
 D EOJ
 Q
GUIEP ;EP
 D PROC
 K ^TMP($J,"BGP7DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP7DPA","^TMP($J,""BGP7DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP7DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP7DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIG(BGPIEN,11,X,0)=BGPDATA,C=C+1
 S ^BGPGUIG(BGPIEN,11,0)="^^"_C_"^"_C_"^"_DT_"^"
 S DA=BGPIEN,DIK="^BGPGUIG(" D IX1^DIK
 D ENDLOG
 K ^TMP($J,"BGP7DPA")
 S ZTREQ="@"
 Q
...
GUIECP ;EP
 K ^TMP($J,"BGP7DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP7DPA","^TMP($J,""BGP7DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP7DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP7DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIG(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
ENDLOG ;-- write the end of the log
 D NOW^%DTC
 S BGPNOW=$G(%)
 S DIE="^BGPGUIG(",DA=BGPIEN,DR=".04////"_BGPNOW_";.06////C"
 D ^DIE
 K DIE,DR,DA
 Q
 ;
PROC ;EP
 ...
 K ^XTMP("BGP7DPA",BGPGPRAJ,BGPGPRAH)
 S ^XTMP("BGP7DPA",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
 ...
 ..S BGPIISO=1,BGPISST="A" D PROCCY^BGP7D1
 .Q
 ...
 .S X=$O(^DIBT("B","RPMS DEMO PATIENT NAMES",0)) I X Q:$D(^DIBT(X,1,DFN))
 .I $G(BGPSTMP) S ^DIBT(BGPSTMP,1,DFN)="" Q
 .S BGPIISO=1,BGPISST=BGPRT1,BGPISSO=1 D PROCCY^BGP7D1
 .Q
 K ^XTMP("BGP15TAX",$J),^XTMP("BGPSNOMEDSUBSET",$J)
 Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP7DPA",XBRC="",XBRX="EOJ^BGP7DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
CPPRINT ;EP - called from xbdbque
 ...
 K BGPQ
 D CPHEADER
 S BGPGYR=2017,BGPGYR=$O(^BGPCTRL("B",BGPGYR,0))
...
TSKMNCP ;EP ENTRY POINT FROM TASKMAN
 ...
 I $D(IOM)#2,IOM S ZTIO=ZTIO_";"_IOM I $D(IOSL)#2,IOSL S ZTIO=ZTIO_";"_IOSL
 K ZTSAVE S ZTSAVE("BGP*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="CPPRINT^BGP7DPA",ZTDTH="",ZTDESC="CRS 15 SCHED REPORT DENOM" D ^%ZTLOAD D EOJ Q
 Q
STMP ;EP
EN1 ;EP Help
 K BGPQUIT S BGPSTMP=""
