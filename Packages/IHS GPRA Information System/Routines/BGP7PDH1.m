BGP7PDH1 ; IHS/CMI/LAB - cover page for gpra del 0 ;
...
PEDCP ;EP
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPPEDCG(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .Q
 S X=" " D SET(X,1,1)
 ...
 .S X=$P(^BGPPEDCG(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPPEDCG(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .S X="Communities: " D SET(X,1,1) S X=0,N=0,Y="",Z="" F  S X=$O(^BGPPEDCG(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPPEDCG(BGPX,9999,X,0),U)
