BGP8CSI ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for APCH HMR DISPLAY
 D EN^VALM("BGP 18 CMS INDICATOR SELECTION")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
...
INIT ; -- init variables and list array
 K BGPTIND S BGPHIGH=""
 S (X,Y,Z,C)=0 F  S X=$O(^BGPCMSIB("AO",X)) Q:X'=+X  S Y=$O(^BGPCMSIB("AO",X,Y)) Q:Y'=+Y  D
 .S C=C+1,BGPTIND(C,0)=C_")",$E(BGPTIND(C,0),5)=$P(^BGPCMSIB(Y,0),U,4),BGPTIND(C,C)=Y I $D(BGPIND(Y)) S BGPTIND(C,0)="*"_BGPTIND(C,0)
 .Q
 S (VALMCNT,BGPHIGH)=C
 Q
 ;
...
ADD ;EP - add an item to the selected list - called from a protocol
 W !
 S DIR(0)="LO^1:"_BGPHIGH,DIR("A")="Which Topic"
...
ADDALL ;
 ...
 ;
REM ;
 W ! S DIR(0)="LO^1:"_BGPHIGH,DIR("A")="Which Topic(s)" D ^DIR K DIR S:$D(DUOUT) DIRUT=1
 I Y="" W !,"No items selected." G ADDX
 I $D(DIRUT) W !,"No items selected." G ADDX
 D FULL^VALM1 W:$D(IOF) @IOF
