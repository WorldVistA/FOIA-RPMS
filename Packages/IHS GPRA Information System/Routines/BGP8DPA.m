...
GUIEP ;EP
 ...
 K ^TMP($J,"BGP8DPA")
 S IOM=80
 D GUIR^XBLM("PRINT^BGP8DPA","^TMP($J,""BGP8DPA"",")
...
GUIECP ;EP
 K ^TMP($J,"BGP8DPA")
 S IOM=80
 D GUIR^XBLM("CPPRINT^BGP8DPA","^TMP($J,""BGP8DPA"",")
 S X=0,C=0 F  S X=$O(^TMP($J,"BGP8DPA",X)) Q:X'=+X  D
 .S BGPDATA=^TMP($J,"BGP8DPA",X)
 .I BGPDATA="ZZZZZZZ" S BGPDATA=$C(12)
 .S ^BGPGUIR(BGPIEN,11,X,0)=BGPDATA,C=C+1
...
PROC ;EP
 ...
 S ^XTMP("BGP8DPA",0)=$$FMADD^XLFDT(DT,14)_"^"_DT_"^"_"PTS WITH GPRA DATA"
 K BGPAPPT,BGPAPPTS
 S BGPTA=0
 I BGPRT1="A" D  Q
...
ZISCP ;
 ...
 ;
 I $D(IO("Q")) G TSKMNCP
 ;S XBRP="CPPRINT^BGP8DPA",XBRC="",XBRX="EOJ^BGP8DPA",XBNS="BGP"
 ;D ^XBDBQUE
 ;
