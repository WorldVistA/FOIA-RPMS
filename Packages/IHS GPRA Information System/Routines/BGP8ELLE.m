BGP8ELLE ; IHS/CMI/LAB - print ind 03 Feb 2017 10:05 AM ;
...
I1AGE ;EP
 ...
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G,H,I,J,K)=0 F BGPX="TAD.1","TAD.2","TAD.3","TAD.4" D I1AGE1,I1AGE2,I1AGE3,I1AGE4,I1AGE5,I1AGE6
 D I1AGEP
MALE ;
 S BGPHD1="MALE ACTIVE CLINICAL PATIENTS 55+",BGPHD2="# Male Active Clinical Patients"
 S X=" " D S(X,1,1) D H3^BGP8ELL2 S X=" " D S(X,1,1)
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G,H,I,J,K)=0 F BGPX="TAE.1","TAE.2","TAE.3","TAE.4" D I1AGE1,I1AGE2,I1AGE3,I1AGE4,I1AGE5,I1AGE6
 D I1AGEP
FEMALE ;
 S BGPHD1="FEMALE ACTIVE CLINICAL PATIENTS 55+",BGPHD2="# Female Active Clinical Patients"
 S X=" " D S(X,1,1) D H3^BGP8ELL2 S X=" " D S(X,1,1)
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G,H,I,J,K)=0 F BGPX="TAF.1","TAF.2","TAF.3","TAF.4" D I1AGE1,I1AGE2,I1AGE3,I1AGE4,I1AGE5,I1AGE6
 D I1AGEP
 Q
I1AGE1 ;
 S C=C+1
...
I1AGE5 ;
 S G=G+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+20) S BGPPC=$O(^BGPELIIR("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIR(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90561.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGE6 ;
 S H=H+1
 S BGPF="ELD."_$P(BGPX,".")_"."_($P(BGPX,".",2)+16) S BGPPC=$O(^BGPELIIR("C",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPELIIR(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90561.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGEP ;
 ...
 S X=" " D S(X,1,1) S X="# Tobacco Users w/ % of Total Screened" D S(X,1,1)
 ...
 S X=" " D S(X,1,1) S X="A. # Smokers w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,6) S Y=V D S(Y,,X+1)
 S X="A. % Smokers w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,7) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="B. # Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,8) S Y=V D S(Y,,X+1)
 S X="B. % Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,9) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="C. # ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,10) S Y=V D S(Y,,X+1)
 S X="C. % ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,11) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 F X=1:1:4 S V=$P(BGPDAC(X),U,13) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ...
 S X="A. % Smokers w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,7) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="B. # Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,8) S Y=V D S(Y,,X+1)
 S X="B. % Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,9) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="C. # ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,10) S Y=V D S(Y,,X+1)
 S X="C. % ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,11) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# ETS/Smk Home w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,12) S Y=V D S(Y,,X+1)
 S X="% ETS/Smk Home w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAP(X),U,13) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 ...
 S X="A. Smokers w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,7),O=$P(BGPDAP(X),U,7) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="B. Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,9),O=$P(BGPDAP(X),U,9) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="C. ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,11),O=$P(BGPDAP(X),U,11) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="ETS/Smk Home w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAP(X),U,13) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
BL ;
 ...
 S X="A. % Smokers w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,7) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="B. # Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,8) S Y=V D S(Y,,X+1)
 S X="B. % Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,9) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="C. # ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,10) S Y=V D S(Y,,X+1)
 S X="C. % ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,11) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# ETS/Smk Home w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,12) S Y=V D S(Y,,X+1)
 S X="% ETS/Smk Home w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAB(X),U,13) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ;percentage changes
 ...
 S X="A. Smokers w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,7),O=$P(BGPDAB(X),U,7) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="B. Smokeless w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,9),O=$P(BGPDAB(X),U,9) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="C. ENDS Users w/ % of Total Tobacco Users" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,11),O=$P(BGPDAB(X),U,11) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 S X="ETS/Smk Home w/ % of Total Screened" D S(X,1,1)
 F X=1:1:4 S N=$P(BGPDAC(X),U,13),O=$P(BGPDAB(X),U,13) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
