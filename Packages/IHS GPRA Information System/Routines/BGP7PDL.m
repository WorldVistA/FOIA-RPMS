...
DEL ;
 ...
 D ^BGP7PDLH
 D DEL1
 I BGPRTYPE=1 D NONSUM^BGP7PDLN
 I BGPRTYPE=1 D DEVSUM^BGP7PDLD
 ;I BGPRTYPE=1 D SUMMARY
 I BGPRTYPE=1 D SDP^BGP7PDLN
 I BGPRTYPE=1 D SDP^BGP7PDLD
 ;I BGPRTYPE=1 D SDP
 I BGPRTYPE=7 D OTHSUM^BGP7PDLO
...
SAVEDEL ;EP
 ...
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="GPRA 17 DELIMITED OUTPUT",XBQ="N",XBF=0
 .D ^XBGSAVE
...
DEL1 ;EP
 ...
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC=""!(BGPQUIT)  I $P(^BGPINDG(BGPIC,0),U,14) D PRINT2 D
 .Q:BGPQUIT
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),'$D(BGPYNPLT) S BGPINDG=BGPIC D NPL1^BGP7NPLP ;printed nat gpra list
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),$D(BGPYNPLT) S BGPINDG=BGPIC D CT^BGP7DSTM ;printed nat gpra SEARCH TEMPLATE
 Q:BGPQUIT
...
PRINT2 ;
 ...
 .S X=$P(^BGPINDG(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 .S X="Denominator(s):" D S(X,1,1)
 .S BGPNODE=$S(BGPRTYPE=1:81,BGPRTYPE=4:61,BGPRTYPE=7:83,BGPRTYPE=9:85,1:61)
 .S BGPX=0 F  S BGPX=$O(^BGPINDG(BGPIC,BGPNODE,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPINDG(BGPIC,BGPNODE,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...;I $P(^BGPINDG(BGPIC,61,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...;I '$D(^BGPINDG(BGPIC,61,BGPY,11,"B",BGPRTYPE)) Q
 ...I BGPRTYPE=4,'$D(^BGPINDG(BGPIC,BGPNODE,BGPY,12,"B",BGPINDG)) Q
 ...S BGPZ=0 F  S BGPZ=$O(^BGPINDG(BGPIC,BGPNODE,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S Y=^BGPINDG(BGPIC,BGPNODE,BGPY,1,BGPZ,0) D S(Y,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 ...
 .S BGPX=0 F  S BGPX=$O(^BGPINDG(BGPIC,BGPNODE,"B",BGPX)) Q:BGPX'=+BGPX  D
 ..S BGPY=0 F  S BGPY=$O(^BGPINDG(BGPIC,BGPNODE,"B",BGPX,BGPY)) Q:BGPY'=+BGPY  D
 ...;I $P(^BGPINDG(BGPIC,62,BGPY,0),U,2)'[BGPRTYPE Q  ;not a denom def for this report
 ...;I BGPRTYPE=4,BGPINDG'="S",$P(^BGPINDG(BGPIC,62,BGPY,0),U,3)'[BGPINDG Q  ;don't display
 ...;I '$D(^BGPINDG(BGPIC,62,BGPY,11,"B",BGPRTYPE)) Q  ;not this report type
 ...I BGPRTYPE=4,'$D(^BGPINDG(BGPIC,BGPNODE,BGPY,12,"B",BGPINDG)) Q
 ...S BGPZ=0 F  S BGPZ=$O(^BGPINDG(BGPIC,BGPNODE,BGPY,1,BGPZ)) Q:BGPZ'=+BGPZ  D
 ....S X=^BGPINDG(BGPIC,BGPNODE,BGPY,1,BGPZ,0) D S(X,1,1)
 ....Q
 ...Q
 ..Q
 .S X=" " D S(X,1,1)
 ...
 .S X="Logic:" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPINDG(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDG(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .I BGPRTYPE=9 G CALC
 .S X=" " D S(X,1,1) S X="Performance Measure Description:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDG(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDG(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX,0) D S(X,1,1)
 .I $O(^BGPINDG(BGPIC,52,0)) S X=" " D S(X,1,1) S X="Past Performance and/or Target:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDG(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDG(BGPIC,52,BGPX,0) D S(X,1,1)
 .I $O(^BGPINDG(BGPIC,55,0)) S X=" " D S(X,1,1) S X="Source:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDG(BGPIC,55,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDG(BGPIC,55,BGPX,0) D S(X,1,1)
 .I '$O(^BGPINDG(BGPIC,55,0)) D S(" ",1,1)
CALC .X ^BGPINDG(BGPIC,4)
 Q
HEADER ;EP
 S Y=$P(^VA(200,DUZ,0),U,2),$E(Y,35)=$$FMTE^XLFDT(DT) D S(Y,1,1)
 I BGPRTYPE=7 S Y="*** IHS 2017 Other National Measures Report ***" D S(Y,1,1)
 I BGPRTYPE=4 S Y="*** IHS 2017 Clinical Performance Report ***" D S(Y,1,1)
 I BGPRTYPE=1,'$G(BGPNGR09) S Y="*** IHS 2017 National GPRA/GPRAMA Report ***" D S(Y,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S Y="*** IHS 2017 National GPRA/GPRAMA Report, Run Using 2017 Logic ***" D S(Y,1,1)
