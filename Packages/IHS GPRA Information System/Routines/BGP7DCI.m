BGP7DCI ; IHS/CMI/LAB - IHS area GPRA 16 Dec 2006 9:12 AM 19 Feb 2017 11:04 AM ;
 ;;17.1;IHS CLINICAL REPORTING;;MAY 10, 2017;Build 29
 ;
CALCIND ;EP - CALCULATE ALL MEASURES
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC'=+BGPIC  D
 .I BGPRTYPE=1,$P(^BGPINDG(BGPIC,0),U,7)'=1 Q  ;national gpra report
 .I BGPRTYPE=7,$P($G(^BGPINDG(BGPIC,12)),U,1)'=1 Q  ;OTHER NATIONAL
 ...
 .K BGPNUMV
 .K ^TMP($J)
 .I $D(^BGPINDG(BGPIC,1)) X ^BGPINDG(BGPIC,1)
 .K BGPVAL,BGPG,BGPC,BGPALLED,BGPV,A,B,C,D,E,F,G,H,I,J,K,M,N,O,P,Q,R,S,T,V,W,X,Y,Z
 .K ^TMP($J)
 .;Set up global for iCare
 .I $G(BQIGREF)'="",$D(BGPSTOP) S @BQIGREF@(DFN,BGPIC)=$P(^BGPINDG(BGPIC,0),U,3)_"^N/A" Q
 .I $D(BGPSTOP) Q  ;no need to set since no num/denom
 .;loop each individual to set numerator and denominator
 .S BGPI=0 F  S BGPI=$O(^BGPINDGC("B",BGPIC,BGPI)) Q:BGPI'=+BGPI  D
 ..S (BGPNUM,BGPDEN)=0
 ..X ^BGPINDGC(BGPI,1)
 ..X ^BGPINDGC(BGPI,2) ;denominator 1 or 0
 ..;set field counter
 ..S BGPNF=$P(^BGPINDGC(BGPI,0),U,9)
 ..S BGPN=$P(^DD(90558.03,BGPNF,0),U,4)
 ..S N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..D S(BGPRPT,BGPGBL,N,P,BGPNUM),S1("N",BGPNUM)
 ..S BGPDF=$P(^BGPINDGC(BGPI,0),U,8)
 ..S BGPN=$P(^DD(90558.03,BGPDF,0),U,4),N=$P(BGPN,";"),P=$P(BGPN,";",2)
 ..I BGPDEN'="NO" D S(BGPRPT,BGPGBL,N,P,BGPDEN),S1("D",BGPDEN)
 ..I $G(BGPCPPL) D CPL
 .I $D(BGPLIST(BGPIC)) D STMP^BGP7UTL
 .I $G(BGPNPL) D NPL
 .F X=1:1:99 S Y="BGPN"_X K @Y
 .F X=1:1:99 S Y="BGPD"_X K @Y
 Q
...
CPL ;comprehensive pat list check and set xtmp
 ...
 Q:BGPTIME'=1
 Q:$P($G(^BGPINDGC(BGPI,12)),U,1)=""
 X ^BGPINDGC(BGPI,3) Q:'$T
 ...
 .S ^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN)=F_"|||"_$P(^BGPINDGC(BGPI,12),U),BGPCPLC=BGPCPLC+1
 S $P(^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||")=F
 S $P(^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)=$P(^XTMP("BGP28CPL",BGPJ,BGPH,"LIST",C,S,BGPAGEB,DFN),"|||",2)_"#"_$P(^BGPINDGC(BGPI,12),U)
 Q
NPL ;
 Q:BGPTIME'=1
 ...
 .X ^BGPNPLG(BGPX,12) K ^TMP($J) Q:'$T
 .S BGPINDL(BGPIC,BGPX)=$G(BGPINDL(BGPIC,BGPX))+1
 .I $G(BGPYNPLT) S ^XTMP("BGP7DNP",BGPJ,BGPH,"LIST",BGPORD,BGPIC,BGPX,DFN)="" Q
 .S BGPO=$S(BGPRTYPE=7:$P(^BGPNPLG(BGPX,0),U,6),1:$P(^BGPNPLG(BGPX,0),U,5))
 .S A=""
 .I $P(^BGPINDG(BGPIC,0),U,2)=151 S A=BGPAGEE G SL
 .I $P(^BGPINDG(BGPIC,0),U,2)=171 S A=BGPAGEE G SL
 .S A=BGPAGEB
SL .I $P(^BGPNPLG(BGPX,0),U,7)=9,$G(BGPVALUD)]"" S BGPVALUE=BGPVALUD
 .S ^XTMP("BGP7DNP",BGPJ,BGPH,"LIST",BGPORD,BGPIC,BGPO,BGPX,$S($P($G(^AUPNPAT(DFN,11)),U,18)]"":$P(^AUPNPAT(DFN,11),U,18),1:"UNKNOWN"),$P(^DPT(DFN,0),U,2),A,DFN)=$G(BGPVALUE)
...
S1(BQITYP,BQIVAL) ; Return data by patient for iCare into global reference BQIGREF
 ...
 S BQIIDTA=$G(^BGPINDGC(BGPI,0))
 S BQIDTA=$G(^BGPINDGC(BGPI,14))
 ;
 ...
 ;
 ;  Get the GOAL 2017 value and the GOAL 06 value
 ...
 S $P(@BQIGREF@(DFN,BGPIC),U,1)=$P(^BGPINDG(BGPIC,0),U,3)
