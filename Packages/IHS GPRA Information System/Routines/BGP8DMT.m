BGP8DMT ;IHS/CMI/LAB - NATIONAL LIST;
...
QUE ;
 ...
 S ZTRTN="PRINT^BGP8DMT",ZTDESC="BGP 18 MED TAX REPORT",ZTIO=ION,ZTDTH=""
 D ^%ZTLOAD
 D HOME^%ZIS
 D XIT
 Q
...
IPC ;EP
 I '$G(BGPGUI) D XIT  ;cmi/maw 10/30/2009
 S BGPRPTN="Improving Patient Care (IPC)",BGPRT(8)=""
 D START
 Q
PRINT ;
 S (BGPPAGE,BGPQUIT)=0
...
N ;GATHER UP AND DISPLAY ALL NATIONAL GPRA
 ...
 S BGPTNAME="" F  S BGPTNAME=$O(^BGPTAXR("B",BGPTNAME)) Q:BGPTNAME=""!(BGPQUIT)  D
 .S BGPTIEN=0,BGPTIEN=$O(^BGPTAXR("B",BGPTNAME,BGPTIEN))
 .Q:'BGPTIEN  ;oops, error in xref
 .Q:'$D(^BGPTAXR(BGPTIEN,0))  ;oops, error in xref
 .Q:$P(^BGPTAXR(BGPTIEN,0),U,2)'="M"  ;only DRUG/MEDICATION taxonomies in this report
 .S (G,X)=0 F  S X=$O(^BGPTAXR(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  I $D(BGPRT(X)) S G=1
 .Q:'G
 .;now eliminate those in BGPNO
 .S (G,X)=0 F  S X=$O(^BGPTAXR(BGPTIEN,12,"B",X)) Q:X'=+X!(G)  D
 ..I $D(BGPNO(X)) S G=1
 .Q:G
...
INTROT ;introductory text
 ...
 ;;CRS 2018.  The report is organized by (1) taxonomies included in the
