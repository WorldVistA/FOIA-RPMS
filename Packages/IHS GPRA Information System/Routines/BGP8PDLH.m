BGP8PDLH ; IHS/CMI/LAB - cover page ;
 ...
 I BGPRTYPE=4,$G(BGPYRPTH)="C" S X="*** IHS 2018 Selected Measures with Community Specified Report ***" D SET(X,1,1)
 I BGPRTYPE=4,$G(BGPYRPTH)="A" S X="*** IHS 2018 Selected Measures with All Communities Report ***" D SET(X,1,1)
 I BGPRTYPE=4,$G(BGPYRPTH)="P" S X="*** IHS 2018 Selected Measures with Patient Panel Population Report ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S X="*** IHS 2018 National GPRA/GPRAMA Report, Run Using 2018 Logic ***" D SET(X,1,1) G N
 I BGPRTYPE=1,$G(BGPDESGP) S X="*** IHS 2018 National GPRA/GPRAMA Report by Designated Provider ***" D SET(X,1,1) G N
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) S X="*** IHS 2018 National GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) S X="*** IHS 2018 National GPRA/GPRAMA Report Clinical Performance Summaries ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPYGPU) S X="*** IHS 2018 GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=7 S X="*** IHS 2018 Other National Measures Report ***" D SET(X,1,1)
 I BGPRTYPE=6,'$G(BGPEDPP) S X="*** IHS 2018 Patient Education with Community Specified Report ***" D SET(X,1,1)
 I BGPRTYPE=6,$G(BGPEDPP) S X="*** IHS 2018 Patient Education with Patient Panel Population Report ***" D SET(X,1,1)
N ;
 ...
 I BGPRTYPE=4 S X="Measures: "_$P($T(@BGPINDG),";;",2) D SET(X,1,1)
 ...
 I BGPROT'="P" D
 .Q:$D(BGPGUI)
 .S X="A delimited output file called "_BGPDELF D SET(X,1,1) S X="has been placed in the "_$$GETDEDIR^BGP8UTL2()_" directory for your use in Excel or some" D SET(X,1,1) S X="other software package." D SET(X,1,1)
...
ENDTIME ;
 ...
 Q
AREACP ;EP
 I BGPRTYPE=6 S X="*** IHS 2018 Patient Education with Community Specified Report***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPNGR09) S X="*** IHS 2018 National GPRA/GPRAMA Report, Run Using 2018 Logic ***" D SET(X,1,1) G NA
 I BGPRTYPE=1,'$G(BGPYGPU),'$G(BGPSUMON) S X="*** IHS 2018 National GPRA/GPRAMA Report ***" D SET(X,1,1)
 I BGPRTYPE=1,'$G(BGPYGPU),$G(BGPSUMON) S X="*** IHS 2018 National GPRA/GPRAMA Report Clinical Performance Summaries ***" D SET(X,1,1)
 I BGPRTYPE=1,$G(BGPYGPU) S X="*** IHS 2018 GPRA/GPRAMA Performance Report ***" D SET(X,1,1)
 I BGPRTYPE=7 D SET("*** IHS 2018 Other National Measures Report ***",1,1)
NA S X="AREA AGGREGATE" D SET(X,1,1)
 ...
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCR(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .Q
 S X=" " D SET(X,1,1)
 ...
 .S X=$P(^BGPGPDCR(BGPX,0),U,9),X=$O(^AUTTLOC("C",X,0)) S X=$S(X:$P(^DIC(4,X,0),U),1:"?????")
 .S BGPC=BGPC+1,X=BGPC_".  "_$S($P(^BGPGPDCR(BGPX,0),U,17):"*",1:"")_X D SET(X,1,1)
 .S X="Community Taxonomy Name: "_$P(^BGPGPDCR(BGPX,0),U,18) D SET(X,1,1)
 .S X=0,N=0,Y="",Z="" F  S X=$O(^BGPGPDCR(BGPX,9999,X)) Q:X'=+X  S N=N+1,Y=Y_$S(N=1:"",1:";")_$P(^BGPGPDCR(BGPX,9999,X,0),U)
