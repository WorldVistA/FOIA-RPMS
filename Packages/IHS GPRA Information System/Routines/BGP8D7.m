...
TA ;EP - called from elder
 S BGPTOB=$$TOBHF^BGP8D712(DFN,BGPBDATE,BGPEDATE)  ;get last HF from the 4 categories
 ...
 .I $P(F,U,10)["CURRENT"!($P(F,U,10)["CESSATION") S BGPN2=1,BGPVAL=$P(BGPTOB,U,11)_" SCREEN, "_$P(BGPTOB,U,11)_" USER" Q  ;ENDS
 .I BGPUDX]"" S BGPN2=1,BGPVAL=$$DATE^BGP8UTL($P(BGPUDX,U,2))_" SCREEN, "_$$DATE^BGP8UTL($P(BGPUDX,U,2))_" USER" Q
 .I BGPSMDX]"" S BGPN2=1,BGPVAL=$$DATE^BGP8UTL($P(BGPSMDX,U,2))_" SCREEN, "_$$DATE^BGP8UTL($P(BGPSMDX,U,2))_" USER" Q
 .I BGPSLDX]"" S BGPN2=1,BGPVAL=$$DATE^BGP8UTL($P(BGPSLDX,U,2))_" SCREEN, "_$$DATE^BGP8UTL($P(BGPSLDX,U,2))_" USER" Q
 .I ($P(BGPSCPT,U)="1034F")!($P(BGPSCPT,U)="1035F")!($P(BGPSCPT,U)="G0376")!($P(BGPSCPT,U)="G0375")!($P(BGPSCPT,U)=99407)!($P(BGPSCPT,U)="G9276") D  Q
 ..S BGPN2=1,BGPVAL=$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" SCREEN, "_$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" USER" Q
 .I $P(BGPSCPT,U)=99406!($P(BGPSCPT,U)="G8455")!($P(BGPSCPT,U)="G8456")!($P(BGPSCPT,U)="G8402")!($P(BGPSCPT,U)="G8453") S BGPN2=1,BGPVAL=$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" SCREEN, "_$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" USER" Q
 ;BGPN3 - SMOKER OR NOT
 D
 ...
 .I BGPSMDX]"" S BGPN3=1,BGPVAL=BGPVAL_", "_$$DATE^BGP8UTL($P(BGPSMDX,U,2))_" SMOKER" Q
 ...
 ..S BGPN3=1,BGPVAL=BGPVAL_", "_$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" SMOKER" Q
 ;BGPN4 - SMOKELESS
 D
 ...
 .I $P(BGPSCPT,U)="1035F"!($P(BGPSCPT,U)="G8456") S BGPN4=1,BGPVAL=BGPVAL_", "_$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" SMOKELESS" Q
 .I BGPSLDX]"" S BGPN4=1,BGPVAL=BGPVAL_", "_$$DATE^BGP8UTL($P(BGPSLDX,U,2))_" SMOKELESS" Q
 ;BGPN5 - ETS
 ...
 ;ENDS USER
 D
 .I $P(F,U,10)="CURRENT ENDS USER"!($P(F,U,10)="CESSATION ENDS USER") S BGPN11=1 D
 ..S BGPVAL=BGPVAL_", "_$P(F,U,11)_" ENDS"
 I BGPN1,BGPVAL="" D
 .I BGPTOB]"" S BGPVAL=$S($P(BGPTOB,U,1)]"":$P(BGPTOB,U,2)_" SCREEN",$P(BGPTOB,U,5)]"":$P(BGPTOB,U,5)_" SCREEN",$P(BGPTOB,U,8)]"":$P(BGPTOB,U,8)_" SCREEN",$P(BGPTOB,U,11)]"":$P(BGPTOB,U,11)_" SCREEN",1:"") Q
 .I BGPSDX]"" S BGPVAL=$$DATE^BGP8UTL($P(BGPSDX,U,2))_" SCREEN" Q
 .I BGPSCPT]"" S BGPVAL=$$DATE^BGP8UTL($P(BGPSCPT,U,2))_" SCREEN" Q
 .I BGPXPHD]"" S BGPVAL=$$DATE^BGP8UTL($P(BGPXPHD,U,2))_" SCREEN" Q
 .I BGP1320]"" S BGPVAL=$$DATE^BGP8UTL($P(BGP1320,U,2))_" SCREEN" Q
 S V=$S(BGPD1:"UP",1:"")_$S(BGPACTCL:",AC",1:"")_$S(BGPD8:",PREG",1:"")
 S BGPVALUE=V_"|||"_BGPVAL
 ;I DUZ=2881,BGPTIME=1 W !,DFN,"   ",BGPN1,"   ",BGPVALUE,"    ",BGPVAL
 ;
...
PED(P,BDATE,EDATE) ;EP
 ...
 I '$D(BGPG) Q ""
 S SN=$O(^BGPSNOMR("B","TOBACCO SCREEN PATIENT ED",0))
 ...
 .I $P(S,U,1)'="-1",$$ICD^BGP8UTL2($P(S,U,1),$O(^ATXAX("B","BGP TOBACCO DXS",0)),9) S %=T_U_$P(BGPG(X),U) Q
...
DX(P,BDATE,EDATE) ;EP  - WAS THERE SCREENING?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP8UTL1(P,"BGP TOBACCO DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP8UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
 ...
 .S G=$P($$ICDDX^BGP8UTL2(Y),U,2)_U_$P(^AUPNPROB(X,0),U,3)_U_Y
 .Q
 I G]"" Q G
 S G=$$IPLSNOID^BGP8DU(P,"PXRM BGP TOBACCO SCREENED",BDATE,EDATE) I G S G=$P(G,U,2)_U_$P(G,U,3) Q G
 Q G
DXSL(P,BDATE,EDATE) ;EP - WAS THERE A SMOKELESS USER DX?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP8UTL1(P,"BGP GPRA SMOKELESS DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP8UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
 ...
 .Q
 I G]"" Q G
 S G=$$IPLSNOID^BGP8DU(P,"PXRM BGP TOBACCO SMOKELESS",BDATE,EDATE) I G S G=$P(G,U,2)_U_$P(G,U,3) Q G
 Q G
 ;
DXU(P,BDATE,EDATE) ;EP - WAS THERE A USER DX?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP8UTL1(P,"BGP TOBACCO USER DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP8UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
 ...
 .Q
 I G]"" Q G
 S G=$$IPLSNOID^BGP8DU(P,"PXRM BGP CURRENT TOBACCO",BDATE,EDATE) I G S G=$P(G,U,2)_U_$P(G,U,3) Q G
 Q G
DXS(P,BDATE,EDATE) ;EP - WAS THERE A SMOKING USER DX?
 K BGPG
 S BGPG(1)=$$LASTDX^BGP8UTL1(P,"BGP GPRA SMOKING DXS",BDATE,EDATE)
 I BGPG(1)]"" Q $P($$ICDDX^BGP8UTL2($P(BGPG(1),U,4),$P(BGPG(1),U,1)),U,2)_U_$P(BGPG(1),U,3)_U_$P(BGPG(1),U,4)
 ...
 .Q
 I G]"" Q G
 S G=$$IPLSNOID^BGP8DU(P,"PXRM BGP TOBACCO SMOKER",BDATE,EDATE) I G S G=$P(G,U,2)_U_$P(G,U,3) Q G
 Q G
...
TOBACCO0 ;lookup in health status
 ...
 S Y=$P(^AUTTHF(Y,0),U)
 S BGPTOB=Y_"^"_$$DATE^BGP8UTL(9999999-E)_"^"_(9999999-E)
 K Y,E,X
 Q
 ;
LASTHF(P,C,BDATE,EDATE) ;EP - get last factor in category C for patient P
 ...
 S D=$O(O(0))
 I D="" Q D
 Q $$VAL^XBDIQ1(9000010.23,O(D),.01)_"^"_$$DATE^BGP8UTL(9999999-D)_"^"_(9999999-D)
 ;
LOINC(A,B) ;
 NEW %
...
CPTSM(P,BDATE,EDATE) ;EP - did pat have TOBACCO SCREENING cpt?
 NEW X
 S X=$$CPT^BGP8DU(P,BDATE,EDATE,$O(^ATXAX("B","BGP TOBACCO SCREEN CPTS",0)),5)
