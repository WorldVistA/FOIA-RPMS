BGP8PDPA ; IHS/CMI/LAB - print ind 03 Jul 2010 8:56 AM ;
...
I1AGEP ;
 ...
 S X=" " D S(X,1,1) S X="# w/ Exercise Educ w/ % of Total Assessed" D S(X,1,1)
 F X=1:1:8 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="% # w/ Exercise Educ w/ % of Total Assessed" D S(X,1,1)
 F X=1:1:8 S V=$P(BGPDAC(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# w/ Exercise Goal w/ % of Total Assessed" D S(X,1,1)
 F X=1:1:8 S V=$P(BGPDAC(X),U,6) S Y=V D S(Y,,X+1)
 S X="% # w/ Exercise Goal w/ % of Total Assessed" D S(X,1,1)
 F X=1:1:8 S V=$P(BGPDAC(X),U,7) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
PR ; 
 ;S X=^BGPINDR(BGPIC,53,1,0) D S(X,1,1) D H9
