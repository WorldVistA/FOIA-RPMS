BGP8ELLT ; IHS/CMI/LAB - print ind ;
...
PR ; 
 ...
 F X=1:1:3 S V=$P(BGPDAP(X),U,15) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ...
 ;F X=1:1:3 S V=$P(BGPDAP(X),U,17) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="G. # w/ Pain Med Rx w/ % of Total Meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,18) S Y=V D S(Y,,X+1)
 S X="G. % w/ Pain Med Rx w/ % of Total Meds" D S(X,1,1)
 F X=1:1:3 S V=$P(BGPDAP(X),U,19) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ;
 ...
 F X=1:1:3 S V=$P(BGPDAP(X),U,29) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ;
 ...
 F X=1:1:3 S V=$P(BGPDAP(X),U,41) S Y=$$SB^BGP8ELL2($J(V,6,1)) D S(Y,,X+1)
 ;% changes
 ...
 S BGP1="G. # w/ Pain Med Rx",BGPPIE=19 D PPC
 S BGP1="H. # w/ Skeletal Muscle Relaxant Rx",BGPPIE=29 D PPC
 S BGP1="# w/ Exposure to Multiple High-Risk Meds",BGPPIE=41 D PPC
 D ^BGP8ELLU
 Q
PPC ;
 S X=BGP1 D S(X,1,1)
 F X=1:1:3 S N=$P(BGPDAC(X),U,BGPPIE),O=$P(BGPDAP(X),U,BGPPIE) S Y=$$SB^BGP8ELL2($J((N-O),6,1)) D S(Y,,X+1)
 Q
V(T,R,N,P) ;EP
 I $G(BGPAREAA) G VA
 NEW X
