BGP7GAHE ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 17 AREA HEDIS","A",.LORISUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPRPTT,BGPSUL,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPROT,BGPRTIME,BGPFILE) ;EP - called from GUI to produce national gpra report (AO-AGP)
 ...
 ;            from BGP 17 HEDIS DATA CURRENT that match the following:
 ;            (SEE BGP7ASL or CALL ME)
 ;  BGPSUL  - array containing Iens from BGP 17 DATA CURRENT that the user selected
 ;  BGPSUL - ARRAY OF IENS FROM THE D GET^BGP7ASL(.BGPSUL,.BGPFILE,3,....... call that the user selected
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOHED^BGP7GAHE",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA HEDIS REPORT 06" D ^%ZTLOAD
 D UPLOG^BGP7GUA(BGPGIEN,ZTSK)
 Q
AOHED ;
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
 D GUIR^BGPXBLM("PRINT^BGP7PHEP","^TMP($J,""BGPGUI"",")
