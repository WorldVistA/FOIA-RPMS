BGP8D55 ; IHS/CMI/LAB - measure calc ;
...
IASCRN ;EP
 ...
 I BGPACTCL,BGPAGEB>11 S BGPD1=1
 I BGPACTCL,BGPAGEB>8 S BGPD11=1
 I BGPACTUP,BGPAGEB>11 S BGPD9=1
 I BGPACTUP S BGPD10=1
 I BGPACTCB D
 .I BGPAGEB>11 S BGPD1=1
 ...
 .I BGPAGEB>8 S BGPD11=1
 .I BGPAGEB>8,BGPAGEB<12 S BGPD12=1
 I BGPSEX="F",BGPAGEB>13,BGPAGEB<47,BGPACTCL S BGPD8=1
 I '(BGPD1+BGPD2+BGPD3+BGPD4+BGPD5+BGPD6+BGPD7+BGPD8+BGPD9+BGPD10+BGPD11+BGPD12) S BGPSTOP=1 Q
 ;BGPN1 - had screen or dx
 ;BGPN2 - had educ
 ;BGPN3 - had screening
 ;BGPN4 - had positive screen
ALEP ;did they have screening?
 K BGPALL
 K ^TMP($J,"A")
 ...
 I BGPALDX S BGPN1=1,BGPN6=1  ;BGPALDX
 ;set numerators
 S BGPN2=$$ALPED^BGP8D5(DFN,BGPBDATE,BGPEDATE)
 I BGPN1!(BGPN2) S BGPN5=1 ;HAD EITHER SCREENING OR EDUC G.B.7
 ;
 S BGPVALUE="UP"_$S(BGPD8:",AC",1:"")_$S(BGPD8&(BGPD1):",",1:"")_$S(BGPD1!(BGPD12):"AC+BH",1:"")_"|||"
...
ALDX(P,BDATE,EDATE) ;EP
 ...
 ..I $$ICD^BGP8UTL2(BGPP,BGPT,9) S BGPC=1_U_"BH POV "_$P(^AMHPROB(BGPP,0),U)_U_$$DATE^BGP8UTL((9999999-$P(D,".")))_U_(9999999-$P(D,".")) Q
 ..S BGPP=$P($G(^AMHPROB(BGPP,0)),U)
 ..Q:BGPP=""
 ..I $D(^ATXAX(BGPJ,21,"B",BGPP)) S BGPC=1_U_"BH POV "_BGPP_U_$$DATE^BGP8UTL((9999999-$P(D,".")))_U_(9999999-$P(D,".")) Q
 ..Q
 I $P(BGPLAL,U,4)<$P(BGPC,U,4) S BGPLAL=BGPC
 S G=""
 S Y=$$PLTAXID^BGP8DU(P,"BGP ALCOHOL DXS",BDATE,EDATE)
 I Y S G=1_U_$P(Y,U,2)_U_$$DATE^BGP8UTL($P(Y,U,3))_U_D
 I $P(BGPLAL,U,4)<$P(G,U,4) S BGPLAL=G
 S Y=$$IPLSNOID^BGP8DU(P,"PXRM BGP ETOH RELATED DX",BDATE,EDATE)
 I Y S G=1_U_$P(Y,U,2)_U_$$DATE^BGP8UTL($P(Y,U,3))_U_D
 I $P(BGPLAL,U,4)<$P(G,U,4) S BGPLAL=G
 ...
 .I $$ICD^BGP8UTL2(Y,BGPT,9) S G=1_U_"BH PL "_Y_U_$$DATE^BGP8UTL($P(^AMHPPROB(X,0),U,3))_U_$P(^AMHPPROB(X,0),U,3) Q
 .S Y=$P($G(^AMHPROB(Y,0)),U)
 .I $D(^ATXAX(BGPJ,21,"B",Y)) S G=1_U_"BH PL "_Y_U_$$DATE^BGP8UTL($P(^AMHPPROB(X,0),U,3))_U_$P(^AMHPPROB(X,0),U,3) Q
 .Q
 I $P(BGPLAL,U,4)<$P(G,U,4) S BGPLAL=G
 ;
 K BGPG
 S BGPG=$$LASTPRC^BGP8UTL1(P,"BGP ALCOHOL PROCEDURES",BDATE,EDATE)
 I $P(BGPLAL,U,4)<$P(BGPG,U,3) S BGPLAL=1_U_"Proc "_$P(BGPG,U,2)_U_$$DATE^BGP8UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 Q BGPLAL
ALSCRN(P,BDATE,EDATE,BGPALL,SBI) ;EP - alcohol hf or screening pov IN REPORT PERIOD
...
PCC ;check PCC first
 ...
 S BGPX=0 F  S BGPX=$O(^TMP($J,"A",BGPX)) Q:BGPX'=+BGPX  S BGPV=$P(^TMP($J,"A",BGPX),U,5) D
 .I SBI Q:$P(^AUPNVSIT(BGPV,0),U,7)'="A"  ;IF SBI ONLY AMBULATORY
...
PCCEX .;
 ...
 ..S BGPALL(BGPIVD,BGPSC)=1_"^Ex 35^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
PCCHF .;
 ...
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_"^HF "_I_"^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
PCCDX .;
 .S E=0 F  S E=$O(^AUPNVPOV("AD",BGPV,E)) Q:E'=+E  D
 ..S I=$$VAL^XBDIQ1(9000010.07,E,.01)
 ..I '$$ICD^BGP8UTL2($$VALI^XBDIQ1(9000010.07,E,.01),$O(^ATXAX("B",$S('SBI:"BGP ALCOHOL SCREEN DXS",1:"BGP SCREEN FOR ALCOHOLISM DX"),0)),9) Q
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_U_"POV "_I_U_$$DATE^BGP8UTL(BGPVD)_U_BGPVD
PCCCPT .;
 ...
 ..S J=$P(^ICPT(I,0),U,1)
 ..S R=$S($$ICD^BGP8UTL2(I,$O(^ATXAX("B","BGP ALCOHOL POSITIVE SCRN CPTS",0)),1):"POSITIVE",1:"")
 ..S BGPSC=BGPSC+1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^CPT "_J_"^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
PCCMEAS .;now add in v measurements
 .S E=0 F  S E=$O(^AUPNVMSR("AD",BGPV,E)) Q:E'=+E  D
 ...
 ..S BGPALL(BGPIVD,BGPSC)=1_"^Meas "_I_"^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_T_U_$S(T:"POSITIVE: "_R,1:"NEGATIVE: "_R)
BH ;CHECK BH VISITS
 S BGPC="",T="",F=""
 S E=9999999-BDATE,D=9999999-EDATE-1_".99"
 F  S D=$O(^AMHREC("AE",P,D)) Q:D'=+D!($P(D,".")>E)  S V=0 F  S V=$O(^AMHREC("AE",P,D,V)) Q:V'=+V  D
BHEX .;
 .I $G(SBI) Q:$$VAL^XBDIQ1(9002011,V,.07)'="OUTPATIENT"
 .S BGPVD=9999999-$P(D,"."),BGPIVD=$P(D,".")
 .S X=$P($G(^AMHREC(V,14)),U,3)
 .I X="P"!(X="N") D
 ..S R=$S(X="P":"POSITIVE",1:"NEGATIVE")
 ..S BGPSC=BGPSC+1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^BH Ex^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
BHHF .;
 ...
 ..I I'="CAGE 0/4",I'="CAGE 1/4",I'="CAGE 2/4",I'="CAGE 3/4",I'="CAGE 4/4" Q  ;cage only
 ..S R=$S(I["0":"NEGATIVE",1:"POSITIVE")
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_"^BH HF "_I_"^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
 ..Q
BHDX .;
 .S X=0 F  S X=$O(^AMHRPRO("AD",V,X)) Q:X'=+X  D
 ..S I=$$VAL^XBDIQ1(9002011.01,X,.01)
 ..I '$$ICD^BGP8UTL2($$VALI^XBDIQ1(9002011.01,X,.01),$O(^ATXAX("B",$S('SBI:"BGP ALCOHOL SCREEN DXS",1:"BGP SCREEN FOR ALCOHOLISM DX"),0)),9),I'="29.1" Q
 ..S BGPSC=BGPSC+1,BGPALL(BGPIVD,BGPSC)=1_U_"BH POV "_I_U_$$DATE^BGP8UTL(BGPVD)_U_BGPVD
 ..Q
BHMEAS .;
 ...
 ..I I="AUDC",$P(^DPT(P,0),U,2)="F",R>2 S T=1
 ..I I="CRFT",R>1,R<7 S T=1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^BH Meas "_I_"^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_T_U_$S(T=1:"POSITIVE: "_R,1:"NEGATIVE")
BHCPT .;now add in CPT codes
 ...
 ..S R=$S($$ICD^BGP8UTL2(I,$O(^ATXAX("B","BGP ALCOHOL POSITIVE SCRN CPTS",0)),1):"POSITIVE",1:"")
 ..S BGPSC=BGPSC+1
 ..S BGPALL(BGPIVD,BGPSC)=1_"^BH CPT "_J_"^"_$$DATE^BGP8UTL(BGPVD)_U_BGPVD_U_$S(R="POSITIVE":1,1:0)_U_R
 ..Q
 .Q
 Q
