...
IC1 ;EP
 ...
 S BGPOW=$$OW^BGP7D6(DFN,BGPBMI,BGPAGEE)
 S BGPOB=$$OB^BGP7D6(DFN,BGPBMI,BGPAGEE)
 ...
 S BGPVALUE=""
 S BGPMN=$$MEDNUTRD^BGP7D711(DFN,BGP365,BGPEDATE) I BGPMN]"" S BGPN1=1
 S BGPSN=$$SPECNUTR^BGP7D711(DFN,BGP365,BGPEDATE) I BGPSN]"" S BGPN2=1
 S BGPSPEX=$$SPECEX^BGP7D711(DFN,BGP365,BGPEDATE) I BGPSPEX]"" S BGPN3=1
 S BGPOTH=$$OTHREL^BGP7D711(DFN,BGP365,BGPEDATE) I BGPOTH]"" S BGPN4=1
 I BGPMN]"" S BGPVALUE="MNT: "_$$DATE^BGP7UTL($P(BGPMN,U))_" "_$P(BGPMN,U,2)_$S(BGPMN]"":"; ",1:"")
 I BGPSN]"" S BGPVALUE=BGPVALUE_"NUTR: "_$$DATE^BGP7UTL($P(BGPSN,U))_" "_$P(BGPSN,U,2)_$S(BGPSN]"":"; ",1:"")
 I BGPSPEX]"" S BGPVALUE=BGPVALUE_"EXER ED: "_$$DATE^BGP7UTL($P(BGPSPEX,U))_" "_$P(BGPSPEX,U,2)_$S(BGPSPEX]"":"; ",1:"")
 I BGPOTH]"" S BGPVALUE=BGPVALUE_"LIFE: "_$$DATE^BGP7UTL($P(BGPOTH,U))_" "_$P(BGPOTH,U,2)
 S V=$S(BGPD1:"AC-OW",1:"")
...
QUIT(P,BD,ED) ;
 NEW Y,X,G,T
 S Y=$$LASTDX^BGP7UTL1(P,"BGP TOBACCO PAST USE DXS",BD,ED)
 I Y Q 1_U_"QUIT: "_$$DATE^BGP7UTL($P(Y,U,3))_" POV "_$P(Y,U,2)
 S Y=$$PLTAXID^BGP7DU(DFN,"BGP TOBACCO PAST USE DXS",BD,ED)
 I Y Q 1_U_"QUIT: "_$$DATE^BGP7UTL($P(Y,U,3))_" "_$P(Y,U,2)
 S Y=$$IPLSNOID^BGP7DU(DFN,"PXRM BGP QUIT TOBACCO",BD,ED)
 I Y Q 1_U_"QUIT: "_$$DATE^BGP7UTL($P(Y,U,3))_" "_$P(Y,U,2)
 S G=$$LASTHF^BGP7D7(P,"TOBACCO",BD,ED)
 I $P(G,U)["PREVIOUS" Q 1_U_"QUIT: "_$P(G,U,2)_" HF: "_$P(G,U,1)
 S G=$$LASTHF^BGP7D7(P,"TOBACCO (SMOKING)",BD,ED)
 I $P(G,U)["PREVIOUS" Q 1_U_"QUIT: "_$P(G,U,2)_" HF: "_$P(G,U,1)
 S G=$$LASTHF^BGP7D7(P,"TOBACCO (SMOKELESS - CHEWING/DIP)",BD,ED)
 I $P(G,U)["PREVIOUS" Q 1_U_"QUIT: "_$P(G,U,2)_" HF: "_$P(G,U,1)
 Q ""
 ;
GETLAST(P,BD,ED) ;
 NEW BGPLAST,S,C,T
 S S=$$LASTHF^BGP7D7(P,"TOBACCO (SMOKING)",BD,ED)
 I S]"" S BGPLAST(9999999-$P(S,U,3))=S I $P(S,U,1)["CESSATION"!($P(S,U,1)["CURRENT")!($P(S,U,1)["LIGHT")!($P(S,U,1)["HEAVY") Q S
 S C=$$LASTHF^BGP7D7(P,"TOBACCO (SMOKELESS - CHEWING/DIP)",BD,ED)
...
NEWSTUFF ;new stuff
 ...
 ..S X=$P(BGPTOBS,U,3)
 ..S Y=$$CPTI^BGP7DU(DFN,$$FMADD^XLFDT(X,1),BGPEDATE,+$$CODEN^ICPTCOD("1034F"))
 ..I Y S BGPTU=1,BGPTOBD=$P(Y,U,2),BGPTOBX="CPT 1034F" Q
 ..S Y=$$CPTI^BGP7DU(DFN,$$FMADD^XLFDT(X,1),BGPEDATE,+$$CODEN^ICPTCOD("1035F"))
 ..I Y S BGPTU=1,BGPTOBD=$P(Y,U,2),BGPTOBX="CPT 1035F"
 ;
 I BGPTOBS="" D  G:Y SET
 .S Y=$$LASTDX^BGP7UTL1(DFN,"BGP TOBACCO USER DXS",BGPBDATE,BGPEDATE)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,3),BGPTOBX=$P(Y,U,2) Q
 .S Y=$$PLTAXID^BGP7DU(DFN,"BGP TOBACCO USER DXS",,BGPEDATE)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,3),BGPTOBX=$P(Y,U,2) Q
 .S Y=$$IPLSNOID^BGP7DU(DFN,"PXRM BGP CURRENT TOBACCO",,BGPEDATE)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,3),BGPTOBX=$P(Y,U,2) Q
 .S Y=$$CPT^BGP7DU(DFN,BGPBDATE,BGPEDATE,$O(^ATXAX("B","BGP TOBACCO USER CPTS",0)),6)
 ...
 ;
 I BGPTOBS="" D  G:Y POT
 .S Y=$$LASTDX^BGP7UTL1(DFN,"BGP TOBACCO USER DXS",BGPBDX,BGPEDX)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,3),BGPTOBX=$P(Y,U,2) Q
 .S Y=$$PLTAXID^BGP7DU(DFN,"BGP TOBACCO USER DXS",,BGPEDATE)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,3),BGPTOBX=$P(Y,U,2) Q
 .S Y=$$IPLSNOID^BGP7DU(DFN,"PXRM BGP CURRENT TOBACCO",,BGPEDATE)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,3),BGPTOBX=$P(Y,U,2) Q
 .S Y=$$CPT^BGP7DU(DFN,BGPBDX,BGPEDX,$O(^ATXAX("B","BGP TOBACCO USER CPTS",0)),6)
 .I Y S BGPTU=1,BGPTOBD=$P(Y,U,2),BGPTOBX=$P(Y,U,3) Q
 ;
 ;
...
POT ;if potential check for 2 codes
 S X=BGPTOBD
 S Y=$$LASTDX^BGP7UTL1(DFN,"BGP TOBACCO PAST USE DXS",$$FMADD^XLFDT(X,1),BGPBDATE)
 I Y S BGPTU="" G SET
 ...
 .I $$ICD^BGP7UTL2(Y,T,9) S BGPREM=1 Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001)
 .I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP QUIT TOBACCO",S)) S BGPREM=1 Q
 .Q
 ;CHECK SNOMED
 ;S Y=$$IPLSNOID^BGP7DU(DFN,"PXRM BGP QUIT TOBACCO",$$FMADD^XLFDT(BGPTOBD,1),BGPBDATE) I Y S BGPREM=1 Q
 ;
 I BGPREM S BGPTU=""
...
SET ;
 ...
 ;I 'BGPTUC S BGPTC1=$$PED^BGP7D711(DFN,BGPBDATE,BGPEDATE,1)
 S BGPTC1=$$PED^BGP7D711(DFN,BGPBDATE,BGPEDATE,1)
 ...
 ;SET BGPVALUE
 S BGPVALUE="UP"_$S(BGPD7:",AC",1:"")_" TOB USER: "_$$DATE^BGP7UTL(BGPTOBD)
 D
