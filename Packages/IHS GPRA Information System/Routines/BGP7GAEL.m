BGP7GAEL ; IHS/CMI/LAB - AREA NATIONAL GPRA REPORT ;
...
TESTNTL ;
 ...
 S BGPSUL(1)=""
 S BGPSUL(2)=""
 D EP(.ERR,1,2522,"BGP 17 AREA ELDER REPORT","A",.BGPSUL,1,3030000,3000000,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
...
EP1 ;
 ...
 S BGPZZ="A"
 S BGPSUCNT=C
 I BGPSUCNT=1 S Y=$O(BGPSUL(0)),X=$P(^BGPEDLCG(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 ...
 S X=0 F  S X=$O(^BGPELIG(X)) Q:X'=+X  S BGPIND(X)=""
 S BGPINDG="E"
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="AOELD^BGP7GAEL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI AREA ELDER REPORT 06" D ^%ZTLOAD
...
AOELD ;
 ...
 D GUIR^BGPXBLM("PRINT^BGP7PHEL","^TMP($J,""BGPGUI"",")
