...
I0302A ;EP
 ...
 ;S BGPIHD=$$IHDCCVD^BGP8D729(DFN,BGPBDATE,BGPEDATE)
 S BGPCHD=0
 S BGPCHD=$$CHD^BGP8D729(DFN,BGPBDATE,BGPEDATE)
...
I0302ASC ;EP
 ...
 S BGPBPC=""
 S BGPBP=$$MEANBP^BGP8D26(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE)
 I BGPBP["unknown" S BGPBPC=$$BPCPT^BGP8D22(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE) I BGPBPC]"" S BGPBP=$P(BGPBPC,U,2)_"^1"
 S BGPBPDEV=$$MEANBP^BGP8D26(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE,1)
 I BGPBPDEV["unknown" S BGPBPCD=$$BPCPT^BGP8D22(DFN,$$FMADD^XLFDT(BGPEDATE,-(2*365)),BGPEDATE,1) I BGPBPCD]"" S BGPBPDEV=$P(BGPBPCD,U,2)_"^1"
 S BGPN1=$S($P(BGPBP,U,2):1,1:0)
 S BGPN16=$S($P(BGPBPDEV,U,2):1,1:0)
 ;S BGPLDL=$$LDL^BGP8D2(DFN,$$FMADD^XLFDT(BGPEDATE,-(5*365)),BGPEDATE,1) ;date^value
 ;I $P(BGPLDL,U) S BGPN11=1  ;had ldl done
 ;S BGPLDL1=$$LDL^BGP8D2(DFN,BGPBDATE,BGPEDATE,1) ;date^value
 ;I $P(BGPLDL1,U) S BGPN2=1  ;had ldl done
 S BGPTOB=$$TOBACCO(DFN,BGP365,BGPEDATE)
 I $P(BGPTOB,U) S BGPN3=1
 S BGPBMI=$$BMI^BGP8D6(DFN,BGPEDATE,BGPAGEE),BGPN4=$S(BGPBMI]"":1,1:0)
 ;S BGPBMIR="" I 'BGPN4 S BGPBMIR=$$REF^BGP8D6(DFN,BGPBDATE,BGPEDATE,BGPAGEE) I $P(BGPBMIR,U) S BGPN4=1,BGPN8=1,BGPBMIR=1_U_"BMI: Ref ht "_$$DATE^BGP8UTL($P(BGPBMIR,U,3))_" wt "_$$DATE^BGP8UTL($P(BGPBMIR,U,6))
 ...
 ;I 'BGPN6 S BGPDEP=$$DEPREF^BGP8D25(DFN,BGPBDATE,BGPEDATE) I BGPDEP S BGPN6=1
 ...
 ;S %=$S($P(BGPLDL1,U,2):"LDL: "_$$DATE^BGP8UTL($P(BGPLDL1,U,2)),1:"")
 ;I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 S %="" I $P(BGPTOB,U) S %="TOB: "_$S($P(BGPTOB,U,3)["/":$P(BGPTOB,U,3),1:$$DATE^BGP8UTL($P(BGPTOB,U,3)))_" "_$P(BGPTOB,U,2)
 I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 I BGPN10 D
 .S %=$S($P(BGPBMI,U)]"":"BMI: "_$J(BGPBMI,5,1),1:$P(BGPBMIR,U,2))
 .I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
 S %=$S($P(BGPLIFED,U):"LIFE: "_$$DATE^BGP8UTL($P(BGPLIFED,U,2))_" "_$P(BGPLIFED,U,3),1:"")
 I %]"" S BGPVALUE=BGPVALUE_$S($P(BGPVALUE,"|||",2)]"":";",1:"") S BGPVALUE=BGPVALUE_" "_%
