BGP8DPEA ;IHS/CMI/LAB - EDUC REPORT;
 ;;18.0;IHS CLINICAL REPORTING;;NOV 21, 2017;Build 51
 ;
 ;
 W:$D(IOF) @IOF
 W !,$$CTR("IHS 2018 Area Aggregate Patient Education Report",80),!!
INTRO ;
 D XIT
...
ASU ;
 ...
 I C=1 S BGPRPTT="F",BGPSUCNT=1,Y=$O(BGPSUL(0)),X=$P(^BGPPEDCR(Y,0),U,9),X=$O(^AUTTLOC("C",X,0)) I X S BGPSUNM=$P(^DIC(4,X,0),U)
 I C>1 S BGPRPTT="A"
ZIS ;call to XBDBQUE
 D ^XBFMK
 S X=0 F  S X=$O(^BGPPEIR(X)) Q:X'=+X  S BGPIND(X)=""
...
DRIVER ;
 ...
 ;
NODEV ;
 S XBRP="",XBRC="NODEV1^BGP8DPEA",XBRX="XIT^BGP8DPEA",XBNS="BGP"
 D ^XBDBQUE
 ;D XIT
 Q
 ;
...
TSKMN ;EP ENTRY POINT FROM TASKMAN
 ...
 S ZTCPU=$G(IOCPU),ZTRTN="DRIVER^BGP8DPEA",ZTDTH="",ZTDESC="PAT ED 09 REPORT" D ^%ZTLOAD D XIT Q
 Q
 ;
XIT ;
 D ^%ZISC
