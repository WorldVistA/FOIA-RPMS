BGP8PDLA ; IHS/CMI/LAB - print ind ;
 ;;18.0;IHS CLINICAL REPORTING;;NOV 21, 2017;Build 51
 ;
 ;
I1AGE ;EP
 ;I BGPRTYPE=9 D ^BGP8PDLB Q
...
I1AGE4 ;
 S F=F+1
 S BGPF=BGPX_"D" S BGPPC=$O(^BGPINDRC("OR",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPINDRC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90560.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
...
I1AGEP ;
 ...
 I BGPRTYPE'=1 S X=" " D S(X,1,1) S X="# w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 I BGPRTYPE'=1 S X="% w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S V=$P(BGPDAC(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
...
PR ; 
 ...
 I BGPRTYPE'=1 S X=" " D S(X,1,1) S X="# w refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S V=$P(BGPDAP(X),U,4) S Y=V D S(Y,,X+1)
 I BGPRTYPE'=1 S X="% w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S V=$P(BGPDAP(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# Quit" D S(X,1,1)
 ...
 I BGPRTYPE'=1 S X="# w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S N=$P(BGPDAC(X),U,5),O=$P(BGPDAP(X),U,5) S Y=$$SB($J((N-O),6,1)) D S(Y,,X+1)
...
BL ;
 ...
 I BGPRTYPE'=1 S X=" " D S(X,1,1) S X="# w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S V=$P(BGPDAB(X),U,4) S Y=V D S(Y,,X+1)
 I BGPRTYPE'=1 S X="% w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S V=$P(BGPDAB(X),U,5) S Y=$$SB($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="# Quit" D S(X,1,1)
 ...
 I BGPRTYPE'=1 S X="# w/ refusal of Counseling" D S(X,1,1)
 I BGPRTYPE'=1 F X=1:1:3 S N=$P(BGPDAC(X),U,5),O=$P(BGPDAB(X),U,5) S Y=$$SB($J((N-O),6,1)) D S(Y,,X+1)
 S X="who quit" D S(X,1,1)
