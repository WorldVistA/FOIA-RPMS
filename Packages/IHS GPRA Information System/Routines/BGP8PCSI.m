BGP8PCSI ; IHS/CMI/LAB - DISPLAY IND LISTS ;
...
EN ;EP -- main entry point for APCH HMR DISPLAY
 D EN^VALM("BGP 18 IPC MEASURE SEL")
 D CLEAR^VALM1
 D FULL^VALM1
 W:$D(IOF) @IOF
 D EOJ
 ...
 ;
HDR ; -- header code
 S VALMHDR(1)="IHS IPC Clinical Performance Measures"
 S VALMHDR(2)="* indicates the performance measure has been selected"
 Q
 ;
INIT ; -- init variables and list array
 K BGPTIND S BGPHIGH=""
 S (X,Y,Z,C)=0 F  S X=$O(^BGPINDR("AIPC",X)) Q:X'=+X  S Y=$O(^BGPINDR("AIPC",X,Y)) Q:Y'=+Y  D
