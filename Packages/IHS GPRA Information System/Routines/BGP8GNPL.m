BGP8GNPL ; IHS/CMI/LAB - CRS 26 Mar 2010 5:09 PM ;
...
TESTNTL ;
 ...
 D EP(.ERR,1,2522,"BGP 18 NATIONAL PAT LISTS",338,.BGPND,.BGPINDL,1,3040000,"A","","","B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPIND,BGPINDL,BGPQTR,BGPPER,BGPLIST,BGPLPRV,BGPLPROV,BGPROT,BGPRTIME,BGPMFITI,BGPVDT,BGPBEN,BGPFILE,BGPDNT) ;EP - called from GUI to produce national gpra report (NTL-GP)
 ; SEE ROUTINE BGP8NPL if you have questions about any of these variables
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
 ...
 ;                        ^BGPINDR("AGPRA",1,ien) xref as the ones with a second
 ...
 ;            entries from BGP 18 NATIONAL PATIENT LISTS that point to that
 ;            measure by using the "B" index on ^BGPNPLR(.  ^BGPNPLR("B",measure ien,ien)
...
EP1 ;
 ...
 I BGPOPTN="CRS 18 OTHER NATIONAL MEASURES PAT LISTS" D  ;maw orig 3/11/2010
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="NTLGP^BGP8GNPL",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI NATIONAL GPRA REPORT LISTS 09" D ^%ZTLOAD
