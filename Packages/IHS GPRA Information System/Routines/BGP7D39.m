...
COVERAGE(P,BDATE,EDATE,MTAX,NTAX,TYPE) ;
 ...
 S C=1_U_PER_U_$S(GAP:1,1:"")_U_TYPE_": IXRD: "_$$DATE^BGP7UTL(BGPISD)_" ["_BGPMP_"] Days="_DAYS
...
HADWARF(P,BDATE,EDATE) ;
 NEW BGPMEDS1,C,A,M
 K BGPMEDS1
 D GETMEDS^BGP7UTL2(P,BDATE,EDATE,"BGP PQA WARFARIN MEDS","BGP PQA WARFARIN NDC",,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q ""  ; NO MEDS
 S (A,C)=0 F  S A=$O(BGPMEDS1(A)) Q:A'=+A  D
 .S M=$P(BGPMEDS1(A),U,4)  ;IEN OF V MED
 .Q:'$D(^AUPNVMED(M,0))
...
HASARB(P,BDATE,EDATE) ;
 NEW BGPMEDS1,C,A,M
 K BGPMEDS1
 D GETMEDS^BGP7UTL2(P,BDATE,EDATE,"BGP PQA ARB NEPRILYSIN MEDS","BGP PQA ARB NEPRILYSIN NDC",,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q ""  ; NO MEDS
 S (A,C)=0 F  S A=$O(BGPMEDS1(A)) Q:A'=+A  D
 .S M=$P(BGPMEDS1(A),U,4)  ;IEN OF V MED
 .Q:'$D(^AUPNVMED(M,0))
