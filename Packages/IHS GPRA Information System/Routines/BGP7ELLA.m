BGP7ELLA ; IHS/CMI/LAB - print ind 20 Jan 2017 4:46 PM ; 
...
I1AGE ;EP
 ...
 K BGPDAC,BGPDAP,BGPDAB S (C,D,E,F,G,H,I,J,K)=0 F BGPX="IIB.1","IIB.2","IIB.3","IIB.4" D I1AGE1,I1AGE2
 D I1AGEP
 Q
I1AGE1 ;
 S C=C+1
...
I1AGEP ;
 ...
 S X="# Total # w/ Influenza/Contra/NMI Refusal (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,2) S Y=V D S(Y,,X+1)
 S X="% Total # w/ Influenza/Contra/NMI Refusal (GPRA)" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,3) S Y=$$SB^BGP7ELL2($J(V,6,1)) D S(Y,,X+1)
 S X=" " D S(X,1,1) S X="A. # w/ Contra/NMI Ref w/ % of Total IZ" D S(X,1,1)
 F X=1:1:4 S V=$P(BGPDAC(X),U,4) S Y=V D S(Y,,X+1)
 S X="A. % w/ Contra/NMI Ref w/ % of Total IZ" D S(X,1,1)
