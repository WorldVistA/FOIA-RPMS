BGP7GGPU ; IHS/CMI/LAB - IHS Diabetes Audit 2003 ;
 ;;17.1;IHS CLINICAL REPORTING;;MAY 10, 2017;Build 29
 ;
 ;
TESTNTL ;
 S ERR=""
 D EP(.ERR,1,2522,"BGP 17 GPRA PERF REPORT",338,1,3040000,3000000,1,1,"B",$$NOW^XLFDT)
 W !,ERR
 Q
EP(BGPRET,BGPUSER,BGPDUZ2,BGPOPTN,BGPTAXI,BGPQTR,BGPPER,BGPVDT,BGPBEN,BGPEXPT,BGPROT,BGPRTIME,BGPMFITI,BGPFILE,BGPYNPN,BGPPN,BGPFP,BGPDP,BGPDNT,BGPCOMMI) ;EP - called from GUI to produce national GPU report (OTH-GPU)
 ;  BGPUSER - DUZ
 ;  BGPDUZ2 - DUZ(2)
 ;  BGPOPTN - OPTION NAME
...
EP1 ;
 ...
 S BGPFILEN="" I BGPEXPT S BGPFILEN="BG171"_$P(^AUTTLOC(DUZ(2),0),U,10)_"."_BGPRPT_"  in directory "_BGPUF
 S BGPDELT=""
 ...
 S BGPGFNM=X
 S DIC="^BGPGUIG(",DIC(0)="L",DIADD=1,DLAYGO=90558.19,DIC("DR")=".02////"_BGPUSER_";.03////"_$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT)_";.05///"_BGPOPTN_";.06///R;.07///"_$G(BGPROT)_";1///"_$G(BGPFILEN)
...
TSKMN ;
 S ZTIO=""
 K ZTSAVE S ZTSAVE("*")=""
 S ZTCPU=$G(IOCPU),ZTRTN="OTHGPU^BGP7GGPU",ZTDTH=$S(BGPRTIME]"":BGPRTIME,1:$$NOW^XLFDT),ZTDESC="GUI GPU GPRA REPORT 06" D ^%ZTLOAD
 D UPLOG^BGP7GUA(BGPGIEN,ZTSK)
 Q
OTHGPU ;
 I $G(BGPSEAT) D ^BGP7D10
 I '$G(BGPSEAT) D ^BGP7D1
 K ^TMP($J,"BGPGUI")
 S IOM=80,BGPIOSL=55
