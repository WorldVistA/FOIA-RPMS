BGP8D62 ;IHS/CMI/LAB - MEASURE 31;
 ;;18.1;IHS CLINICAL REPORTING;;MAY 25, 2018;Build 66
 ;
IBHEDIS ;EP - V18 NATIONAL GPRA
...
CRCP ;EP - called from elder
 ...
 S BGPREF="" I 'BGPN1 S BGPREF=$$REF(DFN,$$FMADD^XLFDT(BGPEDATE,-365),BGPEDATE) I BGPREF S BGPN2=1
 S BGPVALUE=""
 I BGPFOB]"" S BGPVALUE="FOB: "_$$DATE^BGP8UTL($P(BGPFOB,U,2))_" "_$P(BGPFOB,U)
 ...
 I BGPREF]"" S BGPVALUE=BGPVALUE_$S(BGPVALUE]"":"; ",1:"") S BGPVALUE=BGPVALUE_$$DATE^BGP8UTL($P(BGPREF,U,2))_" "_$P(BGPREF,U,3)
...
CRC(P,EDATE) ;EP
 ...
 S X=$$PLTAXND^BGP8DU(P,"BGP COLORECTAL CANCER DXS",EDATE)
 I X Q 1
 S X=$$IPLSNOND^BGP8DU(P,"PXRM COLORECTAL CANCER",EDATE)
 I X Q 1
 ...
 S BGPG=$$LASTPRC^BGP8UTL1(P,"BGP TOTAL COLECTOMY PROCS",$$DOB^AUPNPAT(P),EDATE)
 I BGPG Q 1
...
SIG(P,EDATE,BDATE) ;EP
 NEW BGPLSIG,BGPG,X,T
 S BGPLSIG=""
 I $G(BDATE)="" S BDATE=$$FMADD^XLFDT(EDATE,5*(-365))
 S BGPG=$$LASTPRC^BGP8UTL1(P,"BGP SIG PROCS",BDATE,EDATE)
 I $P(BGPG,U)=1 S BGPLSIG="Proc "_$P(BGPG,U,2)_"^"_$$DATE^BGP8UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 ;
 S T=$O(^ATXAX("B","BGP SIG CPTS",0))
 I T D  I X]"",$P(BGPLSIG,U,3)<$P(X,U,1) S BGPLSIG="CPT "_$P(X,U,2)_U_$$DATE^BGP8UTL($P(X,U,1))_U_$P(X,U,1)
...
COLO(P,EDATE,BDATE) ;EP
 ...
 I $P(BGPG,U)=1 S BGPLCOLO="Proc "_$P(BGPG,U,2)_"^"_$$DATE^BGP8UTL($P(BGPG,U,3))_U_$P(BGPG,U,3)
 ...
 I T D  I X]"",$P(BGPLCOLO,U,3)<$P(X,U,1) S BGPLCOLO="CPT "_$P(X,U,2)_U_$$DATE^BGP8UTL($P(X,U,1))_U_$P(X,U,1)
 .S X=$$CPT^BGP8DU(P,BDATE,EDATE,T,5) I X]"" Q
 .S X=$$TRAN^BGP8DU(P,BDATE,EDATE,T,5)
 Q BGPLCOLO
...
REF(P,BDATE,EDATE) ;EP
 ...
 I BGPT S G=$$RADREF^BGP8UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"SIG: Refused CPT"
 ;S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 ;I BGPT S G=$$RADREF^BGP8UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"BE: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$RADREF^BGP8UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"COLO: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP SIG CPTS",0))
 I BGPT S G=$$CPTREFT^BGP8UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"SIG: Refused CPT"
 ;S BGPT=$O(^ATXAX("B","BGP BE CPTS",0))
 ;I BGPT S G=$$CPTREFT^BGP8UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"BE: Refused CPT"
 S BGPT=$O(^ATXAX("B","BGP COLO CPTS",0))
 I BGPT S G=$$CPTREFT^BGP8UTL1(P,BDATE,EDATE,BGPT) I $P(G,U,1)=1 Q 1_"^"_$P(G,U,2)_"^"_"COLO: Refused CPT"
 S T=$$PRCREFT^BGP8UTL1(P,BDATE,EDATE,$O(^ATXAX("B","BGP COLO PROCS",0)))
 I T Q "1^"_$P(T,U,2)_"^ COLO: Refused Proc"
 S T=$$PRCREFT^BGP8UTL1(P,BDATE,EDATE,$O(^ATXAX("B","BGP SIG PROCS",0)))
