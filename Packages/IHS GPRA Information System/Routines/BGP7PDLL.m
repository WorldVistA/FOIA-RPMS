BGP7PDLL ; IHS/CMI/LAB - print ind 1 ;
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF="G.F.1"_BGPX S BGPPC=$O(^BGPINDGC("C",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF="G.G.1"_BGPX S BGPPC=$O(^BGPINDGC("C",BGPF,0))
 ...
 Q
I1AGE3 ;
 S BGPF="G.H.1"_BGPX S BGPPC=$O(^BGPINDGC("C",BGPF,0))
 S E=E+1
 S BGPDF=$P(^BGPINDGC(BGPPC,0),U,8)
 S BGPNP=$P(^DD(90558.03,BGPDF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(E),U,6)=$$V^BGP7DP1C(1,BGPRPT,N,P)
 S $P(BGPDAP(E),U,6)=$$V^BGP7DP1C(2,BGPRPT,N,P)
 S $P(BGPDAB(E),U,6)=$$V^BGP7DP1C(3,BGPRPT,N,P)
 ;set 6th piece to numerator and 7th to %
 S BGPNF=$P(^BGPINDGC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90558.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(E),U,7)=$$V^BGP7DP1C(1,BGPRPT,N,P),$P(BGPDAC(E),U,8)=$S($P(BGPDAC(E),U,6):($P(BGPDAC(E),U,7)/$P(BGPDAC(E),U,6)*100),1:"")
 S $P(BGPDAP(E),U,7)=$$V^BGP7DP1C(2,BGPRPT,N,P),$P(BGPDAP(E),U,8)=$S($P(BGPDAP(E),U,6):($P(BGPDAP(E),U,7)/$P(BGPDAP(E),U,6)*100),1:"")
 S $P(BGPDAB(E),U,7)=$$V^BGP7DP1C(3,BGPRPT,N,P),$P(BGPDAB(E),U,8)=$S($P(BGPDAB(E),U,6):($P(BGPDAB(E),U,7)/$P(BGPDAB(E),U,6)*100),1:"")
 Q
 ;
I1AGEP ;
