...
VS ;EP
 ...
 I BGPD10 S D=D_",TOB"
 S BGPVALUE="AC"_$S(BGPD10:",TOB",1:"")_"|||"_$S('BGPN1:"No visit in time period",1:"Visit in time period "_$$DATE^BGP8UTL(BGPQV))_" Urgent Care Only: "_$$DATE^BGP8UTL(BGPURO)
...
TOBUSER(P,BDATE) ;EP
 ...
 S BGPTUSER=""
 S BGPTOBP=$$TOBHF^BGP8D712(P,$$DOB^AUPNPAT(P),$$FMADD^XLFDT(BDATE,-1))
 S BGPSDX=$$DXU(P,$$DOB^AUPNPAT(P),$$FMADD^XLFDT(BDATE,-1),BGPEDATE)
 S BGPSCPT=$$CPTSM^BGP8D7(P,$$DOB^AUPNPAT(P),$$FMADD^XLFDT(BDATE,-1))
 S %=""
 ...
 .I $P(F,U,10)["CURRENT"!($P(F,U,10)["CESSATION") S BGPTUSER=1 Q
...
CHRONIC(D) ;
 ...
 I $D(^ATXAX(TM,21,"B",D))!($$NDC(D,TN)) Q 1
 Q 0
DXU(P,BDATE,EDATE,RPED) ;EP - WAS THERE A USER DX?
 NEW D,X,BGPG,Y,G,T,S
 K BGPG
 ...
 .Q:$P(^AUPNPROB(X,0),U,8)>RPED
 .S D=$P(^AUPNPROB(X,0),U,13)
 .Q:D>RPED
 .S Y=$P(^AUPNPROB(X,0),U)
 .I $$ICD^BGP8UTL2(Y,T,9) S G=$P($$ICDDX^BGP8UTL2(Y),U,2)_U_$S(D]"":D,1:$P(^AUPNPROB(X,0),U,3))_U_Y Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001) I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP CURRENT TOBACCO")) S G=$P($$ICDDX^BGP8UTL2(Y),U,2)_U_$S(D]"":D,1:$P(^AUPNPROB(X,0),U,3))_U_Y
 .Q
 Q G
