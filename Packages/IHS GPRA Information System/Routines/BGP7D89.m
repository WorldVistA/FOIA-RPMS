BGP7D89 ; IHS/CMI/LAB - measure C 03 Jul 2010 7:05 AM ;
...
VS ;EP
 ...
 I BGPD10 S D=D_",TOB"
 S BGPVALUE="AC"_$S(BGPD10:",TOB",1:"")_"|||"_$S('BGPN1:"No visit in time period",1:"Visit in time period "_$$DATE^BGP7UTL(BGPQV))_" Urgent Care Only: "_$$DATE^BGP7UTL(BGPURO)
...
TOBUSER(P,BDATE) ;EP
 ...
 S BGPTUSER=""
 S BGPTOBP=$$TOBHF^BGP7D712(P,$$DOB^AUPNPAT(P),$$FMADD^XLFDT(BDATE,-1))
 S BGPSDX=$$DXU(P,$$DOB^AUPNPAT(P),$$FMADD^XLFDT(BDATE,-1),BGPEDATE)
 S BGPSCPT=$$CPTSM^BGP7D7(P,$$DOB^AUPNPAT(P),$$FMADD^XLFDT(BDATE,-1))
 S %=""
...
PRIMMED ;
 ...
 K BGPMEDS1
 D GETMEDS^BGP7UTL2(DFN,SD,BGPEDATE,,,,,.BGPMEDS1)
 I '$D(BGPMEDS1) Q  ;
 ...
 .S BGPDX=BGPDX_BGPD1_") "_$$DATE^BGP7UTL(FD)_" "_$$VAL^XBDIQ1(50,D,.01)
 .I $$UP^XLFSTR($P($G(^AUPNVMED(M,11)),U))["RETURNED TO STOCK" S BGPN1=BGPN1+1 D  Q
 ..I BGPNX]"" S BGPNX=BGPNX_"; "
 ..S BGPNX=BGPNX_BGPD1_") "_$$DATE^BGP7UTL(FD)_" "_$$VAL^XBDIQ1(50,D,.01)_"-RTS"
...
DXU(P,BDATE,EDATE,RPED) ;EP - WAS THERE A USER DX?
 ...
 .I $$ICD^BGP7UTL2(Y,T,9) S G=$P($$ICDDX^BGP7UTL2(Y),U,2)_U_$S(D]"":D,1:$P(^AUPNPROB(X,0),U,3))_U_Y Q
 .S S=$$VAL^XBDIQ1(9000011,X,80001) I S]"",$D(^XTMP("BGPSNOMEDSUBSET",$J,"PXRM BGP CURRENT TOBACCO")) S G=$P($$ICDDX^BGP7UTL2(Y),U,2)_U_$S(D]"":D,1:$P(^AUPNPROB(X,0),U,3))_U_Y
 .Q
 Q G
