BGP7PDLT ; IHS/CMI/LAB - print ind 1 03 Jul 2010 7:40 AM ;
...
I1AGE ;EP  special age tallies
 ...
 I '$G(BGPSUMON) D HEADER^BGP7DPH Q:BGPQUIT  W !,^BGPINDG(BGPIC,53,1,0) W:$D(^BGPINDG(BGPIC,53,2,0)) !,^BGPINDG(BGPIC,53,2,0)  D HPA^BGP7DPH
...
I1AGE1 ;
 ;gather up all #'s
 S C=C+1
 S BGPF=156_"."_BGPPAD_".1"_BGPX S BGPPC=$O(^BGPINDGC("OR",BGPF,0))
...
I1AGE2 ;
 S D=D+1
 S BGPF=156_"."_BGPPAD_".2"_BGPX  S BGPPC=$O(^BGPINDGC("OR",BGPF,0))
...
I1AGE3 ;
 S E=E+1
 S BGPF=156_"."_BGPPAD_".3"_BGPX  S BGPPC=$O(^BGPINDGC("OR",BGPF,0))
 ;set 4th piece to numerator and 5th to %
 S BGPNF=$P(^BGPINDGC(BGPPC,0),U,9)
 S BGPNP=$P(^DD(90558.03,BGPNF,0),U,4),N=$P(BGPNP,";"),P=$P(BGPNP,";",2)
 S $P(BGPDAC(E),U,6)=$$V^BGP7DP1C(1,BGPRPT,N,P),$P(BGPDAC(E),U,7)=$S($P(BGPDAC(E),U,2):($P(BGPDAC(E),U,6)/$P(BGPDAC(E),U,2)*100),1:"")
 S $P(BGPDAP(E),U,6)=$$V^BGP7DP1C(2,BGPRPT,N,P),$P(BGPDAP(E),U,7)=$S($P(BGPDAP(E),U,2):($P(BGPDAP(E),U,6)/$P(BGPDAP(E),U,2)*100),1:"")
 S $P(BGPDAB(E),U,6)=$$V^BGP7DP1C(3,BGPRPT,N,P),$P(BGPDAB(E),U,7)=$S($P(BGPDAB(E),U,2):($P(BGPDAB(E),U,6)/$P(BGPDAB(E),U,2)*100),1:"")
 Q
I1AGEP ;
...
PR ; 
 I $Y>(BGPIOSL-13) D:'$G(BGPSUMON) HEADER^BGP7DPH Q:BGPQUIT  W !,^BGPINDG(BGPIC,53,1,0) W:$D(^BGPINDG(BGPIC,53,2,0)) !,^BGPINDG(BGPIC,53,2,0) D HPA^BGP7DPH
...
BL ;
 D:'$G(BGPSUMON) HEADER^BGP7DPH Q:BGPQUIT  W !,^BGPINDG(BGPIC,53,1,0) W:$D(^BGPINDG(BGPIC,53,2,0)) !,^BGPINDG(BGPIC,53,2,0) D HPA^BGP7DPH
