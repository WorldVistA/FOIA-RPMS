...
DEL ;
 ...
 I BGPRTYPE=1 D DEVSUM^BGP8PDLD
 ;I BGPRTYPE=1 D SUMMARY
 I BGPRTYPE=1 D SDP^BGP8PDLN
 I BGPRTYPE=1 D SDP^BGP8PDLD
 ;I BGPRTYPE=1 D SDP
...
SUMONLY ;
 ...
 I '$D(BGPGUI) D
 .;I $P($G(^BGPSITE(DUZ(2),0)),U,14) ;S XBUF=$P(^BGPSITE(DUZ(2),0),U,14)
 .S XBFLT=1,XBFN=BGPDELF_".txt",XBMED="F",XBTLE="GPRA 18.0 DELIMITED OUTPUT",XBQ="N",XBF=0
 .D ^XBGSAVE
 .K XBFLT,XBFN,XBMED,XBTLE,XBE,XBF
 I $D(BGPGUI) D
 .S (C,X)=0 F  S X=$O(^BGPDATA(X)) Q:X'=+X  S C=C+1,^BGPGUIR(BGPGIEN,12,C,0)=^BGPDATA(X)
...
DEL1 ;EP
 ...
 S BGPIC=0 F  S BGPIC=$O(BGPIND(BGPIC)) Q:BGPIC=""!(BGPQUIT)  I $P(^BGPINDR(BGPIC,0),U,14) D PRINT2 D
 .Q:BGPQUIT
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),'$D(BGPYNPLT) S BGPINDG=BGPIC D NPL1^BGP8NPLP ;printed nat gpra list
 .I $G(BGPNPL),$D(BGPINDL(BGPIC)),$D(BGPYNPLT) S BGPINDG=BGPIC D CT^BGP8DSTM ;printed nat gpra SEARCH TEMPLATE
 Q:BGPQUIT
...
PRINT2 ;
 ...
 .S X=$P(^BGPINDR(BGPIC,0),U,3) D S(X,1,1)
 .S X=" " D S(X,1,1)
 ...
 .S X="Logic:" D S(X,1,1)
 .S BGPX=0 F  S BGPX=$O(^BGPINDR(BGPIC,BGPNODE,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDR(BGPIC,BGPNODE,BGPX,0) D S(X,1,1)
 .I BGPRTYPE=9 G CALC
 .S X=" " D S(X,1,1) S X="Performance Measure Description:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDR(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDR(BGPIC,$S($G(BGPNGR09):57,1:51),BGPX,0) D S(X,1,1)
 .I $O(^BGPINDR(BGPIC,52,0)) S X=" " D S(X,1,1) S X="Past Performance and/or Target:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDR(BGPIC,52,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDR(BGPIC,52,BGPX,0) D S(X,1,1)
 .I $O(^BGPINDR(BGPIC,55,0)) S X=" " D S(X,1,1) S X="Source:" D S(X,1,1) S BGPX=0 F  S BGPX=$O(^BGPINDR(BGPIC,55,BGPX)) Q:BGPX'=+BGPX  D
 ..S X=^BGPINDR(BGPIC,55,BGPX,0) D S(X,1,1)
 .I '$O(^BGPINDR(BGPIC,55,0)) D S(" ",1,1)
CALC .X ^BGPINDR(BGPIC,4)
 Q
HEADER ;EP
 ...
 I $G(BGPEXPT) S X="A file will be created called BG180"_$P(^AUTTLOC(DUZ(2),0),U,10)_"."_BGPRPT_"." D S(X,1,1)
