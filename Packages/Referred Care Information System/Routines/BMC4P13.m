...
INDX ;INDEX NEW INDECIES "BA", "BB" AND "CD" ;Patch 2
 S BMC="BMC*4.0*2" Q:$$INSTALLD(BMC)
...
P5 ;Add Opt for Vendor
 S BMC="BMC*4.0*5" Q:$$INSTALLD(BMC)
...
P6 ;FX CHS PO'S WITH FY 00 INSTEAD OF FY 10
 S BMC="BMC*4.0*6" Q:$$INSTALLD(BMC)
...
P7 ;Add Report opt for the printed C32
 S BMC="BMC*4.0*7" Q:$$INSTALLD(BMC)
...
P8 ;Add Report TOC Opt and Edit TOC Opt
 S BMC="BMC*4.0*8" Q:$$INSTALLD(BMC)
...
PCCLNK  ;Add RCIS to PCC Visit Merge Utility
 ...
 Q
P9 ;Add Report Opt
 S BMC="BMC*4.0*9" ;Q:$$INSTALLD(BMC)
 ...
 Q
P10 ;Patch 10
 ;S BMC="BMC*4.0*10" Q:$$INSTALLD(BMC)
...
P12 ;Add Report Opt and Call-in options 
 S BMC="BMC*4.0*12" Q:$$INSTALLD(BMC)
