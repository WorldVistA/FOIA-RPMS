...
DEV S %ZIS="OPQ" D ^%ZIS I POP S IOP=ION D ^%ZIS Q
 ...
 X ^%ZOSF("UCI") S ZTRTN="START^AGMCRP",ZTUCI=Y,ZTIO="",ZTDESC="Alpha List MEDICARE/RR Holders for "_$P(^AUTTLOC(DUZ(2),0),U,2)_".",AGQIO=IO F G="AGQIO","AGPTS" S ZTSAVE(G)=""
...
START ;EP - From TaskMan.
 ...
 S DFN=0
 F  S DFN=$O(^AUPNRRE(DFN)) Q:+DFN<1  D
 .S AGFLAG=0
 .;if there is an HRN for this person and data in VA PATIENT
 .I $D(^AUPNPAT(DFN,41,DUZ(2))),$D(DPT(DFN,0)) D
 ..I AGPTS="A" D   ;active people only
 ...
 ...I $P($G(^AUPNPAT(DFN,41,DUZ(2),0)),U,3)'=""!($P($G(^DPT(DFN,.35)),U)'="") S AGFLAG=1
 ..I AGPTS="B" S AGFLAG=1
 ..I AGFLAG,'$D(^TMP($J,$P(^DPT(DFN,0),U),DFN)) D
 ...S ^TMP($J,$P(^DPT(DFN,0),U),DFN)="R",AGTOT=AGTOT+1
...
PRINT ;Print a Patient's MEDICARE/RAILROAD file.
 ...
 F I=0:0 S I=$O(^AUPNMCR(DFN,11,I)) Q:I=""  D
