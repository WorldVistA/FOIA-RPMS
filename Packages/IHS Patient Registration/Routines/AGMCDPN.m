...
DEV S %ZIS="OPQ" D ^%ZIS I POP S IOP=ION D ^%ZIS Q
 ...
 X ^%ZOSF("UCI")
 S ZTRTN="START^AGMCDPN"
 S ZTUCI=Y,ZTIO=""
 S ZTDESC="MCAID Accounts by Account Number for "_$P(^AUTTLOC(DUZ(2),0),U,2)_"."
...
WRITE D NOW^AG S X=AGTIME D CTR^AG S AGTIME=X
 ...
 F  S AGMID=$O(^TMP($J,AGMID)) Q:AGMID=""  D  Q:AGEND
 .F  S AGNM=$O(^TMP($J,AGMID,AGNM)) Q:AGNM=""  D  Q:AGEND
 ..F  S AGST=$O(^TMP($J,AGMID,AGNM,AGST)) Q:AGST=""  D  Q:AGEND
 ...F  S AGCTY=$O(^TMP($J,AGMID,AGNM,AGST,AGCTY))  Q:AGCTY=""  D  Q:AGEND
 ....F  S DFN=$O(^TMP($J,AGMID,AGNM,AGST,AGCTY,DFN)) Q:DFN=""  D  Q:AGEND
 .....W AGNM
 ...
 .....S AGELGBEG=""
 .....F  S AGELGBEG=$O(^TMP($J,AGMID,AGNM,AGST,AGCTY,DFN,AGELGBEG))  Q:AGELGBEG=""  D
 ......S AGREC=$G(^TMP($J,AGMID,AGNM,AGST,AGCTY,DFN,AGELGBEG))
...
HDR ;
 ...
 .E  S AGPTS1="ACTIVE"
 .W !?8,AGPTS1_" PATIENTS - MEDICAID ACCOUNTS BY ACCOUNT NUMBER"
...
ALL ;
 ...
 .D COUNTY
 .S ^TMP($J,AGMID,$P(^DPT(DFN,0),U),AGST,AGCTY,DFN,AGELGBEG)=AGELGEND_U_AGPLAN_U_AGRATE_U_AGCOV
 Q
OPEN ;
 S AGELGDT=0
 F  S AGELGDT=$O(^AUPNMCD(AGIMID,11,AGELGDT)) Q:AGELGDT=""  D
