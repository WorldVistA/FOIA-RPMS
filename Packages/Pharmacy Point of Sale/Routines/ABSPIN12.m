ABSPIN12 ; ;03/02/18
 D DE G BEGIN
DE S DIE="^ABSP(9002313.56,D0,""INSURER-ASSIGNED #"",",DIC=DIE,DP=9002313.6,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^ABSP(9002313.56,D0,"INSURER-ASSIGNED #",DA,""))=""
...
BEGIN S DNM="ABSPIN12",DQ=1
1 S DW="0;2",DV="Fa",DU="",DLB="INSURER-ASSIGNED #",DIFLD=.02
 S DE(DW)="C1^ABSPIN12"
 G RE
...
X1 K:$L(X)>30!($L(X)<1) X
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="0;3",DV="Fa",DU="",DLB="MED-CAL SUBSCRIBER ID",DIFLD=.03
 S DE(DW)="C2^ABSPIN12"
 G RE
...
X2 K:$L(X)>10!($L(X)<1) X
 ...
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW="0;4",DV="Fa",DU="",DLB="CA FAMILY PACT ID",DIFLD=.04
 S DE(DW)="C3^ABSPIN12"
 S Y="  PACT CA ID"
 G Y
