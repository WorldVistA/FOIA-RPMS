...
ADDEX ;EP;PROCESS ADDITIONAL EXPORT SITES
 ...
 ;N BYIMDA,BYIMAS,BYIMOUT,COPY
 ;S BYIMDA=DUZ(2)
 ;S BYIMAS=0
 ;F  S BYIMAS=$O(^BYIMPARA(BYIMDA,3,BYIMAS)) Q:'BYIMAS  D
 ;.S X=$G(^BYIMPARA(BYIMDA,3,BYIMAS,0))
 ;.Q:X=""
 ;.S BYIMOUT=$P(X,U,2)
 ;.S BYIMOUT=$$SLASH^BYIMIMM6(BYIMOUT)
 ;.N COPY
 ;.S COPY=$S($ZV["UNIX":"cp",1:"copy")
 ;.S Y=$$JOBWAIT^%HOSTCMD(COPY_" "_OPATH_BYIMFILE_" "_BYIMOUT_BYIMFILE)
...
IMMCNT(DFN) ;COUNT PT IMMS TO BE EXPORTED
 ...
 ;-----
FN ;EP;SET FILE NAME
 S:'$G(ASUFAC) ASUFAC=$P($G(^AUTTLOC(DUZ(2),0)),U,10)
