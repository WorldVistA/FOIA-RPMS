...
SCRN(INDA) ;EP;TO SCREEN IMM'S TO INCLUDE IN EXPORT
 ...
 I BYIMDATE,$P(IMM12,U,18)>BYIMDATE S T=1
...
P8PREP ;SET UP FOR P8 TESTING
 ...
 S X=DAT
 F  S X=$O(^BYIMPARA(DUZ(2),"LAST EXPORT",X)) Q:'X  K ^BYIMPARA(DUZ(2),"LAST EXPORT",X)
 W !!,"Patch 8 testing prep complete."
 I $L(DAT)=7 D
 .S ^BYIMPARA(DUZ(2),"LAST EXPORT",DAT)=$H_U_$H
...
NEW(DFN,BYIMALL,BYIMADM,DDDATE) ;EP;DETERMINE IF PATIENT HAS IMMS THAT HAVEN'T BEEN EXPORTED
 ...
 S INDA("DDATE")=$S($G(DDDATE):DDDATE,1:$O(^BYIMPARA(DUZ(2),"LAST EXPORT",9999999999),-1))
 S Y=0
 S INDA=9999999999
