...
LAB ;EP - CALCULATE LAB
 ...
 .;I '$P($G(APCMLABS(APCMP)),U,1) S F=$P(^APCM25OB(APCMIC,0),U,11) D S^APCM25E1(APCMRPT,APCMIC,"Provider is excluded from this measure as he/she did not order any lab tests with results during the time period.",APCMP,APCMRPTT,APCMTIME,F,1) Q
...
TOTLAB1 ;
 ...
 .I APCMMETH="E" I '$$HOSER^APCM25E6(V,APCMFAC),$P(^AUPNVSIT(V,0),U,7)'="I" Q  ;not a H or 30 or I
...
MEDREC ;EP
 ...
 ..S F=$P(^APCM25OB(APCMIC,0),U,11) D S^APCM25E1(APCMRPT,APCMIC,"Provider is excluded from this measure as he/she did not have any transitions during the EHR reporting period.",APCMP,APCMRPTT,APCMTIME,F,1) Q
 .;set denominator value into field
 .S F=$P(^APCM25OB(APCMIC,0),U,8)  ;denom field for this measure
 ...
 ...S DFN=P D SETLIST^APCM25E1 Q
 ..S DFN=P,APCMVALU="# transitions: "_$P(^TMP($J,"TRANS",APCMP,P),U,1)_"|||"_" # w/mmr: "_+$P(^TMP($J,"TRANS",APCMP,P),U,2)_" "_$P(^TMP($J,"TRANS",APCMP,P),U,3)_"|||0" D SETLIST^APCM25E1
 .;numerator?
 .S F=$P(^APCM25OB(APCMIC,0),U,9)
...
IMAGE(V,R) ;WAS THERE AN IMAGE BEFORE VISIT DATE AND IF SO WAS THIS THE FIRST VISIT AFTER THE IMAGE DATE
 NEW D,A,B,C,P,X,Y,T,G,Z,VST,BB
 ...
 .Q:$$VALI^XBDIQ1(2005,B,8.1)="I"  ;IMPORT API - DON'T COUNT THESE
 ...
 ..S BB=0 F  S BB=$O(VST("DTM",Y,BB)) Q:BB'=+BB!(G)  D
 ...S Z=VST("DTM",Y,BB)
 ...Q:'$D(^AUPNVSIT(Z,0))
...
SEM ;EP
 ...
 ..I $P(APCMATTE("S2.026.EP",APCMP),U,1)="Yes" S F=$P(^APCM25OB(APCMIC,0),U,11) D S^APCM25E1(APCMRPT,APCMIC,"BROADBAND",APCMP,APCMRPTT,APCMTIME,F,1)
 ...
 ..S F=$P(^APCM25OB(APCMIC,0),U,8)
 ..D S^APCM25E1(APCMRPT,APCMIC,$P(APCMATTE(I,APCMP),U,2),APCMP,APCMRPTT,APCMTIME,F) ;THIS IS FOR 2015 LINE
 ..;SET 2016 DENOM AND NUMERATOR FIELDS FOR 2016 - IN ST1
 ..Q
 Q
ST1 ;set denominator value into field
 ...
 S E=$E(APCMEDAT,1,3)_"1231"
 S V="" I $T(PHR^BPHRMUPM)]"" D PHR^BPHRMUPM(DFN,B,E,.V,APCMP)
 ;I DUZ=2881 W !,DFN," ",V
 I $P(V,U)="-1" S APCMSEME=1_U_"("_$P($P(V,U,10),":",1)_")" ;APCMVALU=APCMVALU_"|||Secure message sent: COULD NOT GET SECURE MESSAGE INFORMATION  ("_$P($P(V,U,10),":",1)_")"
 S APCMVALU=APCMVALU_"|||"_"Secure message sent: " D
 .I '$P(V,U,5) S APCMVALU=APCMVALU_"NO|||0" Q
 .I $P($P(V,U,6),".")<B Q
 .I $P($P(V,U,6),".")>E Q
...
ERX ;EPRESCRIBING HOSPITAL
 ...
 S I=$P(^APCM25OB(APCMIC,0),U,1)
 I $G(APCMATTE(I,APCMP))="No" S F=$P(^APCM25OB(APCMIC,0),U,11) D  Q
 .D S^APCM25E1(APCMRPT,APCMIC,"NO ONSITE PHARMACY.",APCMP,APCMRPTT,APCMTIME,F,1)
