AUPN9927 ; IHS/OIT/FBD&NKD - AUPN V99.1 PATCH 27 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 04/25/2018
 ;;99.1;IHS DICTIONARIES (PATIENT);**27**;MAR 9, 1999;Build 2
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("AUPN","99.1","26") XPDQUIT=2  ; AUPN
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 KILL ^TMP("AUPN9927",$J)
 S ^TMP("AUPN9927",$J,1)=" --- AUPN v 99.1, Patch 27, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9927",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9927"",$J,",XMY(1)="",XMY(DUZ)=""
