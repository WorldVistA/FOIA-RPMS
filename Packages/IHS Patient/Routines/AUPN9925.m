AUPN9925 ; IHS/OIT/FBD&NKD - AUPN V99.1 PATCH 25 ENVIRONMENT CHECKS AND POST-INIT PROCESS ; 07/14/2017
 ;;99.1;IHS DICTIONARIES (PATIENT);**25**;MAR 9, 1999;Build 1
 ;
 ;
 D:'$D(IOM) HOME^%ZIS
 ;
 ...
 S:'$$VCHK("AUPN","99.1","24") XPDQUIT=2  ; AUPN
...
MAIL ;----- SEND INSTALL MAIL MESSAGE 
 ...
 KILL ^TMP("AUPN9925",$J)
 S ^TMP("AUPN9925",$J,1)=" --- AUPN v 99.1, Patch 25, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AUPN9925",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AUPN9925"",$J,",XMY(1)="",XMY(DUZ)=""
