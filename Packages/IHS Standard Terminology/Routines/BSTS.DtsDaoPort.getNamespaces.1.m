 ;BSTS.DtsDaoPort.getNamespaces.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getNamespaces.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;35584571;BSTS.DtsDaoPort.getNamespaces
 ;
...
zInvoke(%Client,%Action) public {
 Do %Client.InvokeClient($this,"getNamespaces",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getNamespaces",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getNamespaces")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getNamespaces")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getNamespaces"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"getNamespaces"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getNamespaces"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getNamespaces",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getNamespaces")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getNamespaces).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getNamespaces",top,format,namespacePrefix,input,refOnly,.schema)
