 ;BSTS.DtsDaoPort.findSubsetPropertyTypeByName.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findSubsetPropertyTypeByName.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;2B5A556C;BSTS.DtsDaoPort.findSubsetPropertyTypeByName
 ;
...
zInvoke(%Client,%Action,name,subsetId) public {
 ...
 Do %Client.InvokeClient($this,"findSubsetPropertyTypeByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findSubsetPropertyTypeByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findSubsetPropertyTypeByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findSubsetPropertyTypeByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findSubsetPropertyTypeByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findSubsetPropertyTypeByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findSubsetPropertyTypeByName"
   ...
   If tag="" Set tag=typesPrefix_"findSubsetPropertyTypeByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findSubsetPropertyTypeByName"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findSubsetPropertyTypeByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findSubsetPropertyTypeByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findSubsetPropertyTypeByNameResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findSubsetPropertyTypeByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findSubsetPropertyTypeByName",top,format,namespacePrefix,input,refOnly,.schema)
