 ;BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;7A4F4B5A;BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,propType,qualType,pattern,options) public {
 Set ..propType=$get(propType),..qualType=$get(qualType),..pattern=$get(pattern),..options=$get(options)
 Do %Client.InvokeClient($this,"findConceptsWithPropertyQualifierMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithPropertyQualifierMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithPropertyQualifierMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithPropertyQualifierMatching"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="propType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TPropertyType"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findConceptsWithPropertyQualifierMatching",top,format,namespacePrefix,input,refOnly,.schema)
