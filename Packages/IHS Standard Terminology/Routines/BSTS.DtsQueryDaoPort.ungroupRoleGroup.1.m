 ;BSTS.DtsQueryDaoPort.ungroupRoleGroup.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.ungroupRoleGroup.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;4266622F;BSTS.DtsQueryDaoPort.ungroupRoleGroup
 ;
...
%NormalizeObject() public {
	...
	If m%roleGroupNum Set:i%roleGroupNum'="" i%roleGroupNum=(..roleGroupNumNormalize(i%roleGroupNum))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%roleGroupNum Set iv=..roleGroupNum If iv'="" Set rc=(..roleGroupNumIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"roleGroupNum",iv)
	Quit sc }
zInvoke(%Client,%Action,concept,roleGroupNum) public {
 Set ..concept=$get(concept),..roleGroupNum=$get(roleGroupNum)
 Do %Client.InvokeClient($this,"ungroupRoleGroup",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.ungroupRoleGroup",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.ungroupRoleGroup")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.ungroupRoleGroup")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.ungroupRoleGroup"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="ungroupRoleGroup"
   ...
   If tag="" Set tag=typesPrefix_"ungroupRoleGroup"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"ungroupRoleGroup"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..roleGroupNum
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"roleGroupNum"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"roleGroupNum>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.ungroupRoleGroup",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.ungroupRoleGroup")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..roleGroupNum'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.ungroupRoleGroup).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.ungroupRoleGroup",top,format,namespacePrefix,input,refOnly,.schema)
zconceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..concept=newobject
	Quit newobject }
zroleGroupNumDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zroleGroupNumIsValid(%val) public {
...
zroleGroupNumNormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zroleGroupNumXSDToLogical(%val) public {
