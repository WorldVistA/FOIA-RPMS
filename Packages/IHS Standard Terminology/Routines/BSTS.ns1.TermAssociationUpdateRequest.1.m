 ;BSTS.ns1.TermAssociationUpdateRequest.1
 ;(C)InterSystems, generated for class BSTS.ns1.TermAssociationUpdateRequest.  Do NOT edit. 08/21/2017 07:46:00PM
 ;;356D6449;BSTS.ns1.TermAssociationUpdateRequest
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..newAssociation
 s proporef=..oldAssociation
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..newAssociation)=1 Set r%newAssociation=r%newAssociation.%ConstructClone(1,.cloned),i%newAssociation=""
		If $isobject(..oldAssociation)=1 Set r%oldAssociation=r%oldAssociation.%ConstructClone(1,.cloned),i%oldAssociation=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TermAssociationUpdateRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TermAssociationUpdateRequest")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TermAssociationUpdateRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TermAssociationUpdateRequest"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TermAssociationUpdateRequest"
   ...
   If tag="" Set tag=typesPrefix_"TermAssociationUpdateRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TermAssociationUpdateRequest"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"oldAssociation "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="oldAssociation",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TTermAssociation"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..newAssociation
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TTermAssociation","XMLSUMMARY")'="":-1,1:1)
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"newAssociation "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="newAssociation",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TTermAssociation"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TermAssociationUpdateRequest" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TermAssociationUpdateRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TermAssociationUpdateRequest")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="oldAssociation" {
   ...
   If data'="" Set ..oldAssociation=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="newAssociation" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
   ...
   If data'="" Set ..newAssociation=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
...
zXMLIsObjectEmpty(ignoreNull)
 If $IsObject(..oldAssociation) Quit 0
 If $IsObject(..newAssociation) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TermAssociationUpdateRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TermAssociationUpdateRequest",top,format,namespacePrefix,input,refOnly,.schema)
znewAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TTermAssociation).%New() If newobject="" Quit ""
	Set ..newAssociation=newobject
	Quit newobject }
zoldAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TTermAssociation).%New() If newobject="" Quit ""
	Set ..oldAssociation=newobject
	Quit newobject }
