 ;BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;2B416345;BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching
 ;
...
zInvoke(%Client,%Action,associationType,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findTermsWithInverseTermAssociationMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithInverseTermAssociationMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithInverseTermAssociationMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithInverseTermAssociationMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findTermsWithInverseTermAssociationMatching",top,format,namespacePrefix,input,refOnly,.schema)
