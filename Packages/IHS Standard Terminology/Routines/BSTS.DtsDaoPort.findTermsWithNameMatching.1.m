 ;BSTS.DtsDaoPort.findTermsWithNameMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findTermsWithNameMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;425A587A;BSTS.DtsDaoPort.findTermsWithNameMatching
 ;
...
zInvoke(%Client,%Action,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findTermsWithNameMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findTermsWithNameMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithNameMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findTermsWithNameMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findTermsWithNameMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithNameMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithNameMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithNameMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findTermsWithNameMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithNameMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findTermsWithNameMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findTermsWithNameMatching",top,format,namespacePrefix,input,refOnly,.schema)
