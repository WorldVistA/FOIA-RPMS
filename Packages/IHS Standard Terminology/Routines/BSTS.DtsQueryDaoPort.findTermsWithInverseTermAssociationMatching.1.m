 ;BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;3945714A;BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching
 ;
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching"))
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findTermsWithInverseTermAssociationMatching",top,format,namespacePrefix,input,refOnly,.schema)
