 ;BSTS.ns1.TSubsetAttributeSetDescriptor.1
 ;(C)InterSystems, generated for class BSTS.ns1.TSubsetAttributeSetDescriptor.  Do NOT edit. 08/21/2017 07:46:02PM
 ;;72535A42;BSTS.ns1.TSubsetAttributeSetDescriptor
 ;
%NormalizeObject() public {
	...
	If m%fetchConceptCount Set:i%fetchConceptCount'="" i%fetchConceptCount=(..fetchConceptCountNormalize(i%fetchConceptCount))
	If m%fetchExpression Set:i%fetchExpression'="" i%fetchExpression=(..fetchExpressionNormalize(i%fetchExpression))
	...
	If m%versions Set:i%versions'="" i%versions=(..versionsNormalize(i%versions))
	Quit 1 }
%ValidateObject(force=0) public {
	...
	Set iv=..fetchConceptCount If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"fetchConceptCount"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%fetchConceptCount Set rc=(..fetchConceptCountIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"fetchConceptCount",iv)
	Set iv=..fetchExpression If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"fetchExpression"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%fetchExpression Set rc=(..fetchExpressionIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"fetchExpression",iv)
	...
	Set iv=..versions If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"versions"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%versions Set rc=(..versionsIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"versions",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TSubsetAttributeSetDescriptor",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSubsetAttributeSetDescriptor")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TSubsetAttributeSetDescriptor")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TSubsetAttributeSetDescriptor"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TSubsetAttributeSetDescriptor"
   ...
   If tag="" Set tag=typesPrefix_"TSubsetAttributeSetDescriptor"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TSubsetAttributeSetDescriptor"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..versions
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"versions"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"versions>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..versionAttributeSetDescriptor
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TAttributeSetDescriptor","XMLSUMMARY")'="":-1,1:1)
     Set id=+$get(idlist(temp*val))
     If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
   }
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"versionAttributeSetDescriptor "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="versionAttributeSetDescriptor",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..fetchExpression
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"fetchExpression"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"fetchExpression>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..fetchConceptCount
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"fetchConceptCount"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"fetchConceptCount>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TSubsetAttributeSetDescriptor" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TSubsetAttributeSetDescriptor",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSubsetAttributeSetDescriptor")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="versions" {
   ...
   Set ..versions=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="versionAttributeSetDescriptor" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
     Set class=$select(($get(@(tree)@(ref,"a","type","u"))=1):$get(@(tree)@(ref,"a","type")),1:"")
     If (class="") || (class="TAttributeSetDescriptor") {
       Set class="BSTS.ns1.TAttributeSetDescriptor"
     } Else {
       If $length(class,":")=2 Set class=$piece(class,":",2)
       Set class=$get(^oddCOM("BSTS.ns1.TAttributeSetDescriptor",85,"s",class))_$get(^oddXML("BSTS.ns1.TAttributeSetDescriptor","s",class)) If class="" Set class=0
       Set tmp=$select($get(@(tree)@(ref,"a","type","v"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","type","v"))))
       If (class=0)||(($listlength(class)>1)&&(tmp="")) Set class=$get(@(tree)@(ref,"a","type")) Goto XMLImportBadType
       If $listlength(class)>1 {
         For tmpi=1:1:$listlength(class) {
   ...
   If data'="" Set ..versionAttributeSetDescriptor=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="fetchExpression" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
   ...
   Set ..fetchExpression=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="fetchConceptCount" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..versions'="" Quit 0
 If $IsObject(..versionAttributeSetDescriptor) Quit 0
 If ..fetchExpression'="" Quit 0
 If ..fetchConceptCount'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TSubsetAttributeSetDescriptor).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TSubsetAttributeSetDescriptor",top,format,namespacePrefix,input,refOnly,.schema)
zfetchConceptCountDisplayToLogi(%val) public {
	Quit ''%val }
zfetchConceptCountIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zfetchConceptCountLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zfetchConceptCountNormalize(%val) public {
	Quit %val\1 }
zfetchConceptCountXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zfetchExpressionDisplayToLogica(%val) public {
	Quit ''%val }
zfetchExpressionIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zfetchExpressionLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zfetchExpressionNormalize(%val) public {
	Quit %val\1 }
zfetchExpressionXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
