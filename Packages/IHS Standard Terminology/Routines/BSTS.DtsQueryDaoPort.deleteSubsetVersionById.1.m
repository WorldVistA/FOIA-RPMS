 ;BSTS.DtsQueryDaoPort.deleteSubsetVersionById.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteSubsetVersionById.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;57474742;BSTS.DtsQueryDaoPort.deleteSubsetVersionById
 ;
...
%NormalizeObject() public {
	...
	If m%versionId Set:i%versionId'="" i%versionId=(..versionIdNormalize(i%versionId))
	Quit 1 }
%ValidateObject(force=0) public {
	...
	If m%versionId Set iv=..versionId If iv'="" Set rc=(..versionIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"versionId",iv)
	Quit sc }
zInvoke(%Client,%Action,versionId,subsetId) public {
 Set ..versionId=$get(versionId),..subsetId=$get(subsetId)
 Do %Client.InvokeClient($this,"deleteSubsetVersionById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteSubsetVersionByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteSubsetVersionById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteSubsetVersionById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteSubsetVersionById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteSubsetVersionById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteSubsetVersionById"
   ...
   If tag="" Set tag=typesPrefix_"deleteSubsetVersionById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteSubsetVersionById"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..versionId
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"versionId"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"versionId>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..subsetId
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteSubsetVersionById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteSubsetVersionById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteSubsetVersionByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteSubsetVersionById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteSubsetVersionById",top,format,namespacePrefix,input,refOnly,.schema)
