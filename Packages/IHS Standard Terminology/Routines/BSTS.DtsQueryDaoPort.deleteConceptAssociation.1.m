 ;BSTS.DtsQueryDaoPort.deleteConceptAssociation.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteConceptAssociation.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;436F502B;BSTS.DtsQueryDaoPort.deleteConceptAssociation
 ;
...
zInvoke(%Client,%Action,conceptAssociation) public {
 ...
 Do %Client.InvokeClient($this,"deleteConceptAssociation",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteConceptAssociationResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteConceptAssociation",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteConceptAssociation")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteConceptAssociation")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteConceptAssociation"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteConceptAssociation"
   ...
   If tag="" Set tag=typesPrefix_"deleteConceptAssociation"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteConceptAssociation"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteConceptAssociation",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteConceptAssociation")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteConceptAssociationResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteConceptAssociation).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteConceptAssociation",top,format,namespacePrefix,input,refOnly,.schema)
