 ;BSTS.DtsQueryDaoPort.createNamespaceRevision.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.createNamespaceRevision.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;42305A30;BSTS.DtsQueryDaoPort.createNamespaceRevision
 ;
...
%NormalizeObject() public {
	...
	If m%comments Set:i%comments'="" i%comments=(..commentsNormalize(i%comments))
...
%ValidateObject(force=0) public {
	...
	If m%comments Set iv=..comments If iv'="" Set rc=(..commentsIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"comments",iv)
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,comments) public {
 Set ..namespaceId=$get(namespaceId),..comments=$get(comments)
 Do %Client.InvokeClient($this,"createNamespaceRevision",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="createNamespaceRevisionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.createNamespaceRevision",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.createNamespaceRevision")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.createNamespaceRevision")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.createNamespaceRevision"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="createNamespaceRevision"
   ...
   If tag="" Set tag=typesPrefix_"createNamespaceRevision"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"createNamespaceRevision"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..comments
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"comments"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"comments>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.createNamespaceRevision",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.createNamespaceRevision")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="createNamespaceRevisionResult" {
   ...
   If (class="") || (class="TNamespaceRevision") {
     Set class="BSTS.ns1.TNamespaceRevision"
   } Else {
     If $length(class,":")=2 Set class=$piece(class,":",2)
     Set class=$get(^oddCOM("BSTS.ns1.TNamespaceRevision",85,"t",class))_$get(^oddXML("BSTS.ns1.TNamespaceRevision","t",class)) If class="" Set class=0
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.createNamespaceRevision).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.createNamespaceRevision",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TNamespaceRevision).%New() If newobject="" Quit ""
