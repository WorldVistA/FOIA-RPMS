 ;BSTS.DtsQueryDaoPort.createRoleGroup.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.createRoleGroup.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;466C322B;BSTS.DtsQueryDaoPort.createRoleGroup
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,concept,roles) public {
 Set ..concept=$get(concept),..roles=$get(roles)
 Do %Client.InvokeClient($this,"createRoleGroup",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="createRoleGroupResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.createRoleGroup",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.createRoleGroup")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.createRoleGroup")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.createRoleGroup"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="createRoleGroup"
   ...
   If tag="" Set tag=typesPrefix_"createRoleGroup"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"createRoleGroup"""_xsiAttrs,xsiAttrs=""
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   While k'="" {
     If $IsObject(val) {
         Set topArg="roles",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TRole"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
     } Else {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.createRoleGroup",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.createRoleGroup")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.createRoleGroup).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.createRoleGroup",top,format,namespacePrefix,input,refOnly,.schema)
