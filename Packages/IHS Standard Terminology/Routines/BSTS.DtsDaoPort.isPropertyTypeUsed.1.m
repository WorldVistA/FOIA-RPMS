 ;BSTS.DtsDaoPort.isPropertyTypeUsed.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.isPropertyTypeUsed.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;642B704A;BSTS.DtsDaoPort.isPropertyTypeUsed
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,propType) public {
 Set ..propType=$get(propType)
 Do %Client.InvokeClient($this,"isPropertyTypeUsed",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="isPropertyTypeUsedResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.isPropertyTypeUsed",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.isPropertyTypeUsed")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.isPropertyTypeUsed")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.isPropertyTypeUsed"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isPropertyTypeUsed"
   ...
   If tag="" Set tag=typesPrefix_"isPropertyTypeUsed"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isPropertyTypeUsed"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.isPropertyTypeUsed",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.isPropertyTypeUsed")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="isPropertyTypeUsedResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.isPropertyTypeUsed).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.isPropertyTypeUsed",top,format,namespacePrefix,input,refOnly,.schema)
