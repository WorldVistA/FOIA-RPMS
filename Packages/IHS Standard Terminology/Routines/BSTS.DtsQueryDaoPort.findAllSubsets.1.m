 ;BSTS.DtsQueryDaoPort.findAllSubsets.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findAllSubsets.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;386C526E;BSTS.DtsQueryDaoPort.findAllSubsets
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,asd) public {
 Set ..asd=$get(asd)
 Do %Client.InvokeClient($this,"findAllSubsets",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findAllSubsets",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findAllSubsets")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findAllSubsets")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findAllSubsets"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"findAllSubsets"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findAllSubsets"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findAllSubsets",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findAllSubsets")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findAllSubsets).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findAllSubsets",top,format,namespacePrefix,input,refOnly,.schema)
