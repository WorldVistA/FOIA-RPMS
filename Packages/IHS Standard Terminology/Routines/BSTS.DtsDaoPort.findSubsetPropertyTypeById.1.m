 ;BSTS.DtsDaoPort.findSubsetPropertyTypeById.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findSubsetPropertyTypeById.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;4448515A;BSTS.DtsDaoPort.findSubsetPropertyTypeById
 ;
...
zInvoke(%Client,%Action,id,subsetId) public {
 ...
 Do %Client.InvokeClient($this,"findSubsetPropertyTypeById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findSubsetPropertyTypeByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findSubsetPropertyTypeById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findSubsetPropertyTypeById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findSubsetPropertyTypeById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findSubsetPropertyTypeById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findSubsetPropertyTypeById"
   ...
   If tag="" Set tag=typesPrefix_"findSubsetPropertyTypeById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findSubsetPropertyTypeById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findSubsetPropertyTypeById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findSubsetPropertyTypeById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findSubsetPropertyTypeByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findSubsetPropertyTypeById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findSubsetPropertyTypeById",top,format,namespacePrefix,input,refOnly,.schema)
