 ;BSTS.DtsQueryDaoPort.addSubsetVersionProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addSubsetVersionProperty.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;31597535;BSTS.DtsQueryDaoPort.addSubsetVersionProperty
 ;
...
zInvoke(%Client,%Action,version,property) public {
 ...
 Do %Client.InvokeClient($this,"addSubsetVersionProperty",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addSubsetVersionPropertyResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addSubsetVersionProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addSubsetVersionProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addSubsetVersionProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addSubsetVersionProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addSubsetVersionProperty"
   ...
   If tag="" Set tag=typesPrefix_"addSubsetVersionProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addSubsetVersionProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addSubsetVersionProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addSubsetVersionProperty")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="addSubsetVersionPropertyResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addSubsetVersionProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addSubsetVersionProperty",top,format,namespacePrefix,input,refOnly,.schema)
