 ;BSTS.DtsDaoPort.getAssociationTypes.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getAssociationTypes.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;3251324D;BSTS.DtsDaoPort.getAssociationTypes
 ;
...
zInvoke(%Client,%Action,namespaceId) public {
 ...
 Do %Client.InvokeClient($this,"getAssociationTypes",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getAssociationTypes",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getAssociationTypes")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getAssociationTypes")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getAssociationTypes"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getAssociationTypes"
   ...
   If tag="" Set tag=typesPrefix_"getAssociationTypes"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getAssociationTypes"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getAssociationTypes",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getAssociationTypes")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getAssociationTypes).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getAssociationTypes",top,format,namespacePrefix,input,refOnly,.schema)
