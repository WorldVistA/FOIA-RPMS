 ;BSTS.DtsQueryDaoPort.executeSQLUpdate.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.executeSQLUpdate.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;69344775;BSTS.DtsQueryDaoPort.executeSQLUpdate
 ;
...
zInvoke(%Client,%Action,sql) public {
 Set ..sql=$get(sql)
 Do %Client.InvokeClient($this,"executeSQLUpdate",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="executeSQLUpdateResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.executeSQLUpdate",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeSQLUpdate")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.executeSQLUpdate")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.executeSQLUpdate"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="executeSQLUpdate"
   ...
   If tag="" Set tag=typesPrefix_"executeSQLUpdate"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"executeSQLUpdate"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.executeSQLUpdate",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeSQLUpdate")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.executeSQLUpdate).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.executeSQLUpdate",top,format,namespacePrefix,input,refOnly,.schema)
