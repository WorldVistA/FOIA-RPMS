 ;BSTS.DtsQueryDaoPort.hasNamespacePermission.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.hasNamespacePermission.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;4A665058;BSTS.DtsQueryDaoPort.hasNamespacePermission
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,permission) public {
 Set ..namespaceId=$get(namespaceId),..permission=$get(permission)
 Do %Client.InvokeClient($this,"hasNamespacePermission",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="hasNamespacePermissionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.hasNamespacePermission",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasNamespacePermission")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.hasNamespacePermission")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.hasNamespacePermission"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="hasNamespacePermission"
   ...
   If tag="" Set tag=typesPrefix_"hasNamespacePermission"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"hasNamespacePermission"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.hasNamespacePermission",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasNamespacePermission")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="hasNamespacePermissionResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.hasNamespacePermission).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.hasNamespacePermission",top,format,namespacePrefix,input,refOnly,.schema)
