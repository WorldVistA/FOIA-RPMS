 ;BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;72317A6A;BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences
 ;
...
%NormalizeObject() public {
	...
	If m%removeReferences Set:i%removeReferences'="" i%removeReferences=(..removeReferencesNormalize(i%removeReferences))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%removeReferences Set iv=..removeReferences If iv'="" Set rc=(..removeReferencesIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"removeReferences",iv)
	Quit sc }
zInvoke(%Client,%Action,roleType,removeReferences) public {
 Set ..roleType=$get(roleType),..removeReferences=$get(removeReferences)
 Do %Client.InvokeClient($this,"deleteRoleTypeWithReferences",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteRoleTypeWithReferencesResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteRoleTypeWithReferences"
   ...
   If tag="" Set tag=typesPrefix_"deleteRoleTypeWithReferences"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteRoleTypeWithReferences"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..removeReferences
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"removeReferences"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"removeReferences>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteRoleTypeWithReferencesResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..removeReferences'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteRoleTypeWithReferences",top,format,namespacePrefix,input,refOnly,.schema)
...
zremoveReferencesDisplayToLogic(%val) public {
	Quit ''%val }
zremoveReferencesIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zremoveReferencesLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zremoveReferencesNormalize(%val) public {
	Quit %val\1 }
zremoveReferencesXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
