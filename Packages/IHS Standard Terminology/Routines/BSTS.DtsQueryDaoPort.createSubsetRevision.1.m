 ;BSTS.DtsQueryDaoPort.createSubsetRevision.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.createSubsetRevision.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;53667376;BSTS.DtsQueryDaoPort.createSubsetRevision
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,subsetId,comments) public {
 Set ..subsetId=$get(subsetId),..comments=$get(comments)
 Do %Client.InvokeClient($this,"createSubsetRevision",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="createSubsetRevisionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.createSubsetRevision",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.createSubsetRevision")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.createSubsetRevision")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.createSubsetRevision"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="createSubsetRevision"
   ...
   If tag="" Set tag=typesPrefix_"createSubsetRevision"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"createSubsetRevision"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.createSubsetRevision",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.createSubsetRevision")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="createSubsetRevisionResult" {
   ...
   If (class="") || (class="TSubsetRevision") {
     Set class="BSTS.ns1.TSubsetRevision"
   } Else {
     If $length(class,":")=2 Set class=$piece(class,":",2)
     Set class=$get(^oddCOM("BSTS.ns1.TSubsetRevision",85,"t",class))_$get(^oddXML("BSTS.ns1.TSubsetRevision","t",class)) If class="" Set class=0
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.createSubsetRevision).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.createSubsetRevision",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TSubsetRevision).%New() If newobject="" Quit ""
