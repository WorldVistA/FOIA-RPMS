 ;BSTS.DtsQueryDaoPort.fetchAuthorityById.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchAuthorityById.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;426C346F;BSTS.DtsQueryDaoPort.fetchAuthorityById
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,authorityId,asd) public {
 Set ..authorityId=$get(authorityId),..asd=$get(asd)
 Do %Client.InvokeClient($this,"fetchAuthorityById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="fetchAuthorityByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchAuthorityById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchAuthorityById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchAuthorityById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchAuthorityById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchAuthorityById"
   ...
   If tag="" Set tag=typesPrefix_"fetchAuthorityById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchAuthorityById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchAuthorityById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchAuthorityById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="fetchAuthorityByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchAuthorityById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchAuthorityById",top,format,namespacePrefix,input,refOnly,.schema)
