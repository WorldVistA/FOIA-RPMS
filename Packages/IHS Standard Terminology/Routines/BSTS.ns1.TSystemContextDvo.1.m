 ;BSTS.ns1.TSystemContextDvo.1
 ;(C)InterSystems, generated for class BSTS.ns1.TSystemContextDvo.  Do NOT edit. 08/21/2017 07:45:59PM
 ;;5950546E;BSTS.ns1.TSystemContextDvo
 ;
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%contextKey Set:i%contextKey'="" i%contextKey=(..contextKeyNormalize(i%contextKey))
	If m%contextValue Set:i%contextValue'="" i%contextValue=(..contextValueNormalize(i%contextValue))
	Quit 1 }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	Set iv=..contextKey If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"contextKey"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%contextKey Set rc=(..contextKeyIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"contextKey",iv)
	Set iv=..contextValue If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"contextValue"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%contextValue Set rc=(..contextValueIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"contextValue",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TSystemContextDvo",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSystemContextDvo")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TSystemContextDvo")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TSystemContextDvo"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TSystemContextDvo"
   ...
   If tag="" Set tag=typesPrefix_"TSystemContextDvo"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TSystemContextDvo"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..contextKey
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"contextKey"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"contextKey>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..contextValue
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"contextValue"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"contextValue>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TSystemContextDvo",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSystemContextDvo")
 If encoded {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..contextKey'="" Quit 0
 If ..contextValue'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TSystemContextDvo).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TSystemContextDvo",top,format,namespacePrefix,input,refOnly,.schema)
