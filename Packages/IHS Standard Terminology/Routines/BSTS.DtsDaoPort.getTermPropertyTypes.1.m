 ;BSTS.DtsDaoPort.getTermPropertyTypes.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getTermPropertyTypes.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;62766A50;BSTS.DtsDaoPort.getTermPropertyTypes
 ;
...
zInvoke(%Client,%Action,namespaceId) public {
 ...
 Do %Client.InvokeClient($this,"getTermPropertyTypes",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getTermPropertyTypes",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getTermPropertyTypes")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getTermPropertyTypes")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getTermPropertyTypes"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getTermPropertyTypes"
   ...
   If tag="" Set tag=typesPrefix_"getTermPropertyTypes"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getTermPropertyTypes"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getTermPropertyTypes",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getTermPropertyTypes")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getTermPropertyTypes).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getTermPropertyTypes",top,format,namespacePrefix,input,refOnly,.schema)
