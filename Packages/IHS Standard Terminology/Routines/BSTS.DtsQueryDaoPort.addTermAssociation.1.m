 ;BSTS.DtsQueryDaoPort.addTermAssociation.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addTermAssociation.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;53696A45;BSTS.DtsQueryDaoPort.addTermAssociation
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,newTermAssociation) public {
 Set ..newTermAssociation=$get(newTermAssociation)
 Do %Client.InvokeClient($this,"addTermAssociation",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addTermAssociationResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addTermAssociation",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addTermAssociation")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addTermAssociation")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addTermAssociation"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addTermAssociation"
   ...
   If tag="" Set tag=typesPrefix_"addTermAssociation"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addTermAssociation"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addTermAssociation",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addTermAssociation")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="addTermAssociationResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addTermAssociation).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addTermAssociation",top,format,namespacePrefix,input,refOnly,.schema)
