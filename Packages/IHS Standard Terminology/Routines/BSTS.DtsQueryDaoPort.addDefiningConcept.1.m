 ;BSTS.DtsQueryDaoPort.addDefiningConcept.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addDefiningConcept.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;466A5568;BSTS.DtsQueryDaoPort.addDefiningConcept
 ;
...
zInvoke(%Client,%Action,concept,definingConcept) public {
 ...
 Do %Client.InvokeClient($this,"addDefiningConcept",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addDefiningConceptResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addDefiningConcept",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addDefiningConcept")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addDefiningConcept")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addDefiningConcept"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addDefiningConcept"
   ...
   If tag="" Set tag=typesPrefix_"addDefiningConcept"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addDefiningConcept"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addDefiningConcept",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addDefiningConcept")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="addDefiningConceptResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addDefiningConcept).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addDefiningConcept",top,format,namespacePrefix,input,refOnly,.schema)
