 ;BSTS.DtsDaoPort.deleteVersionByName.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.deleteVersionByName.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;772F5531;BSTS.DtsDaoPort.deleteVersionByName
 ;
...
%NormalizeObject() public {
	...
	If m%arg0 Set:i%arg0'="" i%arg0=(..arg0Normalize(i%arg0))
	If m%arg1 Set:i%arg1'="" i%arg1=(..arg1Normalize(i%arg1))
	Quit 1 }
%ValidateObject(force=0) public {
	...
	If m%arg0 Set iv=..arg0 If iv'="" Set rc=(..arg0IsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"arg0",iv)
	If m%arg1 Set iv=..arg1 If iv'="" Set rc=(..arg1IsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"arg1",iv)
	Quit sc }
zInvoke(%Client,%Action,arg0,arg1) public {
 Set ..arg0=$get(arg0),..arg1=$get(arg1)
 Do %Client.InvokeClient($this,"deleteVersionByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteVersionByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.deleteVersionByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteVersionByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.deleteVersionByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.deleteVersionByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteVersionByName"
   ...
   If tag="" Set tag=typesPrefix_"deleteVersionByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteVersionByName"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..arg0
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"arg0"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"arg0>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..arg1
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"arg1"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"arg1>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.deleteVersionByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteVersionByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteVersionByNameResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.deleteVersionByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.deleteVersionByName",top,format,namespacePrefix,input,refOnly,.schema)
...
zarg1DisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zarg1IsValid(%val) public {
...
zarg1Normalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zarg1XSDToLogical(%val) public {
