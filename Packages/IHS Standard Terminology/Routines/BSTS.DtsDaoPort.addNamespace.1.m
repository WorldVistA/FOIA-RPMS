 ;BSTS.DtsDaoPort.addNamespace.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.addNamespace.  Do NOT edit. 08/21/2017 07:46:09PM
 ;;4D383834;BSTS.DtsDaoPort.addNamespace
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,namespace) public {
 Set ..namespace=$get(namespace)
 Do %Client.InvokeClient($this,"addNamespace",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addNamespaceResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.addNamespace",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.addNamespace")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.addNamespace")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.addNamespace"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"addNamespace"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addNamespace"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.addNamespace",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.addNamespace")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.addNamespace).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.addNamespace",top,format,namespacePrefix,input,refOnly,.schema)
