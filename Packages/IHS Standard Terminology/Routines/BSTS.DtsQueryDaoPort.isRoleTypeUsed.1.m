 ;BSTS.DtsQueryDaoPort.isRoleTypeUsed.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.isRoleTypeUsed.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;35766F31;BSTS.DtsQueryDaoPort.isRoleTypeUsed
 ;
...
zInvoke(%Client,%Action,roleType) public {
 ...
 Do %Client.InvokeClient($this,"isRoleTypeUsed",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="isRoleTypeUsedResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.isRoleTypeUsed",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isRoleTypeUsed")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.isRoleTypeUsed")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.isRoleTypeUsed"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"isRoleTypeUsed"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isRoleTypeUsed"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.isRoleTypeUsed",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isRoleTypeUsed")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.isRoleTypeUsed).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.isRoleTypeUsed",top,format,namespacePrefix,input,refOnly,.schema)
