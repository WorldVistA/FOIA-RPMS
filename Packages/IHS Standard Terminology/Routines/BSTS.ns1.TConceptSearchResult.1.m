 ;BSTS.ns1.TConceptSearchResult.1
 ;(C)InterSystems, generated for class BSTS.ns1.TConceptSearchResult.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;6B52724B;BSTS.ns1.TConceptSearchResult
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..ontylogConcept
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..ontylogConcept)=1 Set r%ontylogConcept=r%ontylogConcept.%ConstructClone(1,.cloned),i%ontylogConcept=""
	}
	Quit 1 }
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%rank Set:i%rank'="" i%rank=(..rankNormalize(i%rank))
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%ontylogConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	If m%rank Set iv=..rank If iv'="" Set rc=(..rankIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"rank",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TConceptSearchResult",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TConceptSearchResult")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TConceptSearchResult")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TConceptSearchResult"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TConceptSearchResult"
   ...
   If tag="" Set tag=typesPrefix_"TConceptSearchResult"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TConceptSearchResult"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..rank
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"rank"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"rank>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..ontylogConcept
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"ontylogConcept "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="ontylogConcept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TConceptSearchResult" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TConceptSearchResult",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TConceptSearchResult")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="ontylogConcept" {
   ...
   If data'="" Set ..ontylogConcept=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
...
zXMLIsObjectEmpty(ignoreNull)
 If ..rank'="" Quit 0
 If $IsObject(..ontylogConcept) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TConceptSearchResult).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TConceptSearchResult",top,format,namespacePrefix,input,refOnly,.schema)
zontylogConceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..ontylogConcept=newobject
	Quit newobject }
zrankDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zrankIsValid(%val) public {
...
zrankNormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zrankXSDToLogical(%val) public {
