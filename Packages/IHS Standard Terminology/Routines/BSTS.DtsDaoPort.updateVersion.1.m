 ;BSTS.DtsDaoPort.updateVersion.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.updateVersion.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;69413057;BSTS.DtsDaoPort.updateVersion
 ;
...
zInvoke(%Client,%Action,arg0,arg1) public {
 ...
 Do %Client.InvokeClient($this,"updateVersion",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateVersionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.updateVersion",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateVersion")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.updateVersion")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.updateVersion"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"updateVersion"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateVersion"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="arg0",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TContentVersion"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..arg1
 If $IsObject(val) , deepFlag {
     Set topArg="arg1",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TContentVersion"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.updateVersion",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateVersion")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.updateVersion).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.updateVersion",top,format,namespacePrefix,input,refOnly,.schema)
