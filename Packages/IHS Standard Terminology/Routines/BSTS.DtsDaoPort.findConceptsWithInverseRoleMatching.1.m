 ;BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;46622F50;BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,roleType,pattern,options) public {
 Set ..roleType=$get(roleType),..pattern=$get(pattern),..options=$get(options)
 Do %Client.InvokeClient($this,"findConceptsWithInverseRoleMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching"))
 If $IsObject(namespaces) {
   If namespaces.Stable,namespaces.CurrentNamespace="http://ws.dao.dtsserver.apelon.com/",'tmpi||(typesPrefix'="") {
     Set topAttrs=""
   } Else {
     Set popAtEnd=1,saveTopPrefix=topPrefix,saveTypesPrefix=typesPrefix,saveAttrsPrefix=attrsPrefix,saveUsePrefix=usePrefix
     Set sc=namespaces.PushNodeForExport("http://ws.dao.dtsserver.apelon.com/",$get(local,0),tmpi,"",,.topPrefix,.topAttrs,.typesPrefix,.attrsPrefix,.usePrefix)
     If 'sc Quit sc
   }
   Set beginprefix=$select(namespaces.ElementQualified&&usePrefix:typesPrefix,1:"")
   If topAttrs'="" Set temp=temp_" "_topAttrs
   If tag="" Set tag="findConceptsWithInverseRoleMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithInverseRoleMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithInverseRoleMatching"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="roleType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TRoleType"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 Set nsIndex=$get(@(tree)@("ns","http://ws.dao.dtsserver.apelon.com/"))
 Set (node,ref)=nodeArg
 If ($listget($get(@(tree)@(node,0)),1)'="e")||(tag'=@(tree)@(node)) Goto XMLImportMalformed
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptsWithInverseRoleMatching",top,format,namespacePrefix,input,refOnly,.schema)
