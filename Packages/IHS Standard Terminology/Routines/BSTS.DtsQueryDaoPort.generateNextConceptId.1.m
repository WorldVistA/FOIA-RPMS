 ;BSTS.DtsQueryDaoPort.generateNextConceptId.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.generateNextConceptId.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;4A5A4164;BSTS.DtsQueryDaoPort.generateNextConceptId
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,idCount) public {
 Set ..namespaceId=$get(namespaceId),..idCount=$get(idCount)
 Do %Client.InvokeClient($this,"generateNextConceptId",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="generateNextConceptIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.generateNextConceptId",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateNextConceptId")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.generateNextConceptId")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.generateNextConceptId"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="generateNextConceptId"
   ...
   If tag="" Set tag=typesPrefix_"generateNextConceptId"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"generateNextConceptId"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.generateNextConceptId",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateNextConceptId")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="generateNextConceptIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.generateNextConceptId).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.generateNextConceptId",top,format,namespacePrefix,input,refOnly,.schema)
