 ;BSTS.DtsDaoPort.getAssociationNavChildContext.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getAssociationNavChildContext.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;6A586744;BSTS.DtsDaoPort.getAssociationNavChildContext
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..arg2)=1 Set r%arg2=r%arg2.%ConstructClone(1,.cloned),i%arg2=""
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%arg2 If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
...
exit	Quit sc }
zInvoke(%Client,%Action,focusConcept,attributeSetDescriptor,arg2) public {
 Set ..focusConcept=$get(focusConcept),..attributeSetDescriptor=$get(attributeSetDescriptor),..arg2=$get(arg2)
 Do %Client.InvokeClient($this,"getAssociationNavChildContext",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getAssociationNavChildContextResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getAssociationNavChildContext",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getAssociationNavChildContext")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getAssociationNavChildContext")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getAssociationNavChildContext"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getAssociationNavChildContext"
   ...
   If tag="" Set tag=typesPrefix_"getAssociationNavChildContext"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getAssociationNavChildContext"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..arg2
 If $IsObject(val) , deepFlag {
     Set topArg="arg2",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getAssociationNavChildContext",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getAssociationNavChildContext")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="getAssociationNavChildContextResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..arg2) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getAssociationNavChildContext).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getAssociationNavChildContext",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TNavChildContext).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
zarg2NewObject() public {
