 ;BSTS.ns1.TQualifierType.1
 ;(C)InterSystems, generated for class BSTS.ns1.TQualifierType.  Do NOT edit. 08/21/2017 07:46:00PM
 ;;73596348;BSTS.ns1.TQualifierType
 ;
%NormalizeObject() public {
	...
	If m%qualifies Set:i%qualifies'="" i%qualifies=(..qualifiesNormalize(i%qualifies))
...
%ValidateObject(force=0) public {
	...
	If m%qualifies Set iv=..qualifies If iv'="" Set rc=(..qualifiesIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"qualifies",iv)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TQualifierType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TQualifierType")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TQualifierType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TQualifierType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"TQualifierType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TQualifierType"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..qualifies
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.TQualifiesItemType","NAMESPACE")
   ...
   Set %xmlmsg=currentIndent_beginprefix_"qualifies"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TQualifiesItemType"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"qualifies>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TQualifierType",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TQualifierType")
 If encoded {
 ...
 If tag="qualifies" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             If data="" Set data=$c(0)
             If data'="" Goto:('$s(data'[","&&(",CA,TA,CP,TP,NP,VP,AP,SP,EP,"[(","_$select(data=$c(0):"",1:data)_",")):1,1:$$Error^%apiOBJ(7205,data,",CA,TA,CP,TP,NP,VP,AP,SP,EP"))) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..qualifies=data
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TQualifierType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TQualifierType",top,format,namespacePrefix,input,refOnly,.schema)
zqualifiesIsValid(%val) public {
