 ;BSTS.DtsQueryDaoPort.addConceptAssociation.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addConceptAssociation.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;664D4569;BSTS.DtsQueryDaoPort.addConceptAssociation
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..conceptAssociation)=1 Set r%conceptAssociation=r%conceptAssociation.%ConstructClone(1,.cloned),i%conceptAssociation=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%conceptAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,conceptAssociation) public {
 Set ..conceptAssociation=$get(conceptAssociation)
 Do %Client.InvokeClient($this,"addConceptAssociation",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addConceptAssociationResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addConceptAssociation",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addConceptAssociation")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addConceptAssociation")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addConceptAssociation"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addConceptAssociation"
   ...
   If tag="" Set tag=typesPrefix_"addConceptAssociation"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addConceptAssociation"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptAssociation
 If $IsObject(val) , deepFlag {
     Set topArg="conceptAssociation",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptAssociation"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addConceptAssociation",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addConceptAssociation")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="addConceptAssociationResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..conceptAssociation) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addConceptAssociation).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addConceptAssociation",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptAssociation).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
zconceptAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptAssociation).%New() If newobject="" Quit ""
	Set ..conceptAssociation=newobject
	Quit newobject }
