 ;BSTS.DtsQueryDaoPort.deleteUserContextProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteUserContextProperty.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;48695249;BSTS.DtsQueryDaoPort.deleteUserContextProperty
 ;
...
zInvoke(%Client,%Action,key) public {
 Set ..key=$get(key)
 Do %Client.InvokeClient($this,"deleteUserContextProperty",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteUserContextProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteUserContextProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteUserContextProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteUserContextProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteUserContextProperty"
   ...
   If tag="" Set tag=typesPrefix_"deleteUserContextProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteUserContextProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteUserContextProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteUserContextProperty")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteUserContextProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteUserContextProperty",top,format,namespacePrefix,input,refOnly,.schema)
