 ;BSTS.DtsQueryDaoPort.fetchAuthorityByName.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchAuthorityByName.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;75326637;BSTS.DtsQueryDaoPort.fetchAuthorityByName
 ;
...
%ValidateObject(force=0) public {
	...
	If m%authorityName Set iv=..authorityName If iv'="" Set rc=(..authorityNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"authorityName",iv)
	Quit sc }
zInvoke(%Client,%Action,authorityName,asd) public {
 Set ..authorityName=$get(authorityName),..asd=$get(asd)
 Do %Client.InvokeClient($this,"fetchAuthorityByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="fetchAuthorityByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchAuthorityByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchAuthorityByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchAuthorityByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchAuthorityByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchAuthorityByName"
   ...
   If tag="" Set tag=typesPrefix_"fetchAuthorityByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchAuthorityByName"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="asd",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchAuthorityByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchAuthorityByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="fetchAuthorityByNameResult" {
   ...
   If (class="") || (class="TAuthority") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TAuthority",85,"t",class))_$get(^oddXML("BSTS.ns1.TAuthority","t",class)) If class="" Set class=0
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchAuthorityByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchAuthorityByName",top,format,namespacePrefix,input,refOnly,.schema)
