 ;BSTS.DtsDaoPort.addVersion.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.addVersion.  Do NOT edit. 08/21/2017 07:46:09PM
 ;;514A7532;BSTS.DtsDaoPort.addVersion
 ;
...
zInvoke(%Client,%Action,arg0) public {
 Set ..arg0=$get(arg0)
 Do %Client.InvokeClient($this,"addVersion",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addVersionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.addVersion",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.addVersion")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.addVersion")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.addVersion"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"addVersion"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addVersion"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.addVersion",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.addVersion")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.addVersion).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.addVersion",top,format,namespacePrefix,input,refOnly,.schema)
