 ;BSTS.ns1.TSynonym.1
 ;(C)InterSystems, generated for class BSTS.ns1.TSynonym.  Do NOT edit. 08/21/2017 07:45:59PM
 ;;48326139;BSTS.ns1.TSynonym
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..concept)=1 Set r%concept=r%concept.%ConstructClone(1,.cloned),i%concept=""
		If $isobject(..term)=1 Set r%term=r%term.%ConstructClone(1,.cloned),i%term=""
	}
	Quit 1 }
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%localAddition Set:i%localAddition'="" i%localAddition=(..localAdditionNormalize(i%localAddition))
	If m%namespaceId Set:i%namespaceId'="" i%namespaceId=(..namespaceIdNormalize(i%namespaceId))
	If m%preferred Set:i%preferred'="" i%preferred=(..preferredNormalize(i%preferred))
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%concept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%term If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	Set iv=..localAddition If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"localAddition"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%localAddition Set rc=(..localAdditionIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"localAddition",iv)
	...
	Set iv=..preferred If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"preferred"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%preferred Set rc=(..preferredIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"preferred",iv)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TSynonym",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSynonym")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TSynonym")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TSynonym"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"TSynonym"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TSynonym"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TConceptItem","XMLSUMMARY")'="":-1,1:1)
     Set id=+$get(idlist(temp*val))
     If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
   }
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"concept "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="concept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TConceptItem"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..associationType
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"associationType "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="associationType",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..term
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TTermItem","XMLSUMMARY")'="":-1,1:1)
     Set id=+$get(idlist(temp*val))
     If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
   }
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"term "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="term",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TTermItem"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..preferred
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"preferred"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"preferred>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..localAddition
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"localAddition"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"localAddition>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..revisionIn
 If val'="" {
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TSynonym" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TSynonym",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSynonym")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="concept" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If (class="") || (class="TConceptItem") {
       Set class="BSTS.ns1.TConceptItem"
     } Else {
       If $length(class,":")=2 Set class=$piece(class,":",2)
       Set class=$get(^oddCOM("BSTS.ns1.TConceptItem",85,"s",class))_$get(^oddXML("BSTS.ns1.TConceptItem","s",class)) If class="" Set class=0
   ...
   If data'="" Set ..concept=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="associationType" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
   ...
   If data'="" Set ..associationType=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="term" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     ...
     Set class=$select(($get(@(tree)@(ref,"a","type","u"))=1):$get(@(tree)@(ref,"a","type")),1:"")
     If (class="") || (class="TTermItem") {
       Set class="BSTS.ns1.TTermItem"
     } Else {
       If $length(class,":")=2 Set class=$piece(class,":",2)
       Set class=$get(^oddCOM("BSTS.ns1.TTermItem",85,"s",class))_$get(^oddXML("BSTS.ns1.TTermItem","s",class)) If class="" Set class=0
       Set tmp=$select($get(@(tree)@(ref,"a","type","v"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","type","v"))))
       If (class=0)||(($listlength(class)>1)&&(tmp="")) Set class=$get(@(tree)@(ref,"a","type")) Goto XMLImportBadType
       If $listlength(class)>1 {
         For tmpi=1:1:$listlength(class) {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..concept) Quit 0
 If $IsObject(..associationType) Quit 0
 If $IsObject(..term) Quit 0
 If ..preferred'="" Quit 0
 If ..localAddition'="" Quit 0
 If ..revisionIn'="" Quit 0
 If ..revisionOut'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TSynonym).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TSynonym",top,format,namespacePrefix,input,refOnly,.schema)
zassociationTypeNewObject() public {
	...
	Quit newobject }
zconceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptItem).%New() If newobject="" Quit ""
	Set ..concept=newobject
	Quit newobject }
zlocalAdditionDisplayToLogical(%val) public {
	Quit ''%val }
zlocalAdditionIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zlocalAdditionLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zlocalAdditionNormalize(%val) public {
	Quit %val\1 }
zlocalAdditionXSDToLogical(%val) public {
...
zpreferredDisplayToLogical(%val) public {
	Quit ''%val }
zpreferredIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zpreferredLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zpreferredNormalize(%val) public {
	Quit %val\1 }
zpreferredXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
...
zrevisionOutXSDToLogical(%val)
 ...
 If $zdatetimeh(dt,3,1,,,,,,,"")="" Quit ""
 Quit dt
ztermNewObject() public {
	Set newobject=##class(BSTS.ns1.TTermItem).%New() If newobject="" Quit ""
	Set ..term=newobject
	Quit newobject }
