 ;BSTS.DtsDaoPort.findSubsetQualifierTypeById.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findSubsetQualifierTypeById.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;37677477;BSTS.DtsDaoPort.findSubsetQualifierTypeById
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,id,subsetId) public {
 Set ..id=$get(id),..subsetId=$get(subsetId)
 Do %Client.InvokeClient($this,"findSubsetQualifierTypeById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findSubsetQualifierTypeByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findSubsetQualifierTypeById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findSubsetQualifierTypeById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findSubsetQualifierTypeById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findSubsetQualifierTypeById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findSubsetQualifierTypeById"
   ...
   If tag="" Set tag=typesPrefix_"findSubsetQualifierTypeById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findSubsetQualifierTypeById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findSubsetQualifierTypeById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findSubsetQualifierTypeById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findSubsetQualifierTypeByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findSubsetQualifierTypeById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findSubsetQualifierTypeById",top,format,namespacePrefix,input,refOnly,.schema)
