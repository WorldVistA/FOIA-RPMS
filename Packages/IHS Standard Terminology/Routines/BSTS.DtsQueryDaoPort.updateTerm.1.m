 ;BSTS.DtsQueryDaoPort.updateTerm.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateTerm.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;52365164;BSTS.DtsQueryDaoPort.updateTerm
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newTerm)=1 Set r%newTerm=r%newTerm.%ConstructClone(1,.cloned),i%newTerm=""
		If $isobject(..oldTerm)=1 Set r%oldTerm=r%oldTerm.%ConstructClone(1,.cloned),i%oldTerm=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newTerm If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldTerm If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldTerm,newTerm) public {
 Set ..oldTerm=$get(oldTerm),..newTerm=$get(newTerm)
 Do %Client.InvokeClient($this,"updateTerm",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateTermResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateTerm",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateTerm")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateTerm")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateTerm"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"updateTerm"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateTerm"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="oldTerm",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TTerm"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newTerm
 If $IsObject(val) , deepFlag {
     Set topArg="newTerm",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TTerm"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateTerm",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateTerm")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldTerm) Quit 0
 If $IsObject(..newTerm) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateTerm).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateTerm",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TTerm).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewTermNewObject() public {
	Set newobject=##class(BSTS.ns1.TTerm).%New() If newobject="" Quit ""
	Set ..newTerm=newobject
	Quit newobject }
zoldTermNewObject() public {
	Set newobject=##class(BSTS.ns1.TTerm).%New() If newobject="" Quit ""
	Set ..oldTerm=newobject
	Quit newobject }
