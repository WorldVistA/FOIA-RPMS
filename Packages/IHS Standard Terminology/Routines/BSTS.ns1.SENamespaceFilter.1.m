 ;BSTS.ns1.SENamespaceFilter.1
 ;(C)InterSystems, generated for class BSTS.ns1.SENamespaceFilter.  Do NOT edit. 08/21/2017 07:46:01PM
 ;;362B4E71;BSTS.ns1.SENamespaceFilter
 ;
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%ALLNAMESPACE Set:i%ALLNAMESPACE'="" i%ALLNAMESPACE=(..ALLNAMESPACENormalize(i%ALLNAMESPACE))
...
%ValidateObject(force=0) public {
	...
	Set iv=..ALLNAMESPACE If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"ALLNAMESPACE"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%ALLNAMESPACE Set rc=(..ALLNAMESPACEIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"ALLNAMESPACE",iv)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.SENamespaceFilter",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SENamespaceFilter")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.SENamespaceFilter")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.SENamespaceFilter"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="SENamespaceFilter"
   ...
   If tag="" Set tag=typesPrefix_"SENamespaceFilter"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"SENamespaceFilter"""_xsiAttrs,xsiAttrs=""
 ...
 If 'encoded {
   Set val=..ALLNAMESPACE
   If val'="" {
     Set %xmlmsg=" "_attrsPrefix_"ALL_NAMESPACE="""_val_"""" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 ...
 If encoded {
   Set val=..ALLNAMESPACE
   If val'="" {
     Set %xmlmsg=currentIndent_beginprefix_"ALL_NAMESPACE"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"ALL_NAMESPACE>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.SENamespaceFilter" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.SENamespaceFilter",.imports,.classes)
zXMLImportAttributes()
 ;
 New data,ref
 If ($data(@(tree)@(node,"a","ALL_NAMESPACE"))) {
   Set ref="ALL_NAMESPACE",data=$get(@(tree)@(node,"a",ref))
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SENamespaceFilter")
 If encoded {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..ALLNAMESPACE'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.SENamespaceFilter).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.SENamespaceFilter",top,format,namespacePrefix,input,refOnly,.schema)
zALLNAMESPACEDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zALLNAMESPACEIsValid(%val) public {
...
zALLNAMESPACENormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zALLNAMESPACEXSDToLogical(%val) public {
