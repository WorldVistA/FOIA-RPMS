 ;BSTS.DtsQueryDaoPort.getConceptChildRoots.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getConceptChildRoots.  Do NOT edit. 08/21/2017 07:46:17PM
 ;;6A4C6D6B;BSTS.DtsQueryDaoPort.getConceptChildRoots
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,rootConceptASD,namespaceId) public {
 Set ..rootConceptASD=$get(rootConceptASD),..namespaceId=$get(namespaceId)
 Do %Client.InvokeClient($this,"getConceptChildRoots",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getConceptChildRoots",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getConceptChildRoots")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getConceptChildRoots")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getConceptChildRoots"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getConceptChildRoots"
   ...
   If tag="" Set tag=typesPrefix_"getConceptChildRoots"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getConceptChildRoots"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getConceptChildRoots",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getConceptChildRoots")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getConceptChildRoots).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getConceptChildRoots",top,format,namespacePrefix,input,refOnly,.schema)
