 ;BSTS.DtsQueryDaoPort.isSubConceptOfConcepts.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.isSubConceptOfConcepts.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;5A47634F;BSTS.DtsQueryDaoPort.isSubConceptOfConcepts
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..concept1)=1 Set r%concept1=r%concept1.%ConstructClone(1,.cloned),i%concept1=""
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%concept1 If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set key=""
	For  {
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,concept1,concepts) public {
 Set ..concept1=$get(concept1),..concepts=$get(concepts)
 Do %Client.InvokeClient($this,"isSubConceptOfConcepts",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.isSubConceptOfConcepts",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptOfConcepts")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.isSubConceptOfConcepts")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.isSubConceptOfConcepts"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isSubConceptOfConcepts"
   ...
   If tag="" Set tag=typesPrefix_"isSubConceptOfConcepts"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isSubConceptOfConcepts"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="concept1",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   While k'="" {
     If $IsObject(val) {
         Set topArg="concepts",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
     } Else {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.isSubConceptOfConcepts",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptOfConcepts")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..concept1) Quit 0
 If ..concepts.Count()>0 Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.isSubConceptOfConcepts).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.isSubConceptOfConcepts",top,format,namespacePrefix,input,refOnly,.schema)
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.isSubConceptOfConcepts).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.isSubConceptOfConcepts:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
...
zconceptsGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TOntylogConcept",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."concepts%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
