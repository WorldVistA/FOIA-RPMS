 ;BSTS.DtsQueryDaoPort.syncAuthorityItemId.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.syncAuthorityItemId.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;3231326F;BSTS.DtsQueryDaoPort.syncAuthorityItemId
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,authorityId,idKey,idValue) public {
 Set ..authorityId=$get(authorityId),..idKey=$get(idKey),..idValue=$get(idValue)
 Do %Client.InvokeClient($this,"syncAuthorityItemId",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.syncAuthorityItemId",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.syncAuthorityItemId")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.syncAuthorityItemId")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.syncAuthorityItemId"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="syncAuthorityItemId"
   ...
   If tag="" Set tag=typesPrefix_"syncAuthorityItemId"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"syncAuthorityItemId"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.syncAuthorityItemId",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.syncAuthorityItemId")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.syncAuthorityItemId).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.syncAuthorityItemId",top,format,namespacePrefix,input,refOnly,.schema)
