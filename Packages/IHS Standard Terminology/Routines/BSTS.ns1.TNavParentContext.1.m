 ;BSTS.ns1.TNavParentContext.1
 ;(C)InterSystems, generated for class BSTS.ns1.TNavParentContext.  Do NOT edit. 08/21/2017 07:45:59PM
 ;;53424362;BSTS.ns1.TNavParentContext
 ;
...
%Construct(initvalue) public {
	Set m%parents=0,M%parents=0
	Quit 1 }
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%parents="",r%parents=""
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..focus)=1 Set r%focus=r%focus.%ConstructClone(1,.cloned),i%focus=""
		Set key="" For  Set value=..parents.GetNext(.key) Quit:key=""  Set r%parents(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%parents(key)=""
	}
	Quit 1 }
%Destruct() public {
	Try {
		If $isobject($get(r%parents))=1,$zobjcnt(r%parents)>1 Do r%parents.%Disconnect()
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set key=""
	For  {
		Set key=$order(r%parents(key),1,Poref) Quit:key=""
		If $isobject(Poref)=1 Set:'$data(%objTX(1,+Poref)) %objTX(6,$i(%objTX(6)))=Poref
	}
exit	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TNavParentContext",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TNavParentContext")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TNavParentContext")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TNavParentContext"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TNavParentContext"
   ...
   If tag="" Set tag=typesPrefix_"TNavParentContext"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TNavParentContext"""_xsiAttrs,xsiAttrs=""
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     ...
     If soap12 { Set %xmlmsg=beginprefix_"parents"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.TConceptNode","XMLSUMMARY")'="":"s_TConceptNode",1:"TConceptNode")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"parents "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.TConceptNode","XMLSUMMARY")'="":"s_TConceptNode",1:"TConceptNode")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       ...
       If +id'=0 {
         Set %xmlmsg=currentIndent_beginprefix_"parents "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
       } Else { if id=0 Set id=$increment(idlist)
         Set topArg="parents",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TConceptNode"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
       }
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"parents "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
   ...
   If encoded {
     ...
     Set %xmlmsg=endprefix_"parents>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TNavParentContext" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TNavParentContext",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TNavParentContext")
 If encoded {
   ...
   If encoded,$$XMLImportId() {
     ...
     If encoded,(($data(@(tree)@(ref,"a","arrayType")))||(($get(@(tree)@(ref,"a","type","u"))=1)&&($piece($get(@(tree)@(ref,"a","type")),":",2)="Array"))||($select($get(@(tree)@(ref,"a","itemType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","itemType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","nodeType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","nodeType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","arraySize","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","arraySize","u"))))="http://www.w3.org/2003/05/soap-encoding")) {
       ...
       While element'="" {
         ...
         If data'="" Do ..parents.Insert(data)
         }
       ...
       Quit:('sc)  Set ref=loopref
         ...
         If $listget($get(@(tree)@(ref,0)),1)'="e" Goto XMLImportMalformedNoTag
         If 'encoded,(@(tree)@(ref)'="parents") Goto XMLImportBadTag
         If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
         If encoded,$$XMLImportId() {
           Set data=idlist(ref)
         } Else {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..parents.Count()>0 Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TNavParentContext).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TNavParentContext",top,format,namespacePrefix,input,refOnly,.schema)
zfocusNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..focus=newobject
	Quit newobject }
zparentsBuildValueArray(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zparentsCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
zparentsCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
	...
	quit $extract(odbc,2,*) }
zparentsDisplayToCollection(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zparentsGetObject(force=0) public {
	Quit $select(r%parents="":$select(i%parents="":"",1:$listbuild(i%parents_"")),(''..parents.%GetSwizzleObject(force,.oid)):oid,1:"") }
zparentsGetObjectId(force=0) public {
	Quit $listget(..parentsGetObject(force)) }
zparentsGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%parents=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.TConceptNode",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."parents%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zparentsOdbcToCollection(val="") public {
	set logical="",out=""
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zparentsSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%parents=newvalue Quit 1
	If newvalue="" Kill i%parents,r%parents Set i%parents="",r%parents="" Quit 1
	Set oref=r%parents Kill i%parents,r%parents Set i%parents="",r%parents=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%parents(i)=value,i%parents(i)=""
	Quit 1 }
