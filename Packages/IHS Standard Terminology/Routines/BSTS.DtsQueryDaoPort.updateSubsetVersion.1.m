 ;BSTS.DtsQueryDaoPort.updateSubsetVersion.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateSubsetVersion.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;73785734;BSTS.DtsQueryDaoPort.updateSubsetVersion
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newVersion
 s proporef=..oldVersion
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newVersion)=1 Set r%newVersion=r%newVersion.%ConstructClone(1,.cloned),i%newVersion=""
		If $isobject(..oldVersion)=1 Set r%oldVersion=r%oldVersion.%ConstructClone(1,.cloned),i%oldVersion=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newVersion If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldVersion If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldVersion,newVersion) public {
 Set ..oldVersion=$get(oldVersion),..newVersion=$get(newVersion)
 Do %Client.InvokeClient($this,"updateSubsetVersion",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateSubsetVersionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateSubsetVersion",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubsetVersion")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateSubsetVersion")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateSubsetVersion"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateSubsetVersion"
   ...
   If tag="" Set tag=typesPrefix_"updateSubsetVersion"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateSubsetVersion"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="oldVersion",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TContentVersion"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newVersion
 If $IsObject(val) , deepFlag {
     Set topArg="newVersion",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TContentVersion"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateSubsetVersion",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubsetVersion")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateSubsetVersionResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldVersion) Quit 0
 If $IsObject(..newVersion) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateSubsetVersion).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateSubsetVersion",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TContentVersion).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewVersionNewObject() public {
	Set newobject=##class(BSTS.ns1.TContentVersion).%New() If newobject="" Quit ""
	Set ..newVersion=newobject
	Quit newobject }
zoldVersionNewObject() public {
	Set newobject=##class(BSTS.ns1.TContentVersion).%New() If newobject="" Quit ""
	Set ..oldVersion=newobject
	Quit newobject }
