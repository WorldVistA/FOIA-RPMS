 ;BSTS.DtsQueryDaoPort.hasAuthorityPermission.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.hasAuthorityPermission.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;4A382B6F;BSTS.DtsQueryDaoPort.hasAuthorityPermission
 ;
...
%ValidateObject(force=0) public {
	...
	If m%permission Set iv=..permission If iv'="" Set rc=(..permissionIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"permission",iv)
	Quit sc }
zInvoke(%Client,%Action,authorityId,permission) public {
 Set ..authorityId=$get(authorityId),..permission=$get(permission)
 Do %Client.InvokeClient($this,"hasAuthorityPermission",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="hasAuthorityPermissionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.hasAuthorityPermission",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasAuthorityPermission")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.hasAuthorityPermission")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.hasAuthorityPermission"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="hasAuthorityPermission"
   ...
   If tag="" Set tag=typesPrefix_"hasAuthorityPermission"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"hasAuthorityPermission"""_xsiAttrs,xsiAttrs=""
 ...
 If val'="" {
   ...
   Set temp1=$select((temp1'="http://apelon.com/dtsserver/ws/dtsquery")&&$IsObject(namespaces):namespaces.GetNamespacePrefix(temp1,.temp),1:typesPrefix)
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.hasAuthorityPermission",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasAuthorityPermission")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="hasAuthorityPermissionResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.hasAuthorityPermission).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.hasAuthorityPermission",top,format,namespacePrefix,input,refOnly,.schema)
