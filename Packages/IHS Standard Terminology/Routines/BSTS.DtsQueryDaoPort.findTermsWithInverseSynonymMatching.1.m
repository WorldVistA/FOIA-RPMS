 ;BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;4A793266;BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching
 ;
...
zInvoke(%Client,%Action,synonymType,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findTermsWithInverseSynonymMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithInverseSynonymMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithInverseSynonymMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithInverseSynonymMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findTermsWithInverseSynonymMatching",top,format,namespacePrefix,input,refOnly,.schema)
