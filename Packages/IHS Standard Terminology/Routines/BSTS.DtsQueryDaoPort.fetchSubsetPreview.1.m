 ;BSTS.DtsQueryDaoPort.fetchSubsetPreview.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchSubsetPreview.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;3037525A;BSTS.DtsQueryDaoPort.fetchSubsetPreview
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,options) public {
 Set ..options=$get(options)
 Do %Client.InvokeClient($this,"fetchSubsetPreview",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchSubsetPreview",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetPreview")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchSubsetPreview")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchSubsetPreview"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchSubsetPreview"
   ...
   If tag="" Set tag=typesPrefix_"fetchSubsetPreview"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchSubsetPreview"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="options",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TPreviewOptions"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchSubsetPreview",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetPreview")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchSubsetPreview).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchSubsetPreview",top,format,namespacePrefix,input,refOnly,.schema)
...
zoptionsNewObject() public {
	Set newobject=##class(BSTS.ns1.TPreviewOptions).%New() If newobject="" Quit ""
