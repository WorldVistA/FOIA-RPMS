 ;BSTS.DtsQueryDaoPort.getSubsetVersion.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getSubsetVersion.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;36357238;BSTS.DtsQueryDaoPort.getSubsetVersion
 ;
...
zInvoke(%Client,%Action,subsetId) public {
 ...
 Do %Client.InvokeClient($this,"getSubsetVersion",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getSubsetVersionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getSubsetVersion",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetVersion")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getSubsetVersion")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getSubsetVersion"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getSubsetVersion"
   ...
   If tag="" Set tag=typesPrefix_"getSubsetVersion"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getSubsetVersion"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getSubsetVersion",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetVersion")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getSubsetVersion).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getSubsetVersion",top,format,namespacePrefix,input,refOnly,.schema)
