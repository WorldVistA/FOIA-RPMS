 ;BSTS.ns1.TVersionedContent.1
 ;(C)InterSystems, generated for class BSTS.ns1.TVersionedContent.  Do NOT edit. 08/21/2017 07:46:01PM
 ;;68514671;BSTS.ns1.TVersionedContent
 ;
...
%Construct(initvalue) public {
	Set m%contentVersions=0,M%contentVersions=0,m%properties=0,M%properties=0
	Quit 1 }
...
%Destruct() public {
	Try {
		If $isobject($get(r%contentVersions))=1,$zobjcnt(r%contentVersions)>1 Do r%contentVersions.%Disconnect()
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TVersionedContent",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TVersionedContent")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TVersionedContent")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TVersionedContent"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TVersionedContent"
   ...
   If tag="" Set tag=typesPrefix_"TVersionedContent"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TVersionedContent"""_xsiAttrs,xsiAttrs=""
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TVersionedContent" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TVersionedContent",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TVersionedContent")
 If encoded {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TVersionedContent).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TVersionedContent",top,format,namespacePrefix,input,refOnly,.schema)
zauthorityNewObject() public {
	Set newobject=##class(BSTS.ns1.TAuthority).%New() If newobject="" Quit ""
	Set ..authority=newobject
	Quit newobject }
zcontentVersionNewObject() public {
	Set newobject=##class(BSTS.ns1.TContentVersion).%New() If newobject="" Quit ""
	Set ..contentVersion=newobject
	Quit newobject }
zcontentVersionsBuildValueArray(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zcontentVersionsCollectionToDis(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
zcontentVersionsCollectionToOdb(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
	...
	quit $extract(odbc,2,*) }
zcontentVersionsDisplayToCollec(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zcontentVersionsGetObject(force=0) public {
	Quit $select(r%contentVersions="":$select(i%contentVersions="":"",1:$listbuild(i%contentVersions_"")),(''..contentVersions.%GetSwizzleObject(force,.oid)):oid,1:"") }
zcontentVersionsGetObjectId(force=0) public {
	Quit $listget(..contentVersionsGetObject(force)) }
zcontentVersionsGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%contentVersions=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.TContentVersion",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."contentVersions%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zcontentVersionsOdbcToCollectio(val="") public {
	set logical="",out=""
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zcontentVersionsSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%contentVersions=newvalue Quit 1
	If newvalue="" Kill i%contentVersions,r%contentVersions Set i%contentVersions="",r%contentVersions="" Quit 1
	Set oref=r%contentVersions Kill i%contentVersions,r%contentVersions Set i%contentVersions="",r%contentVersions=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%contentVersions(i)=value,i%contentVersions(i)=""
	Quit 1 }
zwritableDisplayToLogical(%val) public {
	Quit ''%val }
zwritableIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zwritableLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zwritableNormalize(%val) public {
	Quit %val\1 }
zwritableXSDToLogical(%val) public {
