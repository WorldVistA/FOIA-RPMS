 ;BSTS.DtsQueryDaoPort.addPropertyType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addPropertyType.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;386A4157;BSTS.DtsQueryDaoPort.addPropertyType
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,newPropertyType) public {
 Set ..newPropertyType=$get(newPropertyType)
 Do %Client.InvokeClient($this,"addPropertyType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addPropertyTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addPropertyType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addPropertyType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addPropertyType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addPropertyType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addPropertyType"
   ...
   If tag="" Set tag=typesPrefix_"addPropertyType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addPropertyType"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addPropertyType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addPropertyType")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addPropertyType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addPropertyType",top,format,namespacePrefix,input,refOnly,.schema)
