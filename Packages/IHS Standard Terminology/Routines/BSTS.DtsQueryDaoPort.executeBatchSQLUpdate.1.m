 ;BSTS.DtsQueryDaoPort.executeBatchSQLUpdate.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.executeBatchSQLUpdate.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;6B653769;BSTS.DtsQueryDaoPort.executeBatchSQLUpdate
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,sqlItems) public {
 Set ..sqlItems=$get(sqlItems)
 Do %Client.InvokeClient($this,"executeBatchSQLUpdate",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.executeBatchSQLUpdate",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeBatchSQLUpdate")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.executeBatchSQLUpdate")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.executeBatchSQLUpdate"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="executeBatchSQLUpdate"
   ...
   If tag="" Set tag=typesPrefix_"executeBatchSQLUpdate"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"executeBatchSQLUpdate"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.executeBatchSQLUpdate",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeBatchSQLUpdate")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.executeBatchSQLUpdate).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.executeBatchSQLUpdate",top,format,namespacePrefix,input,refOnly,.schema)
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.executeBatchSQLUpdate).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.executeBatchSQLUpdate:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
