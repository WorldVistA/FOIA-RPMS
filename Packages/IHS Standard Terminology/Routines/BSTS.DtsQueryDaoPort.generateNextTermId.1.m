 ;BSTS.DtsQueryDaoPort.generateNextTermId.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.generateNextTermId.  Do NOT edit. 08/21/2017 07:46:17PM
 ;;54647677;BSTS.DtsQueryDaoPort.generateNextTermId
 ;
...
zInvoke(%Client,%Action,namespaceId,idCount) public {
 ...
 Do %Client.InvokeClient($this,"generateNextTermId",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="generateNextTermIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.generateNextTermId",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateNextTermId")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.generateNextTermId")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.generateNextTermId"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="generateNextTermId"
   ...
   If tag="" Set tag=typesPrefix_"generateNextTermId"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"generateNextTermId"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.generateNextTermId",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateNextTermId")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="generateNextTermIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.generateNextTermId).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.generateNextTermId",top,format,namespacePrefix,input,refOnly,.schema)
