 ;BSTS.DtsQueryDaoPort.findSubset.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findSubset.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;6C773649;BSTS.DtsQueryDaoPort.findSubset
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..filter)=1 Set r%filter=r%filter.%ConstructClone(1,.cloned),i%filter=""
	}
	Quit 1 }
%Destruct() public {
	Try {
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%filter If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,filter,asd) public {
 Set ..filter=$get(filter),..asd=$get(asd)
 Do %Client.InvokeClient($this,"findSubset",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findSubset",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findSubset")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findSubset")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findSubset"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"findSubset"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findSubset"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="filter",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TDataTypeFilter"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findSubset",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findSubset")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..filter) Quit 0
 If $IsObject(..asd) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findSubset).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findSubset",top,format,namespacePrefix,input,refOnly,.schema)
...
zasdNewObject() public {
	...
	Set ..asd=newobject
	Quit newobject }
zfilterNewObject() public {
	Set newobject=##class(BSTS.ns1.TDataTypeFilter).%New() If newobject="" Quit ""
	Set ..filter=newobject
	Quit newobject }
