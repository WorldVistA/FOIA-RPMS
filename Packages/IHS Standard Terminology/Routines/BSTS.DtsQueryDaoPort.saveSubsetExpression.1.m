 ;BSTS.DtsQueryDaoPort.saveSubsetExpression.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.saveSubsetExpression.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;742B7852;BSTS.DtsQueryDaoPort.saveSubsetExpression
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,subsetId,expression) public {
 Set ..subsetId=$get(subsetId),..expression=$get(expression)
 Do %Client.InvokeClient($this,"saveSubsetExpression",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.saveSubsetExpression",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.saveSubsetExpression")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.saveSubsetExpression")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.saveSubsetExpression"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="saveSubsetExpression"
   ...
   If tag="" Set tag=typesPrefix_"saveSubsetExpression"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"saveSubsetExpression"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="expression",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TSubsetExpression"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.saveSubsetExpression",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.saveSubsetExpression")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.saveSubsetExpression).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.saveSubsetExpression",top,format,namespacePrefix,input,refOnly,.schema)
