 ;BSTS.DtsQueryDaoPort.getVersionByTime.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getVersionByTime.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;57756771;BSTS.DtsQueryDaoPort.getVersionByTime
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,time) public {
 Set ..namespaceId=$get(namespaceId),..time=$get(time)
 Do %Client.InvokeClient($this,"getVersionByTime",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getVersionByTimeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getVersionByTime",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getVersionByTime")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getVersionByTime")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getVersionByTime"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getVersionByTime"
   ...
   If tag="" Set tag=typesPrefix_"getVersionByTime"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getVersionByTime"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getVersionByTime",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getVersionByTime")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getVersionByTime).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getVersionByTime",top,format,namespacePrefix,input,refOnly,.schema)
