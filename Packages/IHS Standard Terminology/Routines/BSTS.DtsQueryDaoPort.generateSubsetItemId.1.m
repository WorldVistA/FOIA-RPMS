 ;BSTS.DtsQueryDaoPort.generateSubsetItemId.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.generateSubsetItemId.  Do NOT edit. 08/21/2017 07:46:17PM
 ;;36654A33;BSTS.DtsQueryDaoPort.generateSubsetItemId
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,subsetId,idKey,idCount) public {
 Set ..subsetId=$get(subsetId),..idKey=$get(idKey),..idCount=$get(idCount)
 Do %Client.InvokeClient($this,"generateSubsetItemId",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="generateSubsetItemIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.generateSubsetItemId",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateSubsetItemId")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.generateSubsetItemId")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.generateSubsetItemId"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="generateSubsetItemId"
   ...
   If tag="" Set tag=typesPrefix_"generateSubsetItemId"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"generateSubsetItemId"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.generateSubsetItemId",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateSubsetItemId")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="generateSubsetItemIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.generateSubsetItemId).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.generateSubsetItemId",top,format,namespacePrefix,input,refOnly,.schema)
