 ;BSTS.ns1.TPropertyType.1
 ;(C)InterSystems, generated for class BSTS.ns1.TPropertyType.  Do NOT edit. 08/21/2017 07:46:00PM
 ;;78437078;BSTS.ns1.TPropertyType
 ;
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TPropertyType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TPropertyType")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TPropertyType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TPropertyType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"TPropertyType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TPropertyType"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..attachesTo
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.TModifiesItemType","NAMESPACE")
   ...
   Set %xmlmsg=currentIndent_beginprefix_"attachesTo"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TModifiesItemType"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"attachesTo>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TPropertyType",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TPropertyType")
 If encoded {
 ...
 If tag="attachesTo" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             If data="" Set data=$c(0)
             If data'="" Goto:('$s(data'[","&&(",C,T,N,V,A,S,E,"[(","_$select(data=$c(0):"",1:data)_",")):1,1:$$Error^%apiOBJ(7205,data,",C,T,N,V,A,S,E"))) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..attachesTo=data
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TPropertyType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TPropertyType",top,format,namespacePrefix,input,refOnly,.schema)
