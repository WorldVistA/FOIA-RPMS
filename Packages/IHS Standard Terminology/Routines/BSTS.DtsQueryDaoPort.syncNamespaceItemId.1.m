 ;BSTS.DtsQueryDaoPort.syncNamespaceItemId.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.syncNamespaceItemId.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;4567676B;BSTS.DtsQueryDaoPort.syncNamespaceItemId
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,idKey,idValue) public {
 Set ..namespaceId=$get(namespaceId),..idKey=$get(idKey),..idValue=$get(idValue)
 Do %Client.InvokeClient($this,"syncNamespaceItemId",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.syncNamespaceItemId",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.syncNamespaceItemId")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.syncNamespaceItemId")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.syncNamespaceItemId"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="syncNamespaceItemId"
   ...
   If tag="" Set tag=typesPrefix_"syncNamespaceItemId"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"syncNamespaceItemId"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.syncNamespaceItemId",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.syncNamespaceItemId")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.syncNamespaceItemId).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.syncNamespaceItemId",top,format,namespacePrefix,input,refOnly,.schema)
