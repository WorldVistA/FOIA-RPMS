 ;BSTS.ns1.TSubsetPermissionItem.1
 ;(C)InterSystems, generated for class BSTS.ns1.TSubsetPermissionItem.  Do NOT edit. 08/21/2017 07:46:01PM
 ;;4465316C;BSTS.ns1.TSubsetPermissionItem
 ;
...
%ValidateObject(force=0) public {
	...
	Set iv=..subsetName If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"subsetName"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%subsetName Set rc=(..subsetNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"subsetName",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TSubsetPermissionItem",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSubsetPermissionItem")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TSubsetPermissionItem")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TSubsetPermissionItem"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TSubsetPermissionItem"
   ...
   If tag="" Set tag=typesPrefix_"TSubsetPermissionItem"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TSubsetPermissionItem"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..permission
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.TSubsetPermission","NAMESPACE")
   ...
   Set %xmlmsg=currentIndent_beginprefix_"permission"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TSubsetPermission"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"permission>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TSubsetPermissionItem",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSubsetPermissionItem")
 If encoded {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TSubsetPermissionItem).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TSubsetPermissionItem",top,format,namespacePrefix,input,refOnly,.schema)
