 ;BSTS.DtsDaoPort.findTermsByName.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findTermsByName.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;517A5856;BSTS.DtsDaoPort.findTermsByName
 ;
...
%NormalizeObject() public {
	...
	If m%termName Set:i%termName'="" i%termName=(..termNameNormalize(i%termName))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%termName Set iv=..termName If iv'="" Set rc=(..termNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"termName",iv)
	Quit sc }
zInvoke(%Client,%Action,termName,namespaceId,attributeSetDescriptor) public {
 Set ..termName=$get(termName),..namespaceId=$get(namespaceId),..attributeSetDescriptor=$get(attributeSetDescriptor)
 Do %Client.InvokeClient($this,"findTermsByName",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findTermsByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findTermsByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findTermsByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsByName"
   ...
   If tag="" Set tag=typesPrefix_"findTermsByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsByName"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..termName
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"termName"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"termName>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..namespaceId
 If val'="" {
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="attributeSetDescriptor",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TTermAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findTermsByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsByName")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findTermsByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findTermsByName",top,format,namespacePrefix,input,refOnly,.schema)
