 ;BSTS.DtsQueryDaoPort.generateNamespaceItemId.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.generateNamespaceItemId.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;7576566E;BSTS.DtsQueryDaoPort.generateNamespaceItemId
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,idKey,idCount) public {
 Set ..namespaceId=$get(namespaceId),..idKey=$get(idKey),..idCount=$get(idCount)
 Do %Client.InvokeClient($this,"generateNamespaceItemId",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="generateNamespaceItemIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.generateNamespaceItemId",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateNamespaceItemId")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.generateNamespaceItemId")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.generateNamespaceItemId"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="generateNamespaceItemId"
   ...
   If tag="" Set tag=typesPrefix_"generateNamespaceItemId"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"generateNamespaceItemId"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.generateNamespaceItemId",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.generateNamespaceItemId")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="generateNamespaceItemIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.generateNamespaceItemId).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.generateNamespaceItemId",top,format,namespacePrefix,input,refOnly,.schema)
...
zidCountDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zidCountIsValid(%val) public {
...
zidCountNormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zidCountXSDToLogical(%val) public {
