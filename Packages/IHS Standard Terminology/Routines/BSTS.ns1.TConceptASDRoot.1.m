 ;BSTS.ns1.TConceptASDRoot.1
 ;(C)InterSystems, generated for class BSTS.ns1.TConceptASDRoot.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;69557655;BSTS.ns1.TConceptASDRoot
 ;
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TConceptASDRoot",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TConceptASDRoot")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TConceptASDRoot")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TConceptASDRoot"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TConceptASDRoot"
   ...
   If tag="" Set tag=typesPrefix_"TConceptASDRoot"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TConceptASDRoot"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     ...
     Set topArg="item",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TConceptAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TConceptASDRoot" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TConceptASDRoot",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TConceptASDRoot")
 If encoded {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TConceptASDRoot).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TConceptASDRoot",top,format,namespacePrefix,input,refOnly,.schema)
