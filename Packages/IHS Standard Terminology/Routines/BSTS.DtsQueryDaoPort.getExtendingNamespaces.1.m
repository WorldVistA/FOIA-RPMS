 ;BSTS.DtsQueryDaoPort.getExtendingNamespaces.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getExtendingNamespaces.  Do NOT edit. 08/21/2017 07:46:17PM
 ;;37456A4B;BSTS.DtsQueryDaoPort.getExtendingNamespaces
 ;
...
%NormalizeObject() public {
	...
	If m%subsNamespaceId Set:i%subsNamespaceId'="" i%subsNamespaceId=(..subsNamespaceIdNormalize(i%subsNamespaceId))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%subsNamespaceId Set iv=..subsNamespaceId If iv'="" Set rc=(..subsNamespaceIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"subsNamespaceId",iv)
	Quit sc }
zInvoke(%Client,%Action,subsNamespaceId) public {
 Set ..subsNamespaceId=$get(subsNamespaceId)
 Do %Client.InvokeClient($this,"getExtendingNamespaces",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getExtendingNamespaces",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getExtendingNamespaces")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getExtendingNamespaces")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getExtendingNamespaces"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getExtendingNamespaces"
   ...
   If tag="" Set tag=typesPrefix_"getExtendingNamespaces"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getExtendingNamespaces"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..subsNamespaceId
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"subsNamespaceId"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"subsNamespaceId>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getExtendingNamespaces",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getExtendingNamespaces")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..subsNamespaceId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getExtendingNamespaces).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getExtendingNamespaces",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TNamespace",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
...
%ResultSet(newvalue) public {
	...
	Quit 1 }
zsubsNamespaceIdDisplayToLogica(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zsubsNamespaceIdIsValid(%val) public {
...
zsubsNamespaceIdNormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zsubsNamespaceIdXSDToLogical(%val) public {
