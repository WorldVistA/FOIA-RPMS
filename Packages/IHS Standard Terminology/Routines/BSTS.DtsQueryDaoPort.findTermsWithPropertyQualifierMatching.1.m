 ;BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;72324E56;BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching
 ;
...
zInvoke(%Client,%Action,propType,qualType,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findTermsWithPropertyQualifierMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithPropertyQualifierMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithPropertyQualifierMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithPropertyQualifierMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findTermsWithPropertyQualifierMatching",top,format,namespacePrefix,input,refOnly,.schema)
