 ;BSTS.DtsQueryDaoPort.updateSubset.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateSubset.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;3952614E;BSTS.DtsQueryDaoPort.updateSubset
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newSubset
 s proporef=..oldSubset
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newSubset)=1 Set r%newSubset=r%newSubset.%ConstructClone(1,.cloned),i%newSubset=""
		If $isobject(..oldSubset)=1 Set r%oldSubset=r%oldSubset.%ConstructClone(1,.cloned),i%oldSubset=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newSubset If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldSubset If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldSubset,newSubset) public {
 Set ..oldSubset=$get(oldSubset),..newSubset=$get(newSubset)
 Do %Client.InvokeClient($this,"updateSubset",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateSubsetResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateSubset",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubset")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateSubset")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateSubset"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"updateSubset"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateSubset"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="oldSubset",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TSubset"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newSubset
 If $IsObject(val) , deepFlag {
     Set topArg="newSubset",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TSubset"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateSubset",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubset")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldSubset) Quit 0
 If $IsObject(..newSubset) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateSubset).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateSubset",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TSubset).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewSubsetNewObject() public {
	Set newobject=##class(BSTS.ns1.TSubset).%New() If newobject="" Quit ""
	Set ..newSubset=newobject
	Quit newobject }
zoldSubsetNewObject() public {
	Set newobject=##class(BSTS.ns1.TSubset).%New() If newobject="" Quit ""
	Set ..oldSubset=newobject
	Quit newobject }
