 ;BSTS.DtsQueryDaoPort.isSubConceptByCode.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.isSubConceptByCode.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;30356B4C;BSTS.DtsQueryDaoPort.isSubConceptByCode
 ;
...
%NormalizeObject() public {
	...
	If m%concept2 Set:i%concept2'="" i%concept2=(..concept2Normalize(i%concept2))
	If m%conceptCode1 Set:i%conceptCode1'="" i%conceptCode1=(..conceptCode1Normalize(i%conceptCode1))
...
%ValidateObject(force=0) public {
	...
	If m%concept2 Set iv=..concept2 If iv'="" Set rc=(..concept2IsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"concept2",iv)
	If m%conceptCode1 Set iv=..conceptCode1 If iv'="" Set rc=(..conceptCode1IsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptCode1",iv)
	...
	Quit sc }
zInvoke(%Client,%Action,conceptCode1,concept2,namespaceId) public {
 Set ..conceptCode1=$get(conceptCode1),..concept2=$get(concept2),..namespaceId=$get(namespaceId)
 Do %Client.InvokeClient($this,"isSubConceptByCode",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="isSubConceptByCodeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.isSubConceptByCode",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptByCode")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.isSubConceptByCode")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.isSubConceptByCode"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isSubConceptByCode"
   ...
   If tag="" Set tag=typesPrefix_"isSubConceptByCode"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isSubConceptByCode"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptCode1
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptCode1"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"conceptCode1>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..concept2
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"concept2"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"concept2>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..namespaceId
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.isSubConceptByCode",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptByCode")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="isSubConceptByCodeResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If ..conceptCode1'="" Quit 0
 If ..concept2'="" Quit 0
 If ..namespaceId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.isSubConceptByCode).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.isSubConceptByCode",top,format,namespacePrefix,input,refOnly,.schema)
