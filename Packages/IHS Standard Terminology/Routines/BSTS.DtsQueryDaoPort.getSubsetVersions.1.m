 ;BSTS.DtsQueryDaoPort.getSubsetVersions.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getSubsetVersions.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;6E753469;BSTS.DtsQueryDaoPort.getSubsetVersions
 ;
...
zInvoke(%Client,%Action,subsetId) public {
 ...
 Do %Client.InvokeClient($this,"getSubsetVersions",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getSubsetVersions",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetVersions")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getSubsetVersions")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getSubsetVersions"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getSubsetVersions"
   ...
   If tag="" Set tag=typesPrefix_"getSubsetVersions"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getSubsetVersions"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getSubsetVersions",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetVersions")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getSubsetVersions).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getSubsetVersions",top,format,namespacePrefix,input,refOnly,.schema)
