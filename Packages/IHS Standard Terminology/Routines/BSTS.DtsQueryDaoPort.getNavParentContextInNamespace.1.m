 ;BSTS.DtsQueryDaoPort.getNavParentContextInNamespace.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getNavParentContextInNamespace.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;574D6B39;BSTS.DtsQueryDaoPort.getNavParentContextInNamespace
 ;
...
zInvoke(%Client,%Action,focusConcept,focusConceptASD,namespace) public {
 ...
 Do %Client.InvokeClient($this,"getNavParentContextInNamespace",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getNavParentContextInNamespaceResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getNavParentContextInNamespace",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getNavParentContextInNamespace")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getNavParentContextInNamespace")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getNavParentContextInNamespace"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getNavParentContextInNamespace"
   ...
   If tag="" Set tag=typesPrefix_"getNavParentContextInNamespace"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getNavParentContextInNamespace"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getNavParentContextInNamespace",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getNavParentContextInNamespace")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="getNavParentContextInNamespaceResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getNavParentContextInNamespace).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getNavParentContextInNamespace",top,format,namespacePrefix,input,refOnly,.schema)
