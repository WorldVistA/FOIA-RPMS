 ;BSTS.DtsDaoPort.updateNamespaceProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.updateNamespaceProperty.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;69484D36;BSTS.DtsDaoPort.updateNamespaceProperty
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..namespaceId)=1 Set r%namespaceId=r%namespaceId.%ConstructClone(1,.cloned),i%namespaceId=""
		If $isobject(..newProp)=1 Set r%newProp=r%newProp.%ConstructClone(1,.cloned),i%newProp=""
		If $isobject(..oldProp)=1 Set r%oldProp=r%oldProp.%ConstructClone(1,.cloned),i%oldProp=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%namespaceId If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%newProp If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldProp If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,namespaceId,oldProp,newProp) public {
 Set ..namespaceId=$get(namespaceId),..oldProp=$get(oldProp),..newProp=$get(newProp)
 Do %Client.InvokeClient($this,"updateNamespaceProperty",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateNamespacePropertyResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.updateNamespaceProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateNamespaceProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.updateNamespaceProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.updateNamespaceProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateNamespaceProperty"
   ...
   If tag="" Set tag=typesPrefix_"updateNamespaceProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateNamespaceProperty"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="namespaceId",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TNamespace"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..oldProp
 If $IsObject(val) , deepFlag {
     Set topArg="oldProp",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TProperty"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newProp
 If $IsObject(val) , deepFlag {
     Set topArg="newProp",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TProperty"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.updateNamespaceProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateNamespaceProperty")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateNamespacePropertyResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..namespaceId) Quit 0
 If $IsObject(..oldProp) Quit 0
 If $IsObject(..newProp) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.updateNamespaceProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.updateNamespaceProperty",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TProperty).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znamespaceIdNewObject() public {
	Set newobject=##class(BSTS.ns1.TNamespace).%New() If newobject="" Quit ""
	Set ..namespaceId=newobject
	Quit newobject }
znewPropNewObject() public {
	Set newobject=##class(BSTS.ns1.TProperty).%New() If newobject="" Quit ""
	Set ..newProp=newobject
	Quit newobject }
zoldPropNewObject() public {
	Set newobject=##class(BSTS.ns1.TProperty).%New() If newobject="" Quit ""
	Set ..oldProp=newobject
	Quit newobject }
