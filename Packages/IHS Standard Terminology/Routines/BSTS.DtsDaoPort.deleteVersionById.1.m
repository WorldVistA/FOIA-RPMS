 ;BSTS.DtsDaoPort.deleteVersionById.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.deleteVersionById.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;354C6B46;BSTS.DtsDaoPort.deleteVersionById
 ;
...
zInvoke(%Client,%Action,arg0,arg1) public {
 ...
 Do %Client.InvokeClient($this,"deleteVersionById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteVersionByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.deleteVersionById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteVersionById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.deleteVersionById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.deleteVersionById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteVersionById"
   ...
   If tag="" Set tag=typesPrefix_"deleteVersionById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteVersionById"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..arg0
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"arg0"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"arg0>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..arg1
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.deleteVersionById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteVersionById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteVersionByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.deleteVersionById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.deleteVersionById",top,format,namespacePrefix,input,refOnly,.schema)
...
zarg0DisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zarg0IsValid(%val) public {
...
zarg0Normalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zarg0XSDToLogical(%val) public {
