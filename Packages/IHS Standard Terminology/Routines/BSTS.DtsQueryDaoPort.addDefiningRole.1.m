 ;BSTS.DtsQueryDaoPort.addDefiningRole.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addDefiningRole.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;6A734B6A;BSTS.DtsQueryDaoPort.addDefiningRole
 ;
...
zInvoke(%Client,%Action,concept,role) public {
 ...
 Do %Client.InvokeClient($this,"addDefiningRole",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addDefiningRoleResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addDefiningRole",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addDefiningRole")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addDefiningRole")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addDefiningRole"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addDefiningRole"
   ...
   If tag="" Set tag=typesPrefix_"addDefiningRole"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addDefiningRole"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addDefiningRole",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addDefiningRole")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addDefiningRole).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addDefiningRole",top,format,namespacePrefix,input,refOnly,.schema)
