 ;BSTS.DtsQueryDaoPort.updateTermAssociation.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateTermAssociation.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;4A50725A;BSTS.DtsQueryDaoPort.updateTermAssociation
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newTermAssociation
 s proporef=..oldTermAssociation
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newTermAssociation)=1 Set r%newTermAssociation=r%newTermAssociation.%ConstructClone(1,.cloned),i%newTermAssociation=""
		If $isobject(..oldTermAssociation)=1 Set r%oldTermAssociation=r%oldTermAssociation.%ConstructClone(1,.cloned),i%oldTermAssociation=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newTermAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldTermAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldTermAssociation,newTermAssociation) public {
 Set ..oldTermAssociation=$get(oldTermAssociation),..newTermAssociation=$get(newTermAssociation)
 Do %Client.InvokeClient($this,"updateTermAssociation",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateTermAssociationResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateTermAssociation",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateTermAssociation")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateTermAssociation")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateTermAssociation"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateTermAssociation"
   ...
   If tag="" Set tag=typesPrefix_"updateTermAssociation"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateTermAssociation"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldTermAssociation
 If $IsObject(val) , deepFlag {
     Set topArg="oldTermAssociation",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TTermAssociation"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newTermAssociation
 If $IsObject(val) , deepFlag {
     Set topArg="newTermAssociation",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TTermAssociation"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateTermAssociation",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateTermAssociation")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateTermAssociationResult" {
   ...
   If (class="") || (class="TTermAssociation") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TTermAssociation",85,"t",class))_$get(^oddXML("BSTS.ns1.TTermAssociation","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldTermAssociation) Quit 0
 If $IsObject(..newTermAssociation) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateTermAssociation).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateTermAssociation",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TTermAssociation).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewTermAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TTermAssociation).%New() If newobject="" Quit ""
	Set ..newTermAssociation=newobject
	Quit newobject }
zoldTermAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TTermAssociation).%New() If newobject="" Quit ""
	Set ..oldTermAssociation=newobject
	Quit newobject }
