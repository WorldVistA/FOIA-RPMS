 ;BSTS.DtsDaoPort.findConceptsWithRoleMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptsWithRoleMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;304D652F;BSTS.DtsDaoPort.findConceptsWithRoleMatching
 ;
...
zInvoke(%Client,%Action,roleType,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findConceptsWithRoleMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptsWithRoleMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithRoleMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptsWithRoleMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptsWithRoleMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithRoleMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithRoleMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithRoleMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptsWithRoleMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithRoleMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptsWithRoleMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptsWithRoleMatching",top,format,namespacePrefix,input,refOnly,.schema)
