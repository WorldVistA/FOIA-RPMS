 ;BSTS.ns1.TAssociationTypeList.1
 ;(C)InterSystems, generated for class BSTS.ns1.TAssociationTypeList.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;6A776E4B;BSTS.ns1.TAssociationTypeList
 ;
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TAssociationTypeList",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TAssociationTypeList")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TAssociationTypeList")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TAssociationTypeList"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TAssociationTypeList"
   ...
   If tag="" Set tag=typesPrefix_"TAssociationTypeList"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TAssociationTypeList"""_xsiAttrs,xsiAttrs=""
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     ...
     If soap12 { Set %xmlmsg=beginprefix_"types"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.TAssociationType","XMLSUMMARY")'="":"s_TAssociationType",1:"TAssociationType")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"types "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.TAssociationType","XMLSUMMARY")'="":"s_TAssociationType",1:"TAssociationType")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       ...
       If +id'=0 {
         ...
         Set topArg="types",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TAssociationTypeList" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TAssociationTypeList",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TAssociationTypeList")
 If encoded {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TAssociationTypeList).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TAssociationTypeList",top,format,namespacePrefix,input,refOnly,.schema)
...
ztypesGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TAssociationType",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."types%i"(),oref.OrefStorage=oref.Storage+1
