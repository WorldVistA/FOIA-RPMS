 ;BSTS.DtsDaoPort.updateAuthority.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.updateAuthority.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;2B71374E;BSTS.DtsDaoPort.updateAuthority
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..arg0)=1 Set r%arg0=r%arg0.%ConstructClone(1,.cloned),i%arg0=""
		If $isobject(..arg1)=1 Set r%arg1=r%arg1.%ConstructClone(1,.cloned),i%arg1=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%arg0 If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%arg1 If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.updateAuthority",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateAuthority")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.updateAuthority")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.updateAuthority"))
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="arg0",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAuthority"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..arg1
 If $IsObject(val) , deepFlag {
     Set topArg="arg1",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAuthority"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.updateAuthority",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateAuthority")
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If $IsObject(..arg0) Quit 0
 If $IsObject(..arg1) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.updateAuthority).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.updateAuthority",top,format,namespacePrefix,input,refOnly,.schema)
