 ;BSTS.DtsQueryDaoPort.findRoleTypeByName.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findRoleTypeByName.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;3445686F;BSTS.DtsQueryDaoPort.findRoleTypeByName
 ;
...
zInvoke(%Client,%Action,name,namespaceId) public {
 ...
 Do %Client.InvokeClient($this,"findRoleTypeByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findRoleTypeByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findRoleTypeByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findRoleTypeByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findRoleTypeByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findRoleTypeByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findRoleTypeByName"
   ...
   If tag="" Set tag=typesPrefix_"findRoleTypeByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findRoleTypeByName"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findRoleTypeByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findRoleTypeByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findRoleTypeByNameResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findRoleTypeByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findRoleTypeByName",top,format,namespacePrefix,input,refOnly,.schema)
