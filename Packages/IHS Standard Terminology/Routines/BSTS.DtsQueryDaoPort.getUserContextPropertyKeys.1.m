 ;BSTS.DtsQueryDaoPort.getUserContextPropertyKeys.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getUserContextPropertyKeys.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;66684D2F;BSTS.DtsQueryDaoPort.getUserContextPropertyKeys
 ;
...
zInvoke(%Client,%Action) public {
 Do %Client.InvokeClient($this,"getUserContextPropertyKeys",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getUserContextPropertyKeys",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getUserContextPropertyKeys")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getUserContextPropertyKeys")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getUserContextPropertyKeys"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getUserContextPropertyKeys"
   ...
   If tag="" Set tag=typesPrefix_"getUserContextPropertyKeys"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getUserContextPropertyKeys"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getUserContextPropertyKeys",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getUserContextPropertyKeys")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getUserContextPropertyKeys).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getUserContextPropertyKeys",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	...
	for i=1:1:$listlength(val) {
		set item=##class(BSTS.DtsQueryDaoPort.getUserContextPropertyKeys).%ResultLogicalToDisplay($listget(val,i))
...
%ResultCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=##class(BSTS.DtsQueryDaoPort.getUserContextPropertyKeys).%ResultLogicalToOdbc($listget(val,i))
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.getUserContextPropertyKeys).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.getUserContextPropertyKeys:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
