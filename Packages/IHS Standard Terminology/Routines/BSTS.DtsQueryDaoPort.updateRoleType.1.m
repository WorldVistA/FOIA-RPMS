 ;BSTS.DtsQueryDaoPort.updateRoleType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateRoleType.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;57597A63;BSTS.DtsQueryDaoPort.updateRoleType
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..newRoleType
 s proporef=..oldRoleType
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..newRoleType)=1 Set r%newRoleType=r%newRoleType.%ConstructClone(1,.cloned),i%newRoleType=""
		If $isobject(..oldRoleType)=1 Set r%oldRoleType=r%oldRoleType.%ConstructClone(1,.cloned),i%oldRoleType=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newRoleType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldRoleType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,oldRoleType,newRoleType) public {
 Set ..oldRoleType=$get(oldRoleType),..newRoleType=$get(newRoleType)
 Do %Client.InvokeClient($this,"updateRoleType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateRoleTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateRoleType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateRoleType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateRoleType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateRoleType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"updateRoleType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateRoleType"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="oldRoleType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TRoleType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newRoleType
 If $IsObject(val) , deepFlag {
     Set topArg="newRoleType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TRoleType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateRoleType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateRoleType")
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If $IsObject(..oldRoleType) Quit 0
 If $IsObject(..newRoleType) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateRoleType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateRoleType",top,format,namespacePrefix,input,refOnly,.schema)
...
znewRoleTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TRoleType).%New() If newobject="" Quit ""
	Set ..newRoleType=newobject
	Quit newobject }
zoldRoleTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TRoleType).%New() If newobject="" Quit ""
	Set ..oldRoleType=newobject
	Quit newobject }
