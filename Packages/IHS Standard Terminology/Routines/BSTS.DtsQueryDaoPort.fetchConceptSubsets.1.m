 ;BSTS.DtsQueryDaoPort.fetchConceptSubsets.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchConceptSubsets.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;3354714B;BSTS.DtsQueryDaoPort.fetchConceptSubsets
 ;
...
%NormalizeObject() public {
	...
	If m%conceptNamespaceId Set:i%conceptNamespaceId'="" i%conceptNamespaceId=(..conceptNamespaceIdNormalize(i%conceptNamespaceId))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%conceptNamespaceId Set iv=..conceptNamespaceId If iv'="" Set rc=(..conceptNamespaceIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptNamespaceId",iv)
	Quit sc }
zInvoke(%Client,%Action,conceptId,conceptNamespaceId) public {
 Set ..conceptId=$get(conceptId),..conceptNamespaceId=$get(conceptNamespaceId)
 Do %Client.InvokeClient($this,"fetchConceptSubsets",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchConceptSubsets",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchConceptSubsets")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchConceptSubsets")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchConceptSubsets"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchConceptSubsets"
   ...
   If tag="" Set tag=typesPrefix_"fetchConceptSubsets"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchConceptSubsets"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptNamespaceId
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptNamespaceId"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"conceptNamespaceId>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchConceptSubsets",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchConceptSubsets")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="return" {
   ...
   If (class="") || (class="TSubset") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TSubset",85,"t",class))_$get(^oddXML("BSTS.ns1.TSubset","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..conceptId'="" Quit 0
 If ..conceptNamespaceId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchConceptSubsets).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchConceptSubsets",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TSubset",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
