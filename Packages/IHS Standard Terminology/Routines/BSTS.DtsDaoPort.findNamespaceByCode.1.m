 ;BSTS.DtsDaoPort.findNamespaceByCode.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findNamespaceByCode.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;77666977;BSTS.DtsDaoPort.findNamespaceByCode
 ;
...
%NormalizeObject() public {
	...
	If m%namespaceCode Set:i%namespaceCode'="" i%namespaceCode=(..namespaceCodeNormalize(i%namespaceCode))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%namespaceCode Set iv=..namespaceCode If iv'="" Set rc=(..namespaceCodeIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"namespaceCode",iv)
	Quit sc }
zInvoke(%Client,%Action,namespaceCode,attributeSetDescriptor) public {
 Set ..namespaceCode=$get(namespaceCode),..attributeSetDescriptor=$get(attributeSetDescriptor)
 Do %Client.InvokeClient($this,"findNamespaceByCode",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findNamespaceByCodeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findNamespaceByCode",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findNamespaceByCode")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findNamespaceByCode")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findNamespaceByCode"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findNamespaceByCode"
   ...
   If tag="" Set tag=typesPrefix_"findNamespaceByCode"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findNamespaceByCode"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..namespaceCode
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"namespaceCode"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"namespaceCode>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..attributeSetDescriptor
 If $IsObject(val) , deepFlag {
     Set topArg="attributeSetDescriptor",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TContentAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findNamespaceByCode",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findNamespaceByCode")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findNamespaceByCodeResult" {
   ...
   If (class="") || (class="TNamespace") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TNamespace",85,"t",class))_$get(^oddXML("BSTS.ns1.TNamespace","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..namespaceCode'="" Quit 0
 If $IsObject(..attributeSetDescriptor) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findNamespaceByCode).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findNamespaceByCode",top,format,namespacePrefix,input,refOnly,.schema)
...
zattributeSetDescriptorNewObjec() public {
	Set newobject=##class(BSTS.ns1.TContentAttributeSetDescriptor).%New() If newobject="" Quit ""
