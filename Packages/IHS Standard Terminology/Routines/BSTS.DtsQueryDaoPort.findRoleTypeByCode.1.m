 ;BSTS.DtsQueryDaoPort.findRoleTypeByCode.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findRoleTypeByCode.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;45706537;BSTS.DtsQueryDaoPort.findRoleTypeByCode
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,code,namespaceId) public {
 Set ..code=$get(code),..namespaceId=$get(namespaceId)
 Do %Client.InvokeClient($this,"findRoleTypeByCode",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findRoleTypeByCodeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findRoleTypeByCode",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findRoleTypeByCode")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findRoleTypeByCode")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findRoleTypeByCode"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findRoleTypeByCode"
   ...
   If tag="" Set tag=typesPrefix_"findRoleTypeByCode"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findRoleTypeByCode"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findRoleTypeByCode",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findRoleTypeByCode")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findRoleTypeByCodeResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findRoleTypeByCode).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findRoleTypeByCode",top,format,namespacePrefix,input,refOnly,.schema)
