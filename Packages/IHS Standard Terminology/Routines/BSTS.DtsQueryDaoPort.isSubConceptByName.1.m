 ;BSTS.DtsQueryDaoPort.isSubConceptByName.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.isSubConceptByName.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;31364841;BSTS.DtsQueryDaoPort.isSubConceptByName
 ;
...
%NormalizeObject() public {
	...
	If m%conceptName1 Set:i%conceptName1'="" i%conceptName1=(..conceptName1Normalize(i%conceptName1))
	If m%conceptName2 Set:i%conceptName2'="" i%conceptName2=(..conceptName2Normalize(i%conceptName2))
...
%ValidateObject(force=0) public {
	...
	If m%conceptName1 Set iv=..conceptName1 If iv'="" Set rc=(..conceptName1IsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptName1",iv)
	If m%conceptName2 Set iv=..conceptName2 If iv'="" Set rc=(..conceptName2IsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptName2",iv)
	...
	Quit sc }
zInvoke(%Client,%Action,conceptName1,conceptName2,namespaceId) public {
 Set ..conceptName1=$get(conceptName1),..conceptName2=$get(conceptName2),..namespaceId=$get(namespaceId)
 Do %Client.InvokeClient($this,"isSubConceptByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="isSubConceptByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.isSubConceptByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.isSubConceptByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.isSubConceptByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isSubConceptByName"
   ...
   If tag="" Set tag=typesPrefix_"isSubConceptByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isSubConceptByName"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptName1
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptName1"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"conceptName1>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..conceptName2
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptName2"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"conceptName2>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..namespaceId
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.isSubConceptByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="isSubConceptByNameResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If ..conceptName1'="" Quit 0
 If ..conceptName2'="" Quit 0
 If ..namespaceId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.isSubConceptByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.isSubConceptByName",top,format,namespacePrefix,input,refOnly,.schema)
