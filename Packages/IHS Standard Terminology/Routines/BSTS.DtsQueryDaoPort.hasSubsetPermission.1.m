 ;BSTS.DtsQueryDaoPort.hasSubsetPermission.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.hasSubsetPermission.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;5235334D;BSTS.DtsQueryDaoPort.hasSubsetPermission
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,subsetId,permission) public {
 Set ..subsetId=$get(subsetId),..permission=$get(permission)
 Do %Client.InvokeClient($this,"hasSubsetPermission",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="hasSubsetPermissionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.hasSubsetPermission",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasSubsetPermission")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.hasSubsetPermission")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.hasSubsetPermission"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="hasSubsetPermission"
   ...
   If tag="" Set tag=typesPrefix_"hasSubsetPermission"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"hasSubsetPermission"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.hasSubsetPermission",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasSubsetPermission")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="hasSubsetPermissionResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.hasSubsetPermission).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.hasSubsetPermission",top,format,namespacePrefix,input,refOnly,.schema)
