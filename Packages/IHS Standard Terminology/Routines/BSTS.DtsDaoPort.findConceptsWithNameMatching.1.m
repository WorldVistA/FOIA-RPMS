 ;BSTS.DtsDaoPort.findConceptsWithNameMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptsWithNameMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;364F6E61;BSTS.DtsDaoPort.findConceptsWithNameMatching
 ;
...
zInvoke(%Client,%Action,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findConceptsWithNameMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptsWithNameMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithNameMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptsWithNameMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptsWithNameMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithNameMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithNameMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithNameMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptsWithNameMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithNameMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptsWithNameMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptsWithNameMatching",top,format,namespacePrefix,input,refOnly,.schema)
