 ;BSTS.ns1.SEConceptExcludeFilter.1
 ;(C)InterSystems, generated for class BSTS.ns1.SEConceptExcludeFilter.  Do NOT edit. 08/21/2017 07:46:01PM
 ;;2F516D46;BSTS.ns1.SEConceptExcludeFilter
 ;
...
%NormalizeObject() public {
	...
	If m%modifier Set:i%modifier'="" i%modifier=(..modifierNormalize(i%modifier))
...
%ValidateObject(force=0) public {
	...
	Set iv=..modifier If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"modifier"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%modifier Set rc=(..modifierIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"modifier",iv)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.SEConceptExcludeFilter",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SEConceptExcludeFilter")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.SEConceptExcludeFilter")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.SEConceptExcludeFilter"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="SEConceptExcludeFilter"
   ...
   If tag="" Set tag=typesPrefix_"SEConceptExcludeFilter"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"SEConceptExcludeFilter"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..modifier
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.SEConceptFilterModifier","NAMESPACE")
   Set temp1=$select((temp1'="http://apelon.com/dtsserver/types")&&$IsObject(namespaces):namespaces.GetNamespacePrefix(temp1,.temp),1:typesPrefix)
   Set %xmlmsg=currentIndent_beginprefix_"modifier"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"SEConceptFilterModifier"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"modifier>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..concept
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     ...
     Set topArg="concept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.SEConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.SEConceptExcludeFilter" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.SEConceptExcludeFilter",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SEConceptExcludeFilter")
 If encoded {
 ...
 If tag="modifier" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             If data="" Set data=$c(0)
             If data'="" Goto:('$s(data'[","&&(",ALL,ONLY_DESCENDANTS_OF,ONLY_CHILD_OF,"[(","_$select(data=$c(0):"",1:data)_",")):1,1:$$Error^%apiOBJ(7205,data,",ALL,ONLY_DESCENDANTS_OF,ONLY_CHILD_OF"))) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..modifier=data
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.SEConceptExcludeFilter).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.SEConceptExcludeFilter",top,format,namespacePrefix,input,refOnly,.schema)
zconceptNewObject() public {
	Set newobject=##class(BSTS.ns1.SEConcept).%New() If newobject="" Quit ""
	Set ..concept=newobject
	Quit newobject }
zmodifierIsValid(%val) public {
