 ;BSTS.DtsQueryDaoPort.fetchSubsetNamespaces.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchSubsetNamespaces.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;35517568;BSTS.DtsQueryDaoPort.fetchSubsetNamespaces
 ;
...
zInvoke(%Client,%Action,subsetId) public {
 ...
 Do %Client.InvokeClient($this,"fetchSubsetNamespaces",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchSubsetNamespaces",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetNamespaces")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchSubsetNamespaces")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchSubsetNamespaces"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchSubsetNamespaces"
   ...
   If tag="" Set tag=typesPrefix_"fetchSubsetNamespaces"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchSubsetNamespaces"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchSubsetNamespaces",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetNamespaces")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchSubsetNamespaces).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchSubsetNamespaces",top,format,namespacePrefix,input,refOnly,.schema)
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.fetchSubsetNamespaces).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.fetchSubsetNamespaces:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
