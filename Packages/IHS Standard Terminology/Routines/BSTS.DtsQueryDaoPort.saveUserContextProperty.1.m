 ;BSTS.DtsQueryDaoPort.saveUserContextProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.saveUserContextProperty.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;6B707261;BSTS.DtsQueryDaoPort.saveUserContextProperty
 ;
...
zInvoke(%Client,%Action,key,value) public {
 ...
 Do %Client.InvokeClient($this,"saveUserContextProperty",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.saveUserContextProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.saveUserContextProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.saveUserContextProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.saveUserContextProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="saveUserContextProperty"
   ...
   If tag="" Set tag=typesPrefix_"saveUserContextProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"saveUserContextProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.saveUserContextProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.saveUserContextProperty")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.saveUserContextProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.saveUserContextProperty",top,format,namespacePrefix,input,refOnly,.schema)
