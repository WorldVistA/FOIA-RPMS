 ;BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;796E6C50;BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..synonymType)=1 Set r%synonymType=r%synonymType.%ConstructClone(1,.cloned),i%synonymType=""
	}
	Quit 1 }
%Destruct() public {
	Try {
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%synonymType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,synonymType,pattern,options) public {
 Set ..synonymType=$get(synonymType),..pattern=$get(pattern),..options=$get(options)
 Do %Client.InvokeClient($this,"findConceptsWithSynonymMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithSynonymMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithSynonymMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithSynonymMatching"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="synonymType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..synonymType) Quit 0
 If ..pattern'="" Quit 0
 If $IsObject(..options) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findConceptsWithSynonymMatching",top,format,namespacePrefix,input,refOnly,.schema)
