 ;BSTS.DtsQueryDaoPort.updatePropertyType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updatePropertyType.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;672B5346;BSTS.DtsQueryDaoPort.updatePropertyType
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newPropertyType
 s proporef=..oldPropertyType
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newPropertyType)=1 Set r%newPropertyType=r%newPropertyType.%ConstructClone(1,.cloned),i%newPropertyType=""
		If $isobject(..oldPropertyType)=1 Set r%oldPropertyType=r%oldPropertyType.%ConstructClone(1,.cloned),i%oldPropertyType=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newPropertyType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldPropertyType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldPropertyType,newPropertyType) public {
 Set ..oldPropertyType=$get(oldPropertyType),..newPropertyType=$get(newPropertyType)
 Do %Client.InvokeClient($this,"updatePropertyType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updatePropertyTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updatePropertyType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updatePropertyType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updatePropertyType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updatePropertyType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updatePropertyType"
   ...
   If tag="" Set tag=typesPrefix_"updatePropertyType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updatePropertyType"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldPropertyType
 If $IsObject(val) , deepFlag {
     Set topArg="oldPropertyType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TPropertyType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newPropertyType
 If $IsObject(val) , deepFlag {
     Set topArg="newPropertyType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TPropertyType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updatePropertyType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updatePropertyType")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updatePropertyTypeResult" {
   ...
   If (class="") || (class="TPropertyType") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TPropertyType",85,"t",class))_$get(^oddXML("BSTS.ns1.TPropertyType","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldPropertyType) Quit 0
 If $IsObject(..newPropertyType) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updatePropertyType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updatePropertyType",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TPropertyType).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewPropertyTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TPropertyType).%New() If newobject="" Quit ""
	Set ..newPropertyType=newobject
	Quit newobject }
zoldPropertyTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TPropertyType).%New() If newobject="" Quit ""
	Set ..oldPropertyType=newobject
	Quit newobject }
