 ;BSTS.ns1.TNavChildContext.1
 ;(C)InterSystems, generated for class BSTS.ns1.TNavChildContext.  Do NOT edit. 08/21/2017 07:45:59PM
 ;;65643865;BSTS.ns1.TNavChildContext
 ;
...
%Construct(initvalue) public {
	Set m%children=0,M%children=0
	Quit 1 }
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%children="",r%children=""
	Set i%"%%OID"=""
	If deep>0 {
		Set key="" For  Set value=..children.GetNext(.key) Quit:key=""  Set r%children(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%children(key)=""
		If $isobject(..focus)=1 Set r%focus=r%focus.%ConstructClone(1,.cloned),i%focus=""
	}
	Quit 1 }
%Destruct() public {
	Try {
		If $isobject($get(r%children))=1,$zobjcnt(r%children)>1 Do r%children.%Disconnect()
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set key=""
	For  {
		Set key=$order(r%children(key),1,Poref) Quit:key=""
		If $isobject(Poref)=1 Set:'$data(%objTX(1,+Poref)) %objTX(6,$i(%objTX(6)))=Poref
	}
	Set Poref=r%focus If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TNavChildContext",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TNavChildContext")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TNavChildContext")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TNavChildContext"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TNavChildContext"
   ...
   If tag="" Set tag=typesPrefix_"TNavChildContext"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TNavChildContext"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TOntylogConcept","XMLSUMMARY")'="":-1,1:1)
     Set id=+$get(idlist(temp*val))
     If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
   }
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"focus "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="focus",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     ...
     Set temp="",temp1=$parameter("BSTS.ns1.TConceptNode","NAMESPACE")
     ...
     If soap12 { Set %xmlmsg=beginprefix_"children"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.TConceptNode","XMLSUMMARY")'="":"s_TConceptNode",1:"TConceptNode")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"children "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.TConceptNode","XMLSUMMARY")'="":"s_TConceptNode",1:"TConceptNode")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       If encoded,'inlineFlag {
         Set temp=$select($parameter("BSTS.ns1.TConceptNode","XMLSUMMARY")'="":-1,1:1)
         Set id=+$get(idlist(temp*val))
         If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
       }
       If +id'=0 {
         Set %xmlmsg=currentIndent_beginprefix_"children "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
       } Else { if id=0 Set id=$increment(idlist)
         Set topArg="children",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TConceptNode"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
       }
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"children "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
   ...
   If encoded {
     ...
     Set %xmlmsg=endprefix_"children>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TNavChildContext" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TNavChildContext",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TNavChildContext")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="focus" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If (class="") || (class="TOntylogConcept") {
       Set class="BSTS.ns1.TOntylogConcept"
     } Else {
       If $length(class,":")=2 Set class=$piece(class,":",2)
       Set class=$get(^oddCOM("BSTS.ns1.TOntylogConcept",85,"s",class))_$get(^oddXML("BSTS.ns1.TOntylogConcept","s",class)) If class="" Set class=0
     ...
     If encoded,(($data(@(tree)@(ref,"a","arrayType")))||(($get(@(tree)@(ref,"a","type","u"))=1)&&($piece($get(@(tree)@(ref,"a","type")),":",2)="Array"))||($select($get(@(tree)@(ref,"a","itemType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","itemType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","nodeType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","nodeType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","arraySize","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","arraySize","u"))))="http://www.w3.org/2003/05/soap-encoding")) {
       ...
       While element'="" {
         ...
         If $$XMLImportId() {
           ...
           If 'sc Goto XMLImportExit
           Set class=$select(($get(@(tree)@(ref,"a","type","u"))=1):$get(@(tree)@(ref,"a","type")),1:"")
           If (class="") || (class="TConceptNode") {
             Set class="BSTS.ns1.TConceptNode"
           } Else {
             If $length(class,":")=2 Set class=$piece(class,":",2)
             Set class=$get(^oddCOM("BSTS.ns1.TConceptNode",85,"s",class))_$get(^oddXML("BSTS.ns1.TConceptNode","s",class)) If class="" Set class=0
             Set tmp=$select($get(@(tree)@(ref,"a","type","v"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","type","v"))))
             If (class=0)||(($listlength(class)>1)&&(tmp="")) Set class=$get(@(tree)@(ref,"a","type")) Goto XMLImportBadType
             If $listlength(class)>1 {
               For tmpi=1:1:$listlength(class) {
       ...
       Quit:('sc)  Set ref=loopref
         ...
         If $listget($get(@(tree)@(ref,0)),1)'="e" Goto XMLImportMalformedNoTag
         If 'encoded,(@(tree)@(ref)'="children") Goto XMLImportBadTag
         If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
         If encoded,$$XMLImportId() {
           Set data=idlist(ref)
         } Else {
...
zXMLIsObjectEmpty(ignoreNull)
 If $IsObject(..focus) Quit 0
 If ..children.Count()>0 Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TNavChildContext).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TNavChildContext",top,format,namespacePrefix,input,refOnly,.schema)
zchildrenBuildValueArray(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zchildrenCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
zchildrenCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
	...
	quit $extract(odbc,2,*) }
zchildrenDisplayToCollection(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zchildrenGetObject(force=0) public {
	Quit $select(r%children="":$select(i%children="":"",1:$listbuild(i%children_"")),(''..children.%GetSwizzleObject(force,.oid)):oid,1:"") }
zchildrenGetObjectId(force=0) public {
	Quit $listget(..childrenGetObject(force)) }
zchildrenGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%children=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.TConceptNode",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."children%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zchildrenOdbcToCollection(val="") public {
	set logical="",out=""
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zchildrenSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%children=newvalue Quit 1
	If newvalue="" Kill i%children,r%children Set i%children="",r%children="" Quit 1
	Set oref=r%children Kill i%children,r%children Set i%children="",r%children=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%children(i)=value,i%children(i)=""
	Quit 1 }
zfocusNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..focus=newobject
	Quit newobject }
