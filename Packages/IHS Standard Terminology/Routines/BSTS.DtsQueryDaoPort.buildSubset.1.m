 ;BSTS.DtsQueryDaoPort.buildSubset.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.buildSubset.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;4F646F36;BSTS.DtsQueryDaoPort.buildSubset
 ;
...
zInvoke(%Client,%Action,subsetId) public {
 ...
 Do %Client.InvokeClient($this,"buildSubset",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="buildSubsetResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.buildSubset",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.buildSubset")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.buildSubset")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.buildSubset"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"buildSubset"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"buildSubset"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.buildSubset",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.buildSubset")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.buildSubset).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.buildSubset",top,format,namespacePrefix,input,refOnly,.schema)
