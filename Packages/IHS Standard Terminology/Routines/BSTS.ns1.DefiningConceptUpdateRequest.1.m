 ;BSTS.ns1.DefiningConceptUpdateRequest.1
 ;(C)InterSystems, generated for class BSTS.ns1.DefiningConceptUpdateRequest.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;6F43786A;BSTS.ns1.DefiningConceptUpdateRequest
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..addDefiningConcept
 s proporef=..concept
 s proporef=..deleteDefiningConcept
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..addDefiningConcept)=1 Set r%addDefiningConcept=r%addDefiningConcept.%ConstructClone(1,.cloned),i%addDefiningConcept=""
		If $isobject(..concept)=1 Set r%concept=r%concept.%ConstructClone(1,.cloned),i%concept=""
		If $isobject(..deleteDefiningConcept)=1 Set r%deleteDefiningConcept=r%deleteDefiningConcept.%ConstructClone(1,.cloned),i%deleteDefiningConcept=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%addDefiningConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%concept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%deleteDefiningConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.DefiningConceptUpdateRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.DefiningConceptUpdateRequest")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.DefiningConceptUpdateRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.DefiningConceptUpdateRequest"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="DefiningConceptUpdateRequest"
   ...
   If tag="" Set tag=typesPrefix_"DefiningConceptUpdateRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"DefiningConceptUpdateRequest"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     ...
     Set topArg="concept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..deleteDefiningConcept
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TOntylogConcept","XMLSUMMARY")'="":-1,1:1)
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"deleteDefiningConcept "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="deleteDefiningConcept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..addDefiningConcept
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
     Set temp=$select($parameter("BSTS.ns1.TOntylogConcept","XMLSUMMARY")'="":-1,1:1)
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"addDefiningConcept "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="addDefiningConcept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.DefiningConceptUpdateRequest" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.DefiningConceptUpdateRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.DefiningConceptUpdateRequest")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="deleteDefiningConcept" {
   ...
   If data'="" Set ..deleteDefiningConcept=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="addDefiningConcept" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
   ...
   If data'="" Set ..addDefiningConcept=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..deleteDefiningConcept) Quit 0
 If $IsObject(..addDefiningConcept) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.DefiningConceptUpdateRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.DefiningConceptUpdateRequest",top,format,namespacePrefix,input,refOnly,.schema)
zaddDefiningConceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..addDefiningConcept=newobject
	Quit newobject }
zconceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..concept=newobject
	Quit newobject }
zdeleteDefiningConceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..deleteDefiningConcept=newobject
	Quit newobject }
