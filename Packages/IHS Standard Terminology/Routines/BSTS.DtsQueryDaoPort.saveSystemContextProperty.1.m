 ;BSTS.DtsQueryDaoPort.saveSystemContextProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.saveSystemContextProperty.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;70423568;BSTS.DtsQueryDaoPort.saveSystemContextProperty
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,key,value) public {
 Set ..key=$get(key),..value=$get(value)
 Do %Client.InvokeClient($this,"saveSystemContextProperty",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.saveSystemContextProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.saveSystemContextProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.saveSystemContextProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.saveSystemContextProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="saveSystemContextProperty"
   ...
   If tag="" Set tag=typesPrefix_"saveSystemContextProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"saveSystemContextProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.saveSystemContextProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.saveSystemContextProperty")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.saveSystemContextProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.saveSystemContextProperty",top,format,namespacePrefix,input,refOnly,.schema)
