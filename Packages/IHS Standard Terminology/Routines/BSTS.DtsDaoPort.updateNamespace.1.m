 ;BSTS.DtsDaoPort.updateNamespace.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.updateNamespace.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;6733465A;BSTS.DtsDaoPort.updateNamespace
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..newNamespace
 s proporef=..oldNamespace
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..newNamespace)=1 Set r%newNamespace=r%newNamespace.%ConstructClone(1,.cloned),i%newNamespace=""
		If $isobject(..oldNamespace)=1 Set r%oldNamespace=r%oldNamespace.%ConstructClone(1,.cloned),i%oldNamespace=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newNamespace If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldNamespace If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,oldNamespace,newNamespace) public {
 Set ..oldNamespace=$get(oldNamespace),..newNamespace=$get(newNamespace)
 Do %Client.InvokeClient($this,"updateNamespace",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateNamespaceResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.updateNamespace",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateNamespace")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.updateNamespace")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.updateNamespace"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateNamespace"
   ...
   If tag="" Set tag=typesPrefix_"updateNamespace"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateNamespace"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldNamespace
 If $IsObject(val) , deepFlag {
     Set topArg="oldNamespace",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TNamespace"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newNamespace
 If $IsObject(val) , deepFlag {
     Set topArg="newNamespace",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TNamespace"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.updateNamespace",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateNamespace")
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If $IsObject(..oldNamespace) Quit 0
 If $IsObject(..newNamespace) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.updateNamespace).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.updateNamespace",top,format,namespacePrefix,input,refOnly,.schema)
...
znewNamespaceNewObject() public {
	Set newobject=##class(BSTS.ns1.TNamespace).%New() If newobject="" Quit ""
	Set ..newNamespace=newobject
	Quit newobject }
zoldNamespaceNewObject() public {
	Set newobject=##class(BSTS.ns1.TNamespace).%New() If newobject="" Quit ""
	Set ..oldNamespace=newobject
	Quit newobject }
