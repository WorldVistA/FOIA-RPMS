 ;BSTS.DtsQueryDaoPort.fetchSubsetConcepts.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchSubsetConcepts.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;6F434473;BSTS.DtsQueryDaoPort.fetchSubsetConcepts
 ;
...
%NormalizeObject() public {
	...
	If m%snapshotTime Set:i%snapshotTime'="" i%snapshotTime=(..snapshotTimeNormalize(i%snapshotTime))
...
%ValidateObject(force=0) public {
	...
	If m%snapshotTime Set iv=..snapshotTime If iv'="" Set rc=(..snapshotTimeIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"snapshotTime",iv)
	If m%subsetId Set iv=..subsetId If iv'="" Set rc=(..subsetIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"subsetId",iv)
	Quit sc }
zInvoke(%Client,%Action,subsetId,snapshotTime,asd) public {
 Set ..subsetId=$get(subsetId),..snapshotTime=$get(snapshotTime),..asd=$get(asd)
 Do %Client.InvokeClient($this,"fetchSubsetConcepts",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchSubsetConcepts",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetConcepts")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchSubsetConcepts")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchSubsetConcepts"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchSubsetConcepts"
   ...
   If tag="" Set tag=typesPrefix_"fetchSubsetConcepts"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchSubsetConcepts"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..snapshotTime
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"snapshotTime"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"dateTime""",1:"")_">"_$select(val="":"",1:$translate(val," ","T")_"Z")_endprefix_"snapshotTime>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..asd
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchSubsetConcepts",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetConcepts")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..subsetId'="" Quit 0
 If ..snapshotTime'="" Quit 0
 If $IsObject(..asd) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchSubsetConcepts).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchSubsetConcepts",top,format,namespacePrefix,input,refOnly,.schema)
...
zsnapshotTimeIsValid(%val)
	...
	Quit 1
zsnapshotTimeLogicalToXSD(%val)
	Quit $select(%val="":"",1:$translate(%val," ","T")_"Z")
zsnapshotTimeNormalize(%val)
...
zsnapshotTimeOdbcToLogical(%val)
