 ;BSTS.DtsQueryDaoPort.fetchSubsetConceptCount.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchSubsetConceptCount.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;4D564C67;BSTS.DtsQueryDaoPort.fetchSubsetConceptCount
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,subset) public {
 Set ..subset=$get(subset)
 Do %Client.InvokeClient($this,"fetchSubsetConceptCount",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="fetchSubsetConceptCountResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchSubsetConceptCount",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetConceptCount")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchSubsetConceptCount")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchSubsetConceptCount"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchSubsetConceptCount"
   ...
   If tag="" Set tag=typesPrefix_"fetchSubsetConceptCount"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchSubsetConceptCount"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchSubsetConceptCount",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchSubsetConceptCount")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="fetchSubsetConceptCountResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchSubsetConceptCount).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchSubsetConceptCount",top,format,namespacePrefix,input,refOnly,.schema)
