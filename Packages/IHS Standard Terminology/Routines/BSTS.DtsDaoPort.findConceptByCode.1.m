 ;BSTS.DtsDaoPort.findConceptByCode.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptByCode.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;6D62476F;BSTS.DtsDaoPort.findConceptByCode
 ;
...
%NormalizeObject() public {
	...
	If m%conceptCode Set:i%conceptCode'="" i%conceptCode=(..conceptCodeNormalize(i%conceptCode))
...
%ValidateObject(force=0) public {
	...
	If m%conceptCode Set iv=..conceptCode If iv'="" Set rc=(..conceptCodeIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptCode",iv)
	...
	Quit sc }
zInvoke(%Client,%Action,conceptCode,namespaceId,attributeSetDescriptor) public {
 Set ..conceptCode=$get(conceptCode),..namespaceId=$get(namespaceId),..attributeSetDescriptor=$get(attributeSetDescriptor)
 Do %Client.InvokeClient($this,"findConceptByCode",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findConceptByCodeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptByCode",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptByCode")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptByCode")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptByCode"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptByCode"
   ...
   If tag="" Set tag=typesPrefix_"findConceptByCode"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptByCode"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptCode
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptCode"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"conceptCode>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..namespaceId
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptByCode",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptByCode")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findConceptByCodeResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..conceptCode'="" Quit 0
 If ..namespaceId'="" Quit 0
 If $IsObject(..attributeSetDescriptor) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptByCode).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptByCode",top,format,namespacePrefix,input,refOnly,.schema)
