 ;BSTS.ns1.SynonymUpdateRequest.1
 ;(C)InterSystems, generated for class BSTS.ns1.SynonymUpdateRequest.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;6A464432;BSTS.ns1.SynonymUpdateRequest
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..newSynonym
 s proporef=..oldSynonym
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..newSynonym)=1 Set r%newSynonym=r%newSynonym.%ConstructClone(1,.cloned),i%newSynonym=""
		If $isobject(..oldSynonym)=1 Set r%oldSynonym=r%oldSynonym.%ConstructClone(1,.cloned),i%oldSynonym=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newSynonym If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldSynonym If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.SynonymUpdateRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SynonymUpdateRequest")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.SynonymUpdateRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.SynonymUpdateRequest"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="SynonymUpdateRequest"
   ...
   If tag="" Set tag=typesPrefix_"SynonymUpdateRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"SynonymUpdateRequest"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"oldSynonym "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="oldSynonym",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TSynonym"),local=1,savexsiAttrs=xsiAttrs
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"newSynonym "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="newSynonym",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TSynonym"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.SynonymUpdateRequest" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.SynonymUpdateRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SynonymUpdateRequest")
 If encoded {
...
zXMLIsObjectEmpty(ignoreNull)
 If $IsObject(..oldSynonym) Quit 0
 If $IsObject(..newSynonym) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.SynonymUpdateRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.SynonymUpdateRequest",top,format,namespacePrefix,input,refOnly,.schema)
znewSynonymNewObject() public {
	Set newobject=##class(BSTS.ns1.TSynonym).%New() If newobject="" Quit ""
	Set ..newSynonym=newobject
	Quit newobject }
zoldSynonymNewObject() public {
	Set newobject=##class(BSTS.ns1.TSynonym).%New() If newobject="" Quit ""
	Set ..oldSynonym=newobject
	Quit newobject }
