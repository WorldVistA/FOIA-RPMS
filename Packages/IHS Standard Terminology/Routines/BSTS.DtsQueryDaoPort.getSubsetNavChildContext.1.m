 ;BSTS.DtsQueryDaoPort.getSubsetNavChildContext.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getSubsetNavChildContext.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;68707754;BSTS.DtsQueryDaoPort.getSubsetNavChildContext
 ;
...
zInvoke(%Client,%Action,focusConcept,subsetId,namespace,focusConceptASD) public {
 ...
 Do %Client.InvokeClient($this,"getSubsetNavChildContext",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getSubsetNavChildContextResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getSubsetNavChildContext",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetNavChildContext")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getSubsetNavChildContext")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getSubsetNavChildContext"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getSubsetNavChildContext"
   ...
   If tag="" Set tag=typesPrefix_"getSubsetNavChildContext"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getSubsetNavChildContext"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getSubsetNavChildContext",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetNavChildContext")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="getSubsetNavChildContextResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getSubsetNavChildContext).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getSubsetNavChildContext",top,format,namespacePrefix,input,refOnly,.schema)
