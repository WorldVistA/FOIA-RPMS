 ;BSTS.DtsQueryDaoPort.deleteSystemContextProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteSystemContextProperty.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;6E344261;BSTS.DtsQueryDaoPort.deleteSystemContextProperty
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,key) public {
 Set ..key=$get(key)
 Do %Client.InvokeClient($this,"deleteSystemContextProperty",%Action)
 Quit }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteSystemContextProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteSystemContextProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteSystemContextProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteSystemContextProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteSystemContextProperty"
   ...
   If tag="" Set tag=typesPrefix_"deleteSystemContextProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteSystemContextProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteSystemContextProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteSystemContextProperty")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteSystemContextProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteSystemContextProperty",top,format,namespacePrefix,input,refOnly,.schema)
