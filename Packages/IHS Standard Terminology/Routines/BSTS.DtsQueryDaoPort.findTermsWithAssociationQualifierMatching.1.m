 ;BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;53557566;BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching
 ;
...
zInvoke(%Client,%Action,assocType,qualType,pattern,options,inverse) public {
 ...
 Do %Client.InvokeClient($this,"findTermsWithAssociationQualifierMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithAssociationQualifierMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithAssociationQualifierMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithAssociationQualifierMatching"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..options
 If $IsObject(val) , deepFlag {
     Set topArg="options",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TTermSearchOptions"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..inverse
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findTermsWithAssociationQualifierMatching",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TTerm",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
