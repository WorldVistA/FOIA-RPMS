 ;BSTS.DtsQueryDaoPort.updateAuthority.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateAuthority.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;5A4D4178;BSTS.DtsQueryDaoPort.updateAuthority
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..newAuthority
 s proporef=..oldAuthority
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..newAuthority)=1 Set r%newAuthority=r%newAuthority.%ConstructClone(1,.cloned),i%newAuthority=""
		If $isobject(..oldAuthority)=1 Set r%oldAuthority=r%oldAuthority.%ConstructClone(1,.cloned),i%oldAuthority=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newAuthority If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldAuthority If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,oldAuthority,newAuthority) public {
 Set ..oldAuthority=$get(oldAuthority),..newAuthority=$get(newAuthority)
 Do %Client.InvokeClient($this,"updateAuthority",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateAuthorityResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateAuthority",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateAuthority")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateAuthority")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateAuthority"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateAuthority"
   ...
   If tag="" Set tag=typesPrefix_"updateAuthority"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateAuthority"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldAuthority
 If $IsObject(val) , deepFlag {
     Set topArg="oldAuthority",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAuthority"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newAuthority
 If $IsObject(val) , deepFlag {
     Set topArg="newAuthority",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAuthority"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateAuthority",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateAuthority")
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If $IsObject(..oldAuthority) Quit 0
 If $IsObject(..newAuthority) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateAuthority).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateAuthority",top,format,namespacePrefix,input,refOnly,.schema)
...
znewAuthorityNewObject() public {
	Set newobject=##class(BSTS.ns1.TAuthority).%New() If newobject="" Quit ""
	Set ..newAuthority=newobject
	Quit newobject }
zoldAuthorityNewObject() public {
	Set newobject=##class(BSTS.ns1.TAuthority).%New() If newobject="" Quit ""
	Set ..oldAuthority=newobject
	Quit newobject }
