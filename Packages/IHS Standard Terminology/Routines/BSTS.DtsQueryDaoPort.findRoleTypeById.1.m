 ;BSTS.DtsQueryDaoPort.findRoleTypeById.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findRoleTypeById.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;4C6E766C;BSTS.DtsQueryDaoPort.findRoleTypeById
 ;
...
zInvoke(%Client,%Action,id,namespaceId) public {
 ...
 Do %Client.InvokeClient($this,"findRoleTypeById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findRoleTypeByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findRoleTypeById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findRoleTypeById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findRoleTypeById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findRoleTypeById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findRoleTypeById"
   ...
   If tag="" Set tag=typesPrefix_"findRoleTypeById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findRoleTypeById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findRoleTypeById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findRoleTypeById")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findRoleTypeById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findRoleTypeById",top,format,namespacePrefix,input,refOnly,.schema)
