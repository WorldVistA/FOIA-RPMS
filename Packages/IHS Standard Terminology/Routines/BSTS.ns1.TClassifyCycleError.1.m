 ;BSTS.ns1.TClassifyCycleError.1
 ;(C)InterSystems, generated for class BSTS.ns1.TClassifyCycleError.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;7A656759;BSTS.ns1.TClassifyCycleError
 ;
...
%NormalizeObject() public {
	...
	If m%cycleNumber Set:i%cycleNumber'="" i%cycleNumber=(..cycleNumberNormalize(i%cycleNumber))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	Set iv=..cycleNumber If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"cycleNumber"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%cycleNumber Set rc=(..cycleNumberIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"cycleNumber",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TClassifyCycleError",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TClassifyCycleError")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TClassifyCycleError")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TClassifyCycleError"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TClassifyCycleError"
   ...
   If tag="" Set tag=typesPrefix_"TClassifyCycleError"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TClassifyCycleError"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     ...
     Set topArg="concept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..cycleNumber
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"cycleNumber"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"cycleNumber>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..roleType
 If $IsObject(val) , deepFlag {
   Set id=""
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TClassifyCycleError" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TClassifyCycleError",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TClassifyCycleError")
 If encoded {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..cycleNumber'="" Quit 0
 If $IsObject(..roleType) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TClassifyCycleError).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TClassifyCycleError",top,format,namespacePrefix,input,refOnly,.schema)
zconceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TConcept).%New() If newobject="" Quit ""
	Set ..concept=newobject
	Quit newobject }
zcycleNumberDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zcycleNumberIsValid(%val) public {
...
zcycleNumberNormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zcycleNumberXSDToLogical(%val) public {
