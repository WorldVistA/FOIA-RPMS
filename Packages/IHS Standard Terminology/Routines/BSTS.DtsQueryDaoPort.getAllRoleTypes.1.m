 ;BSTS.DtsQueryDaoPort.getAllRoleTypes.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getAllRoleTypes.  Do NOT edit. 08/21/2017 07:46:17PM
 ;;38394463;BSTS.DtsQueryDaoPort.getAllRoleTypes
 ;
...
zInvoke(%Client,%Action) public {
 Do %Client.InvokeClient($this,"getAllRoleTypes",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getAllRoleTypes",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getAllRoleTypes")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getAllRoleTypes")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getAllRoleTypes"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getAllRoleTypes"
   ...
   If tag="" Set tag=typesPrefix_"getAllRoleTypes"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getAllRoleTypes"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getAllRoleTypes",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getAllRoleTypes")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getAllRoleTypes).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getAllRoleTypes",top,format,namespacePrefix,input,refOnly,.schema)
