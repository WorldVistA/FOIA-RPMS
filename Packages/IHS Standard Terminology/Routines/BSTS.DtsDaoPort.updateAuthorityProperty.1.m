 ;BSTS.DtsDaoPort.updateAuthorityProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.updateAuthorityProperty.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;67656E6C;BSTS.DtsDaoPort.updateAuthorityProperty
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,authority,oldProp,newProp) public {
 Set ..authority=$get(authority),..oldProp=$get(oldProp),..newProp=$get(newProp)
 Do %Client.InvokeClient($this,"updateAuthorityProperty",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateAuthorityPropertyResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.updateAuthorityProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateAuthorityProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.updateAuthorityProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.updateAuthorityProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateAuthorityProperty"
   ...
   If tag="" Set tag=typesPrefix_"updateAuthorityProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateAuthorityProperty"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="authority",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAuthority"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.updateAuthorityProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.updateAuthorityProperty")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateAuthorityPropertyResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.updateAuthorityProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.updateAuthorityProperty",top,format,namespacePrefix,input,refOnly,.schema)
