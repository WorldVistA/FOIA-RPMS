 ;BSTS.DtsQueryDaoPort.updateQualifierType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateQualifierType.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;7039654E;BSTS.DtsQueryDaoPort.updateQualifierType
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newQualifierType
 s proporef=..oldQualifierType
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newQualifierType)=1 Set r%newQualifierType=r%newQualifierType.%ConstructClone(1,.cloned),i%newQualifierType=""
		If $isobject(..oldQualifierType)=1 Set r%oldQualifierType=r%oldQualifierType.%ConstructClone(1,.cloned),i%oldQualifierType=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newQualifierType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldQualifierType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldQualifierType,newQualifierType) public {
 Set ..oldQualifierType=$get(oldQualifierType),..newQualifierType=$get(newQualifierType)
 Do %Client.InvokeClient($this,"updateQualifierType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateQualifierTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateQualifierType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateQualifierType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateQualifierType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateQualifierType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateQualifierType"
   ...
   If tag="" Set tag=typesPrefix_"updateQualifierType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateQualifierType"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldQualifierType
 If $IsObject(val) , deepFlag {
     Set topArg="oldQualifierType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TQualifierType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newQualifierType
 If $IsObject(val) , deepFlag {
     Set topArg="newQualifierType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TQualifierType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateQualifierType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateQualifierType")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateQualifierTypeResult" {
   ...
   If (class="") || (class="TQualifierType") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TQualifierType",85,"t",class))_$get(^oddXML("BSTS.ns1.TQualifierType","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldQualifierType) Quit 0
 If $IsObject(..newQualifierType) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateQualifierType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateQualifierType",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TQualifierType).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewQualifierTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TQualifierType).%New() If newobject="" Quit ""
	Set ..newQualifierType=newobject
	Quit newobject }
zoldQualifierTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TQualifierType).%New() If newobject="" Quit ""
	Set ..oldQualifierType=newobject
	Quit newobject }
