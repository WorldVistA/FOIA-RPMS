 ;BSTS.DtsQueryDaoPort.getSubsetNavParentContext.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getSubsetNavParentContext.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;6A736F39;BSTS.DtsQueryDaoPort.getSubsetNavParentContext
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,focusConcept,subsetId,namespace,focusConceptASD) public {
 Set ..focusConcept=$get(focusConcept),..subsetId=$get(subsetId),..namespace=$get(namespace),..focusConceptASD=$get(focusConceptASD)
 Do %Client.InvokeClient($this,"getSubsetNavParentContext",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getSubsetNavParentContextResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getSubsetNavParentContext",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetNavParentContext")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getSubsetNavParentContext")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getSubsetNavParentContext"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getSubsetNavParentContext"
   ...
   If tag="" Set tag=typesPrefix_"getSubsetNavParentContext"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getSubsetNavParentContext"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getSubsetNavParentContext",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetNavParentContext")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="getSubsetNavParentContextResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getSubsetNavParentContext).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getSubsetNavParentContext",top,format,namespacePrefix,input,refOnly,.schema)
