 ;BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;524F7974;BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching
 ;
...
zInvoke(%Client,%Action,associationType,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findConceptsWithConceptAssociationMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithConceptAssociationMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithConceptAssociationMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithConceptAssociationMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptsWithConceptAssociationMatching",top,format,namespacePrefix,input,refOnly,.schema)
