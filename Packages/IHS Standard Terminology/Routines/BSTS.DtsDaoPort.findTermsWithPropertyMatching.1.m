 ;BSTS.DtsDaoPort.findTermsWithPropertyMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findTermsWithPropertyMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;7833305A;BSTS.DtsDaoPort.findTermsWithPropertyMatching
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,propertyType,pattern,options) public {
 Set ..propertyType=$get(propertyType),..pattern=$get(pattern),..options=$get(options)
 Do %Client.InvokeClient($this,"findTermsWithPropertyMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findTermsWithPropertyMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithPropertyMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findTermsWithPropertyMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findTermsWithPropertyMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithPropertyMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithPropertyMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithPropertyMatching"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="propertyType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TPropertyType"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findTermsWithPropertyMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithPropertyMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findTermsWithPropertyMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findTermsWithPropertyMatching",top,format,namespacePrefix,input,refOnly,.schema)
