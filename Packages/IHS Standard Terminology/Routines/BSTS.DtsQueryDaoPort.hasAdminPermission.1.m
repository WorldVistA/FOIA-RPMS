 ;BSTS.DtsQueryDaoPort.hasAdminPermission.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.hasAdminPermission.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;3764736D;BSTS.DtsQueryDaoPort.hasAdminPermission
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,permission) public {
 Set ..permission=$get(permission)
 Do %Client.InvokeClient($this,"hasAdminPermission",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="hasAdminPermissionResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.hasAdminPermission",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasAdminPermission")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.hasAdminPermission")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.hasAdminPermission"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="hasAdminPermission"
   ...
   If tag="" Set tag=typesPrefix_"hasAdminPermission"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"hasAdminPermission"""_xsiAttrs,xsiAttrs=""
 ...
 If val'="" {
   ...
   Set %xmlmsg=currentIndent_beginprefix_"permission"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TAdminPermission"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"permission>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.hasAdminPermission",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.hasAdminPermission")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="hasAdminPermissionResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.hasAdminPermission).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.hasAdminPermission",top,format,namespacePrefix,input,refOnly,.schema)
