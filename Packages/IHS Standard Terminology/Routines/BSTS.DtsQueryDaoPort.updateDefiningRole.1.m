 ;BSTS.DtsQueryDaoPort.updateDefiningRole.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateDefiningRole.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;66526B65;BSTS.DtsQueryDaoPort.updateDefiningRole
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,concept,deleteRole,addRole) public {
 Set ..concept=$get(concept),..deleteRole=$get(deleteRole),..addRole=$get(addRole)
 Do %Client.InvokeClient($this,"updateDefiningRole",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateDefiningRoleResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateDefiningRole",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateDefiningRole")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateDefiningRole")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateDefiningRole"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateDefiningRole"
   ...
   If tag="" Set tag=typesPrefix_"updateDefiningRole"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateDefiningRole"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..deleteRole
 If $IsObject(val) , deepFlag {
     Set topArg="deleteRole",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TRole"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..addRole
 If $IsObject(val) , deepFlag {
     Set topArg="addRole",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TRole"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateDefiningRole",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateDefiningRole")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateDefiningRoleResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateDefiningRole).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateDefiningRole",top,format,namespacePrefix,input,refOnly,.schema)
