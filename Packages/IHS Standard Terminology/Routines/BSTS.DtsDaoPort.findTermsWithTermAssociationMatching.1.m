 ;BSTS.DtsDaoPort.findTermsWithTermAssociationMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findTermsWithTermAssociationMatching.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;4B654A51;BSTS.DtsDaoPort.findTermsWithTermAssociationMatching
 ;
...
zInvoke(%Client,%Action,associationType,pattern,options) public {
 ...
 Do %Client.InvokeClient($this,"findTermsWithTermAssociationMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findTermsWithTermAssociationMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithTermAssociationMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findTermsWithTermAssociationMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findTermsWithTermAssociationMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findTermsWithTermAssociationMatching"
   ...
   If tag="" Set tag=typesPrefix_"findTermsWithTermAssociationMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findTermsWithTermAssociationMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findTermsWithTermAssociationMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findTermsWithTermAssociationMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findTermsWithTermAssociationMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findTermsWithTermAssociationMatching",top,format,namespacePrefix,input,refOnly,.schema)
