 ;BSTS.DtsQueryDaoPort.fetchTermHistory.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchTermHistory.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;41766756;BSTS.DtsQueryDaoPort.fetchTermHistory
 ;
...
%NormalizeObject() public {
	...
	If m%termId Set:i%termId'="" i%termId=(..termIdNormalize(i%termId))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%termId Set iv=..termId If iv'="" Set rc=(..termIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"termId",iv)
	Quit sc }
zInvoke(%Client,%Action,termId,namespaceId,fromTime,asd) public {
 Set ..termId=$get(termId),..namespaceId=$get(namespaceId),..fromTime=$get(fromTime),..asd=$get(asd)
 Do %Client.InvokeClient($this,"fetchTermHistory",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchTermHistory",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchTermHistory")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchTermHistory")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchTermHistory"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchTermHistory"
   ...
   If tag="" Set tag=typesPrefix_"fetchTermHistory"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchTermHistory"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..termId
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"termId"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"termId>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..namespaceId
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchTermHistory",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchTermHistory")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchTermHistory).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchTermHistory",top,format,namespacePrefix,input,refOnly,.schema)
