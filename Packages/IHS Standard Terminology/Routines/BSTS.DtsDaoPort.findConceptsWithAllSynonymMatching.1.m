 ;BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;49744D2F;BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,pattern,options) public {
 Set ..pattern=$get(pattern),..options=$get(options)
 Do %Client.InvokeClient($this,"findConceptsWithAllSynonymMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithAllSynonymMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithAllSynonymMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithAllSynonymMatching"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptsWithAllSynonymMatching",top,format,namespacePrefix,input,refOnly,.schema)
