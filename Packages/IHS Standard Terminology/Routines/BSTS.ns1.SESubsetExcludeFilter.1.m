 ;BSTS.ns1.SESubsetExcludeFilter.1
 ;(C)InterSystems, generated for class BSTS.ns1.SESubsetExcludeFilter.  Do NOT edit. 08/21/2017 07:46:01PM
 ;;7858754C;BSTS.ns1.SESubsetExcludeFilter
 ;
%NormalizeObject() public {
	...
	If m%rebuild Set:i%rebuild'="" i%rebuild=(..rebuildNormalize(i%rebuild))
...
%ValidateObject(force=0) public {
	...
	Set iv=..rebuild If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"rebuild"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%rebuild Set rc=(..rebuildIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"rebuild",iv)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.SESubsetExcludeFilter",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SESubsetExcludeFilter")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.SESubsetExcludeFilter")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.SESubsetExcludeFilter"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="SESubsetExcludeFilter"
   ...
   If tag="" Set tag=typesPrefix_"SESubsetExcludeFilter"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"SESubsetExcludeFilter"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..rebuild
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"rebuild"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"rebuild>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.SESubsetExcludeFilter" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.SESubsetExcludeFilter",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.SESubsetExcludeFilter")
 If encoded {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.SESubsetExcludeFilter).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.SESubsetExcludeFilter",top,format,namespacePrefix,input,refOnly,.schema)
zrebuildDisplayToLogical(%val) public {
	Quit ''%val }
zrebuildIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zrebuildLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zrebuildNormalize(%val) public {
	Quit %val\1 }
zrebuildXSDToLogical(%val) public {
