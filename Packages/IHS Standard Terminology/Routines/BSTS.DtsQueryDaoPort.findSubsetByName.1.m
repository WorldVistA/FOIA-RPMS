 ;BSTS.DtsQueryDaoPort.findSubsetByName.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findSubsetByName.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;787A662B;BSTS.DtsQueryDaoPort.findSubsetByName
 ;
...
%NormalizeObject() public {
	...
	If m%subsetName Set:i%subsetName'="" i%subsetName=(..subsetNameNormalize(i%subsetName))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%subsetName Set iv=..subsetName If iv'="" Set rc=(..subsetNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"subsetName",iv)
	Quit sc }
zInvoke(%Client,%Action,subsetName,asd) public {
 Set ..subsetName=$get(subsetName),..asd=$get(asd)
 Do %Client.InvokeClient($this,"findSubsetByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findSubsetByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findSubsetByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findSubsetByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findSubsetByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findSubsetByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findSubsetByName"
   ...
   If tag="" Set tag=typesPrefix_"findSubsetByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findSubsetByName"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..subsetName
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"subsetName"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"subsetName>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..asd
 If $IsObject(val) , deepFlag {
     Set topArg="asd",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TSubsetAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findSubsetByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findSubsetByName")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..subsetName'="" Quit 0
 If $IsObject(..asd) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findSubsetByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findSubsetByName",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TSubset).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
zasdNewObject() public {
	Set newobject=##class(BSTS.ns1.TSubsetAttributeSetDescriptor).%New() If newobject="" Quit ""
	Set ..asd=newobject
	Quit newobject }
