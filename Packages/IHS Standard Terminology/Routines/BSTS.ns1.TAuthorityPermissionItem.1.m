 ;BSTS.ns1.TAuthorityPermissionItem.1
 ;(C)InterSystems, generated for class BSTS.ns1.TAuthorityPermissionItem.  Do NOT edit. 08/21/2017 07:46:00PM
 ;;52633048;BSTS.ns1.TAuthorityPermissionItem
 ;
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%authorityId Set:i%authorityId'="" i%authorityId=(..authorityIdNormalize(i%authorityId))
	If m%authorityName Set:i%authorityName'="" i%authorityName=(..authorityNameNormalize(i%authorityName))
	If m%permission Set:i%permission'="" i%permission=(..permissionNormalize(i%permission))
	Quit 1 }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	Set iv=..authorityId If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"authorityId"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%authorityId Set rc=(..authorityIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"authorityId",iv)
	Set iv=..authorityName If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"authorityName"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%authorityName Set rc=(..authorityNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"authorityName",iv)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TAuthorityPermissionItem",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TAuthorityPermissionItem")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TAuthorityPermissionItem")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TAuthorityPermissionItem"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TAuthorityPermissionItem"
   ...
   If tag="" Set tag=typesPrefix_"TAuthorityPermissionItem"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TAuthorityPermissionItem"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..authorityName
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"authorityName"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"authorityName>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..permission
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.TAuthorityPermission","NAMESPACE")
   ...
   Set %xmlmsg=currentIndent_beginprefix_"permission"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TAuthorityPermission"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"permission>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TAuthorityPermissionItem",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TAuthorityPermissionItem")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="authorityId" {
   ...
   Set ..authorityId=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="authorityName" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else { Goto:'sc XMLImportExit
 ...
 If tag="permission" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             If data="" Set data=$c(0)
             If data'="" Goto:('$s(data'[","&&(",R,M,"[(","_$select(data=$c(0):"",1:data)_",")):1,1:$$Error^%apiOBJ(7205,data,",R,M"))) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..permission=data
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..authorityName'="" Quit 0
 If ..permission'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TAuthorityPermissionItem).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TAuthorityPermissionItem",top,format,namespacePrefix,input,refOnly,.schema)
