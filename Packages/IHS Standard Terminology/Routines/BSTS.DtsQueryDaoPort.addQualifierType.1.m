 ;BSTS.DtsQueryDaoPort.addQualifierType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.addQualifierType.  Do NOT edit. 08/21/2017 07:46:13PM
 ;;69744A38;BSTS.DtsQueryDaoPort.addQualifierType
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,newQualifierType) public {
 Set ..newQualifierType=$get(newQualifierType)
 Do %Client.InvokeClient($this,"addQualifierType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addQualifierTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.addQualifierType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addQualifierType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.addQualifierType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.addQualifierType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="addQualifierType"
   ...
   If tag="" Set tag=typesPrefix_"addQualifierType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addQualifierType"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.addQualifierType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.addQualifierType")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.addQualifierType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.addQualifierType",top,format,namespacePrefix,input,refOnly,.schema)
