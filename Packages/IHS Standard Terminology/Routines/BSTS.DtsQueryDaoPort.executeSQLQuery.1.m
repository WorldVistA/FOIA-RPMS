 ;BSTS.DtsQueryDaoPort.executeSQLQuery.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.executeSQLQuery.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;3633364B;BSTS.DtsQueryDaoPort.executeSQLQuery
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..sqlItem)=1 Set r%sqlItem=r%sqlItem.%ConstructClone(1,.cloned),i%sqlItem=""
	}
	Quit 1 }
%Destruct() public {
	Try {
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%sqlItem If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,sqlItem) public {
 Set ..sqlItem=$get(sqlItem)
 Do %Client.InvokeClient($this,"executeSQLQuery",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.executeSQLQuery",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeSQLQuery")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.executeSQLQuery")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.executeSQLQuery"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="executeSQLQuery"
   ...
   If tag="" Set tag=typesPrefix_"executeSQLQuery"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"executeSQLQuery"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="sqlItem",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.SqlItem"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.executeSQLQuery",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeSQLQuery")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="return" {
   ...
   If (class="") || (class="stringArray") {
     Set class="BSTS.ns2.stringArray"
   } Else {
     If $length(class,":")=2 Set class=$piece(class,":",2)
     Set class=$get(^oddCOM("BSTS.ns2.stringArray",85,"t",class))_$get(^oddXML("BSTS.ns2.stringArray","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..sqlItem) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.executeSQLQuery).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.executeSQLQuery",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns2.stringArray",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
...
%ResultSet(newvalue) public {
	...
	Quit 1 }
zsqlItemNewObject() public {
	Set newobject=##class(BSTS.ns1.SqlItem).%New() If newobject="" Quit ""
	Set ..sqlItem=newobject
	Quit newobject }
