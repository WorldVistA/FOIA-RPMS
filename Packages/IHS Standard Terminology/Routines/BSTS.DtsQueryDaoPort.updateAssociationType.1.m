 ;BSTS.DtsQueryDaoPort.updateAssociationType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateAssociationType.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;73533731;BSTS.DtsQueryDaoPort.updateAssociationType
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newAssocType
 s proporef=..oldAssocType
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newAssocType)=1 Set r%newAssocType=r%newAssocType.%ConstructClone(1,.cloned),i%newAssocType=""
		If $isobject(..oldAssocType)=1 Set r%oldAssocType=r%oldAssocType.%ConstructClone(1,.cloned),i%oldAssocType=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newAssocType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldAssocType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldAssocType,newAssocType) public {
 Set ..oldAssocType=$get(oldAssocType),..newAssocType=$get(newAssocType)
 Do %Client.InvokeClient($this,"updateAssociationType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateAssociationTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateAssociationType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateAssociationType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateAssociationType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateAssociationType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateAssociationType"
   ...
   If tag="" Set tag=typesPrefix_"updateAssociationType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateAssociationType"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldAssocType
 If $IsObject(val) , deepFlag {
     Set topArg="oldAssocType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newAssocType
 If $IsObject(val) , deepFlag {
     Set topArg="newAssocType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateAssociationType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateAssociationType")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateAssociationTypeResult" {
   ...
   If (class="") || (class="TAssociationType") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TAssociationType",85,"t",class))_$get(^oddXML("BSTS.ns1.TAssociationType","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldAssocType) Quit 0
 If $IsObject(..newAssocType) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateAssociationType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateAssociationType",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TAssociationType).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewAssocTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TAssociationType).%New() If newobject="" Quit ""
	Set ..newAssocType=newobject
	Quit newobject }
zoldAssocTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TAssociationType).%New() If newobject="" Quit ""
	Set ..oldAssocType=newobject
	Quit newobject }
