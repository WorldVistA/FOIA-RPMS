 ;BSTS.DtsDaoPort.getNamespacePropertyTypes.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getNamespacePropertyTypes.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;7777694C;BSTS.DtsDaoPort.getNamespacePropertyTypes
 ;
...
zInvoke(%Client,%Action,namespaceId) public {
 ...
 Do %Client.InvokeClient($this,"getNamespacePropertyTypes",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getNamespacePropertyTypes",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getNamespacePropertyTypes")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getNamespacePropertyTypes")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getNamespacePropertyTypes"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getNamespacePropertyTypes"
   ...
   If tag="" Set tag=typesPrefix_"getNamespacePropertyTypes"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getNamespacePropertyTypes"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getNamespacePropertyTypes",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getNamespacePropertyTypes")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getNamespacePropertyTypes).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getNamespacePropertyTypes",top,format,namespacePrefix,input,refOnly,.schema)
