 ;BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;742F3661;BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,associationType,pattern,options) public {
 Set ..associationType=$get(associationType),..pattern=$get(pattern),..options=$get(options)
 Do %Client.InvokeClient($this,"findConceptsWithInverseConceptAssociationMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithInverseConceptAssociationMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithInverseConceptAssociationMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithInverseConceptAssociationMatching"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="associationType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findConceptsWithInverseConceptAssociationMatching",top,format,namespacePrefix,input,refOnly,.schema)
