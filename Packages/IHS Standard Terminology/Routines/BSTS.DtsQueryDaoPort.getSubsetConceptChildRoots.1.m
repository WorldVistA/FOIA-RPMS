 ;BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;65506750;BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..rootConceptASD)=1 Set r%rootConceptASD=r%rootConceptASD.%ConstructClone(1,.cloned),i%rootConceptASD=""
	}
	Quit 1 }
%Destruct() public {
	Try {
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%rootConceptASD If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,rootConceptASD,namespace,subsetId) public {
 Set ..rootConceptASD=$get(rootConceptASD),..namespace=$get(namespace),..subsetId=$get(subsetId)
 Do %Client.InvokeClient($this,"getSubsetConceptChildRoots",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getSubsetConceptChildRoots"
   ...
   If tag="" Set tag=typesPrefix_"getSubsetConceptChildRoots"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getSubsetConceptChildRoots"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..rootConceptASD
 If $IsObject(val) , deepFlag {
     Set topArg="rootConceptASD",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="return" {
   ...
   If (class="") || (class="TConceptNode") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TConceptNode",85,"t",class))_$get(^oddXML("BSTS.ns1.TConceptNode","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..rootConceptASD) Quit 0
 If $IsObject(..namespace) Quit 0
 If ..subsetId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getSubsetConceptChildRoots",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TConceptNode",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
