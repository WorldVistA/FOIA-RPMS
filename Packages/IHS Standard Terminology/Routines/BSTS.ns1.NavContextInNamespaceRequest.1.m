 ;BSTS.ns1.NavContextInNamespaceRequest.1
 ;(C)InterSystems, generated for class BSTS.ns1.NavContextInNamespaceRequest.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;64337544;BSTS.ns1.NavContextInNamespaceRequest
 ;
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.NavContextInNamespaceRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.NavContextInNamespaceRequest")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.NavContextInNamespaceRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.NavContextInNamespaceRequest"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="NavContextInNamespaceRequest"
   ...
   If tag="" Set tag=typesPrefix_"NavContextInNamespaceRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"NavContextInNamespaceRequest"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"focusConcept "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="focusConcept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"focusConceptAsd "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="focusConceptAsd",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TConceptAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.NavContextInNamespaceRequest" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.NavContextInNamespaceRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.NavContextInNamespaceRequest")
 If encoded {
...
XMLImportElements() ;
 ...
 If tag="focusConceptAsd" {
   ...
   If data'="" Set ..focusConceptAsd=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.NavContextInNamespaceRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.NavContextInNamespaceRequest",top,format,namespacePrefix,input,refOnly,.schema)
