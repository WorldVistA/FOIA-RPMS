 ;BSTS.ns1.TSubsetExpression.1
 ;(C)InterSystems, generated for class BSTS.ns1.TSubsetExpression.  Do NOT edit. 08/21/2017 07:45:59PM
 ;;46323847;BSTS.ns1.TSubsetExpression
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..namespaceFilters
...
%Construct(initvalue) public {
	Set m%namespaceFilters=0,M%namespaceFilters=0,m%subsetFilters=0,M%subsetFilters=0
	Quit 1 }
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%namespaceFilters="",r%namespaceFilters=""
	Set i%subsetFilters="",r%subsetFilters=""
	Set i%"%%OID"=""
	If deep>0 {
		Set key="" For  Set value=..namespaceFilters.GetNext(.key) Quit:key=""  Set r%namespaceFilters(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%namespaceFilters(key)=""
...
%Destruct() public {
	Try {
		If $isobject($get(r%namespaceFilters))=1,$zobjcnt(r%namespaceFilters)>1 Do r%namespaceFilters.%Disconnect()
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set key=""
	For  {
		Set key=$order(r%namespaceFilters(key),1,Poref) Quit:key=""
...
%ValidateObject(force=0) public {
	...
	If m%sql Set iv=..sql If iv'="" Set rc=(..sqlIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"sql",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TSubsetExpression",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSubsetExpression")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TSubsetExpression")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TSubsetExpression"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TSubsetExpression"
   ...
   If tag="" Set tag=typesPrefix_"TSubsetExpression"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TSubsetExpression"""_xsiAttrs,xsiAttrs=""
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     ...
     Set temp="",temp1=$parameter("BSTS.ns1.SENamespaceFilter","NAMESPACE")
     ...
     If soap12 { Set %xmlmsg=beginprefix_"namespaceFilters"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.SENamespaceFilter","XMLSUMMARY")'="":"s_SENamespaceFilter",1:"SENamespaceFilter")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"namespaceFilters "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.SENamespaceFilter","XMLSUMMARY")'="":"s_SENamespaceFilter",1:"SENamespaceFilter")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       If encoded,'inlineFlag {
         Set temp=$select($parameter("BSTS.ns1.SENamespaceFilter","XMLSUMMARY")'="":-1,1:1)
         Set id=+$get(idlist(temp*val))
         If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
       }
       If +id'=0 {
         Set %xmlmsg=currentIndent_beginprefix_"namespaceFilters "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
       } Else { if id=0 Set id=$increment(idlist)
         Set topArg="namespaceFilters",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.SENamespaceFilter"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
       }
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"namespaceFilters "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
   ...
   If encoded {
     ...
     Set %xmlmsg=endprefix_"namespaceFilters>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TSubsetExpression" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TSubsetExpression",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TSubsetExpression")
 If encoded {
     ...
     If encoded,(($data(@(tree)@(ref,"a","arrayType")))||(($get(@(tree)@(ref,"a","type","u"))=1)&&($piece($get(@(tree)@(ref,"a","type")),":",2)="Array"))||($select($get(@(tree)@(ref,"a","itemType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","itemType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","nodeType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","nodeType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","arraySize","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","arraySize","u"))))="http://www.w3.org/2003/05/soap-encoding")) {
       ...
       While element'="" {
         ...
         If $$XMLImportId() {
           ...
           If (class="") || (class="SENamespaceFilter") {
             Set class="BSTS.ns1.SENamespaceFilter"
           } Else {
             If $length(class,":")=2 Set class=$piece(class,":",2)
             Set class=$get(^oddCOM("BSTS.ns1.SENamespaceFilter",85,"s",class))_$get(^oddXML("BSTS.ns1.SENamespaceFilter","s",class)) If class="" Set class=0
         ...
         If data'="" Do ..namespaceFilters.Insert(data)
         }
       ...
       Quit:('sc)  Set ref=loopref
         ...
         If $listget($get(@(tree)@(ref,0)),1)'="e" Goto XMLImportMalformedNoTag
         If 'encoded,(@(tree)@(ref)'="namespaceFilters") Goto XMLImportBadTag
         If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
         If encoded,$$XMLImportId() {
           Set data=idlist(ref)
         } Else {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..namespaceFilters.Count()>0 Quit 0
 If ..subsetFilters.Count()>0 Quit 0
 If ..sql'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TSubsetExpression).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TSubsetExpression",top,format,namespacePrefix,input,refOnly,.schema)
znamespaceFiltersBuildValueArra(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
znamespaceFiltersCollectionToDi(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
znamespaceFiltersCollectionToOd(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
	...
	quit $extract(odbc,2,*) }
znamespaceFiltersDisplayToColle(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
znamespaceFiltersGetObject(force=0) public {
	Quit $select(r%namespaceFilters="":$select(i%namespaceFilters="":"",1:$listbuild(i%namespaceFilters_"")),(''..namespaceFilters.%GetSwizzleObject(force,.oid)):oid,1:"") }
znamespaceFiltersGetObjectId(force=0) public {
	Quit $listget(..namespaceFiltersGetObject(force)) }
znamespaceFiltersGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%namespaceFilters=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.SENamespaceFilter",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."namespaceFilters%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
znamespaceFiltersOdbcToCollecti(val="") public {
	set logical="",out=""
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
znamespaceFiltersSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%namespaceFilters=newvalue Quit 1
	If newvalue="" Kill i%namespaceFilters,r%namespaceFilters Set i%namespaceFilters="",r%namespaceFilters="" Quit 1
	Set oref=r%namespaceFilters Kill i%namespaceFilters,r%namespaceFilters Set i%namespaceFilters="",r%namespaceFilters=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%namespaceFilters(i)=value,i%namespaceFilters(i)=""
	Quit 1 }
