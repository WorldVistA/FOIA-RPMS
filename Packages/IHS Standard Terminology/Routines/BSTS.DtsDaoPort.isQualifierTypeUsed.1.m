 ;BSTS.DtsDaoPort.isQualifierTypeUsed.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.isQualifierTypeUsed.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;704B4574;BSTS.DtsDaoPort.isQualifierTypeUsed
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,type) public {
 Set ..type=$get(type)
 Do %Client.InvokeClient($this,"isQualifierTypeUsed",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="isQualifierTypeUsedResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.isQualifierTypeUsed",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.isQualifierTypeUsed")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.isQualifierTypeUsed")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.isQualifierTypeUsed"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isQualifierTypeUsed"
   ...
   If tag="" Set tag=typesPrefix_"isQualifierTypeUsed"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isQualifierTypeUsed"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="type",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TQualifierType"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.isQualifierTypeUsed",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.isQualifierTypeUsed")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="isQualifierTypeUsedResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.isQualifierTypeUsed).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.isQualifierTypeUsed",top,format,namespacePrefix,input,refOnly,.schema)
