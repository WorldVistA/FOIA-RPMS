 ;BSTS.DtsQueryDaoPort.updateSubsetVersionProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateSubsetVersionProperty.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;46437A55;BSTS.DtsQueryDaoPort.updateSubsetVersionProperty
 ;
...
zInvoke(%Client,%Action,version,oldProp,newProp) public {
 ...
 Do %Client.InvokeClient($this,"updateSubsetVersionProperty",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateSubsetVersionPropertyResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateSubsetVersionProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubsetVersionProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateSubsetVersionProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateSubsetVersionProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateSubsetVersionProperty"
   ...
   If tag="" Set tag=typesPrefix_"updateSubsetVersionProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateSubsetVersionProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateSubsetVersionProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubsetVersionProperty")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateSubsetVersionPropertyResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateSubsetVersionProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateSubsetVersionProperty",top,format,namespacePrefix,input,refOnly,.schema)
