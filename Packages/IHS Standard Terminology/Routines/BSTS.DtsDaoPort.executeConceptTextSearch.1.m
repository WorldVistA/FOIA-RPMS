 ;BSTS.DtsDaoPort.executeConceptTextSearch.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.executeConceptTextSearch.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;4C49302B;BSTS.DtsDaoPort.executeConceptTextSearch
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..casd)=1 Set r%casd=r%casd.%ConstructClone(1,.cloned),i%casd=""
	}
	Quit 1 }
%Destruct() public {
	Try {
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%casd If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,sql,casd) public {
 Set ..sql=$get(sql),..casd=$get(casd)
 Do %Client.InvokeClient($this,"executeConceptTextSearch",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.executeConceptTextSearch",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.executeConceptTextSearch")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.executeConceptTextSearch")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.executeConceptTextSearch"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="executeConceptTextSearch"
   ...
   If tag="" Set tag=typesPrefix_"executeConceptTextSearch"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"executeConceptTextSearch"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="casd",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.executeConceptTextSearch",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.executeConceptTextSearch")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="return" {
   ...
   If (class="") || (class="TConceptSearchResult") {
     Set class="BSTS.ns1.TConceptSearchResult"
   } Else {
     If $length(class,":")=2 Set class=$piece(class,":",2)
     Set class=$get(^oddCOM("BSTS.ns1.TConceptSearchResult",85,"t",class))_$get(^oddXML("BSTS.ns1.TConceptSearchResult","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..sql'="" Quit 0
 If $IsObject(..casd) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.executeConceptTextSearch).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.executeConceptTextSearch",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TConceptSearchResult",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
