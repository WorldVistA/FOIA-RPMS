 ;BSTS.DtsDaoPort.addAuthority.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.addAuthority.  Do NOT edit. 08/21/2017 07:46:09PM
 ;;3957454E;BSTS.DtsDaoPort.addAuthority
 ;
...
zInvoke(%Client,%Action,arg0) public {
 Set ..arg0=$get(arg0)
 Do %Client.InvokeClient($this,"addAuthority",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="addAuthorityResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.addAuthority",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.addAuthority")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.addAuthority")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.addAuthority"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"addAuthority"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"addAuthority"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.addAuthority",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.addAuthority")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.addAuthority).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.addAuthority",top,format,namespacePrefix,input,refOnly,.schema)
