 ;BSTS.ns1.TRole.1
 ;(C)InterSystems, generated for class BSTS.ns1.TRole.  Do NOT edit. 08/21/2017 07:46:00PM
 ;;526A6D75;BSTS.ns1.TRole
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..roleType)=1 Set r%roleType=r%roleType.%ConstructClone(1,.cloned),i%roleType=""
		If $isobject(..valueConcept)=1 Set r%valueConcept=r%valueConcept.%ConstructClone(1,.cloned),i%valueConcept=""
	}
	Quit 1 }
%NormalizeObject() public {
	If '$system.CLS.GetModified() Quit 1
	If m%groupNum Set:i%groupNum'="" i%groupNum=(..groupNumNormalize(i%groupNum))
	...
	If m%roleModifier Set:i%roleModifier'="" i%roleModifier=(..roleModifierNormalize(i%roleModifier))
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%roleType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%valueConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	Set sc=1
	If '$system.CLS.GetModified() Quit sc
	If m%groupNum Set iv=..groupNum If iv'="" Set rc=(..groupNumIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"groupNum",iv)
	...
	Set iv=..roleModifier If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"roleModifier"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%roleModifier Set rc=(..roleModifierIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"roleModifier",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TRole",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TRole")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TRole")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TRole"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"TRole"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TRole"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..roleModifier
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.TRoleModifier","NAMESPACE")
   ...
   Set %xmlmsg=currentIndent_beginprefix_"roleModifier"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TRoleModifier"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"roleModifier>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..roleType
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"roleType "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="roleType",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TRoleType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..valueConcept
 If $IsObject(val) , deepFlag {
   Set id=""
   If encoded,'inlineFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"valueConcept "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="valueConcept",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
   }
 }
 Set val=..groupNum
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"groupNum"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"groupNum>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..revisionIn
 If val'="" {
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TRole" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TRole",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TRole")
 If encoded {
 ...
 If tag="roleModifier" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             If data'="" Goto:('$s(data'[","&&(",1,2,3,4,5,6,7,8,9,"[(","_$select(data=$c(0):"",1:data)_",")):1,1:$$Error^%apiOBJ(7205,data,",1,2,3,4,5,6,7,8,9"))) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..roleModifier=data
...
zXMLIsObjectEmpty(ignoreNull)
 If ..roleModifier'="" Quit 0
 If $IsObject(..roleType) Quit 0
 If $IsObject(..valueConcept) Quit 0
 If ..groupNum'="" Quit 0
 If ..revisionIn'="" Quit 0
 If ..revisionOut'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TRole).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TRole",top,format,namespacePrefix,input,refOnly,.schema)
zgroupNumDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zgroupNumIsValid(%val) public {
...
zgroupNumNormalize(%val) public {
	Quit $select($zu(115,13)&&(%val=$c(0)):"",1:%val\1) }
zgroupNumXSDToLogical(%val) public {
...
zrevisionOutXSDToLogical(%val)
 ...
 If $zdatetimeh(dt,3,1,,,,,,,"")="" Quit ""
 Quit dt
zroleModifierIsValid(%val) public {
	Q $s(%val'[","&&(",1,2,3,4,5,6,7,8,9,"[(","_$select(%val=$c(0):"",1:%val)_",")):1,1:$$Error^%apiOBJ(7205,%val,",1,2,3,4,5,6,7,8,9")) }
zroleTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TRoleType).%New() If newobject="" Quit ""
	Set ..roleType=newobject
	Quit newobject }
zvalueConceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..valueConcept=newobject
	Quit newobject }
