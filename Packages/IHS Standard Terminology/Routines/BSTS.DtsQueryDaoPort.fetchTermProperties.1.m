 ;BSTS.DtsQueryDaoPort.fetchTermProperties.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchTermProperties.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;42347935;BSTS.DtsQueryDaoPort.fetchTermProperties
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,term) public {
 Set ..term=$get(term)
 Do %Client.InvokeClient($this,"fetchTermProperties",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="fetchTermPropertiesResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchTermProperties",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchTermProperties")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchTermProperties")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchTermProperties"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchTermProperties"
   ...
   If tag="" Set tag=typesPrefix_"fetchTermProperties"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchTermProperties"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchTermProperties",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchTermProperties")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="fetchTermPropertiesResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchTermProperties).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchTermProperties",top,format,namespacePrefix,input,refOnly,.schema)
