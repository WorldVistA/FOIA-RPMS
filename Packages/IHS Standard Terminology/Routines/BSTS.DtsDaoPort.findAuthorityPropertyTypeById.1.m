 ;BSTS.DtsDaoPort.findAuthorityPropertyTypeById.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findAuthorityPropertyTypeById.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;44696B78;BSTS.DtsDaoPort.findAuthorityPropertyTypeById
 ;
...
zInvoke(%Client,%Action,id,authorityId) public {
 ...
 Do %Client.InvokeClient($this,"findAuthorityPropertyTypeById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findAuthorityPropertyTypeByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findAuthorityPropertyTypeById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityPropertyTypeById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findAuthorityPropertyTypeById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findAuthorityPropertyTypeById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findAuthorityPropertyTypeById"
   ...
   If tag="" Set tag=typesPrefix_"findAuthorityPropertyTypeById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findAuthorityPropertyTypeById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findAuthorityPropertyTypeById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityPropertyTypeById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findAuthorityPropertyTypeByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findAuthorityPropertyTypeById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findAuthorityPropertyTypeById",top,format,namespacePrefix,input,refOnly,.schema)
