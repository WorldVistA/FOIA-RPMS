 ;BSTS.DtsQueryDaoPort.getAllLicenses.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getAllLicenses.  Do NOT edit. 08/21/2017 07:46:17PM
 ;;5233354C;BSTS.DtsQueryDaoPort.getAllLicenses
 ;
...
%NormalizeObject() public {
	...
	If m%condition Set:i%condition'="" i%condition=(..conditionNormalize(i%condition))
	Quit 1 }
%ValidateObject(force=0) public {
	...
	If m%condition Set iv=..condition If iv'="" Set rc=(..conditionIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"condition",iv)
	Quit sc }
zInvoke(%Client,%Action,condition) public {
 Set ..condition=$get(condition)
 Do %Client.InvokeClient($this,"getAllLicenses",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getAllLicenses",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getAllLicenses")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getAllLicenses")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getAllLicenses"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"getAllLicenses"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getAllLicenses"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..condition
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"condition"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"condition>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getAllLicenses",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getAllLicenses")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getAllLicenses).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getAllLicenses",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	...
	for i=1:1:$listlength(val) {
		set item=##class(BSTS.DtsQueryDaoPort.getAllLicenses).%ResultLogicalToDisplay($listget(val,i))
...
%ResultCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=##class(BSTS.DtsQueryDaoPort.getAllLicenses).%ResultLogicalToOdbc($listget(val,i))
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.getAllLicenses).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.getAllLicenses:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
