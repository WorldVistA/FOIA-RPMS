 ;BSTS.DtsDaoPort.deleteAuthorityByName.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.deleteAuthorityByName.  Do NOT edit. 08/21/2017 07:46:09PM
 ;;38797464;BSTS.DtsDaoPort.deleteAuthorityByName
 ;
...
zInvoke(%Client,%Action,arg0) public {
 Set ..arg0=$get(arg0)
 Do %Client.InvokeClient($this,"deleteAuthorityByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteAuthorityByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.deleteAuthorityByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteAuthorityByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.deleteAuthorityByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.deleteAuthorityByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteAuthorityByName"
   ...
   If tag="" Set tag=typesPrefix_"deleteAuthorityByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteAuthorityByName"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.deleteAuthorityByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteAuthorityByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteAuthorityByNameResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.deleteAuthorityByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.deleteAuthorityByName",top,format,namespacePrefix,input,refOnly,.schema)
