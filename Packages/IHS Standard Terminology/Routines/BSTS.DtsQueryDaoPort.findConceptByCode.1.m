 ;BSTS.DtsQueryDaoPort.findConceptByCode.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findConceptByCode.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;65557873;BSTS.DtsQueryDaoPort.findConceptByCode
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,conceptCode,namespaceId,asd) public {
 Set ..conceptCode=$get(conceptCode),..namespaceId=$get(namespaceId),..asd=$get(asd)
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findConceptByCode",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptByCode")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findConceptByCode")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findConceptByCode"))
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findConceptByCode",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptByCode")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findConceptByCode).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findConceptByCode",top,format,namespacePrefix,input,refOnly,.schema)
