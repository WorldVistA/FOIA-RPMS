 ;BSTS.DtsDaoPort.findAuthorityQualifierTypeByName.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findAuthorityQualifierTypeByName.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;742F5A59;BSTS.DtsDaoPort.findAuthorityQualifierTypeByName
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,name,authorityId) public {
 Set ..name=$get(name),..authorityId=$get(authorityId)
 Do %Client.InvokeClient($this,"findAuthorityQualifierTypeByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findAuthorityQualifierTypeByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findAuthorityQualifierTypeByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityQualifierTypeByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findAuthorityQualifierTypeByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findAuthorityQualifierTypeByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findAuthorityQualifierTypeByName"
   ...
   If tag="" Set tag=typesPrefix_"findAuthorityQualifierTypeByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findAuthorityQualifierTypeByName"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findAuthorityQualifierTypeByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityQualifierTypeByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findAuthorityQualifierTypeByNameResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findAuthorityQualifierTypeByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findAuthorityQualifierTypeByName",top,format,namespacePrefix,input,refOnly,.schema)
