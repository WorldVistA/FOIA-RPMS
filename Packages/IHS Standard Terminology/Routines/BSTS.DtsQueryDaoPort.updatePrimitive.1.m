 ;BSTS.DtsQueryDaoPort.updatePrimitive.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updatePrimitive.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;66544271;BSTS.DtsQueryDaoPort.updatePrimitive
 ;
...
%ValidateObject(force=0) public {
	...
	If m%primitive Set iv=..primitive If iv'="" Set rc=(..primitiveIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"primitive",iv)
	Quit sc }
zInvoke(%Client,%Action,concept,primitive) public {
 Set ..concept=$get(concept),..primitive=$get(primitive)
 Do %Client.InvokeClient($this,"updatePrimitive",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updatePrimitiveResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updatePrimitive",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updatePrimitive")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updatePrimitive")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updatePrimitive"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updatePrimitive"
   ...
   If tag="" Set tag=typesPrefix_"updatePrimitive"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updatePrimitive"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updatePrimitive",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updatePrimitive")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updatePrimitive).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updatePrimitive",top,format,namespacePrefix,input,refOnly,.schema)
