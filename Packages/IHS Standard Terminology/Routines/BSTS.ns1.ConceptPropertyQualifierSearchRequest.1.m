 ;BSTS.ns1.ConceptPropertyQualifierSearchRequest.1
 ;(C)InterSystems, generated for class BSTS.ns1.ConceptPropertyQualifierSearchRequest.  Do NOT edit. 08/21/2017 07:45:57PM
 ;;51396A38;BSTS.ns1.ConceptPropertyQualifierSearchRequest
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..propType)=1 Set r%propType=r%propType.%ConstructClone(1,.cloned),i%propType=""
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%propType If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
...
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.ConceptPropertyQualifierSearchRequest",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.ConceptPropertyQualifierSearchRequest")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.ConceptPropertyQualifierSearchRequest")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.ConceptPropertyQualifierSearchRequest"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="ConceptPropertyQualifierSearchRequest"
   ...
   If tag="" Set tag=typesPrefix_"ConceptPropertyQualifierSearchRequest"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"ConceptPropertyQualifierSearchRequest"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
   ...
   If +id'=0 {
     Set %xmlmsg=currentIndent_beginprefix_"propType "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   } Else { if id=0 Set id=$increment(idlist)
     Set topArg="propType",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TPropertyType"),local=1,savexsiAttrs=xsiAttrs
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.ConceptPropertyQualifierSearchRequest" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.ConceptPropertyQualifierSearchRequest",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.ConceptPropertyQualifierSearchRequest")
 If encoded {
...
zXMLIsObjectEmpty(ignoreNull)
 If $IsObject(..propType) Quit 0
 If $IsObject(..qualType) Quit 0
 If ..pattern'="" Quit 0
 If $IsObject(..options) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.ConceptPropertyQualifierSearchRequest).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.ConceptPropertyQualifierSearchRequest",top,format,namespacePrefix,input,refOnly,.schema)
zoptionsNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptSearchOptions).%New() If newobject="" Quit ""
	Set ..options=newobject
	Quit newobject }
zpropTypeNewObject() public {
	Set newobject=##class(BSTS.ns1.TPropertyType).%New() If newobject="" Quit ""
	Set ..propType=newobject
	Quit newobject }
