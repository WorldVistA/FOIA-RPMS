 ;BSTS.DtsQueryDaoPort.executeSQLUpdates.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.executeSQLUpdates.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;79783638;BSTS.DtsQueryDaoPort.executeSQLUpdates
 ;
...
%Construct(operation) public {
	Set m%%Result=0,M%%Result=0,m%sqlItems=0,M%sqlItems=0
	Quit ..%OnNew(.operation) }
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%%Result="",r%%Result=""
	Set i%sqlItems="",r%sqlItems=""
	Set i%"%%OID"=""
	If deep>0 {
		Set key="" For  Set value=..sqlItems.GetNext(.key) Quit:key=""  Set r%sqlItems(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%sqlItems(key)=""
	}
	Quit 1 }
%Destruct() public {
	Try {
		If $isobject($get(r%%Result))=1,$zobjcnt(r%%Result)>1 Do r%%Result.%Disconnect()
		If $isobject($get(r%sqlItems))=1,$zobjcnt(r%sqlItems)>1 Do r%sqlItems.%Disconnect()
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set key=""
	For  {
		Set key=$order(r%sqlItems(key),1,Poref) Quit:key=""
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,sql,sqlItems) public {
 Set ..sql=$get(sql),..sqlItems=$get(sqlItems)
 Do %Client.InvokeClient($this,"executeSQLUpdates",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.executeSQLUpdates",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeSQLUpdates")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.executeSQLUpdates")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.executeSQLUpdates"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="executeSQLUpdates"
   ...
   If tag="" Set tag=typesPrefix_"executeSQLUpdates"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"executeSQLUpdates"""_xsiAttrs,xsiAttrs=""
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   While k'="" {
     If $IsObject(val) {
         Set topArg="sqlItems",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.SqlItem"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"sqlItems "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.executeSQLUpdates",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.executeSQLUpdates")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..sql'="" Quit 0
 If ..sqlItems.Count()>0 Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.executeSQLUpdates).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.executeSQLUpdates",top,format,namespacePrefix,input,refOnly,.schema)
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.executeSQLUpdates).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.executeSQLUpdates:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
...
zsqlItemsBuildValueArray(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zsqlItemsCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	set odbc="",tDelimLen=$l(delim)
	for i=1:1:$listlength(val) {
		set item=$Listget(val,i)
		if item'["""",item'[delim,$length($zstrip(item,"<>W"))=$length(item) set odbc=odbc_delim_item continue
	...
	quit $extract(odbc,tDelimLen+1,*) }
zsqlItemsCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
		if item'["""",item'[",",$length($zstrip(item,"<>W"))=$length(item) set odbc=odbc_","_item continue
	...
	quit $extract(odbc,2,*) }
zsqlItemsDisplayToCollection(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	set logical="",out="",tDelimLen=$l(delim)
hloop	if $extract(val,1,tDelimLen)=delim { goto hdel } if val="" { goto hexit } if $extract(val)="""" { goto hquote } goto hdefault
hdefault	set stuff=$piece(val,delim),out=out_stuff,val=$extract(val,$length(stuff)+1,*) goto hloop
hquote	set stuff=$find(val,"""",2) if stuff=0 set stuff=$length(val)+10
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zsqlItemsGetObject(force=0) public {
	Quit $select(r%sqlItems="":$select(i%sqlItems="":"",1:$listbuild(i%sqlItems_"")),(''..sqlItems.%GetSwizzleObject(force,.oid)):oid,1:"") }
zsqlItemsGetObjectId(force=0) public {
	Quit $listget(..sqlItemsGetObject(force)) }
zsqlItemsGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%sqlItems=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.SqlItem",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."sqlItems%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zsqlItemsOdbcToCollection(val="") public {
	set logical="",out=""
hloop	if $extract(val)="," { goto hdel } if val="" { goto hexit } if $extract(val)="""" { goto hquote } goto hdefault
hdefault	set stuff=$piece(val,","),out=out_stuff,val=$extract(val,$length(stuff)+1,*) goto hloop
hquote	set stuff=$find(val,"""",2) if stuff=0 set stuff=$length(val)+10
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zsqlItemsSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%sqlItems=newvalue Quit 1
	If newvalue="" Kill i%sqlItems,r%sqlItems Set i%sqlItems="",r%sqlItems="" Quit 1
	Set oref=r%sqlItems Kill i%sqlItems,r%sqlItems Set i%sqlItems="",r%sqlItems=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%sqlItems(i)=value,i%sqlItems(i)=""
	Quit 1 }
