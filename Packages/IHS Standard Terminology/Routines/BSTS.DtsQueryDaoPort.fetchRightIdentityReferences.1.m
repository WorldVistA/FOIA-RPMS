 ;BSTS.DtsQueryDaoPort.fetchRightIdentityReferences.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchRightIdentityReferences.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;745A4B41;BSTS.DtsQueryDaoPort.fetchRightIdentityReferences
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,roleType) public {
 Set ..roleType=$get(roleType)
 Do %Client.InvokeClient($this,"fetchRightIdentityReferences",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchRightIdentityReferences",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchRightIdentityReferences")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchRightIdentityReferences")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchRightIdentityReferences"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchRightIdentityReferences"
   ...
   If tag="" Set tag=typesPrefix_"fetchRightIdentityReferences"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchRightIdentityReferences"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchRightIdentityReferences",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchRightIdentityReferences")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="return" {
   ...
   If (class="") || (class="TRoleType") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TRoleType",85,"t",class))_$get(^oddXML("BSTS.ns1.TRoleType","t",class)) If class="" Set class=0
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchRightIdentityReferences).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchRightIdentityReferences",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TRoleType",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
