 ;BSTS.DtsQueryDaoPort.deleteSubsetVersionByName.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteSubsetVersionByName.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;5656792B;BSTS.DtsQueryDaoPort.deleteSubsetVersionByName
 ;
...
%NormalizeObject() public {
	...
	If m%versionName Set:i%versionName'="" i%versionName=(..versionNameNormalize(i%versionName))
	Quit 1 }
%ValidateObject(force=0) public {
	...
	If m%versionName Set iv=..versionName If iv'="" Set rc=(..versionNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"versionName",iv)
	Quit sc }
zInvoke(%Client,%Action,versionName,subsetId) public {
 Set ..versionName=$get(versionName),..subsetId=$get(subsetId)
 Do %Client.InvokeClient($this,"deleteSubsetVersionByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteSubsetVersionByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteSubsetVersionByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteSubsetVersionByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteSubsetVersionByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteSubsetVersionByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteSubsetVersionByName"
   ...
   If tag="" Set tag=typesPrefix_"deleteSubsetVersionByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteSubsetVersionByName"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..versionName
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"versionName"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"versionName>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..subsetId
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteSubsetVersionByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteSubsetVersionByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteSubsetVersionByNameResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..%Result'="" Quit 0
 If ..versionName'="" Quit 0
 If ..subsetId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteSubsetVersionByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteSubsetVersionByName",top,format,namespacePrefix,input,refOnly,.schema)
