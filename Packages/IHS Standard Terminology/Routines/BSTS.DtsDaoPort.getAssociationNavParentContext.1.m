 ;BSTS.DtsDaoPort.getAssociationNavParentContext.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getAssociationNavParentContext.  Do NOT edit. 08/21/2017 07:46:11PM
 ;;41503352;BSTS.DtsDaoPort.getAssociationNavParentContext
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,focusConcept,attributeSetDescriptor,namespace) public {
 Set ..focusConcept=$get(focusConcept),..attributeSetDescriptor=$get(attributeSetDescriptor),..namespace=$get(namespace)
 Do %Client.InvokeClient($this,"getAssociationNavParentContext",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getAssociationNavParentContextResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getAssociationNavParentContext",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getAssociationNavParentContext")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getAssociationNavParentContext")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getAssociationNavParentContext"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getAssociationNavParentContext"
   ...
   If tag="" Set tag=typesPrefix_"getAssociationNavParentContext"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getAssociationNavParentContext"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..namespace
 If $IsObject(val) , deepFlag {
     Set topArg="namespace",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TAssociationType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getAssociationNavParentContext",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getAssociationNavParentContext")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="getAssociationNavParentContextResult" {
   ...
   If (class="") || (class="TNavParentContext") {
     Set class="BSTS.ns1.TNavParentContext"
   } Else {
     If $length(class,":")=2 Set class=$piece(class,":",2)
     Set class=$get(^oddCOM("BSTS.ns1.TNavParentContext",85,"t",class))_$get(^oddXML("BSTS.ns1.TNavParentContext","t",class)) If class="" Set class=0
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getAssociationNavParentContext).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getAssociationNavParentContext",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TNavParentContext).%New() If newobject="" Quit ""
