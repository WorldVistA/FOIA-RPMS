 ;BSTS.DtsDaoPort.getVersions.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.getVersions.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;46416270;BSTS.DtsDaoPort.getVersions
 ;
...
zInvoke(%Client,%Action,namespaceId) public {
 ...
 Do %Client.InvokeClient($this,"getVersions",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.getVersions",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getVersions")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.getVersions")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.getVersions"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"getVersions"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getVersions"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.getVersions",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.getVersions")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.getVersions).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.getVersions",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TContentVersion",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
