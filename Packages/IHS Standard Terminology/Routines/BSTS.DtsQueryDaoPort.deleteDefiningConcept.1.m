 ;BSTS.DtsQueryDaoPort.deleteDefiningConcept.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteDefiningConcept.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;704F6671;BSTS.DtsQueryDaoPort.deleteDefiningConcept
 ;
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..definingConcept)=1 Set r%definingConcept=r%definingConcept.%ConstructClone(1,.cloned),i%definingConcept=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%definingConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,concept,definingConcept) public {
 Set ..concept=$get(concept),..definingConcept=$get(definingConcept)
 Do %Client.InvokeClient($this,"deleteDefiningConcept",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteDefiningConceptResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteDefiningConcept",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteDefiningConcept")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteDefiningConcept")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteDefiningConcept"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteDefiningConcept"
   ...
   If tag="" Set tag=typesPrefix_"deleteDefiningConcept"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteDefiningConcept"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..definingConcept
 If $IsObject(val) , deepFlag {
     Set topArg="definingConcept",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteDefiningConcept",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteDefiningConcept")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteDefiningConceptResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..definingConcept) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteDefiningConcept).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteDefiningConcept",top,format,namespacePrefix,input,refOnly,.schema)
...
zconceptNewObject() public {
	...
	Quit newobject }
zdefiningConceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..definingConcept=newobject
	Quit newobject }
