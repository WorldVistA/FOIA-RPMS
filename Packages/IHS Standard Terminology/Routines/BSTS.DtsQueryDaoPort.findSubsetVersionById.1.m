 ;BSTS.DtsQueryDaoPort.findSubsetVersionById.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findSubsetVersionById.  Do NOT edit. 08/21/2017 07:46:16PM
 ;;44344B30;BSTS.DtsQueryDaoPort.findSubsetVersionById
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,id,subsetId,asd) public {
 Set ..id=$get(id),..subsetId=$get(subsetId),..asd=$get(asd)
 Do %Client.InvokeClient($this,"findSubsetVersionById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findSubsetVersionByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findSubsetVersionById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findSubsetVersionById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findSubsetVersionById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findSubsetVersionById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findSubsetVersionById"
   ...
   If tag="" Set tag=typesPrefix_"findSubsetVersionById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findSubsetVersionById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findSubsetVersionById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findSubsetVersionById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findSubsetVersionByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findSubsetVersionById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findSubsetVersionById",top,format,namespacePrefix,input,refOnly,.schema)
