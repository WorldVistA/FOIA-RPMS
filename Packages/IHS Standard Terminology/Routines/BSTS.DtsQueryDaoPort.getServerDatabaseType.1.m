 ;BSTS.DtsQueryDaoPort.getServerDatabaseType.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.getServerDatabaseType.  Do NOT edit. 08/21/2017 07:46:18PM
 ;;6F393654;BSTS.DtsQueryDaoPort.getServerDatabaseType
 ;
...
zInvoke(%Client,%Action) public {
 Do %Client.InvokeClient($this,"getServerDatabaseType",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="getServerDatabaseTypeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.getServerDatabaseType",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getServerDatabaseType")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.getServerDatabaseType")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.getServerDatabaseType"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="getServerDatabaseType"
   ...
   If tag="" Set tag=typesPrefix_"getServerDatabaseType"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"getServerDatabaseType"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.getServerDatabaseType",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.getServerDatabaseType")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="getServerDatabaseTypeResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.getServerDatabaseType).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.getServerDatabaseType",top,format,namespacePrefix,input,refOnly,.schema)
