 ;BSTS.DtsDaoPort.findAuthorityPropertyTypeByName.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findAuthorityPropertyTypeByName.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;4876616B;BSTS.DtsDaoPort.findAuthorityPropertyTypeByName
 ;
...
zInvoke(%Client,%Action,name,authorityId) public {
 ...
 Do %Client.InvokeClient($this,"findAuthorityPropertyTypeByName",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findAuthorityPropertyTypeByNameResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findAuthorityPropertyTypeByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityPropertyTypeByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findAuthorityPropertyTypeByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findAuthorityPropertyTypeByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findAuthorityPropertyTypeByName"
   ...
   If tag="" Set tag=typesPrefix_"findAuthorityPropertyTypeByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findAuthorityPropertyTypeByName"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findAuthorityPropertyTypeByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityPropertyTypeByName")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findAuthorityPropertyTypeByNameResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findAuthorityPropertyTypeByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findAuthorityPropertyTypeByName",top,format,namespacePrefix,input,refOnly,.schema)
