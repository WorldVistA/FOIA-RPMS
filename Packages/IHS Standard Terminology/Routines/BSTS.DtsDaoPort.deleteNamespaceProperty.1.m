 ;BSTS.DtsDaoPort.deleteNamespaceProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.deleteNamespaceProperty.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;516B456D;BSTS.DtsDaoPort.deleteNamespaceProperty
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceId,property) public {
 Set ..namespaceId=$get(namespaceId),..property=$get(property)
 Do %Client.InvokeClient($this,"deleteNamespaceProperty",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteNamespacePropertyResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.deleteNamespaceProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteNamespaceProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.deleteNamespaceProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.deleteNamespaceProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteNamespaceProperty"
   ...
   If tag="" Set tag=typesPrefix_"deleteNamespaceProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteNamespaceProperty"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.deleteNamespaceProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.deleteNamespaceProperty")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteNamespacePropertyResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.deleteNamespaceProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.deleteNamespaceProperty",top,format,namespacePrefix,input,refOnly,.schema)
