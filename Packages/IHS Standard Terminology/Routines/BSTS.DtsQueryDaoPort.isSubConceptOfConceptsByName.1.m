 ;BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName.  Do NOT edit. 08/21/2017 07:46:19PM
 ;;65754449;BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName
 ;
...
%NormalizeObject() public {
	...
	If m%conceptName Set:i%conceptName'="" i%conceptName=(..conceptNameNormalize(i%conceptName))
	If m%concepts Set key="" For  Set key=$order(i%concepts(key),1,data) Quit:key=""  Set:data'="" i%concepts(key)=..conceptsNormalize(data)
...
%ValidateObject(force=0) public {
	...
	If m%conceptName Set iv=..conceptName If iv'="" Set rc=(..conceptNameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptName",iv)
	If m%concepts Set key="",rc=1 For  Set key=$order(i%concepts(key),1,val) Quit:key=""  If val'="" Set rc=..conceptsIsValid(val) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"concepts"_"("_key_")",val)
	...
	Quit sc }
zInvoke(%Client,%Action,conceptName,concepts,namespaceId) public {
 Set ..conceptName=$get(conceptName),..concepts=$get(concepts),..namespaceId=$get(namespaceId)
 Do %Client.InvokeClient($this,"isSubConceptOfConceptsByName",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isSubConceptOfConceptsByName"
   ...
   If tag="" Set tag=typesPrefix_"isSubConceptOfConceptsByName"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isSubConceptOfConceptsByName"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptName
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptName"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"conceptName>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set aval=..concepts
 Set k="" Set val=aval.GetNext(.k) If k'="" {
   While k'="" {
     If val'="" {
       Set %xmlmsg=currentIndent_beginprefix_"concepts"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"string""",1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"concepts>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName")
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..conceptName'="" Quit 0
 If ..concepts.Count()>0 Quit 0
 If ..namespaceId'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName",top,format,namespacePrefix,input,refOnly,.schema)
...
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName).%ResultDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName:%Result",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
...
zconceptsCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	set odbc="",tDelimLen=$l(delim)
	for i=1:1:$listlength(val) {
		set item=##class(BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName).conceptsLogicalToDisplay($listget(val,i))
		if item'["""",item'[delim,$length($zstrip(item,"<>W"))=$length(item) set odbc=odbc_delim_item continue
		for j=$length(item,""""):-1:2 set $piece(item,"""",j)=""""_$piece(item,"""",j)
		set odbc=odbc_delim_""""_item_""""
	}
...
zconceptsCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=##class(BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName).conceptsLogicalToOdbc($listget(val,i))
		if item'["""",item'[",",$length($zstrip(item,"<>W"))=$length(item) set odbc=odbc_","_item continue
		for j=$length(item,""""):-1:2 set $piece(item,"""",j)=""""_$piece(item,"""",j)
		set odbc=odbc_","""_item_""""
	}
...
hdel	set logical=logical_$listbuild(out),out="",val=$extract(val,tDelimLen+1,*) goto hloop
hexit	set logical=logical_$listbuild(out)
	for stuff=1:1:$Listlength(logical) { set $List(logical,stuff) = ##class(BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName).conceptsDisplayToLogical($List(logical,stuff)) }
	quit logical }
...
zconceptsGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.DtsQueryDaoPort.isSubConceptOfConceptsByName:concepts",oref.ElementClassType="datatype",oref.Owner=+$this,oref.Storage=$this."concepts%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
...
zconceptsSet(newvalue) public {
	...
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set i%concepts(i)=value
	Quit 1 }
