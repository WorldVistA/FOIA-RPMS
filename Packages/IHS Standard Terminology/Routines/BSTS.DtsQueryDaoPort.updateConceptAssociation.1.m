 ;BSTS.DtsQueryDaoPort.updateConceptAssociation.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateConceptAssociation.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;4D366F63;BSTS.DtsQueryDaoPort.updateConceptAssociation
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   ...
   s sc = 1
 s proporef=..%Result
 s proporef=..newConceptAssociation
 s proporef=..oldConceptAssociation
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	If deep>0 {
		...
		If $isobject(..newConceptAssociation)=1 Set r%newConceptAssociation=r%newConceptAssociation.%ConstructClone(1,.cloned),i%newConceptAssociation=""
		If $isobject(..oldConceptAssociation)=1 Set r%oldConceptAssociation=r%oldConceptAssociation.%ConstructClone(1,.cloned),i%oldConceptAssociation=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%newConceptAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%oldConceptAssociation If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
zInvoke(%Client,%Action,oldConceptAssociation,newConceptAssociation) public {
 Set ..oldConceptAssociation=$get(oldConceptAssociation),..newConceptAssociation=$get(newConceptAssociation)
 Do %Client.InvokeClient($this,"updateConceptAssociation",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateConceptAssociationResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateConceptAssociation",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateConceptAssociation")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateConceptAssociation")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateConceptAssociation"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateConceptAssociation"
   ...
   If tag="" Set tag=typesPrefix_"updateConceptAssociation"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateConceptAssociation"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..oldConceptAssociation
 If $IsObject(val) , deepFlag {
     Set topArg="oldConceptAssociation",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptAssociation"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..newConceptAssociation
 If $IsObject(val) , deepFlag {
     Set topArg="newConceptAssociation",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptAssociation"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateConceptAssociation",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateConceptAssociation")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateConceptAssociationResult" {
   ...
   If (class="") || (class="TConceptAssociation") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TConceptAssociation",85,"t",class))_$get(^oddXML("BSTS.ns1.TConceptAssociation","t",class)) If class="" Set class=0
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..oldConceptAssociation) Quit 0
 If $IsObject(..newConceptAssociation) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateConceptAssociation).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateConceptAssociation",top,format,namespacePrefix,input,refOnly,.schema)
%ResultNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptAssociation).%New() If newobject="" Quit ""
	Set ..%Result=newobject
	Quit newobject }
znewConceptAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptAssociation).%New() If newobject="" Quit ""
	Set ..newConceptAssociation=newobject
	Quit newobject }
zoldConceptAssociationNewObject() public {
	Set newobject=##class(BSTS.ns1.TConceptAssociation).%New() If newobject="" Quit ""
	Set ..oldConceptAssociation=newobject
	Quit newobject }
