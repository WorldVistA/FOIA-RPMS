 ;BSTS.ns1.TContentVersion.1
 ;(C)InterSystems, generated for class BSTS.ns1.TContentVersion.  Do NOT edit. 08/21/2017 07:46:00PM
 ;;38575148;BSTS.ns1.TContentVersion
 ;
...
%NormalizeObject() public {
	...
	If m%attachesTo Set:i%attachesTo'="" i%attachesTo=(..attachesToNormalize(i%attachesTo))
	...
	If m%releaseDate Set:i%releaseDate'="" i%releaseDate=(..releaseDateNormalize(i%releaseDate))
	If m%revisionDate Set:i%revisionDate'="" i%revisionDate=(..revisionDateNormalize(i%revisionDate))
	Quit 1 }
...
%ValidateObject(force=0) public {
	...
	If m%attachesTo Set iv=..attachesTo If iv'="" Set rc=(..attachesToIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"attachesTo",iv)
	...
	If m%releaseDate Set iv=..releaseDate If iv'="" Set rc=(..releaseDateIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"releaseDate",iv)
	If m%revisionDate Set iv=..revisionDate If iv'="" Set rc=(..revisionDateIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"revisionDate",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TContentVersion",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TContentVersion")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TContentVersion")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TContentVersion"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TContentVersion"
   ...
   If tag="" Set tag=typesPrefix_"TContentVersion"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TContentVersion"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..revisionDate
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"revisionDate"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"dateTime""",1:"")_">"_$select(val="":"",1:$translate(val," ","T")_"Z")_endprefix_"revisionDate>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..releaseDate
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"releaseDate"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"dateTime""",1:"")_">"_$select(val="":"",1:$translate(val," ","T")_"Z")_endprefix_"releaseDate>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..attachesTo
 If val'="" {
   Set temp="",temp1=$parameter("BSTS.ns1.TContentType","NAMESPACE")
   ...
   Set %xmlmsg=currentIndent_beginprefix_"attachesTo"_$select(xsitype:" "_xsiPrefix_"type="""_temp1_"TContentType"""_temp,1:"")_">"_$select(val=$c(0):"",1:$select((val["<")||(val[">")||(val["&"):"<![CDATA["_$replace(val,"]]>","]]]]><![CDATA[>")_"]]>",1:val))_endprefix_"attachesTo>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set aval=..properties
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TContentVersion" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TContentVersion",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TContentVersion")
 If encoded {
 ...
 If tag="revisionDate" {
   ...
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             Set data=$zstrip(data,"<>W",$c(13,10)) If data'="" Set data=..revisionDateXSDToLogical(data) Goto:data="" XMLImportErr Goto:('..revisionDateIsValid(data)) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..revisionDate=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="releaseDate" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             Set data=$zstrip(data,"<>W",$c(13,10)) If data'="" Set data=..releaseDateXSDToLogical(data) Goto:data="" XMLImportErr Goto:('..releaseDateIsValid(data)) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..releaseDate=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="attachesTo" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     ...
     If +$listget($get(@(tree)@(ref,0)),7,0) { Set data=""
             ...
             If data="" Set data=$c(0)
             If data'="" Goto:('$s(data'[","&&(",N,S,A,"[(","_$select(data=$c(0):"",1:data)_",")):1,1:$$Error^%apiOBJ(7205,data,",N,S,A"))) XMLImportErr
     }
     If encoded,($data(@(tree)@(ref,"a","id"))) Set idlist(ref)=data
   }
   Set ..attachesTo=data
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..id'="" Quit 0
 If ..name'="" Quit 0
 If ..revisionDate'="" Quit 0
 If ..releaseDate'="" Quit 0
 If ..attachesTo'="" Quit 0
 If ..properties.Count()>0 Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TContentVersion).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TContentVersion",top,format,namespacePrefix,input,refOnly,.schema)
zattachesToIsValid(%val) public {
...
zreleaseDateIsValid(%val)
	...
	Quit 1
zreleaseDateLogicalToXSD(%val)
	Quit $select(%val="":"",1:$translate(%val," ","T")_"Z")
zreleaseDateNormalize(%val)
...
zreleaseDateXSDToLogical(%val)
 ...
 Quit dt
zrevisionDateIsValid(%val)
	Quit:$zu(115,13)&&(%val=$c(0)) 1 New val Set val=%val,%val=$select(%val=(%val\1):$zd(%val,3,,,,,,,"error")_" 00:00:00",%val?1.2N1":"2N1":"2N.1(1"."1.N):$zd($h,3)_" "_%val,$length(%val)=10:%val_" 00:00:00",1:%val) Quit:($length(%val)<19||($zdth(%val,3,,,,,,,,"")="")) $$Error^%apiOBJ(7208,val)
	Quit 1
zrevisionDateLogicalToXSD(%val)
	Quit $select(%val="":"",1:$translate(%val," ","T")_"Z")
zrevisionDateNormalize(%val)
	Quit $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",%val=(%val\1):$zd(%val,3,,,,,,,"error")_" 00:00:00",%val?1.2N1":"2N1":"2N.1(1"."1.N):$zd($h,3)_" "_%val,$l(%val)=10:%val_" 00:00:00",$zdth(%val,3,,,,,,,,"")="":"error",1:$zdt($zdth($p(%val,"."),3,,,,,,,,""),3)_$s(+$p(%val,".",2)=0:"",1:+("."_$p(%val,".",2))))
zrevisionDateOdbcToLogical(%val)
 quit:%val="" "" if $zdth(%val,3,,,,,,,,"")'="" { s %val=$zdt($zdth(%val,3),3,,$l($p(%val,".",2))) } elseif $zdth(%val,-1,,,,,,,,"")'="" { s %val=$zdt($zdth(%val,-1),3,,$l($p(%val,".",2))) } quit $s(%val'[".":%val,1:$zstrip($zstrip(%val,">","0"),">","."))
	Quit
zrevisionDateXSDToLogical(%val)
 New len,dt,d,f,s,t,z
 If $length($get(%val),"T")'=2 Quit ""
 Set dt=$translate(%val,"T"," ")
 Set len=$length(%val)
