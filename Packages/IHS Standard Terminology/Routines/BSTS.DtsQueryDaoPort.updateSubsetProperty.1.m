 ;BSTS.DtsQueryDaoPort.updateSubsetProperty.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateSubsetProperty.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;5162494A;BSTS.DtsQueryDaoPort.updateSubsetProperty
 ;
...
exit	Quit sc }
zInvoke(%Client,%Action,subset,oldProp,newProp) public {
 Set ..subset=$get(subset),..oldProp=$get(oldProp),..newProp=$get(newProp)
 Do %Client.InvokeClient($this,"updateSubsetProperty",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateSubsetPropertyResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateSubsetProperty",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubsetProperty")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateSubsetProperty")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateSubsetProperty"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateSubsetProperty"
   ...
   If tag="" Set tag=typesPrefix_"updateSubsetProperty"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateSubsetProperty"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="subset",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TSubset"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateSubsetProperty",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateSubsetProperty")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateSubsetPropertyResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateSubsetProperty).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateSubsetProperty",top,format,namespacePrefix,input,refOnly,.schema)
