 ;BSTS.DtsQueryDaoPort.deleteNamespaceByCode.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.deleteNamespaceByCode.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;52664B75;BSTS.DtsQueryDaoPort.deleteNamespaceByCode
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,namespaceCode) public {
 Set ..namespaceCode=$get(namespaceCode)
 Do %Client.InvokeClient($this,"deleteNamespaceByCode",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="deleteNamespaceByCodeResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.deleteNamespaceByCode",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteNamespaceByCode")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.deleteNamespaceByCode")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.deleteNamespaceByCode"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="deleteNamespaceByCode"
   ...
   If tag="" Set tag=typesPrefix_"deleteNamespaceByCode"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"deleteNamespaceByCode"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.deleteNamespaceByCode",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.deleteNamespaceByCode")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="deleteNamespaceByCodeResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.deleteNamespaceByCode).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.deleteNamespaceByCode",top,format,namespacePrefix,input,refOnly,.schema)
