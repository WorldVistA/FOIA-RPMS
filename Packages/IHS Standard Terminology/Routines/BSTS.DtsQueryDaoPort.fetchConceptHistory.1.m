 ;BSTS.DtsQueryDaoPort.fetchConceptHistory.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchConceptHistory.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;79694668;BSTS.DtsQueryDaoPort.fetchConceptHistory
 ;
...
%NormalizeObject() public {
	...
	If m%conceptId Set:i%conceptId'="" i%conceptId=(..conceptIdNormalize(i%conceptId))
	If m%fromTime Set:i%fromTime'="" i%fromTime=(..fromTimeNormalize(i%fromTime))
...
%ValidateObject(force=0) public {
	...
	If m%conceptId Set iv=..conceptId If iv'="" Set rc=(..conceptIdIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"conceptId",iv)
	If m%fromTime Set iv=..fromTime If iv'="" Set rc=(..fromTimeIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"fromTime",iv)
	...
	Quit sc }
zInvoke(%Client,%Action,conceptId,namespaceId,fromTime,asd) public {
 Set ..conceptId=$get(conceptId),..namespaceId=$get(namespaceId),..fromTime=$get(fromTime),..asd=$get(asd)
 Do %Client.InvokeClient($this,"fetchConceptHistory",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchConceptHistory",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchConceptHistory")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchConceptHistory")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchConceptHistory"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchConceptHistory"
   ...
   If tag="" Set tag=typesPrefix_"fetchConceptHistory"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchConceptHistory"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..conceptId
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"conceptId"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"int""",1:"")_">"_val_endprefix_"conceptId>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..namespaceId
 If val'="" {
 ...
 Set val=..fromTime
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"fromTime"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"dateTime""",1:"")_">"_$select(val="":"",1:$translate(val," ","T")_"Z")_endprefix_"fromTime>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..asd
 If $IsObject(val) , deepFlag {
     Set topArg="asd",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptAttributeSetDescriptor"),local=1,savexsiAttrs=xsiAttrs
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchConceptHistory",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchConceptHistory")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchConceptHistory).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchConceptHistory",top,format,namespacePrefix,input,refOnly,.schema)
...
zasdNewObject() public {
	...
	Set ..asd=newobject
	Quit newobject }
zconceptIdDisplayToLogical(%val) public {
	Q $s(%val="":"",$zu(115,13)&&(%val=$c(0)):"",1:+$in(%val,"",%val)) }
zconceptIdIsValid(%val) public {
...
zfromTimeIsValid(%val)
	...
	Quit 1
zfromTimeLogicalToXSD(%val)
	Quit $select(%val="":"",1:$translate(%val," ","T")_"Z")
zfromTimeNormalize(%val)
