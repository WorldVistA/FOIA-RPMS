 ;BSTS.DtsQueryDaoPort.updateDefiningConcept.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.updateDefiningConcept.  Do NOT edit. 08/21/2017 07:46:20PM
 ;;41415877;BSTS.DtsQueryDaoPort.updateDefiningConcept
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
   i $d(Seen(+$this)) q 1
   Set Seen(+$this)=$this
   s sc = 1
 s proporef=..addedDefiningConcept
 s proporef=..concept
 s proporef=..deletedDefiningConcept
...
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%"%%OID"=""
	If deep>0 {
		If $isobject(..addedDefiningConcept)=1 Set r%addedDefiningConcept=r%addedDefiningConcept.%ConstructClone(1,.cloned),i%addedDefiningConcept=""
		If $isobject(..concept)=1 Set r%concept=r%concept.%ConstructClone(1,.cloned),i%concept=""
		If $isobject(..deletedDefiningConcept)=1 Set r%deletedDefiningConcept=r%deletedDefiningConcept.%ConstructClone(1,.cloned),i%deletedDefiningConcept=""
	}
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set Poref=r%addedDefiningConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%concept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
	Set Poref=r%deletedDefiningConcept If Poref'="",'$data(%objTX(1,+Poref)) Set %objTX(6,$i(%objTX(6)))=Poref
exit	Quit sc }
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,concept,deletedDefiningConcept,addedDefiningConcept) public {
 Set ..concept=$get(concept),..deletedDefiningConcept=$get(deletedDefiningConcept),..addedDefiningConcept=$get(addedDefiningConcept)
 Do %Client.InvokeClient($this,"updateDefiningConcept",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="updateDefiningConceptResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.updateDefiningConcept",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateDefiningConcept")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.updateDefiningConcept")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.updateDefiningConcept"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="updateDefiningConcept"
   ...
   If tag="" Set tag=typesPrefix_"updateDefiningConcept"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"updateDefiningConcept"""_xsiAttrs,xsiAttrs=""
 ...
 If $IsObject(val) , deepFlag {
     Set topArg="concept",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..deletedDefiningConcept
 If $IsObject(val) , deepFlag {
     Set topArg="deletedDefiningConcept",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..addedDefiningConcept
 If $IsObject(val) , deepFlag {
     Set topArg="addedDefiningConcept",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TOntylogConcept"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.updateDefiningConcept",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.updateDefiningConcept")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="updateDefiningConceptResult" {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If $IsObject(..deletedDefiningConcept) Quit 0
 If $IsObject(..addedDefiningConcept) Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.updateDefiningConcept).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.updateDefiningConcept",top,format,namespacePrefix,input,refOnly,.schema)
...
zaddedDefiningConceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..addedDefiningConcept=newobject
	Quit newobject }
zconceptNewObject() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..concept=newobject
	Quit newobject }
zdeletedDefiningConceptNewObjec() public {
	Set newobject=##class(BSTS.ns1.TOntylogConcept).%New() If newobject="" Quit ""
	Set ..deletedDefiningConcept=newobject
	Quit newobject }
