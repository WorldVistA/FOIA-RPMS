 ;BSTS.DtsDaoPort.executeSQL.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.executeSQL.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;53316D36;BSTS.DtsDaoPort.executeSQL
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,sql) public {
 Set ..sql=$get(sql)
 Do %Client.InvokeClient($this,"executeSQL",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.executeSQL",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.executeSQL")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.executeSQL")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.executeSQL"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag=typesPrefix_"executeSQL"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"executeSQL"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.executeSQL",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.executeSQL")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.executeSQL).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.executeSQL",top,format,namespacePrefix,input,refOnly,.schema)
