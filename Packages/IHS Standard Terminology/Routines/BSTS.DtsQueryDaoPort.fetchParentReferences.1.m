 ;BSTS.DtsQueryDaoPort.fetchParentReferences.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.fetchParentReferences.  Do NOT edit. 08/21/2017 07:46:14PM
 ;;4A30784D;BSTS.DtsQueryDaoPort.fetchParentReferences
 ;
...
zInvoke(%Client,%Action,roleType) public {
 ...
 Do %Client.InvokeClient($this,"fetchParentReferences",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.fetchParentReferences",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchParentReferences")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.fetchParentReferences")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.fetchParentReferences"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="fetchParentReferences"
   ...
   If tag="" Set tag=typesPrefix_"fetchParentReferences"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"fetchParentReferences"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.fetchParentReferences",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.fetchParentReferences")
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.fetchParentReferences).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.fetchParentReferences",top,format,namespacePrefix,input,refOnly,.schema)
