 ;BSTS.ns1.TContentVersionList.1
 ;(C)InterSystems, generated for class BSTS.ns1.TContentVersionList.  Do NOT edit. 08/21/2017 07:45:58PM
 ;;734E6C34;BSTS.ns1.TContentVersionList
 ;
...
%Construct(initvalue) public {
	Set m%versions=0,M%versions=0
	Quit 1 }
%ConstructCloneInit(object,deep=0,cloned,location) public {
	Set i%versions="",r%versions=""
	Set i%"%%OID"=""
	If deep>0 {
		Set key="" For  Set value=..versions.GetNext(.key) Quit:key=""  Set r%versions(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%versions(key)=""
	}
	Quit 1 }
%Destruct() public {
	Try {
		If $isobject($get(r%versions))=1,$zobjcnt(r%versions)>1 Do r%versions.%Disconnect()
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set key=""
	For  {
		Set key=$order(r%versions(key),1,Poref) Quit:key=""
		If $isobject(Poref)=1 Set:'$data(%objTX(1,+Poref)) %objTX(6,$i(%objTX(6)))=Poref
	}
exit	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TContentVersionList",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TContentVersionList")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TContentVersionList")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TContentVersionList"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TContentVersionList"
   ...
   If tag="" Set tag=typesPrefix_"TContentVersionList"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TContentVersionList"""_xsiAttrs,xsiAttrs=""
 ...
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     ...
     If soap12 { Set %xmlmsg=beginprefix_"versions"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.TContentVersion","XMLSUMMARY")'="":"s_TContentVersion",1:"TContentVersion")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"versions "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.TContentVersion","XMLSUMMARY")'="":"s_TContentVersion",1:"TContentVersion")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       ...
       If +id'=0 {
         Set %xmlmsg=currentIndent_beginprefix_"versions "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
       } Else { if id=0 Set id=$increment(idlist)
         Set topArg="versions",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TContentVersion"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
       }
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"versions "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
   ...
   If encoded {
     ...
     Set %xmlmsg=endprefix_"versions>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TContentVersionList" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TContentVersionList",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TContentVersionList")
 If encoded {
   ...
   If encoded,$$XMLImportId() {
     ...
     If encoded,(($data(@(tree)@(ref,"a","arrayType")))||(($get(@(tree)@(ref,"a","type","u"))=1)&&($piece($get(@(tree)@(ref,"a","type")),":",2)="Array"))||($select($get(@(tree)@(ref,"a","itemType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","itemType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","nodeType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","nodeType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","arraySize","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","arraySize","u"))))="http://www.w3.org/2003/05/soap-encoding")) {
       ...
       While element'="" {
         ...
         If data'="" Do ..versions.Insert(data)
         }
       ...
       Quit:('sc)  Set ref=loopref
         ...
         If $listget($get(@(tree)@(ref,0)),1)'="e" Goto XMLImportMalformedNoTag
         If 'encoded,(@(tree)@(ref)'="versions") Goto XMLImportBadTag
         If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
         If encoded,$$XMLImportId() {
           Set data=idlist(ref)
         } Else {
...
zXMLIsObjectEmpty(ignoreNull)
 If ..versions.Count()>0 Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TContentVersionList).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TContentVersionList",top,format,namespacePrefix,input,refOnly,.schema)
zversionsBuildValueArray(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zversionsCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
zversionsCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
	...
	quit $extract(odbc,2,*) }
zversionsDisplayToCollection(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zversionsGetObject(force=0) public {
	Quit $select(r%versions="":$select(i%versions="":"",1:$listbuild(i%versions_"")),(''..versions.%GetSwizzleObject(force,.oid)):oid,1:"") }
zversionsGetObjectId(force=0) public {
	Quit $listget(..versionsGetObject(force)) }
zversionsGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%versions=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.TContentVersion",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."versions%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zversionsOdbcToCollection(val="") public {
	set logical="",out=""
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zversionsSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%versions=newvalue Quit 1
	If newvalue="" Kill i%versions,r%versions Set i%versions="",r%versions="" Quit 1
	Set oref=r%versions Kill i%versions,r%versions Set i%versions="",r%versions=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%versions(i)=value,i%versions(i)=""
	Quit 1 }
