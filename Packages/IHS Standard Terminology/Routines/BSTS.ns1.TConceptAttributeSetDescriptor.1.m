 ;BSTS.ns1.TConceptAttributeSetDescriptor.1
 ;(C)InterSystems, generated for class BSTS.ns1.TConceptAttributeSetDescriptor.  Do NOT edit. 08/21/2017 07:46:01PM
 ;;70397561;BSTS.ns1.TConceptAttributeSetDescriptor
 ;
%BindExport(dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
 ...
 s proporef=..inverseRoleTypes
 s proporef=..propertyTypes
 s proporef=..roleTypes
...
%Construct(initvalue) public {
	Set m%associationTypes=0,M%associationTypes=0,m%inverseAssociationTypes=0,M%inverseAssociationTypes=0,m%inverseRoleTypes=0,M%inverseRoleTypes=0
	Set m%propertyTypes=0,M%propertyTypes=0,m%roleTypes=0,M%roleTypes=0,m%synonymTypes=0,M%synonymTypes=0
	Quit 1 }
%ConstructCloneInit(object,deep=0,cloned,location) public {
	...
	Set i%inverseRoleTypes="",r%inverseRoleTypes=""
	Set i%propertyTypes="",r%propertyTypes=""
	Set i%roleTypes="",r%roleTypes=""
	...
	If deep>0 {
		...
		Set key="" For  Set value=..inverseRoleTypes.GetNext(.key) Quit:key=""  Set r%inverseRoleTypes(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%inverseRoleTypes(key)=""
		...
		Set key="" For  Set value=..roleTypes.GetNext(.key) Quit:key=""  Set r%roleTypes(key)=$select(value="":"",1:value.%ConstructClone(1,.cloned)),i%roleTypes(key)=""
...
%Destruct() public {
	Try {
		...
		If $isobject($get(r%inverseRoleTypes))=1,$zobjcnt(r%inverseRoleTypes)>1 Do r%inverseRoleTypes.%Disconnect()
		If $isobject($get(r%propertyTypes))=1,$zobjcnt(r%propertyTypes)>1 Do r%propertyTypes.%Disconnect()
		If $isobject($get(r%roleTypes))=1,$zobjcnt(r%roleTypes)>1 Do r%roleTypes.%Disconnect()
...
%NormalizeObject() public {
	...
	If m%allInverseRoleTypes Set:i%allInverseRoleTypes'="" i%allInverseRoleTypes=(..allInverseRoleTypesNormalize(i%allInverseRoleTypes))
	If m%allPropertyTypes Set:i%allPropertyTypes'="" i%allPropertyTypes=(..allPropertyTypesNormalize(i%allPropertyTypes))
	If m%allRoleTypes Set:i%allRoleTypes'="" i%allRoleTypes=(..allRoleTypesNormalize(i%allRoleTypes))
	...
	If m%definedViewAttributes Set:i%definedViewAttributes'="" i%definedViewAttributes=(..definedViewAttributesNormalize(i%definedViewAttributes))
	If m%name Set:i%name'="" i%name=(..nameNormalize(i%name))
	If m%queryHasSubs Set:i%queryHasSubs'="" i%queryHasSubs=(..queryHasSubsNormalize(i%queryHasSubs))
	If m%queryHasSups Set:i%queryHasSups'="" i%queryHasSups=(..queryHasSupsNormalize(i%queryHasSups))
	If m%snapshotDate Set:i%snapshotDate'="" i%snapshotDate=(..snapshotDateNormalize(i%snapshotDate))
	If m%subconcepts Set:i%subconcepts'="" i%subconcepts=(..subconceptsNormalize(i%subconcepts))
	If m%superconcepts Set:i%superconcepts'="" i%superconcepts=(..superconceptsNormalize(i%superconcepts))
	Quit 1 }
...
%AddToSaveSet(depth=3,refresh=0) public {
	...
	Set key=""
	For  {
		Set key=$order(r%inverseRoleTypes(key),1,Poref) Quit:key=""
		If $isobject(Poref)=1 Set:'$data(%objTX(1,+Poref)) %objTX(6,$i(%objTX(6)))=Poref
	}
	Set key=""
	For  {
	...
	Set key=""
	For  {
		Set key=$order(r%roleTypes(key),1,Poref) Quit:key=""
		If $isobject(Poref)=1 Set:'$data(%objTX(1,+Poref)) %objTX(6,$i(%objTX(6)))=Poref
	}
	Set key=""
	For  {
...
%ValidateObject(force=0) public {
	...
	Set iv=..allInverseRoleTypes If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"allInverseRoleTypes"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%allInverseRoleTypes Set rc=(..allInverseRoleTypesIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"allInverseRoleTypes",iv)
	...
	Set iv=..allRoleTypes If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"allRoleTypes"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%allRoleTypes Set rc=(..allRoleTypesIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"allRoleTypes",iv)
	...
	Set iv=..definedViewAttributes If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"definedViewAttributes"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%definedViewAttributes Set rc=(..definedViewAttributesIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"definedViewAttributes",iv)
	If m%name Set iv=..name If iv'="" Set rc=(..nameIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"name",iv)
	Set iv=..queryHasSubs If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"queryHasSubs"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%queryHasSubs Set rc=(..queryHasSubsIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"queryHasSubs",iv)
	Set iv=..queryHasSups If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"queryHasSups"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%queryHasSups Set rc=(..queryHasSupsIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"queryHasSups",iv)
	...
	Set iv=..subconcepts If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"subconcepts"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%subconcepts Set rc=(..subconceptsIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"subconcepts",iv)
	Set iv=..superconcepts If iv="" Set rc=$$Error^%apiOBJ(5659,$classname()_"::"_"superconcepts"_"("_$this_",ID="),sc=$select(+sc:rc,1:$$AppendStatus^%occSystem(sc,rc))
	If m%superconcepts Set rc=(..superconceptsIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"superconcepts",iv)
	Quit sc }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.ns1.TConceptAttributeSetDescriptor",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 New tag,summary,attrsVal,savelocal,aval,k,tmpPrefix,prefixDepth,hasNoContent,hasElement,topAttrs,beginprefix,endprefix,savexsiAttrs,initialxsiAttrs,initlist,initialCR,inlineFlag,popAtEnd,saveTopPrefix,saveTypesPrefix,saveAttrsPrefix,saveUsePrefix,initlist
 Set $ztrap="XMLExportInternalTrap",popAtEnd=0
 Set summary=summaryArg,initialxsiAttrs=xsiAttrs
 If group Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TConceptAttributeSetDescriptor")
 ...
 If 'nocycle,('encoded||inlineFlag) {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.ns1.TConceptAttributeSetDescriptor")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.ns1.TConceptAttributeSetDescriptor"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="TConceptAttributeSetDescriptor"
   ...
   If tag="" Set tag=typesPrefix_"TConceptAttributeSetDescriptor"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"TConceptAttributeSetDescriptor"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..allRoleTypes
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"allRoleTypes"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"allRoleTypes>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set aval=..roleTypes
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     If indentFlag Set %xmlmsg=currentIndent Set currentIndent=currentIndent_indentChars if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     Set temp="",temp1=$parameter("BSTS.ns1.TRoleType","NAMESPACE")
     Set temp1=$select((temp1'="http://apelon.com/dtsserver/types")&&$IsObject(namespaces):namespaces.GetNamespacePrefix(temp1,.temp),1:typesPrefix)
     If soap12 { Set %xmlmsg=beginprefix_"roleTypes"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.TRoleType","XMLSUMMARY")'="":"s_TRoleType",1:"TRoleType")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"roleTypes "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.TRoleType","XMLSUMMARY")'="":"s_TRoleType",1:"TRoleType")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       If encoded,'inlineFlag {
         Set temp=$select($parameter("BSTS.ns1.TRoleType","XMLSUMMARY")'="":-1,1:1)
         Set id=+$get(idlist(temp*val))
         If 'soap12 , (id=0) Set id=$increment(idlist),oreflist(temp*id)=val,idlist(temp*val)=id
       }
       If +id'=0 {
         Set %xmlmsg=currentIndent_beginprefix_"roleTypes "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
       } Else { if id=0 Set id=$increment(idlist)
         Set topArg="roleTypes",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TRoleType"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
       }
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"roleTypes "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
     Set val=aval.GetNext(.k)
   }
   If encoded {
     If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     Set %xmlmsg=endprefix_"roleTypes>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 Set val=..allInverseRoleTypes
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"allInverseRoleTypes"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"allInverseRoleTypes>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set aval=..inverseRoleTypes
 Set k="" Set:deepFlag val=aval.GetNext(.k) If k'="" {
   If encoded {
     If indentFlag Set %xmlmsg=currentIndent Set currentIndent=currentIndent_indentChars if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     Set temp="",temp1=$parameter("BSTS.ns1.TRoleType","NAMESPACE")
     Set temp1=$select((temp1'="http://apelon.com/dtsserver/types")&&$IsObject(namespaces):namespaces.GetNamespacePrefix(temp1,.temp),1:typesPrefix)
     If soap12 { Set %xmlmsg=beginprefix_"inverseRoleTypes"_$select(xsitype:"",1:" "_soapPrefix_"itemType="""_temp1_$select($parameter("BSTS.ns1.TRoleType","XMLSUMMARY")'="":"s_TRoleType",1:"TRoleType")_"""")_" "_soapPrefix_"arraySize="""_aval.Count()_""""_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     } Else { Set %xmlmsg=beginprefix_"inverseRoleTypes "_$select(xsitype:xsiPrefix_"type="""_soapPrefix_"Array""",1:soapPrefix_"arrayType="""_temp1_$select($parameter("BSTS.ns1.TRoleType","XMLSUMMARY")'="":"s_TRoleType",1:"TRoleType")_"["_aval.Count()_"]""")_""_temp_">" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg } }
   }
   While k'="" {
     If $IsObject(val) {
       Set id=""
       ...
       If +id'=0 {
         Set %xmlmsg=currentIndent_beginprefix_"inverseRoleTypes "_$select(soap12:soapPrefix_"ref=""",1:"href=""#")_"id"_id_""" />" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
       } Else { if id=0 Set id=$increment(idlist)
         Set topArg="inverseRoleTypes",summaryArg=1,group=0,createId=$get(id),typeAttr=$select(encoded||xsitype:"*",1:"BSTS.ns1.TRoleType"),local=1,savexsiAttrs=xsiAttrs
         Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
       }
     } Else {
       Set %xmlmsg=currentIndent_beginprefix_"inverseRoleTypes "_xsiPrefix_"nil=""true"""_xsiAttrs_"/>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     }
     Set val=aval.GetNext(.k)
   }
   If encoded {
     If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
     Set %xmlmsg=endprefix_"inverseRoleTypes>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
   }
 }
 Set val=..definedViewAttributes
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"definedViewAttributes"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"definedViewAttributes>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..queryHasSubs
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"queryHasSubs"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"queryHasSubs>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..queryHasSups
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"queryHasSups"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"queryHasSups>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..subconcepts
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"subconcepts"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"subconcepts>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 Set val=..superconcepts
 If val'="" {
   Set %xmlmsg=currentIndent_beginprefix_"superconcepts"_$select(xsitype:" "_xsiPrefix_"type="""_schemaPrefix_"boolean""",1:"")_">"_$s(val:"true",1:"false")_endprefix_"superconcepts>" if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 }
 If indentFlag Set currentIndent=$extract(currentIndent,1,*-$length(indentChars)) Set %xmlmsg=currentIndent if $data(%xmlBlock) { Do xeWrite^%occXMLInternal } else { write %xmlmsg }
 ...
 If 'encoded||inlineFlag {
   ...
   While id'="" {
     ...
     If $classname(val)="BSTS.ns1.TConceptAttributeSetDescriptor" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.ns1.TConceptAttributeSetDescriptor",.imports,.classes)
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.ns1.TConceptAttributeSetDescriptor")
 If encoded {
...
XMLImportElements() ;
     ...
     If encoded,(($data(@(tree)@(ref,"a","arrayType")))||(($get(@(tree)@(ref,"a","type","u"))=1)&&($piece($get(@(tree)@(ref,"a","type")),":",2)="Array"))||($select($get(@(tree)@(ref,"a","itemType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","itemType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","nodeType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","nodeType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","arraySize","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","arraySize","u"))))="http://www.w3.org/2003/05/soap-encoding")) {
       ...
       While element'="" {
         ...
         If $$XMLImportId() {
           ...
           If 'sc Goto XMLImportExit
           Set class=$select(($get(@(tree)@(ref,"a","type","u"))=1):$get(@(tree)@(ref,"a","type")),1:"")
           If (class="") || (class="TRoleType") {
             Set class="BSTS.ns1.TRoleType"
           } Else {
             If $length(class,":")=2 Set class=$piece(class,":",2)
             Set class=$get(^oddCOM("BSTS.ns1.TRoleType",85,"s",class))_$get(^oddXML("BSTS.ns1.TRoleType","s",class)) If class="" Set class=0
             Set tmp=$select($get(@(tree)@(ref,"a","type","v"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","type","v"))))
             If (class=0)||(($listlength(class)>1)&&(tmp="")) Set class=$get(@(tree)@(ref,"a","type")) Goto XMLImportBadType
             If $listlength(class)>1 {
               For tmpi=1:1:$listlength(class) {
         ...
         If data'="" Do ..roleTypes.Insert(data)
         }
         Set element=$order(@(tree)@(loopref,"c",element))
       }
       Quit:('sc)  Set ref=loopref
         ...
         If $listget($get(@(tree)@(ref,0)),1)'="e" Goto XMLImportMalformedNoTag
         If 'encoded,(@(tree)@(ref)'="roleTypes") Goto XMLImportBadTag
         If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
         If encoded,$$XMLImportId() {
           Set data=idlist(ref)
         } Else {
 ...
 If tag="allInverseRoleTypes" {
   ...
   Set ..allInverseRoleTypes=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 For  { Set encodedArray=encoded
 If tag="inverseRoleTypes" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
     Set data=idlist(ref)
   } Else {
     ...
     If encoded,(($data(@(tree)@(ref,"a","arrayType")))||(($get(@(tree)@(ref,"a","type","u"))=1)&&($piece($get(@(tree)@(ref,"a","type")),":",2)="Array"))||($select($get(@(tree)@(ref,"a","itemType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","itemType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","nodeType","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","nodeType","u"))))="http://www.w3.org/2003/05/soap-encoding")||($select($get(@(tree)@(ref,"a","arraySize","u"))="":"",1:$get(@(tree)@("ns#",@(tree)@(ref,"a","arraySize","u"))))="http://www.w3.org/2003/05/soap-encoding")) {
       ...
       While element'="" {
         ...
         If data'="" Do ..inverseRoleTypes.Insert(data)
         }
         Set element=$order(@(tree)@(loopref,"c",element))
       }
       Quit:('sc)  Set ref=loopref
         ...
         If $listget($get(@(tree)@(ref,0)),1)'="e" Goto XMLImportMalformedNoTag
         If 'encoded,(@(tree)@(ref)'="inverseRoleTypes") Goto XMLImportBadTag
         If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
         If encoded,$$XMLImportId() {
           Set data=idlist(ref)
         } Else {
 ...
 If tag="definedViewAttributes" {
   ...
   Set ..definedViewAttributes=data
   If $$XMLLOOP()'=0 Goto XMLImportExit }
 If tag="queryHasSubs" {
   If '$case($listget($get(@(tree)@(ref,0)),3),"":1,nsIndex:1,:0) Goto XMLImportNS
   If encoded,$$XMLImportId() {
...
zXMLIsObjectEmpty(ignoreNull)
 ...
 If ..allRoleTypes'="" Quit 0
 If ..roleTypes.Count()>0 Quit 0
 If ..allInverseRoleTypes'="" Quit 0
 If ..inverseRoleTypes.Count()>0 Quit 0
 If ..definedViewAttributes'="" Quit 0
 If ..queryHasSubs'="" Quit 0
 If ..queryHasSups'="" Quit 0
 If ..subconcepts'="" Quit 0
 If ..superconcepts'="" Quit 0
 Quit 1
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.ns1.TConceptAttributeSetDescriptor).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.ns1.TConceptAttributeSetDescriptor",top,format,namespacePrefix,input,refOnly,.schema)
zallInverseRoleTypesDisplayToLo(%val) public {
	Quit ''%val }
zallInverseRoleTypesIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zallInverseRoleTypesLogicalToXS(%val) public {
	Q $s(%val:"true",1:"false") }
zallInverseRoleTypesNormalize(%val) public {
	Quit %val\1 }
zallInverseRoleTypesXSDToLogica(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zallRoleTypesDisplayToLogical(%val) public {
	Quit ''%val }
zallRoleTypesIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zallRoleTypesLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zallRoleTypesNormalize(%val) public {
	Quit %val\1 }
zallRoleTypesXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zdefinedViewAttributesDisplayTo(%val) public {
	Quit ''%val }
zdefinedViewAttributesIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zdefinedViewAttributesLogicalT2(%val) public {
	Q $s(%val:"true",1:"false") }
zdefinedViewAttributesNormalize(%val) public {
	Quit %val\1 }
zdefinedViewAttributesXSDToLogi(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zinverseRoleTypesBuildValueArra(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zinverseRoleTypesCollectionToDi(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
zinverseRoleTypesCollectionToOd(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
	...
	quit $extract(odbc,2,*) }
zinverseRoleTypesDisplayToColle(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zinverseRoleTypesGetObject(force=0) public {
	Quit $select(r%inverseRoleTypes="":$select(i%inverseRoleTypes="":"",1:$listbuild(i%inverseRoleTypes_"")),(''..inverseRoleTypes.%GetSwizzleObject(force,.oid)):oid,1:"") }
zinverseRoleTypesGetObjectId(force=0) public {
	Quit $listget(..inverseRoleTypesGetObject(force)) }
zinverseRoleTypesGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%inverseRoleTypes=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.TRoleType",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."inverseRoleTypes%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zinverseRoleTypesOdbcToCollecti(val="") public {
	set logical="",out=""
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zinverseRoleTypesSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%inverseRoleTypes=newvalue Quit 1
	If newvalue="" Kill i%inverseRoleTypes,r%inverseRoleTypes Set i%inverseRoleTypes="",r%inverseRoleTypes="" Quit 1
	Set oref=r%inverseRoleTypes Kill i%inverseRoleTypes,r%inverseRoleTypes Set i%inverseRoleTypes="",r%inverseRoleTypes=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%inverseRoleTypes(i)=value,i%inverseRoleTypes(i)=""
	Quit 1 }
zqueryHasSubsDisplayToLogical(%val) public {
	Quit ''%val }
zqueryHasSubsIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zqueryHasSubsLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zqueryHasSubsNormalize(%val) public {
	Quit %val\1 }
zqueryHasSubsXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zqueryHasSupsDisplayToLogical(%val) public {
	Quit ''%val }
zqueryHasSupsIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zqueryHasSupsLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zqueryHasSupsNormalize(%val) public {
	Quit %val\1 }
zqueryHasSupsXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zroleTypesBuildValueArray(value,array) public {
	Quit ##class(%Collection.ListOfObj).BuildValueArray(value,.array)
}
zroleTypesCollectionToDisplay(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	set odbc="",tDelimLen=$l(delim)
	for i=1:1:$listlength(val) {
		set item=$Listget(val,i)
		if item'["""",item'[delim,$length($zstrip(item,"<>W"))=$length(item) set odbc=odbc_delim_item continue
	...
	quit $extract(odbc,tDelimLen+1,*) }
zroleTypesCollectionToOdbc(val="") public {
	set odbc=""
	for i=1:1:$listlength(val) {
		set item=$listget(val,i)
		if item'["""",item'[",",$length($zstrip(item,"<>W"))=$length(item) set odbc=odbc_","_item continue
	...
	quit $extract(odbc,2,*) }
zroleTypesDisplayToCollection(val="",delim) public { Set:'($data(delim)#2) delim=$char(13,10)
	set logical="",out="",tDelimLen=$l(delim)
hloop	if $extract(val,1,tDelimLen)=delim { goto hdel } if val="" { goto hexit } if $extract(val)="""" { goto hquote } goto hdefault
hdefault	set stuff=$piece(val,delim),out=out_stuff,val=$extract(val,$length(stuff)+1,*) goto hloop
hquote	set stuff=$find(val,"""",2) if stuff=0 set stuff=$length(val)+10
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zroleTypesGetObject(force=0) public {
	Quit $select(r%roleTypes="":$select(i%roleTypes="":"",1:$listbuild(i%roleTypes_"")),(''..roleTypes.%GetSwizzleObject(force,.oid)):oid,1:"") }
zroleTypesGetObjectId(force=0) public {
	Quit $listget(..roleTypesGetObject(force)) }
zroleTypesGetSwizzled() public {
	Set oref=##class(%Collection.ListOfObj).%New() If oref="" Quit ""
	Set modstate=$system.CLS.GetModifiedBits() Set r%roleTypes=oref Do $system.CLS.SetModifiedBits(modstate)
	Set oref.ElementType="BSTS.ns1.TRoleType",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."roleTypes%i"(),oref.OrefStorage=oref.Storage+1
	Do $system.CLS.SetModified(oref,0)
	Quit oref }
zroleTypesOdbcToCollection(val="") public {
	set logical="",out=""
hloop	if $extract(val)="," { goto hdel } if val="" { goto hexit } if $extract(val)="""" { goto hquote } goto hdefault
hdefault	set stuff=$piece(val,","),out=out_stuff,val=$extract(val,$length(stuff)+1,*) goto hloop
hquote	set stuff=$find(val,"""",2) if stuff=0 set stuff=$length(val)+10
...
hexit	set logical=logical_$listbuild(out)
	quit logical }
zroleTypesSet(newvalue) public {
	If '$isobject(newvalue),newvalue'="" Quit $$Error^%apiOBJ(5807,newvalue)
	If r%roleTypes=newvalue Quit 1
	If newvalue="" Kill i%roleTypes,r%roleTypes Set i%roleTypes="",r%roleTypes="" Quit 1
	Set oref=r%roleTypes Kill i%roleTypes,r%roleTypes Set i%roleTypes="",r%roleTypes=oref
	Set key="" For i=1:1 Set value=newvalue.GetNext(.key) Quit:key=""  Set r%roleTypes(i)=value,i%roleTypes(i)=""
	Quit 1 }
zsubconceptsDisplayToLogical(%val) public {
	Quit ''%val }
zsubconceptsIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zsubconceptsLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zsubconceptsNormalize(%val) public {
	Quit %val\1 }
zsubconceptsXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
zsuperconceptsDisplayToLogical(%val) public {
	Quit ''%val }
zsuperconceptsIsValid(%val="") public {
	Q $s($isvalidnum(%val,0,0,2)&&(+%val'=2):1,1:$$Error^%apiOBJ(7206,%val)) }
zsuperconceptsLogicalToXSD(%val) public {
	Q $s(%val:"true",1:"false") }
zsuperconceptsNormalize(%val) public {
	Quit %val\1 }
zsuperconceptsXSDToLogical(%val) public {
	Q $case(%val,"true":1,"false":0,1:1,0:0,:"") }
