 ;BSTS.DtsDaoPort.isAssociationTypeUsed.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.isAssociationTypeUsed.  Do NOT edit. 08/21/2017 07:46:12PM
 ;;50476968;BSTS.DtsDaoPort.isAssociationTypeUsed
 ;
...
zInvoke(%Client,%Action,assocType) public {
 ...
 Do %Client.InvokeClient($this,"isAssociationTypeUsed",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="isAssociationTypeUsedResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.isAssociationTypeUsed",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.isAssociationTypeUsed")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.isAssociationTypeUsed")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.isAssociationTypeUsed"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="isAssociationTypeUsed"
   ...
   If tag="" Set tag=typesPrefix_"isAssociationTypeUsed"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"isAssociationTypeUsed"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.isAssociationTypeUsed",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.isAssociationTypeUsed")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="isAssociationTypeUsedResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.isAssociationTypeUsed).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.isAssociationTypeUsed",top,format,namespacePrefix,input,refOnly,.schema)
