 ;BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching.1
 ;(C)InterSystems, generated for class BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching.  Do NOT edit. 08/21/2017 07:46:15PM
 ;;6C365868;BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching
 ;
...
%ValidateObject(force=0) public {
	...
	If m%inverse Set iv=..inverse If iv'="" Set rc=(..inverseIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"inverse",iv)
	If m%pattern Set iv=..pattern If iv'="" Set rc=(..patternIsValid(iv)) If ('rc) Set sc=$$EmbedErr^%occSystem(sc,rc,5802,"pattern",iv)
	Quit sc }
zInvoke(%Client,%Action,assocType,qualType,pattern,options,inverse) public {
 Set ..assocType=$get(assocType),..qualType=$get(qualType),..pattern=$get(pattern),..options=$get(options),..inverse=$get(inverse)
 Do %Client.InvokeClient($this,"findConceptsWithAssociationQualifierMatching",%Action)
 Quit ..%Result }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findConceptsWithAssociationQualifierMatching"
   ...
   If tag="" Set tag=typesPrefix_"findConceptsWithAssociationQualifierMatching"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findConceptsWithAssociationQualifierMatching"""_xsiAttrs,xsiAttrs=""
 ...
 Set val=..qualType
 If $IsObject(val) , deepFlag {
     Set topArg="qualType",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TQualifierType"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..pattern
 If val'="" {
 ...
 Set val=..options
 If $IsObject(val) , deepFlag {
     Set topArg="options",summaryArg=0,group=0,createId="",typeAttr=$select(xsitype:"*",1:"BSTS.ns1.TConceptSearchOptions"),local=1,savexsiAttrs=xsiAttrs
     Set sc=val.XMLExportInternal() Goto:'sc XMLExportExit Set xsiAttrs=savexsiAttrs
 }
 Set val=..inverse
 If val'="" {
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="return" {
   ...
   If (class="") || (class="TOntylogConcept") {
     ...
     Set class=$get(^oddCOM("BSTS.ns1.TOntylogConcept",85,"t",class))_$get(^oddXML("BSTS.ns1.TOntylogConcept","t",class)) If class="" Set class=0
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsQueryDaoPort.findConceptsWithAssociationQualifierMatching",top,format,namespacePrefix,input,refOnly,.schema)
...
%ResultGetSwizzled() public {
	...
	Set oref.ElementType="BSTS.ns1.TOntylogConcept",oref.ElementClassType="",oref.Owner=+$this,oref.Storage=$this."%Result%i"(),oref.OrefStorage=oref.Storage+1
