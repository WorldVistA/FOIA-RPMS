 ;BSTS.DtsDaoPort.findAuthorityQualifierTypeById.1
 ;(C)InterSystems, generated for class BSTS.DtsDaoPort.findAuthorityQualifierTypeById.  Do NOT edit. 08/21/2017 07:46:10PM
 ;;572B7066;BSTS.DtsDaoPort.findAuthorityQualifierTypeById
 ;
...
%ValidateObject(force=0) public {
	...
	Quit sc }
zInvoke(%Client,%Action,id,authorityId) public {
 Set ..id=$get(id),..authorityId=$get(authorityId)
 Do %Client.InvokeClient($this,"findAuthorityQualifierTypeById",%Action)
 Quit ..%Result }
zNeedsAdjustment(ResultName) public {
 Set ResultName="findAuthorityQualifierTypeByIdResult"
 Quit 1 }
zReset() public {
 Quit }
zXMLDTD(top,format,input,dtdlist)
 Quit ##class(%XML.Implementation).XMLDTD("BSTS.DtsDaoPort.findAuthorityQualifierTypeById",.top,.format,.input,.dtdlist)
zXMLExportInternal()
 ...
 If group Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityQualifierTypeById")
 ...
 If 'nocycle {
   If $data(oreflist($this)) Quit $$Error^%apiOBJ(6296,"BSTS.DtsDaoPort.findAuthorityQualifierTypeById")
   Set oreflist($this)=""
 }
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="BSTS.DtsDaoPort.findAuthorityQualifierTypeById"))
 If $IsObject(namespaces) {
   ...
   If tag="" Set tag="findAuthorityQualifierTypeById"
   ...
   If tag="" Set tag=typesPrefix_"findAuthorityQualifierTypeById"
   Set xsitype=0
 }
 Set local=+$get(local),savelocal=local
 Set endprefix="</"_beginprefix,beginprefix="<"_beginprefix
 If tmpi Set temp=temp_" "_xsiPrefix_"type="""_typesPrefix_"findAuthorityQualifierTypeById"""_xsiAttrs,xsiAttrs=""
...
zXMLGetSchemaImports(imports,classes)
 Quit ##class(%XML.Implementation).XMLGetSchemaImports("BSTS.DtsDaoPort.findAuthorityQualifierTypeById",.imports,.classes)
...
zXMLImportInternal()
 ...
 If bareProjection Quit $$Error^%apiOBJ(6386,"BSTS.DtsDaoPort.findAuthorityQualifierTypeById")
...
XMLLOOP For  { Set child=$order(@(tree)@(node,"c",child)) If (child="")||($listget($get(@(tree)@(child,0)),1)'="w") Quit }
 ...
 If tag="findAuthorityQualifierTypeByIdResult" {
...
zXMLNew(document,node,containerOref="")
	Quit (##class(BSTS.DtsDaoPort.findAuthorityQualifierTypeById).%New())
zXMLSchema(top="",format="",namespacePrefix="",input=0,refOnly=0,schema)
 Quit ##class(%XML.Implementation).XMLSchema("BSTS.DtsDaoPort.findAuthorityQualifierTypeById",top,format,namespacePrefix,input,refOnly,.schema)
