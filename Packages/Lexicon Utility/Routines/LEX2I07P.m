LEX2I07P ;IHS/OIT/FCJ - LEXICON UTILITY PATCH 1007
 ;;2.0;LEXICON UTILITY;**1007**;Sep 23, 1996;Build 1
 ;=================================================================
...
MAIL ;
 ...
 KILL ^TMP("LEX1007",$J)
 D RSLT(" --- LEX v 2.0 Patch 1007, has been installed into this namespace ---")
 F %=1:1 D RSLT($P($T(GREET+%),";",3)) Q:$P($T(GREET+%+1),";",3)="###"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   D RSLT(^(%,0))
 S XMSUB=$P($P($T(+1),";",2)," ",3,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""LEX1007"",$J,",XMY(1)="",XMY(DUZ)=""
 ...
 Q
 ;
RSLT(%) S ^(0)=$G(^TMP("LEX1007",$J,0))+1,^(^(0))=%
 Q
 ;
...
GREET ;;To add to mail message.
 ...
 ;;the Help Desk.  Refer to patch "LEX*2.1*1007".
 ;;  
