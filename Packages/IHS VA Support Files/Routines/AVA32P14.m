AVA32P14 ;IHS/ASDST/GTH - AVA 93.2 PATCH 14 ; [ 10/10/2002   4:36 PM ]
 ;;93.2;VA SUPPORT FILES;**14**;JUL 01, 1993;Build 12
 ;
 ; IHS/ASDST/GTH AVA*93.2*14 09/12/2002
 ;
 I '$G(IOM) D HOME^%ZIS
 ;
...
VCHK(AVAPRE,AVAVER,AVAQUIT) ; Check versions needed.
 ...
 ;
PRE ;EP - From KIDS.
 D BMES^XPDUTL("Beginning pre-install routine (PRE^AVA32P14).")
...
POST ;EP - From KIDS.
 NEW AVA
 D BMES^XPDUTL("Beginning post-install routine (POST^AVA32P14).")
 ;
 D BMES^XPDUTL("Changing sub-file dictionary number in the 9999999.18 multiple...")
 D SFDD
 ;
 D BMES^XPDUTL("Delivering AVA*93.2*14 install message to select users...")
 D MAIL
 ;
...
MAIL ; Send install mail message.
 ...
 KILL ^TMP("AVAP14MS",$J)
 S ^TMP("AVAP14MS",$J,1)=" --- AVA v 93.2, Patch 14, has been installed into this uci ---"
 S %=0
 F  S %=$O(^XTMP("XPDI",XPDA,"BLD",XPDBLD,1,%)) Q:'%   S ^TMP("AVAP14MS",$J,(%+1))=" "_^(%,0)
 S XMSUB=$P($P($T(+1),";",2)," ",4,99),XMDUZ=$S($G(DUZ):DUZ,1:.5),XMTEXT="^TMP(""AVAP14MS"",$J,",XMY(1)="",XMY(DUZ)=""
