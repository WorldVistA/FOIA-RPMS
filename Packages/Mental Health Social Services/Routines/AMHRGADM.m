AMHRGADM ; IHS/CMI/LAB - GAD - MULTIPLE PATS 10 Dec 2008 9:28 AM ;
...
START ;
 W:$D(IOF) @IOF
 D EN^XBVK("AMH")
 W !,$$CTR^AMHLEIN("GAD-2 and GAD-7 Generalized Anxiety Disorder Outcomes - Scores ",80),$$CTR^AMHLEIN("for Multiple Patients",80),!!
 W !,"This option is used to list GAD-2, and GAD-7 Scores for multiple "
 W !,"patients sorted by patient.",!
WHICH ;
 W !!,"Please note:  Only visits with GAD-2, and GAD-7 scores recorded will display",!,"on this list.",!
 D DBHUSR^AMHUTIL
DATES ;
 K AMHED,AMHBD
...
ZIS ;
 ...
 S XBRC="PROC^AMHRGADM",XBRP="PRINT^AMHRGADM",XBNS="AMH",XBRX="XIT^AMHRGADM"
 D ^XBDBQUE
XIT ;
 ...
 ;
BROWSE ;
 S XBRP="VIEWR^XBLM(""PRINT^AMHRGADM"")"
 S XBNS="AMH",XBRC="PROC^AMHRGADM",XBRX="XIT^AMHRGADM",XBIOP=0 D ^XBDBQUE
 Q
 ;
PROC ;
 ;loop through visits and check GAD score
 D XTMP^AMHUTIL("AMHRGADM","BH - GAD SCORES MULT PATS")
 ...
 ..S DFN=$P(AMHV0,U,8)
 ..Q:DFN=""
 ..I '$$HASGAD^AMHRGADO(AMHVIEN) Q  ;no GAD score
 ...
 ..S ^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",$P(^DPT(DFN,0),U,1),DFN,$P((9999999-AMHSD),"."),"BH",AMHVIEN)=""
 ..Q
 .Q
 ...
 ..I $D(AMHRCLNT),'$D(AMHRCLNT(AMHVCC)) Q
 ..S ^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",$P(^DPT(DFN,0),U,1),DFN,$P((9999999-AMHSD),"."),"PCC",AMHVIEN)=""
 Q
PRINT ;EP - called from xbdbque
 S AMHPG=0 K AMHQ D HEADER
 I '$D(^XTMP("AMHRGADM",AMHJOB,AMHBTH)) W !!,"NO PATIENTS/GAD SCORES TO REPORT" G DONE
 S AMHNAME="" F  S AMHNAME=$O(^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",AMHNAME)) Q:AMHNAME=""!($D(AMHQ))  D
 .S DFN=0 F  S DFN=$O(^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",AMHNAME,DFN)) Q:DFN'=+DFN!($D(AMHQ))  D
 ..W ! S AMHDATE="" F  S AMHDATE=$O(^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",AMHNAME,DFN,AMHDATE)) Q:AMHDATE=""!($D(AMHQ))  D
 ...S AMHT="" F  S AMHT=$O(^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",AMHNAME,DFN,AMHDATE,AMHT)) Q:AMHT=""!($D(AMHQ))  D
 ....S AMHV=0 F  S AMHV=$O(^XTMP("AMHRGADM",AMHJOB,AMHBTH,"PATS",AMHNAME,DFN,AMHDATE,AMHT,AMHV)) Q:AMHV'=+AMHV!($D(AMHQ))  D PRINT1
DONE ;
 K ^XTMP("AMHRGADM",AMHJOB,AMHBTH),AMHJOB,AMHBTH
...
PRINT1 ;
 ...
 ..I Y="GAD2" S T=T_$P(^AMHRMSR(X,0),U,4)_" "
 ..I Y="GAD7" S Z=Z_$P(^AMHRMSR(X,0),U,4)_" "
 I AMHT="PCC" D
 .F  S X=$O(^AUPNVMSR("AD",AMHV,X)) Q:X'=+X  D
 ..S Y=$$VAL^XBDIQ1(9000010.01,X,.01) D
 ..I Y="GAD2" S T=T_$P(^AUPNVMSR(X,0),U,4)_" "
 ..I Y="GAD7" S Z=Z_$P(^AUPNVMSR(X,0),U,4)_" "
 W ?34,T,?40,Z
 I AMHT="BH" D
...
HEADER1 ;
 ...
 W !,$$CTR^AMHLEIN("GAD-2, and GAD-7 SCORES FOR MULTIPLE PATIENTS",80),!
 ...
 W !,"PATIENT NAME",?17,"HRN",?24,"Date",?33,"GAD2",?40,"GAD7",?48,"Provider",?58,"CLINIC",?65,"Diagnosis/POV"
 ;W !?33,"-2",?37,"-9",?41,"-T"
 W !,$TR($J("",80)," ","-")
 Q
